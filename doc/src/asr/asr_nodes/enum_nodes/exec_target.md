...
[DoConcurrentLoop](../statement_nodes/DoConcurrentLoop.md), so the choice is
made per loop rather than per compilation. A `do concurrent` loop, an
`!$omp target` region and an `!$omp parallel do` all reach the passes as the
same node, and each one carries the answer to "who runs these iterations".
...
[DoConcurrentLoop](../statement_nodes/DoConcurrentLoop.md), [GpuKernelLaunch](../statement_nodes/GpuKernelLaunch.md), [OMPRegion](../statement_nodes/OMPRegion.md), [exec_space](exec_space.md)
