...
time_section "🧪 Testing POT3D" '
  ...
  print_subsection "Building with default flags with OPEN_MPI"
  FC="$FC --cpp -DOPEN_MPI=yes" ./build_and_run.sh

  print_subsection "Building with optimization flags with OPEN_MPI"
  FC="$FC --cpp --fast --skip-pass=dead_code_removal -DOPEN_MPI=yes" ./build_and_run.sh

  print_subsection "Building POT3D in separate compilation mode with OPEN_MPI"
  FC="$FC --cpp --generate-object-code --skip-pass=pass_array_by_data -DOPEN_MPI=yes" ./build_and_run.sh

  print_subsection "Building with default flags with MPICH"
  FC="$FC --cpp" ./build_and_run.sh

  print_subsection "Building with optimization flags with MPICH"
  FC="$FC --cpp --fast --skip-pass=dead_code_removal" ./build_and_run.sh

  print_subsection "Building POT3D in separate compilation mode with MPICH"
  FC="$FC --cpp --generate-object-code --skip-pass=pass_array_by_data" ./build_and_run.sh
