...
if (abs(sum(s(:)%v) - 10.0d0) > 1.0d-12) error stop "gpu_metal_206: scalar dp component"
...
if (abs(sum(c%u) - 12.0d0) > 1.0d-12) error stop "gpu_metal_206: allocatable dp component"
...
if (abs(sum(d(:)%v) - 20.0d0) > 1.0d-12) error stop "gpu_metal_206: inherited dp component"
if (sum(d(:)%k) /= 10) error stop "gpu_metal_206: inherited integer component"

! 5. array-of-derived-type component with fp64 inside
do concurrent (i = 1:2)
    n%g(i)%v = 3.0d0 * i
end do
if (abs(n%g(1)%v - 3.0d0) > 1.0d-12) error stop "gpu_metal_206: nested array component 1"
if (abs(n%g(2)%v - 6.0d0) > 1.0d-12) error stop "gpu_metal_206: nested array component 2"
...
if (abs(sum(mm(1)%r) - 8.0d0) > 1.0d-12) error stop "gpu_metal_206: matmul component 1"
if (abs(sum(mm(2)%r) - 32.0d0) > 1.0d-12) error stop "gpu_metal_206: matmul component 2"
...
if (abs(sum(pp(:)%v) - 40.0d0) > 1.0d-12) error stop "gpu_metal_206: pointer component"

print *, "gpu_metal_206 ok"
end program
