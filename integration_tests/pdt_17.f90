program pdt_17
    ...
    type(value_t(4)) :: kw_value4 = value_t(4)(value=1.5)
    type(default_t(8)) :: override8 = default_t(k=8)(5.25_8)
    type(default_t) :: default4 = default_t(value=6.5)
    type(pair_t(4)) :: pair4 = pair_t(4)(y=2.5, x=1.5)
    type(pair_t(8)) :: pair8 = pair_t(k=8)(y=4.0_8, x=3.0_8)
    ...
    if (kw_value4%value /= 1.5 .or. kind(kw_value4%value) /= 4) error stop
    ...
    if (pair4%x /= 1.5 .or. pair4%y /= 2.5) error stop
    if (pair8%x /= 3.0_8 .or. pair8%y /= 4.0_8) error stop
    if (kind(pair8%x) /= 8 .or. kind(pair8%y) /= 8) error stop
    if (abs(constant8%value - 1.23456789012345_8) > 1.e-14_8) error stop

    value4 = value_t(4)(8.5)
    value8 = value_t(k=8)(9.25_8)
    pair4 = pair_t(4)(y=8.5, x=7.5)
    if (value4%value /= 8.5 .or. value8%value /= 9.25_8) error stop
    if (pair4%x /= 7.5 .or. pair4%y /= 8.5) error stop
end program
