! Passing an assumed-size CHARACTER(LEN=1) array (UnboundedPointerArray) to a
...
contains
  integer function nf_put_att_text_a(text, n) result(status)
    ...
    ! `text` is an assumed-size character array (UnboundedPointerArray); the
    ! call inserts an UnboundedPointerArray -> StringArraySinglePointer cast.
