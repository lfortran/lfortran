...
def run_test(backend, std):
    ...
    if backend == "gfortran":
        ...
        run_cmd(f"FC=lfortran FFLAGS=\"--openmp\" cmake -DLFORTRAN_BACKEND={backend} -DFAST={fast_tests} -DEXP_FAST={exp_fast_tests} {std_string}" + common,
                cwd=cwd)
    elif backend == "fortran":
        run_cmd(f"FC=lfortran cmake -DLFORTRAN_BACKEND={backend} "
            f"-DFAST={fast_tests} -DEXP_FAST={exp_fast_tests} -DCMAKE_Fortran_FLAGS=\"-fPIC\" {std_string}" + common,
                cwd=cwd)
    else:
        run_cmd(f"FC=lfortran cmake -DLFORTRAN_BACKEND={backend} -DFAST={fast_tests} -DEXP_FAST={exp_fast_tests} {std_string}" + common,
                cwd=cwd)
...
def get_args():
    ...
    parser.add_argument("-ef", "--experimental-fast", action='store_true',
                help="Run supported tests with --experimental-fast")
...
def main():
    ...
    # Setup
    global NO_OF_THREADS, fast_tests, exp_fast_tests, std_f23_tests
    ...
    exp_fast_tests = "yes" if args.experimental_fast else "no"
