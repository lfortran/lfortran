...
Different from C++, the larger the number, the higher the precedence. To follow
LFortran's convention, we need to revert the precedence table.
*/
enum julia_prec {
  Base = 2,
  Pow,         // ^
...
static inline bool is_right_associated_julia(int prec) {
  return prec == julia_prec::Pow || prec == julia_prec::Unary ||
         prec >= julia_prec::LogicalAnd;
}
  ...
  switch (t) {
    ...
    default:
      throw LCompilersException(
          "Cannot represent the binary operator as a string");
  }
}

static inline std::string logicalbinop_to_str_julia(
    const ASR::logicalbinopType t) {
  switch (t) {
    ...
    default:
      throw LCompilersException(
          "Cannot represent the boolean operator as a string");
  }
}

static inline std::string cmpop_to_str_julia(const ASR::cmpopType t) {
  switch (t) {
...
class ASRToJuliaVisitor : public ASR::BaseVisitor<ASRToJuliaVisitor> {
 public:
  ...
  std::string format_type(const std::string& type, const std::string& name,
                          bool use_ref, const std::string& default_value = "") {
    std::string fmt;
    if (use_ref) {
    ...
    if (!default_value.empty()) fmt += " = " + default_value;
  ...
  std::string format_dependencies() {
    std::string fmt;
    if (dependencies.empty()) return fmt;

    for (auto& p : dependencies) {
      fmt += "using Main." + p.first + ": ";
      for (auto it = p.second.begin(); it != p.second.end(); it++) {
        fmt += *it;
        if (std::next(it) != p.second.end()) fmt += ", ";
      }
      fmt += "\n";
    }
    fmt += "\n";
  ...
  std::string format_binop(const std::string& left, const std::string& op,
                           const std::string& right, int left_precedence,
                           int right_precedence, bool is_sub = false) {
    std::string out;
  ...
  void generate_array_decl(std::string& sub, std::string v_m_name,
    ...
    if (!init_default) {
      ...
      sub += v_m_name + " = Array{" + type_name + ", " +
             std::to_string(n_dims) + "}(undef, ";
      if (is_allocate) return;
    }
  ...
  std::string convert_variable_decl(const ASR::Variable_t& v,
    ...
    if (ASRUtils::is_pointer(v.m_type)) {
      ...
      if (ASRUtils::is_integer(*v.m_type)) {
        ...
        if (is_array) {
          generate_array_decl(sub, std::string(v.m_name), type_name, dims,
                              t->m_dims, t->n_dims, init_default);
        } else {
          sub = format_type(type_name, v.m_name, use_ref,
                            init_default ? "0" : "");
        }
        ...
        if (is_array) {
          generate_array_decl(sub, std::string(v.m_name), der_type_name, dims,
                              t->m_dims, t->n_dims, init_default);
        } else {
  ...
  std::string get_function_declaration(const ASR::Function_t& x) {
    ...
    if (x.m_return_var) {
      ASR::Variable_t* return_var =
          LFortran::ASRUtils::EXPR2VAR(x.m_return_var);
      if (ASRUtils::is_integer(*return_var->m_type)) {
        ...
        bool is_float =
            ASR::down_cast<ASR::Real_t>(return_var->m_type)->m_kind == 4;
        if (is_float) {
          ret_type = "Float32";
        } else {
          ret_type = "Float64";
        ...
        bool is_float =
            ASR::down_cast<ASR::Complex_t>(return_var->m_type)->m_kind == 4;
        if (is_float) {
          ret_type = "ComplexF32";
        } else {
          ret_type = "ComplexF64";
    ...
    func += ")";
    if (!ret_type.empty()) func += "::" + ret_type;
  ...
  void visit_Function(const ASR::Function_t& x) {
    if (std::string(x.m_name) == "size" && intrinsic_module) {
      ...
      src.clear();
      return;
    } else if ((std::string(x.m_name) == "int" ||
  ...
  void visit_FunctionCall(const ASR::FunctionCall_t& x) {
    ...
    if (x.m_name->type == ASR::symbolType::ExternalSymbol) {
      ASR::ExternalSymbol_t* e =
          ASR::down_cast<ASR::ExternalSymbol_t>(x.m_name);
      dependencies[std::string(e->m_module_name)].insert(
          std::string(e->m_name));
    }
    ...
    if (sym_info[get_hash((ASR::asr_t*)fn)].intrinsic_function) {
      if (fn_name == "size") {
        ...
        // - Convert real to integer (|A| < 1 => 0, |A| >= 1 => largest integer
        // whose absolute value <= |A| with sign of A)
      ...
      for (size_t i = 0; i < x.n_args; i++) {
        ...
        if (ASR::is_a<ASR::Var_t>(*x.m_args[i].m_value)) {
          ...
          if (use_ref && !is_ref) {
            throw CodeGenError(
                "intent(out) and intent(inout) cannot be used in functions "
                "unless the variables passed in are intent(out) or "
                "intent(inout) in the outer scope");
  ...
  void visit_RealBinOp(const ASR::RealBinOp_t& x) { handle_BinOp(x); }

  void visit_ComplexBinOp(const ASR::ComplexBinOp_t& x) { handle_BinOp(x); }
  ...
  void handle_BinOp(const T& x, bool is_integer_binop = false) {
    ...
    src = format_binop(left, op, right, left_precedence, right_precedence,
  ...
  void visit_Allocate(const ASR::Allocate_t& x) {
    ...
    for (size_t i = 0; i < x.n_args; i++) {
      ...
      if (!ASRUtils::is_array(v->m_type)) continue;
      ...
      if (ASRUtils::is_integer(*v->m_type)) {
        ASR::Integer_t* t = ASR::down_cast<ASR::Integer_t>(v->m_type);
        std::string type_name = "Int" + std::to_string(t->m_kind * 8);
        generate_array_decl(out, std::string(v->m_name), type_name, _dims,
                            nullptr, n_dims, true, true);
      } else if (ASRUtils::is_real(*v->m_type)) {
        ASR::Real_t* t = ASR::down_cast<ASR::Real_t>(v->m_type);
        std::string type_name = "Float32";
        if (t->m_kind == 8) type_name = "Float64";
        ...
        ASR::Derived_t* t = ASR::down_cast<ASR::Derived_t>(v->m_type);
        std::string der_type_name = ASRUtils::symbol_name(t->m_derived_type);
        generate_array_decl(out, std::string(v->m_name), der_type_name, _dims,
                            nullptr, n_dims, true, true);
      } else {
        diag.codegen_error_label("Type number '" +
                                     std::to_string(v->m_type->type) +
    ...
    for (size_t i = 0; i < x.n_body; i++) {
      ...
      if (stmt->type == ASR::case_stmtType::CaseStmt) {
        ...
        if (left.empty() && right.empty()) {
          diag.codegen_error_label("Empty range in select statement",
                                   {x.base.base.loc}, "");
          throw Abort();
        }
        if (left.empty()) {
  ...
  void visit_DoLoop(const ASR::DoLoop_t& x, bool concurrent = false) {
    ...
    if (!c) {
      ...
      if (c->type == ASR::exprType::IntegerConstant) {
        ...
        ASR::IntegerUnaryMinus_t* ium =
            ASR::down_cast<ASR::IntegerUnaryMinus_t>(c);
    ...
    visit_expr(*a);
    out +=
        src + ":" + (increment == 1 ? "" : (std::to_string(increment) + ":"));
    visit_expr(*b);
    out += src + "\n";
    indentation_level += 1;
    for (size_t i = 0; i < x.n_body; i++) {
  ...
  void visit_DoConcurrentLoop(const ASR::DoConcurrentLoop_t& x) {
    const ASR::DoLoop_t do_loop =
        ASR::DoLoop_t{x.base, x.m_head, x.m_body, x.n_body};
  ...
  void visit_Var(const ASR::Var_t& x) {
    ...
    if ((sv->m_intent == ASRUtils::intent_in ||
      ...
      src =
          "(*" + std::string(ASR::down_cast<ASR::Variable_t>(s)->m_name) + ")";
    } else {
  ...
  void visit_Cast(const ASR::Cast_t& x) {
    ...
    switch (x.m_kind) {
      ...
      case (ASR::cast_kindType::RealToInteger): {
        int dest_kind = ASRUtils::extract_kind_from_ttype_t(x.m_type);
        src =
            "Int" + std::to_string(dest_kind * 8) + broadcast + "(" + src + ")";
        break;
      }
      ...
      default:
        throw CodeGenError(
            "Cast kind " + std::to_string(x.m_kind) + " not implemented",
            x.base.base.loc);
    }
    last_expr_precedence = julia_prec::Base;
  }
  ...
  void visit_RealCompare(const ASR::RealCompare_t& x) { handle_Compare(x); }
  ...
  void visit_StringCompare(const ASR::StringCompare_t& x) { handle_Compare(x); }
...
Result<std::string> asr_to_julia(Allocator& al, ASR::TranslationUnit_t& asr,
                                 diag::Diagnostics& diag) {
