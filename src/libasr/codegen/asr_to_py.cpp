 ...
 *  - a .pyx file, which is a Cython file that includes the actual python-callable wrapper
 * functions.
 *
...
public:
    ...
    // Are we assuming arrays to be in C order (row-major)? If not, assume Fortran order
    // (column-major).
    bool c_order;
    ...
    ASRToPyVisitor(bool c_order_, std::string chdr_filename_)
        ...
        , pxdf(chdr_filename_)
    {
        // we need to get get the pxd filename (minus extension), so we can import it in the pyx
        // file knock off ".h" from the c header filename
        pxdf.erase(--pxdf.end());
        pxdf.erase(--pxdf.end());
        // this is an unfortuante hack, but we have to add something so that the pxd and pyx
        // filenames are different (beyond just their extensions). If we don't, the cython emits a
        // warning.
    ...
    helper_visit_arguments(size_t n_args, ASR::expr_t** args)
    {
        ...
        /* get_arg_infos */ for (size_t i = 0; i < n_args; i++) {
            ...
            const char* errmsg1
                = "pywrap does not yet support array dummy arguments with lower bounds other than 1.";
            const char* errmsg2
...
#undef _X
        ...
        /* build_return_strings */ for (auto it = arg_infos.begin(); it != arg_infos.end(); it++) {
            ...
            if (!c.empty() && !c_wip.empty())
                c += ", ";
            if (!fargs.empty() && !fargs_wip.empty())
                fargs += ", ";
            if (!cyargs.empty() && !cyargs_wip.empty())
                cyargs += ", ";
...
#undef _X
        ...
        pyx += pyx_body;
        pyx += "    cdef " + rtnvar_type + " " + rtnvar_name + " = " + pxdf + "." + effective_name
               + " (" + call_args + ")\n";
        pyx += rtn_statement + "\n\n";
    }
};
