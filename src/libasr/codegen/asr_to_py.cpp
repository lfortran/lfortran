...
/*
 *
 * This back-end generates wrapper code that allows Fortran to automatically be
 * called from Python. It also generates a C header file, so I suppose it
 * indirectly generates C wrappers as well. Currently, it outputs Cython, rather
 * than the Python C API directly - much easier to implement. The actual output
 * files are:
 *  - a .h file, containing C-language function declarations *
 *  - a .pxd file, basically containing the same information as the .h file, but
 * in Cython's format.
 *  - a .pyx file, which is a Cython file that includes the actual
 * python-callable wrapper functions.
 *
 *  Currently, this back-end only wraps functions that are marked "bind (c)" in
 * the Fortran source. At some later point we will offer the functionality to
 * generate bind (c) wrapper functions for normal Fortran subprograms, but for
 * now, we don't offer this functionality.
 *
 *  --- H. Snyder, Aug 2021
 *
 * */
...
class ASRToPyVisitor : public ASR::BaseVisitor<ASRToPyVisitor> {
 public:
  // These store the strings that will become the contents of the generated .h,
  // .pxd, .pyx files
  ...
  // Are we assuming arrays to be in C order (row-major)? If not, assume Fortran
  // order (column-major).
  bool c_order;

  // What's the file name of the C header file we're going to generate? (needed
  // for the .pxd)
  ...
  ASRToPyVisitor(bool c_order_, std::string chdr_filename_)
      : c_order(c_order_), chdr_filename(chdr_filename_), pxdf(chdr_filename_) {
    // we need to get get the pxd filename (minus extension), so we can import
    // it in the pyx file knock off ".h" from the c header filename
    pxdf.erase(--pxdf.end());
    pxdf.erase(--pxdf.end());
    // this is an unfortuante hack, but we have to add something so that the pxd
    // and pyx filenames are different (beyond just their extensions). If we
    // don't, the cython emits a warning.
    // TODO we definitely need to change this somehow because right now this
    // "append _pxd" trick exists in two places (bin/lfortran.cpp, and here),
    // which could easily cause breakage.
    pxdf += "_pxd";
  }
  ...
  helper_visit_arguments(size_t n_args, ASR::expr_t **args) {
    ...
    /* get_arg_infos */ for (size_t i = 0; i < n_args; i++) {
      ...
      const char *errmsg1 =
          "pywrap does not yet support array dummy arguments with lower bounds "
          "other than 1.";
      const char *errmsg2 =
          "pywrap can only generate wrappers for array dummy arguments "
          "if the upper bound is a constant integer, or another (scalar) dummy "
          "argument.";

// Generate a sequence of if-blocks to determine the type, using the type list
// defined above
#define _X(ASR_TYPE, KIND, CTYPE_STR)                                \
  if (is_a<ASR_TYPE>(*arg->m_type) &&                                \
    ...
    for (int j = 0; j < this_arg_info.ndims; j++) {                  \
      ...
      if (is_a<ASR::Var_t>(*tmp_arg->m_dims[j].m_length)) {          \
        ASR::Variable_t *dimvar =                                    \
            ASRUtils::EXPR2VAR(tmp_arg->m_dims[j].m_length);         \
...
#undef _X
    ...
    /* mark_array_bound_vars */ for (auto arg_iter = arg_infos.begin();
                                     arg_iter != arg_infos.end(); arg_iter++) {
      ...
      We don't actually want `n` in the python wrapper's arguments - the Python
      programmer shouldn't need to explicitly pass sizes. From the get_arg_infos
      block, we already have the mapping from `x` to `n`, but we also need the
      opposite - we need be able to look at `n` and know that it's related to
      `x`. So let's do a pass over the arg_infos list and assemble that
      information.

      */

      for (auto bound_iter = arg_iter->ubound_varnames.begin();
           bound_iter != arg_iter->ubound_varnames.end(); bound_iter++) {
        for (unsigned int j = 0; j < arg_infos.size(); j++) {
          if (0 ==
              std::string(arg_infos[j].asr_obj->m_name).compare(*bound_iter)) {
            arg_infos[j].i_am_ubound_of.push_back(
                std::make_pair(arg_iter->asr_obj->m_name, j));
          }
        }
      }
    ...
    /* apply_c_order */ if (c_order) {
      for (auto arg_iter = arg_infos.begin(); arg_iter != arg_infos.end();
           arg_iter++) {
        for (auto bound = arg_iter->i_am_ubound_of.begin();
             bound != arg_iter->i_am_ubound_of.end(); bound++) {
    ...
    /* build_return_strings */ for (auto it = arg_infos.begin();
                                    it != arg_infos.end(); it++) {
      ...
      if (it->ndims > 0) {
        ...
        std::string strndims =
            it->ndims > 1 ? ", ndim=" + std::to_string(it->ndims) : "";
        cyargs_wip += "ndarray[" + it->ctype + strndims + mode + "]";
      } else {
        cyargs_wip += it->ctype;
      }

      // Fortran defaults to pass-by-reference, so the C argument is a pointer,
      // unless it is not an array AND it has the value type.
      if (it->ndims > 0 || !it->asr_obj->m_value_attr) {
        c_wip += " *";
        fargs_wip = "&";
        // If the argument is intent(in) and a pointer, it should be a
        // ptr-to-const.
        if (ASR::intentType::In == it->asr_obj->m_intent)
          c_wip = "const " + c_wip;
      }
      ...
      if (it->ndims > 0) {
        fargs_wip += "[0";
        for (int h = 1; h < it->ndims; h++) fargs_wip += ",0";
        fargs_wip += "]";
      }
      ...
      if (!it->i_am_ubound_of.empty()) {
        ...
        pyxbody += " = ";
        pyxbody += i_am_ubound_of.first + ".shape[" +
        ...
        for (unsigned int k = 1; k < it->i_am_ubound_of.size(); k++) {
          ...
          pyxbody += "    assert(" + i_am_ubound_of_k.first + ".shape[" +
                     std::to_string(i_am_ubound_of_k.second) +
                     "] == " + i_am_ubound_of.first + ".shape[" +
                     std::to_string(i_am_ubound_of.second) + "])\n";
        }
      }
      ...
      if (!return_statement.empty() && !rtn_wip.empty())
        return_statement += ", ";
  ...
  void visit_TranslationUnit(const ASR::TranslationUnit_t &x) {
    ...
    pyx_tmp +=
        "from numpy cimport import_array, ndarray, int8_t, int16_t, int32_t, "
        "int64_t\n";
  ...
  void visit_Function(const ASR::Function_t &x) {
    ...
    bool bindc_name_not_given =
        x.m_bindc_name == NULL || !strcmp("", x.m_bindc_name);
    std::string effective_name =
        bindc_name_not_given ? x.m_name : x.m_bindc_name;

    ASR::Variable_t *rtnvar = ASRUtils::EXPR2VAR(x.m_return_var);
    std::string rtnvar_type;
#define _X(ASR_TYPE, KIND, CTYPE_STR)                          \
  if (is_a<ASR_TYPE>(*rtnvar->m_type) &&                       \
      (down_cast<ASR_TYPE>(rtnvar->m_type)->m_kind == KIND)) { \
...
#undef _X
    ...
    std::tie(c_args, cy_args, call_args, pyx_body, rtn_statement) =
        helper_visit_arguments(x.n_args, x.m_args);
    ...
    pyx += pyx_body;
    pyx += "    cdef " + rtnvar_type + " " + rtnvar_name + " = " + pxdf + "." +
           effective_name + " (" + call_args + ")\n";
    pyx += rtn_statement + "\n\n";
  }
};
