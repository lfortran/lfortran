...
static std::string m2s(X86Reg *base, X86Reg *index, uint8_t scale,
                       int32_t disp) {
  std::string r;
  r = "[";
  if (base) r += r2s(*base);
...
static std::string i2s(uint32_t imm32) { return "0x" + hexify(imm32); }

static std::string i2s(uint16_t imm16) { return "0x" + hexify(imm16); }
...
static void ModRM_SIB_disp_bytes(Vec<uint8_t> &code, Allocator &al, uint8_t mod,
                                 uint8_t reg, uint8_t rm, uint8_t base,
                                 uint8_t index, uint8_t scale_index,
                                 int32_t disp) {
...
static void modrm_sib_disp(Vec<uint8_t> &code, Allocator &al, X86Reg reg,
  ...
  if (mem) {
    ...
    if (base_opt) {
      ...
      throw AssemblerError(
          "base_opt or index_opt must be supplied if mem=true");
    }
...
class X86Assembler {
 ...
 public:
...
#ifdef LFORTRAN_ASM_PRINT
  std::string get_asm() { return m_asm_code; }
  ...
  Vec<uint8_t> &get_machine_code() { return m_code; }
  ...
  uint32_t pos() { return m_origin + m_code.size(); }

  uint32_t origin() { return m_origin; }
  ...
  void asm_jne_imm8(uint8_t imm8) { asm_jnz_imm8(imm8); }
  ...
  void asm_mov_r32_m32(X86Reg r32, X86Reg *base, X86Reg *index, uint8_t scale,
                       int32_t disp) {
  ...
  void asm_mov_m32_r32(X86Reg *base, X86Reg *index, uint8_t scale, int32_t disp,
                       X86Reg r32) {
    if (r32 == X86Reg::eax && !base && !index) {
  ...
  void asm_add_m32_r32(X86Reg *base, X86Reg *index, uint8_t scale, int32_t disp,
                       X86Reg r32) {
  ...
  void asm_lea_r32_m32(X86Reg r32, X86Reg *base, X86Reg *index, uint8_t scale,
                       int32_t disp) {
    m_code.push_back(m_al, 0x8d);
    modrm_sib_disp(m_code, m_al, r32, base, index, scale, disp, true);
...
void emit_exit(X86Assembler &a, const std::string &name, uint32_t exit_code);
void emit_data_string(X86Assembler &a, const std::string &label,
                      const std::string &s);
void emit_print(X86Assembler &a, const std::string &msg_label, uint32_t size);
