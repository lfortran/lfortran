...
r2s(X86Reg r32)
{
    switch (r32) {
        ...
        case (X86Reg::edi):
            return "edi";
        default:
            throw AssemblerError("Unknown instruction");
    }
}

static std::string
m2s(X86Reg* base, X86Reg* index, uint8_t scale, int32_t disp)
{
    std::string r;
    r = "[";
    if (base)
...
static void
push_back_uint32(Vec<uint8_t>& code, Allocator& al, uint32_t i32)
{
...
static void
insert_uint32(Vec<uint8_t>& code, size_t pos, uint32_t i32)
{
    code.p[pos] = (i32) &0xFF;
...
static void
push_back_uint16(Vec<uint8_t>& code, Allocator& al, uint16_t i16)
{
...
static void
insert_uint16(Vec<uint8_t>& code, size_t pos, uint16_t i16)
{
    code.p[pos] = (i16) &0xFF;
...
static uint8_t
ModRM_byte(uint8_t mode, uint8_t reg, uint8_t rm)
{
...
static uint8_t
SIB_byte(uint8_t base, uint8_t index, uint8_t scale_index)
{
...
static void
ModRM_SIB_disp_bytes(Vec<uint8_t>& code,
                     Allocator& al,
                     uint8_t mod,
                     uint8_t reg,
                     uint8_t rm,
...
static void
modrm_sib_disp(Vec<uint8_t>& code,
               Allocator& al,
               X86Reg reg,
               ...
               bool mem)
{
    ...
    switch (scale) {
        ...
        case (8):
            ...
            break;
        default:
            throw AssemblerError("Scale must be one of [1, 2, 4, 8]");
    }

    ModRM_SIB_disp_bytes(code, al, mod, reg, rm, base, index, scale_index, disp);
}
...
public:
        ...
        if (!s.defined) {
            switch (type) {
                ...
                case (2):
                    ...
                    break;
                default:
                    throw AssemblerError("Unknown label type");
            }
        }
        return s;
    }
    ...
    void asm_inc_m32(X86Reg* base, X86Reg* index, uint8_t scale, int32_t disp)
    {
        ...
        modrm_sib_disp(m_code, m_al, X86Reg::eax, base, index, scale, disp, true);
    ...
    void asm_xor_r32_r32(X86Reg r32, X86Reg s32)
    {
        ...
        modrm_sib_disp(m_code, m_al, s32, &r32, nullptr, 1, 0, false);
    ...
    void asm_mov_r32_m32(X86Reg r32, X86Reg* base, X86Reg* index, uint8_t scale, int32_t disp)
    {
        if (r32 == X86Reg::eax && !base && !index) {
            ...
            modrm_sib_disp(m_code, m_al, r32, base, index, scale, disp, true);
        }
        EMIT("mov " + r2s(r32) + ", " + m2s(base, index, scale, disp));
    }

    void asm_mov_m32_r32(X86Reg* base, X86Reg* index, uint8_t scale, int32_t disp, X86Reg r32)
    {
        if (r32 == X86Reg::eax && !base && !index) {
    ...
    void asm_sub_r32_imm8(X86Reg r32, uint8_t imm8)
    {
        ...
        modrm_sib_disp(m_code, m_al, X86Reg::ebp, &r32, nullptr, 1, 0, false);
        m_code.push_back(m_al, imm8);
    ...
    void asm_add_m32_r32(X86Reg* base, X86Reg* index, uint8_t scale, int32_t disp, X86Reg r32)
    {
    ...
    {
        m_code.push_back(m_al, 0x83);
        modrm_sib_disp(m_code, m_al, X86Reg::eax, &r32, nullptr, 1, 0, false);
        m_code.push_back(m_al, imm8);
    ...
    {
        m_code.push_back(m_al, 0xF7);
        modrm_sib_disp(m_code, m_al, X86Reg::esi, &r32, nullptr, 1, 0, false);
        EMIT("div " + r2s(r32));
    }
    ...
    {
        m_code.push_back(m_al, 0xF7);
        modrm_sib_disp(m_code, m_al, X86Reg::ebx, &r32, nullptr, 1, 0, false);
        EMIT("neg " + r2s(r32));
    }

    void asm_lea_r32_m32(X86Reg r32, X86Reg* base, X86Reg* index, uint8_t scale, int32_t disp)
    {
        m_code.push_back(m_al, 0x8d);
        modrm_sib_disp(m_code, m_al, r32, base, index, scale, disp, true);
...
void
emit_elf32_header(X86Assembler& a, uint32_t p_flags = 5);
void
emit_elf32_footer(X86Assembler& a);

void
emit_exit(X86Assembler& a, const std::string& name, uint32_t exit_code);
void
emit_data_string(X86Assembler& a, const std::string& label, const std::string& s);
void
emit_print(X86Assembler& a, const std::string& msg_label, uint32_t size);
void
emit_print_int(X86Assembler& a, const std::string& name);
