    ...
    llvm::Value* LLVMUtils::declare_constant_stringArray(Allocator &/*al*/, const ASR::ArrayConstant_t* arr_const){
        ...
        */
        ...
        if(elem_string->m_physical_type == ASR::DescriptorString){
            // Create the character array type [total_len * kind x i8]
            llvm::ArrayType *char_array_type = llvm::ArrayType::get(llvm::Type::getInt8Ty(context), total_len * kind);

            llvm::Constant *const_data_as_array {};
            if (kind == 1) {
                sequence.resize(total_len, ' ');
                const_data_as_array = llvm::ConstantDataArray::getString(context, sequence, false);
            } else if (kind == 2 || kind == 4) {
                std::vector<uint8_t> bytes = utf8_to_unicode_bytes(sequence, kind);
                size_t orig_count = bytes.size() / kind;
                bytes.resize(total_len * kind, 0x00);
                // Pad remaining slots with space character (0x20)
                for (size_t idx = orig_count; idx < (size_t)total_len; ++idx) {
                ...
                sequence.resize(total_len * kind, ' ');
                const_data_as_array = llvm::ConstantDataArray::getString(context, sequence, false);
            }
