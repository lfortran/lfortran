...
//TODO :: Remove Code Above
    ...
    llvm::Value* LLVMUtils::declare_string_constant(const ASR::StringConstant_t* str_const){
        return declare_global_string(
            ASRUtils::get_string_type(str_const->m_type), str_const->m_s, "string_const");
    }
    ...
    llvm::Value* LLVMUtils::declare_constant_stringArray(const ASR::ArrayConstant_t* arr_const){
        ...
        return declare_global_string(
            ASRUtils::get_string_type(arr_const->m_type), (char*)arr_const->m_data, "stringArray_const");
    }
    
    llvm::Value* LLVMUtils::declare_global_string(
    ASR::String_t* str, char* initial_data, std::string name){
        ...
        if(str->m_len_kind == ASR::DeferredLength){
            ...
            if(initial_data) {
                string_constant = builder->CreateGlobalStringPtr(initial_data);
            } else {
                string_constant = builder->CreateGlobalStringPtr(std::string(" ", len));
            }
        }
        switch(str->m_physical_type) {
            case ASR::DescriptorString:{
                ...
                llvm::GlobalVariable* global_string_desc = new llvm::GlobalVariable(
                        *module,
                        ...
                        string_descriptor_constant, name);
    ...
    void LLVMList::list_deepcopy(llvm::Value* src, llvm::Value* dest,
        ...
        llvm::Value* src_data = llvm_utils->CreateLoad2(el_type->getPointerTo(), get_pointer_to_list_data(src));
    ...
    void LLVMDict::rehash(llvm::Value* dict, llvm::Module* module,
        ...
        std::map<std::string, std::map<std::string, int>>& name2memidx) {
        llvm::Value* capacity_ptr = get_pointer_to_capacity(dict);
        llvm::Value* old_capacity = llvm_utils->CreateLoad(capacity_ptr);
        ...
        llvm_utils->start_new_block(loopbody);
        {
            ...
            llvm::BasicBlock *elseBB = llvm::BasicBlock::Create(context, "else");
            llvm::BasicBlock *mergeBB = llvm::BasicBlock::Create(context, "ifcont");
            llvm::Value* is_key_set = llvm_utils->CreateLoad(llvm_utils->create_ptr_gep(key_mask, idx));
    ...
    void LLVMSetLinearProbing::resolve_collision_for_write(
        ...
        linear_prob_happened = builder->CreateOr(linear_prob_happened,
            builder->CreateICmpEQ(
                llvm_utils->CreateLoad(llvm_utils->create_ptr_gep(el_mask, el_hash)),
                llvm::ConstantInt::get(llvm::Type::getInt8Ty(context), llvm::APInt(8, 2)
            ))
        );
        llvm::Value* set_max_2 = builder->CreateSelect(linear_prob_happened,
    ...
    void LLVMSetLinearProbing::rehash(
        ...
        llvm_utils->start_new_block(loopbody);
        {
            ...
            llvm::BasicBlock *elseBB = llvm::BasicBlock::Create(context, "else");
            llvm::BasicBlock *mergeBB = llvm::BasicBlock::Create(context, "ifcont");
            llvm::Value* is_el_set = llvm_utils->CreateLoad(llvm_utils->create_ptr_gep(el_mask, idx));
    ...
    llvm::Value* LLVMSetInterface::len(llvm::Value* set) {
        return llvm_utils->CreateLoad(get_pointer_to_occupancy(set));
    }
