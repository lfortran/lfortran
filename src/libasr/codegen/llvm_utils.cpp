...
llvm::Value* CreateStore(llvm::IRBuilder<>& builder, llvm::Value* x,
                         llvm::Value* y) {
  LFORTRAN_ASSERT(y->getType()->isPointerTy());
  return builder.CreateStore(x, y);
}

llvm::Value* CreateGEP(llvm::IRBuilder<>& builder, llvm::Value* x,
...
llvm::Value* CreateInBoundsGEP(llvm::IRBuilder<>& builder, llvm::Value* x,
                               std::vector<llvm::Value*>& idx) {
  llvm::Type* t = x->getType();
  LFORTRAN_ASSERT(t->isPointerTy());
  llvm::Type* t2 = t->getContainedType(0);
...
llvm::Value* lfortran_calloc(llvm::LLVMContext& context, llvm::Module& module,
                             llvm::IRBuilder<>& builder, llvm::Value* count,
                             llvm::Value* type_size) {
...
LLVMUtils::LLVMUtils(llvm::LLVMContext& context, llvm::IRBuilder<>* _builder)
    : context(context),
...
llvm::Value* LLVMUtils::create_gep(llvm::Value* ds, llvm::Value* idx) {
  std::vector<llvm::Value*> idx_vec = {
      llvm::ConstantInt::get(context, llvm::APInt(32, 0)), idx};
  return LLVM::CreateGEP(*builder, ds, idx_vec);
}
...
llvm::Value* LLVMUtils::lfortran_str_cmp(llvm::Value* left_arg,
...
llvm::Value* LLVMUtils::is_equal_by_value(llvm::Value* left, llvm::Value* right,
  ...
  switch (asr_type->type) {
    ...
    case ASR::ttypeType::Character: {
      if (!are_iterators_set) {
        str_cmp_itr =
            builder->CreateAlloca(llvm::Type::getInt32Ty(context), nullptr);
      }
      llvm::Value* null_char = llvm::ConstantInt::get(
      ...
      llvm::BasicBlock* loophead =
          llvm::BasicBlock::Create(context, "loop.head");
      ...
      start_new_block(loophead);
      {
        ...
        llvm::Value* cond =
            builder->CreateAnd(builder->CreateICmpNE(l, null_char),
    ...
    default: {
      throw LCompilersException(
          "LLVMUtils::is_equal_by_value isn't implemented for " +
...
LLVMList::LLVMList(llvm::LLVMContext& context_, LLVMUtils* llvm_utils_,
                   llvm::IRBuilder<>* builder_)
    : context(context_),
      llvm_utils(std::move(llvm_utils_)),
      builder(std::move(builder_)) {}

LLVMDict::LLVMDict(llvm::LLVMContext& context_, LLVMUtils* llvm_utils_,
                   llvm::IRBuilder<>* builder_)
    : context(context_),
      llvm_utils(std::move(llvm_utils_)),
      builder(std::move(builder_)),
...
LLVMDictOptimizedLinearProbing::LLVMDictOptimizedLinearProbing(
    llvm::LLVMContext& context_, LLVMUtils* llvm_utils_,
    llvm::IRBuilder<>* builder_)
...
llvm::Type* LLVMList::get_list_type(llvm::Type* el_type, std::string& type_code,
  ...
  llvm::StructType* list_desc =
      llvm::StructType::create(context, list_type_vec, "list");
  typecode2listtype[type_code] = std::make_tuple(list_desc, type_size, el_type);
  return list_desc;
}

llvm::Type* LLVMDict::get_dict_type(std::string key_type_code,
  ...
  std::pair<std::string, std::string> llvm_key =
      std::make_pair(key_type_code, value_type_code);
  if (typecode2dicttype.find(llvm_key) != typecode2dicttype.end()) {
    return std::get<0>(typecode2dicttype[llvm_key]);
  }

  llvm::Type* key_list_type = llvm_utils->list_api->get_list_type(
      key_type, key_type_code, key_type_size);
  llvm::Type* value_list_type = llvm_utils->list_api->get_list_type(
      value_type, value_type_code, value_type_size);
  std::vector<llvm::Type*> dict_type_vec = {llvm::Type::getInt32Ty(context),
                                            key_list_type, value_list_type,
                                            llvm::Type::getInt8PtrTy(context)};
  llvm::Type* dict_desc =
      llvm::StructType::create(context, dict_type_vec, "dict");
  typecode2dicttype[llvm_key] =
      std::make_tuple(dict_desc, std::make_pair(key_type_size, value_type_size),
...
void LLVMList::list_init(std::string& type_code, llvm::Value* list,
                         llvm::Module& module, int32_t initial_capacity,
                         int32_t n) {
  if (typecode2listtype.find(type_code) == typecode2listtype.end()) {
  ...
  llvm::Value* arg_size = llvm::ConstantInt::get(
      context, llvm::APInt(32, type_size * initial_capacity));

  llvm::Value* list_data =
  ...
  llvm::Value* current_end_point =
      llvm::ConstantInt::get(context, llvm::APInt(32, n));
  llvm::Value* current_capacity =
      llvm::ConstantInt::get(context, llvm::APInt(32, initial_capacity));
  builder->CreateStore(current_end_point,
...
void LLVMDict::dict_init(std::string key_type_code, std::string value_type_code,
  ...
  size_t mask_size =
      data_layout.getTypeAllocSize(llvm::Type::getInt8Ty(context));
  llvm::Value* llvm_capacity = llvm::ConstantInt::get(
      llvm::Type::getInt32Ty(context), llvm::APInt(32, initial_capacity));
  llvm::Value* llvm_mask_size = llvm::ConstantInt::get(
      llvm::Type::getInt32Ty(context), llvm::APInt(32, mask_size));
  llvm::Value* key_mask = LLVM::lfortran_calloc(context, *module, *builder,
                                                llvm_capacity, llvm_mask_size);
...
void LLVMList::list_deepcopy(llvm::Value* src, llvm::Value* dest,
  ...
  llvm::Value* src_end_point =
      LLVM::CreateLoad(*builder, get_pointer_to_current_end_point(src));
  llvm::Value* src_capacity =
      LLVM::CreateLoad(*builder, get_pointer_to_current_capacity(src));
  ...
  llvm::Value* src_data =
      LLVM::CreateLoad(*builder, get_pointer_to_list_data(src));
  int32_t type_size = std::get<1>(typecode2listtype[src_type_code]);
  llvm::Value* arg_size = builder->CreateMul(
      llvm::ConstantInt::get(context, llvm::APInt(32, type_size)),
      src_capacity);
  llvm::Value* copy_data =
      LLVM::lfortran_malloc(context, module, *builder, arg_size);
  ...
  // which may also contain non-trivial structs (such as in case of
  // list[list[f64]], list[tuple[f64]]). We need to make sure that all the data
  // inside those structs is deepcopied and not just the address of the first
  // element of those structs. Hence we dive deeper into the lowest level of
  // nested types and deepcopy everything properly. If we don't consider this
  // case then the data only from first level of nested types will be deep
  // copied and rest will be shallow copied. The importance of this case can be
  // figured out by goind through, integration_tests/test_list_06.py and
  ...
  if (LLVM::is_llvm_struct(element_type)) {
    ...
    llvm_utils->start_new_block(loophead);
    {
      llvm::Value* cond = builder->CreateICmpSGT(
          src_end_point, LLVM::CreateLoad(*builder, pos_ptr));
      builder->CreateCondBr(cond, loopbody, loopend);
    }
    ...
    llvm_utils->start_new_block(loopbody);
    {
      ...
      llvm::Value* tmp = builder->CreateAdd(
          pos, llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
...
void LLVMDict::dict_deepcopy(llvm::Value* src, llvm::Value* dest,
  ...
  LFORTRAN_ASSERT(src->getType() == dest->getType());
  llvm::Value* src_occupancy =
      LLVM::CreateLoad(*builder, get_pointer_to_occupancy(src));
  ...
  llvm::Value* src_key_mask =
      LLVM::CreateLoad(*builder, get_pointer_to_keymask(src));
  ...
  llvm::Value* llvm_mask_size = llvm::ConstantInt::get(
      llvm::Type::getInt32Ty(context), llvm::APInt(32, mask_size));
  llvm::Value* src_capacity =
      LLVM::CreateLoad(*builder, get_pointer_to_capacity(src));
  llvm::Value* dest_key_mask = LLVM::lfortran_calloc(
      context, *module, *builder, src_capacity, llvm_mask_size);
  builder->CreateMemCpy(dest_key_mask, llvm::MaybeAlign(), src_key_mask,
                        llvm::MaybeAlign(),
                        builder->CreateMul(src_capacity, llvm_mask_size));
...
void LLVMList::write_item(llvm::Value* list, llvm::Value* pos,
  ...
  llvm::Value* list_data =
      LLVM::CreateLoad(*builder, get_pointer_to_list_data(list));
...
void LLVMDict::set_iterators() {
  ...
  LLVM::CreateStore(*builder,
                    llvm::ConstantInt::get(llvm::Type::getInt32Ty(context),
                                           llvm::APInt(32, 0)),
                    idx_ptr);
  hash_value = builder->CreateAlloca(llvm::Type::getInt64Ty(context), nullptr,
                                     "hash_value");
  LLVM::CreateStore(*builder,
                    llvm::ConstantInt::get(llvm::Type::getInt64Ty(context),
                                           llvm::APInt(64, 0)),
                    hash_value);
  hash_iter = builder->CreateAlloca(llvm::Type::getInt64Ty(context), nullptr,
                                    "hash_iter");
  LLVM::CreateStore(*builder,
                    llvm::ConstantInt::get(llvm::Type::getInt64Ty(context),
                                           llvm::APInt(64, 0)),
                    hash_iter);
  polynomial_powers =
      builder->CreateAlloca(llvm::Type::getInt64Ty(context), nullptr, "p_pow");
  LLVM::CreateStore(*builder,
                    llvm::ConstantInt::get(llvm::Type::getInt64Ty(context),
                                           llvm::APInt(64, 1)),
                    polynomial_powers);
...
void LLVMDict::resolve_collision(llvm::Value* capacity, llvm::Value* key_hash,
  ...
  if (!are_iterators_set) {
    pos_ptr = builder->CreateAlloca(llvm::Type::getInt32Ty(context), nullptr);
    is_key_matching_var =
        builder->CreateAlloca(llvm::Type::getInt1Ty(context), nullptr);
  }
  ...
  {
    llvm::Value* pos = LLVM::CreateLoad(*builder, pos_ptr);
    llvm::Value* key_mask_value =
        LLVM::CreateLoad(*builder, llvm_utils->create_ptr_gep(key_mask, pos));
    llvm::Value* is_key_skip = builder->CreateICmpEQ(
        key_mask_value, llvm::ConstantInt::get(llvm::Type::getInt8Ty(context),
                                               llvm::APInt(8, 3)));
    llvm::Value* is_key_set = builder->CreateICmpNE(
        key_mask_value, llvm::ConstantInt::get(llvm::Type::getInt8Ty(context),
                                               llvm::APInt(8, 0)));
    llvm::Value* is_key_matching = llvm::ConstantInt::get(
        llvm::Type::getInt1Ty(context), llvm::APInt(1, 0));
    LLVM::CreateStore(*builder, is_key_matching, is_key_matching_var);
    llvm::Function* fn = builder->GetInsertBlock()->getParent();
    llvm::BasicBlock* thenBB = llvm::BasicBlock::Create(context, "then", fn);
    llvm::BasicBlock* elseBB = llvm::BasicBlock::Create(context, "else");
    llvm::BasicBlock* mergeBB = llvm::BasicBlock::Create(context, "ifcont");
    llvm::Value* compare_keys =
        builder->CreateAnd(is_key_set, builder->CreateNot(is_key_skip));
    builder->CreateCondBr(compare_keys, thenBB, elseBB);
    builder->SetInsertPoint(thenBB);
    {
      llvm::Value* original_key = llvm_utils->list_api->read_item(
          key_list, pos, LLVM::is_llvm_struct(key_asr_type), false);
    ...
    if (for_read) {
      cond = builder->CreateAnd(
          is_key_set,
          builder->CreateNot(LLVM::CreateLoad(*builder, is_key_matching_var)));
  ...
  {
    llvm::Value* pos = LLVM::CreateLoad(*builder, pos_ptr);
    pos = builder->CreateAdd(
        pos, llvm::ConstantInt::get(llvm::Type::getInt32Ty(context),
                                    llvm::APInt(32, 1)));
...
void LLVMDictOptimizedLinearProbing::resolve_collision(
    llvm::Value* capacity, llvm::Value* key_hash, llvm::Value* key,
    llvm::Value* key_list, llvm::Value* key_mask, llvm::Module& module,
    ASR::ttype_t* key_asr_type, bool for_read) {
  if (!are_iterators_set) {
    if (!for_read) {
      pos_ptr = builder->CreateAlloca(llvm::Type::getInt32Ty(context), nullptr);
...
void LLVMDict::resolve_collision_for_write(llvm::Value* dict,
                                           llvm::Value* key_hash,
  ...
  llvm::Value* value_list = get_value_list(dict);
  llvm::Value* key_mask =
      LLVM::CreateLoad(*builder, get_pointer_to_keymask(dict));
  llvm::Value* capacity =
      LLVM::CreateLoad(*builder, get_pointer_to_capacity(dict));
  this->resolve_collision(capacity, key_hash, key, key_list, key_mask, module,
                          key_asr_type);
  llvm::Value* pos = LLVM::CreateLoad(*builder, pos_ptr);
  llvm_utils->list_api->write_item(key_list, pos, key, key_asr_type, module,
                                   false);
  llvm_utils->list_api->write_item(value_list, pos, value, value_asr_type,
                                   module, false);
  llvm::Value* key_mask_value =
      LLVM::CreateLoad(*builder, llvm_utils->create_ptr_gep(key_mask, pos));
  llvm::Value* is_slot_empty = builder->CreateICmpEQ(
      key_mask_value, llvm::ConstantInt::get(llvm::Type::getInt8Ty(context),
                                             llvm::APInt(8, 0)));
...
void LLVMDictOptimizedLinearProbing::resolve_collision_for_write(
    llvm::Value* dict, llvm::Value* key_hash, llvm::Value* key,
    llvm::Value* value, llvm::Module& module, ASR::ttype_t* key_asr_type,
    ASR::ttype_t* value_asr_type) {
  llvm::Value* key_list = get_key_list(dict);
  llvm::Value* value_list = get_value_list(dict);
  llvm::Value* key_mask =
  ...
  linear_prob_happened = builder->CreateOr(
      linear_prob_happened,
      builder->CreateICmpEQ(
          LLVM::CreateLoad(*builder,
                           llvm_utils->create_ptr_gep(key_mask, key_hash)),
          llvm::ConstantInt::get(llvm::Type::getInt8Ty(context),
                                 llvm::APInt(8, 2))));
  llvm::Value* set_max_2 = builder->CreateSelect(
      linear_prob_happened,
      llvm::ConstantInt::get(llvm::Type::getInt8Ty(context), llvm::APInt(8, 2)),
      llvm::ConstantInt::get(llvm::Type::getInt8Ty(context),
                             llvm::APInt(8, 1)));
  LLVM::CreateStore(*builder, set_max_2,
                    llvm_utils->create_ptr_gep(key_mask, key_hash));
  LLVM::CreateStore(*builder, set_max_2,
                    llvm_utils->create_ptr_gep(key_mask, pos));
}

llvm::Value* LLVMDict::resolve_collision_for_read(llvm::Value* dict,
                                                  llvm::Value* key_hash,
                                                  llvm::Value* key,
                                                  llvm::Module& module,
                                                  ASR::ttype_t* key_asr_type) {
  ...
  llvm::Value* pos = LLVM::CreateLoad(*builder, pos_ptr);
  llvm::Value* item =
      llvm_utils->list_api->read_item(value_list, pos, true, false);
  return item;
}
...
llvm::Value* LLVMDictOptimizedLinearProbing::resolve_collision_for_read(
  ...
  llvm::BasicBlock* elseBB = llvm::BasicBlock::Create(context, "else");
  llvm::BasicBlock* mergeBB = llvm::BasicBlock::Create(context, "ifcont");
  llvm::Value* key_mask_value = LLVM::CreateLoad(
      *builder, llvm_utils->create_ptr_gep(key_mask, key_hash));
  llvm::Value* is_prob_not_neeeded = builder->CreateICmpEQ(
      key_mask_value, llvm::ConstantInt::get(llvm::Type::getInt8Ty(context),
                                             llvm::APInt(8, 1)));
  ...
  switch (key_asr_type->type) {
    case ASR::ttypeType::Integer: {
      ...
      llvm::Value* int_hash = builder->CreateZExtOrTrunc(
          builder->CreateSRem(
              key, builder->CreateZExtOrTrunc(capacity, key->getType())),
          capacity->getType());
      return int_hash;
    }
    case ASR::ttypeType::Character: {
      ...
      llvm::Value* null_char = llvm::ConstantInt::get(
          llvm::Type::getInt8Ty(context), llvm::APInt(8, '\0'));
      llvm::Value* p = llvm::ConstantInt::get(llvm::Type::getInt64Ty(context),
                                              llvm::APInt(64, 31));
      llvm::Value* m = llvm::ConstantInt::get(llvm::Type::getInt64Ty(context),
                                              llvm::APInt(64, 100000009));
      if (!are_iterators_set) {
        hash_value = builder->CreateAlloca(llvm::Type::getInt64Ty(context),
                                           nullptr, "hash_value");
        hash_iter = builder->CreateAlloca(llvm::Type::getInt64Ty(context),
                                          nullptr, "hash_iter");
        polynomial_powers = builder->CreateAlloca(
            llvm::Type::getInt64Ty(context), nullptr, "p_pow");
      }
      LLVM::CreateStore(*builder,
                        llvm::ConstantInt::get(llvm::Type::getInt64Ty(context),
                                               llvm::APInt(64, 0)),
      ...
      llvm_utils->start_new_block(loophead);
      {
        ...
        llvm::Value* c =
            LLVM::CreateLoad(*builder, llvm_utils->create_ptr_gep(key, i));
      ...
      llvm_utils->start_new_block(loopbody);
      {
        ...
        c = builder->CreateAdd(
            c, llvm::ConstantInt::get(llvm::Type::getInt64Ty(context),
                                      llvm::APInt(64, 1)));
        ...
        i = builder->CreateAdd(
            i, llvm::ConstantInt::get(llvm::Type::getInt64Ty(context),
                                      llvm::APInt(64, 1)));
    ...
    case ASR::ttypeType::Tuple: {
      llvm::Value* tuple_hash = llvm::ConstantInt::get(
      ...
      for (size_t i = 0; i < asr_tuple->n_type; i++) {
        llvm::Value* llvm_tuple_i = llvm_utils->tuple_api->read_item(
            key, i, LLVM::is_llvm_struct(asr_tuple->m_type[i]));
        tuple_hash = builder->CreateAdd(
            tuple_hash,
            get_key_hash(capacity, llvm_tuple_i, asr_tuple->m_type[i], module));
        tuple_hash = builder->CreateSRem(tuple_hash, capacity);
      }
      return tuple_hash;
    }
    default: {
      throw LCompilersException("Hashing " +
                                ASRUtils::type_to_str_python(key_asr_type) +
...
void LLVMDict::rehash(llvm::Value* dict, llvm::Module* module,
  ...
  llvm::Value* capacity = builder->CreateMul(
      old_capacity, llvm::ConstantInt::get(llvm::Type::getInt32Ty(context),
                                           llvm::APInt(32, 2)));
  capacity = builder->CreateAdd(
      capacity, llvm::ConstantInt::get(llvm::Type::getInt32Ty(context),
                                       llvm::APInt(32, 1)));
  ...
  std::pair<std::string, std::string> dict_type_key =
      std::make_pair(key_type_code, value_type_code);
  llvm::Type* key_llvm_type =
      std::get<2>(typecode2dicttype[dict_type_key]).first;
  llvm::Type* value_llvm_type =
      std::get<2>(typecode2dicttype[dict_type_key]).second;
  int32_t key_type_size = std::get<1>(typecode2dicttype[dict_type_key]).first;
  int32_t value_type_size =
      std::get<1>(typecode2dicttype[dict_type_key]).second;

  llvm::Value* key_list = get_key_list(dict);
  llvm::Value* new_key_list =
      builder->CreateAlloca(llvm_utils->list_api->get_list_type(
                                key_llvm_type, key_type_code, key_type_size),
                            nullptr);
  llvm_utils->list_api->list_init(key_type_code, new_key_list, *module,
                                  capacity, capacity);

  llvm::Value* value_list = get_value_list(dict);
  llvm::Value* new_value_list = builder->CreateAlloca(
      llvm_utils->list_api->get_list_type(value_llvm_type, value_type_code,
                                          value_type_size),
      nullptr);
  llvm_utils->list_api->list_init(value_type_code, new_value_list, *module,
                                  capacity, capacity);
  ...
  llvm::Value* llvm_mask_size = llvm::ConstantInt::get(
      llvm::Type::getInt32Ty(context), llvm::APInt(32, mask_size));
  llvm::Value* new_key_mask = LLVM::lfortran_calloc(context, *module, *builder,
                                                    capacity, llvm_mask_size);
  ...
  {
    llvm::Value* cond = builder->CreateICmpSGT(
        old_capacity, LLVM::CreateLoad(*builder, idx_ptr));
    builder->CreateCondBr(cond, loopbody, loopend);
  }
  ...
  llvm_utils->start_new_block(loopbody);
  {
    ...
    llvm::BasicBlock* elseBB = llvm::BasicBlock::Create(context, "else");
    llvm::BasicBlock* mergeBB = llvm::BasicBlock::Create(context, "ifcont");
    llvm::Value* is_key_set =
        LLVM::CreateLoad(*builder, llvm_utils->create_ptr_gep(key_mask, idx));
    is_key_set = builder->CreateICmpNE(
        is_key_set, llvm::ConstantInt::get(llvm::Type::getInt8Ty(context),
                                           llvm::APInt(8, 0)));
    builder->CreateCondBr(is_key_set, thenBB, elseBB);
    builder->SetInsertPoint(thenBB);
    {
      llvm::Value* key = llvm_utils->list_api->read_item(
          key_list, idx, LLVM::is_llvm_struct(key_asr_type), false);
      llvm::Value* value = llvm_utils->list_api->read_item(
          value_list, idx, LLVM::is_llvm_struct(value_asr_type), false);
      llvm::Value* key_hash =
          get_key_hash(current_capacity, key, key_asr_type, *module);
      this->resolve_collision(current_capacity, key_hash, key, new_key_list,
                              new_key_mask, *module, key_asr_type);
      llvm::Value* pos = LLVM::CreateLoad(*builder, pos_ptr);
      llvm::Value* key_dest =
          llvm_utils->list_api->read_item(new_key_list, pos, true, false);
      llvm_utils->deepcopy(key, key_dest, key_asr_type, *module);
      llvm::Value* value_dest =
          llvm_utils->list_api->read_item(new_value_list, pos, true, false);
      ...
      llvm::Value* set_max_2 = builder->CreateSelect(
          ...
          llvm::ConstantInt::get(llvm::Type::getInt8Ty(context),
                                 llvm::APInt(8, 1)));
      LLVM::CreateStore(*builder, set_max_2,
                        llvm_utils->create_ptr_gep(new_key_mask, key_hash));
      LLVM::CreateStore(*builder, set_max_2,
                        llvm_utils->create_ptr_gep(new_key_mask, pos));
    }
    builder->CreateBr(mergeBB);

    llvm_utils->start_new_block(elseBB);
    llvm_utils->start_new_block(mergeBB);
    idx = builder->CreateAdd(
        idx, llvm::ConstantInt::get(llvm::Type::getInt32Ty(context),
                                    llvm::APInt(32, 1)));
  ...
  LLVM::CreateStore(*builder, LLVM::CreateLoad(*builder, new_key_list),
                    key_list);
  LLVM::CreateStore(*builder, LLVM::CreateLoad(*builder, new_value_list),
                    value_list);
  LLVM::CreateStore(*builder, new_key_mask, get_pointer_to_keymask(dict));
}

void LLVMDict::rehash_all_at_once_if_needed(llvm::Value* dict,
                                            llvm::Module* module,
                                            ASR::ttype_t* key_asr_type,
                                            ASR::ttype_t* value_asr_type) {
  llvm::Function* fn = builder->GetInsertBlock()->getParent();
  ...
  llvm::Value* occupancy =
      LLVM::CreateLoad(*builder, get_pointer_to_occupancy(dict));
  llvm::Value* capacity =
      LLVM::CreateLoad(*builder, get_pointer_to_capacity(dict));
  occupancy = builder->CreateAdd(
      occupancy, llvm::ConstantInt::get(llvm::Type::getInt32Ty(context),
                                        llvm::APInt(32, 1)));
  ...
  // Threshold hash is chosen from
  // https://en.wikipedia.org/wiki/Hash_table#Load_factor
  llvm::Value* load_factor_threshold = llvm::ConstantFP::get(
      llvm::Type::getFloatTy(context), llvm::APFloat((float)0.6));
  builder->CreateCondBr(
      builder->CreateFCmpOGE(load_factor, load_factor_threshold), thenBB,
      elseBB);
  builder->SetInsertPoint(thenBB);
...
llvm::Value* LLVMDict::read_item(llvm::Value* dict, llvm::Value* key,
                                 llvm::Module& module,
                                 ASR::ttype_t* key_asr_type, bool get_pointer) {
  llvm::Value* current_capacity =
      LLVM::CreateLoad(*builder, get_pointer_to_capacity(dict));
  llvm::Value* key_hash =
      get_key_hash(current_capacity, key, key_asr_type, module);
  llvm::Value* value_ptr = this->resolve_collision_for_read(
      dict, key_hash, key, module, key_asr_type);
  if (get_pointer) {
    return value_ptr;
  }
...
llvm::Value* LLVMDict::pop_item(llvm::Value* dict, llvm::Value* key,
  ...
  if (get_pointer) {
    ...
    llvm::Type* llvm_value_type =
        std::get<2>(
            typecode2dicttype[std::make_pair(key_type_code, value_type_code)])
            .second;
    llvm::Value* return_ptr = builder->CreateAlloca(llvm_value_type, nullptr);
    LLVM::CreateStore(*builder, LLVM::CreateLoad(*builder, value_ptr),
                      return_ptr);
    return return_ptr;
  }
...
llvm::Value* LLVMList::read_item(llvm::Value* list, llvm::Value* pos,
                                 bool get_pointer, bool check_index_bound) {
  if (check_index_bound) {
    check_index_within_bounds(list, pos);
  }
  llvm::Value* list_data =
...
LLVMDict::~LLVMDict() { typecode2dicttype.clear(); }
...
void LLVMList::resize_if_needed(llvm::Value* list, llvm::Value* n,
  ...
  builder->SetInsertPoint(thenBB);
  llvm::Value* new_capacity = builder->CreateMul(
      llvm::ConstantInt::get(context, llvm::APInt(32, 2)), capacity);
  new_capacity = builder->CreateAdd(
      new_capacity, llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
  llvm::Value* arg_size = builder->CreateMul(
      llvm::ConstantInt::get(context, llvm::APInt(32, type_size)),
      new_capacity);
  ...
  copy_data =
      LLVM::lfortran_realloc(context, module, *builder, copy_data, arg_size);
  copy_data = builder->CreateBitCast(copy_data, el_type->getPointerTo());
...
void LLVMList::shift_end_point_by_one(llvm::Value* list) {
  ...
  end_point = builder->CreateAdd(
      end_point, llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
...
void LLVMList::append(llvm::Value* list, llvm::Value* item,
                      ASR::ttype_t* asr_type, llvm::Module& module) {
  llvm::Value* current_end_point =
      LLVM::CreateLoad(*builder, get_pointer_to_current_end_point(list));
  llvm::Value* current_capacity =
      LLVM::CreateLoad(*builder, get_pointer_to_current_capacity(list));
  std::string type_code = ASRUtils::get_type_code(asr_type);
  int type_size = std::get<1>(typecode2listtype[type_code]);
  llvm::Type* el_type = std::get<2>(typecode2listtype[type_code]);
  resize_if_needed(list, current_end_point, current_capacity, type_size,
                   el_type, module);
...
void LLVMList::insert_item(llvm::Value* list, llvm::Value* pos,
  ...
  {
    llvm::Value* cond = builder->CreateICmpSGT(
        current_end_point, LLVM::CreateLoad(*builder, pos_ptr));
    builder->CreateCondBr(cond, loopbody, loopend);
  }
  ...
  llvm_utils->start_new_block(loopbody);
  {
    llvm::Value* next_index =
        builder->CreateAdd(LLVM::CreateLoad(*builder, pos_ptr),
...
llvm::Value* LLVMList::find_item_position(llvm::Value* list, llvm::Value* item,
  ...
  LLVM::CreateStore(*builder,
                    llvm::ConstantInt::get(context, llvm::APInt(32, 0)), i);
  llvm::Value* tmp = nullptr;
  ...
  llvm_utils->start_new_block(loophead);
  {
    ...
    llvm::Value* is_item_not_equal = builder->CreateNot(
        llvm_utils->is_equal_by_value(left_arg, item, module, item_type));
  ...
  {
    tmp =
        builder->CreateAdd(LLVM::CreateLoad(*builder, i),
                           llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
  ...
  llvm::Value* cond =
      builder->CreateICmpEQ(LLVM::CreateLoad(*builder, i), current_end_point);
  builder->CreateCondBr(cond, thenBB, elseBB);
  builder->SetInsertPoint(thenBB);
  {
...
void LLVMList::remove(llvm::Value* list, llvm::Value* item,
  ...
  llvm::Value* tmp =
      LLVMList::find_item_position(list, item, item_type, module);
  ...
  {
    llvm::Value* cond = builder->CreateICmpSGT(
        current_end_point, LLVM::CreateLoad(*builder, item_pos));
    builder->CreateCondBr(cond, loopbody, loopend);
  }
  ...
  {
    tmp =
        builder->CreateAdd(LLVM::CreateLoad(*builder, item_pos),
                           llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
...
LLVMTuple::LLVMTuple(llvm::LLVMContext& context_, LLVMUtils* llvm_utils_,
                     llvm::IRBuilder<>* builder_)
...
llvm::Type* LLVMTuple::get_tuple_type(std::string& type_code,
  ...
  llvm::Type* llvm_tuple_type =
      llvm::StructType::create(context, el_types, "tuple");
  typecode2tupletype[type_code] =
      std::make_pair(llvm_tuple_type, el_types.size());
...
void LLVMTuple::tuple_deepcopy(llvm::Value* src, llvm::Value* dest,
  ...
  for (size_t i = 0; i < tuple_type->n_type; i++) {
    llvm::Value* src_item =
        read_item(src, i, LLVM::is_llvm_struct(tuple_type->m_type[i]));
    llvm::Value* dest_item_ptr = read_item(dest, i, true);
    llvm_utils->deepcopy(src_item, dest_item_ptr, tuple_type->m_type[i],
                         module);
  }
}
...
llvm::Value* LLVMTuple::check_tuple_equality(llvm::Value* t1, llvm::Value* t2,
  ...
  for (size_t i = 0; i < tuple_type->n_type; i++) {
    ...
    llvm::Value* is_t1_eq_t2 =
        llvm_utils->is_equal_by_value(t1i, t2i, module, tuple_type->m_type[i]);
