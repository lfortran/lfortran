    ...
    llvm::Type* LLVMUtils::get_type_from_ttype_t(ASR::ttype_t* asr_type,
        ...
        switch (asr_type->type) {
            ...
            case (ASR::ttypeType::String) : {
                ASR::String_t* v_type = ASR::down_cast<ASR::String_t>(asr_type);
                a_kind = v_type->m_kind;
                if(v_type->m_physical_type == ASR::DescriptorString){
    ...
    bool LLVMUtils::is_proper_array_of_strings_llvm_var(ASR::ttype_t* type, llvm::Value* str){
        ...
        switch(ASRUtils::extract_physical_type(type)){
            ...
            case ASR::PointerToDataArray:{
                switch (str_type->m_physical_type){
                    // A Pointer To String Descriptor => `string_descriptor*`
                    case ASR::DescriptorString : {
                        return str->getType()->isPointerTy() && 
                            !str->getType()->getPointerElementType()->isPointerTy() &&
                            (string_descriptor->getPointerTo() == str->getType()) && 
                            (str->getType()->getPointerElementType() == string_descriptor);
                    }
                    default:
                        throw LCompilersException("Unhandled String Physical type");
                }
...
#if LLVM_VERSION_MAJOR < 17
        switch (str_type->m_physical_type){
            case ASR::DescriptorString:{ // Check for => `string_descriptor*`
                return str->getType()->isPointerTy() && 
                    !str->getType()->getPointerElementType()->isPointerTy() &&
                    (string_descriptor->getPointerTo() == str->getType()) && 
                    (str->getType()->getPointerElementType() == string_descriptor);
...
// TODO :: Remove code above.
    ...
    llvm::Value* LLVMUtils::get_string_length(ASR::String_t* str_type, llvm::Value* str, bool get_pointer_to_len){
        LCOMPILERS_ASSERT(is_proper_string_llvm_variable(str_type, str))
        if(!get_pointer_to_len && str_type->m_len && ASR::is_a<ASR::IntegerConstant_t>(*str_type->m_len)){ // Explicit-Constant-CompileTime Length
    ...
    llvm::Value* LLVMUtils::get_stringArray_whole_size(ASR::ttype_t* type){
        ...
        if(ASRUtils::is_value_constant(str->m_len) && ASRUtils::is_fixed_size_array(type)){
            ...
            LCOMPILERS_ASSERT_MSG(false, "Array size and string length must be compile time constants");
        }
...
//TODO :: Remove Code Above
    ...
    llvm::Value* LLVMUtils::get_stringArray_data(ASR::ttype_t* type, llvm::Value* arr_ptr){
        ...
        switch(arr->m_physical_type){
            ...
            default:
                throw LCompilersException("Unhandled Arra Physical Type");
        }
    }
    ...
    llvm::Value* LLVMUtils::declare_string_constant(const ASR::StringConstant_t* str_const){
        return declare_global_string(
            ASRUtils::get_string_type(str_const->m_type), str_const->m_s, true, "string_const");
    }

    llvm::Value* LLVMUtils::declare_constant_stringArray(const ASR::ArrayConstant_t* arr_const){
        // Array of string is just consecutive characters in memory
        // We can declare it as global string only if the type is PointerToDataArray
        LCOMPILERS_ASSERT(ASRUtils::extract_physical_type(arr_const->m_type) == ASR::PointerToDataArray)
        return declare_global_string(
            ASRUtils::get_string_type(arr_const->m_type), (char*)arr_const->m_data, true, "stringArray_const");
    }
    
    llvm::Value* LLVMUtils::declare_global_string(
    ASR::String_t* str, std::string initial_data, bool is_const, std::string name){
        int64_t len = 0; ASRUtils::is_value_constant(str->m_len, len);
    ...
    llvm::Value* LLVMList::get_pointer_to_list_data_using_typecode(llvm::Type* list_type, llvm::Value* list) {
        return llvm_utils->create_gep2(list_type, list, 2);
    }

    llvm::Value* LLVMList::get_pointer_to_current_end_point_using_typecode(llvm::Type* list_type, llvm::Value* list) {
        return llvm_utils->create_gep2(list_type, list, 0);
    }

    llvm::Value* LLVMList::get_pointer_to_current_capacity_using_typecode(llvm::Type* list_type, llvm::Value* list) {
    ...
    void LLVMList::list_init(std::string& type_code, llvm::Value* list,
        ...
        llvm::Value* list_data_ptr = get_pointer_to_list_data_using_typecode(list_type, list);
        ...
        builder->CreateStore(current_end_point, get_pointer_to_current_end_point_using_typecode(list_type, list));
        builder->CreateStore(current_capacity, get_pointer_to_current_capacity_using_typecode(list_type, list));
    }
    ...
    llvm::Value* LLVMDict::get_pointer_to_occupancy_using_typecode(llvm::Type* dict_type, llvm::Value* dict) {
    ...
    llvm::Value* LLVMDict::get_pointer_to_capacity_using_typecode(std::string& /*key_type_code*/, std::string& value_type_code,
                                                    llvm::Value* dict) {
        return llvm_utils->list_api->get_pointer_to_current_capacity_using_typecode(
    ...
    void LLVMList::list_deepcopy(llvm::Value* src, llvm::Value* dest,
        ...
        llvm::Value* src_end_point = llvm_utils->CreateLoad2(llvm::Type::getInt32Ty(context),
            get_pointer_to_current_end_point_using_typecode(list_type, src));
        llvm::Value* src_capacity = llvm_utils->CreateLoad2(llvm::Type::getInt32Ty(context),
            get_pointer_to_current_capacity_using_typecode(list_type, src));
        llvm::Value* dest_end_point_ptr = get_pointer_to_current_end_point_using_typecode(list_type, dest);
        llvm::Value* dest_capacity_ptr = get_pointer_to_current_capacity_using_typecode(list_type, dest);
        ...
        llvm::Value* src_data = llvm_utils->CreateLoad2(el_type->getPointerTo(), get_pointer_to_list_data_using_typecode(list_type, src));
        ...
        if( LLVM::is_llvm_struct(element_type) ) {
            builder->CreateStore(copy_data, get_pointer_to_list_data_using_typecode(list_type, dest));
            ...
            llvm_utils->start_new_block(loopbody);
            {
                ...
                llvm_utils->deepcopy(srci, desti, element_type, element_type, module, name2memidx);
    ...
    void LLVMDict::dict_deepcopy(llvm::Value* src, llvm::Value* dest,
        ...
        llvm::Value* llvm_mask_size = llvm::ConstantInt::get(llvm::Type::getInt32Ty(context),
                                            llvm::APInt(32, mask_size));
        llvm::Value* src_capacity = llvm_utils->CreateLoad2(llvm::Type::getInt32Ty(context), get_pointer_to_capacity(src));
    ...
    void LLVMDictSeparateChaining::deepcopy_key_value_pair_linked_list(
        ...
        llvm_utils->start_new_block(loopbody);
        {
            ...
            llvm_utils->deepcopy(src_key, dest_key_ptr, dict_type->m_key_type, dict_type->m_key_type, module, name2memidx);
            llvm_utils->deepcopy(src_value, dest_value_ptr, dict_type->m_value_type, dict_type->m_value_type, module, name2memidx);
    ...
    void LLVMList::write_item(llvm::Value* list, llvm::Value* pos,
        ...
        llvm_utils->deepcopy(item, element_ptr, asr_type, asr_type, module, name2memidx);
    }
    ...
    void LLVMDictOptimizedLinearProbing::resolve_collision(
        ...
        llvm_utils->start_new_block(loophead);
        {
            ...
            llvm::Value* compare_keys = builder->CreateAnd(is_key_set,
                                            builder->CreateNot(is_key_skip));
            llvm_utils->create_if_else(compare_keys, [&]() {
                llvm::Value* original_key = llvm_utils->list_api->read_item_using_typecode(key_type_code, key_list, pos,
                                false, module, LLVM::is_llvm_struct(key_asr_type));
                is_key_matching = llvm_utils->is_equal_by_value(key, original_key, module,
                                                                key_asr_type);
                LLVM::CreateStore(*builder, is_key_matching, is_key_matching_var);
    ...
    void LLVMDictSeparateChaining::resolve_collision_for_write(
        ...
        builder->SetInsertPoint(thenBB);
        {
            llvm_utils->create_if_else(builder->CreateICmpNE(
                ...
                llvm_utils->deepcopy(key, llvm_utils->create_gep2(kv_pair_type, new_kv_struct, 0), key_asr_type, key_asr_type, module, name2memidx);
                llvm_utils->deepcopy(value, llvm_utils->create_gep2(kv_pair_type, new_kv_struct, 1), value_asr_type, value_asr_type, module, name2memidx);
                LLVM::CreateStore(*builder,
                    llvm::ConstantPointerNull::get(llvm::Type::getInt8Ty(context)->getPointerTo()),
                ...
                llvm_utils->deepcopy(key, llvm_utils->create_gep2(kv_pair_type, key_value_pair_linked_list, 0), key_asr_type, key_asr_type,module, name2memidx);
                llvm_utils->deepcopy(value, llvm_utils->create_gep2(kv_pair_type, key_value_pair_linked_list, 1), value_asr_type, key_asr_type, module, name2memidx);
                LLVM::CreateStore(*builder,
                    llvm::ConstantPointerNull::get(llvm::Type::getInt8Ty(context)->getPointerTo()),
                    llvm_utils->create_gep2(
        ...
        llvm_utils->start_new_block(elseBB);
        {
            ...
            llvm_utils->deepcopy(key, llvm_utils->create_gep2(kv_pair_type, kv_struct, 0), key_asr_type, key_asr_type, module, name2memidx);
            llvm_utils->deepcopy(value, llvm_utils->create_gep2(kv_pair_type, kv_struct, 1), value_asr_type, value_asr_type, module, name2memidx);
        }
        llvm_utils->start_new_block(mergeBB);
        ...
        switch( key_asr_type->type ) {
            ...
            case ASR::ttypeType::String: {
                ...
                llvm_utils->start_new_block(loophead);
                {
                    ...
                    llvm::Value* c = llvm_utils->CreateLoad(llvm_utils->create_ptr_gep(key, i));
    ...
    void LLVMDict::rehash(llvm::Value* dict, llvm::Module* module,
        ...
        llvm_utils->start_new_block(loopbody);
        {
            ...
            builder->SetInsertPoint(thenBB);
            {
                llvm::Value* key = llvm_utils->list_api->read_item_using_typecode(key_type_code, key_list, idx,
                        false, module, LLVM::is_llvm_struct(key_asr_type));
                llvm::Value* value = llvm_utils->list_api->read_item_using_typecode(value_type_code, value_list,
                ...
                llvm_utils->deepcopy(key, key_dest, key_asr_type, key_asr_type, module, name2memidx);
                llvm::Value* value_dest = llvm_utils->list_api->read_item_using_typecode(value_type_code,
                                                new_value_list, pos, false, module, true);
                llvm_utils->deepcopy(value, value_dest, value_asr_type, value_asr_type, module, name2memidx);
        ...
        llvm_utils->list_api->free_data_using_typecode(key_type_code, key_list, module);
        llvm_utils->list_api->free_data_using_typecode(value_type_code, value_list, module);
    ...
    void LLVMDictSeparateChaining::rehash(
        ...
        llvm::Value* rehash_flag = llvm_utils->CreateLoad(get_pointer_to_rehash_flag(dict));
        builder->CreateCondBr(rehash_flag, thenBB_rehash, elseBB_rehash);
        builder->SetInsertPoint(thenBB_rehash);
        old_key_value_pairs_value = llvm_utils->CreateLoad(old_key_value_pairs);
        ...
        old_key_mask_value = llvm_utils->CreateLoad(old_key_mask);
        old_capacity_value = llvm_utils->CreateLoad(old_capacity);
        capacity = llvm_utils->CreateLoad(get_pointer_to_capacity(dict));
    ...
    void LLVMDict::rehash_all_at_once_if_needed(llvm::Value* dict, llvm::Module* module,
        ...
        llvm::Value* occupancy = llvm_utils->CreateLoad2(llvm::Type::getInt32Ty(context), 
                                                         get_pointer_to_occupancy_using_typecode(dict_type, dict));
    ...
    void LLVMDictSeparateChaining::rehash_all_at_once_if_needed(
        ...
        llvm::Value* occupancy = llvm_utils->CreateLoad(get_pointer_to_occupancy(dict));
        llvm::Value* buckets_filled = llvm_utils->CreateLoad(get_pointer_to_number_of_filled_buckets(dict));
        llvm::Value* rehash_condition = llvm_utils->CreateLoad(get_pointer_to_rehash_flag(dict));
    ...
    llvm::Value* LLVMDictSeparateChaining::pop_item(
        ...
        llvm_utils->create_if_else(builder->CreateICmpNE(prev,
            ...
            llvm_utils->create_if_else(builder->CreateICmpEQ(found_next,
                ...
                llvm::Value* key_value_pairs = llvm_utils->CreateLoad(get_pointer_to_key_value_pairs(dict));
                LLVM::CreateStore(*builder, llvm_utils->CreateLoad(found_next),
    ...
    llvm::Value* LLVMList::read_item(llvm::Value* list, llvm::Value* pos,
        ...
        return llvm_utils->CreateLoad(element_ptr);
    }


    llvm::Value* LLVMList::read_item_using_typecode(std::string& type_code, llvm::Value* list, llvm::Value* pos,
                                     bool enable_bounds_checking,
                                     llvm::Module* module, bool get_pointer) {
        if( enable_bounds_checking ) {
            check_index_within_bounds(list, pos, module);
        ...
        llvm::Value* list_data = llvm_utils->CreateLoad2(list_element_type->getPointerTo(), 
                                                         get_pointer_to_list_data_using_typecode(list_type, list));
        llvm::Value* element_ptr = llvm_utils->create_ptr_gep2(list_element_type, list_data, pos);
        if( get_pointer ) {
            return element_ptr;
        }
    ...
    llvm::Value* LLVMList::len_using_typecode(llvm::Type* list_type, llvm::Value* list) {
        return llvm_utils->CreateLoad2(
                llvm::Type::getInt32Ty(context) ,get_pointer_to_current_end_point_using_typecode(
                    list_type, list));
    }
    ...
    void LLVMList::resize_if_needed_using_typecode(std::string& type_code, llvm::Value* list, llvm::Value* n,
        ...
        llvm::Value* arg_size = builder->CreateMul(llvm::ConstantInt::get(context,
                                                   llvm::APInt(32, type_size)),
                                                   new_capacity);
        llvm::Value* copy_data_ptr = get_pointer_to_list_data_using_typecode(list_type, list);
        llvm::Value* copy_data = llvm_utils->CreateLoad2(el_type->getPointerTo() ,copy_data_ptr);
        copy_data = LLVM::lfortran_realloc(context, *module, *builder,
                                           copy_data, arg_size);
        copy_data = builder->CreateBitCast(copy_data, el_type->getPointerTo());
        builder->CreateStore(copy_data, copy_data_ptr);
        builder->CreateStore(new_capacity, get_pointer_to_current_capacity_using_typecode(list_type, list));
        builder->CreateBr(mergeBB);
        llvm_utils->start_new_block(elseBB);
        llvm_utils->start_new_block(mergeBB);
    }
    ...
    void LLVMList::shift_end_point_by_one_using_typecode(std::string& type_code, llvm::Value* list) {
        llvm::Type* list_type = get_list_type(nullptr, type_code, 0);
        llvm::Value* end_point_ptr = get_pointer_to_current_end_point_using_typecode(list_type, list);
        llvm::Value* end_point = llvm_utils->CreateLoad2(llvm::Type::getInt32Ty(context),
                                                            end_point_ptr);
        end_point = builder->CreateAdd(end_point, llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
        builder->CreateStore(end_point, end_point_ptr);
    ...
    void LLVMList::append(llvm::Value* list, llvm::Value* item,
        ...
        llvm::Value* current_end_point = llvm_utils->CreateLoad2(llvm::Type::getInt32Ty(context),
                                                                 get_pointer_to_current_end_point_using_typecode(list_type, list));
        llvm::Value* current_capacity = llvm_utils->CreateLoad2(llvm::Type::getInt32Ty(context),
    ...
    void LLVMList::insert_item(llvm::Value* list, llvm::Value* pos,
        ...
        LLVM::CreateStore(*builder, read_item_using_typecode(type_code, list, pos, false, module, false), tmp_ptr);
        ...
        llvm_utils->start_new_block(loopbody);
        {
            llvm::Value* next_index = builder->CreateAdd(
                            llvm_utils->CreateLoad(pos_ptr),
                            llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
            tmp = read_item_using_typecode(type_code, list, next_index, false, module, false);
    ...
    void LLVMList::reserve(llvm::Value* list, llvm::Value* n,
         ...
         *
         */
        llvm::Value* capacity = llvm_utils->CreateLoad(get_pointer_to_current_capacity(list));
        std::string type_code = ASRUtils::get_type_code(asr_type);
        int type_size = std::get<1>(typecode2listtype[type_code]);
        llvm::Type* el_type = std::get<2>(typecode2listtype[type_code]);
    ...
    void LLVMList::reverse(std::string& type_code, llvm::Value* list, llvm::Module* module) {
        ...
        llvm_utils->start_new_block(loopbody);
        {
            tmp = read_item_using_typecode(type_code, list, llvm_utils->CreateLoad(i),
                false, module, false);    // tmp = list[i]
            write_item_using_typecode(type_code, list, llvm_utils->CreateLoad(i),
                        read_item_using_typecode(type_code, list, llvm_utils->CreateLoad(j),
                        false, module, false),
    ...
    llvm::Value* LLVMList::find_item_position(llvm::Value* list,
        ...
        llvm_utils->start_new_block(loophead);
        {
            llvm::Value* left_arg = read_item_using_typecode(type_code, list, llvm_utils->CreateLoad(i),
    ...
    void LLVMList::remove(llvm::Value* list, llvm::Value* item,
        ...
        llvm_utils->start_new_block(loopbody);
        {
            ...
            write_item_using_typecode(type_code, list, llvm_utils->CreateLoad(item_pos),
                read_item_using_typecode(type_code, list, tmp, false, module, false), false, module);
            LLVM::CreateStore(*builder, tmp, item_pos);
        }
        builder->CreateBr(loophead);
    ...
    llvm::Value* LLVMList::pop_position(llvm::Value* list, llvm::Value* pos,
        ...
        if( LLVM::is_llvm_struct(list_element_type) ) {
            ...
            llvm::AllocaInst *target = llvm_utils->CreateAlloca(
                ...
                "pop_position_item");
            llvm_utils->deepcopy(item, target, list_element_type, list_element_type, module, name2memidx);
            item = target;
        }
    ...
    void LLVMList::free_data_using_typecode(std::string& type_code, llvm::Value* list, llvm::Module* module) {
        llvm::Type* list_type = get_list_type(nullptr, type_code, 0);
        llvm::Type* list_el_type = std::get<2>(typecode2listtype[type_code]);
        llvm::Value* data = llvm_utils->CreateLoad2(list_el_type->getPointerTo(), get_pointer_to_list_data_using_typecode(list_type, list));
        LLVM::lfortran_free(context, *module, *builder, data);
    }
    ...
    llvm::Value* LLVMSetLinearProbing::get_pointer_to_occupancy_using_typecode(llvm::Type* set_type, llvm::Value* set) {
    ...
    llvm::Value* LLVMSetSeparateChaining::get_pointer_to_occupancy_using_typecode(llvm::Type* set_type,llvm::Value* set) {
        return llvm_utils->create_gep2(set_type, set, 0);
    }
    ...
    void LLVMSetLinearProbing::resolve_collision(
        ...
        llvm_utils->start_new_block(loophead);
        {
            ...
            llvm_utils->create_if_else(compare_elems, [&]() {
                llvm::Value* original_el = llvm_utils->list_api->read_item_using_typecode(el_type_code, el_list, pos,
                                false, module, LLVM::is_llvm_struct(el_asr_type));
    ...
    void LLVMSetSeparateChaining::resolve_collision_for_write(
        ...
        builder->SetInsertPoint(thenBB);
        {
            llvm_utils->create_if_else(builder->CreateICmpNE(
                ...
                llvm_utils->deepcopy(el, llvm_utils->create_gep(new_el_struct, 0), el_asr_type, el_asr_type, module, name2memidx);
                LLVM::CreateStore(*builder,
                    llvm::ConstantPointerNull::get(llvm::Type::getInt8Ty(context)->getPointerTo()),
                ...
                llvm_utils->deepcopy(el, llvm_utils->create_gep(el_linked_list, 0), el_asr_type, el_asr_type, module, name2memidx);
                LLVM::CreateStore(*builder,
                    llvm::ConstantPointerNull::get(llvm::Type::getInt8Ty(context)->getPointerTo()),
        ...
        {
            llvm::Value* el_struct = builder->CreateBitCast(el_struct_i8, el_struct_type->getPointerTo());
            llvm_utils->deepcopy(el, llvm_utils->create_gep(el_struct, 0), el_asr_type, el_asr_type, module, name2memidx);
        }
        llvm_utils->start_new_block(mergeBB);
    ...
    void LLVMSetLinearProbing::rehash(
        ...
        llvm_utils->start_new_block(loopbody);
        {
            ...
            builder->SetInsertPoint(thenBB);
            {
                llvm::Value* el = llvm_utils->list_api->read_item_using_typecode(el_type_code, el_list, idx,
                        false, module, LLVM::is_llvm_struct(el_asr_type));
                ...
                llvm_utils->deepcopy(el, el_dest, el_asr_type, el_asr_type, module, name2memidx);
        ...
        llvm_utils->list_api->free_data_using_typecode(el_type_code, el_list, module);
    ...
    void LLVMSetSeparateChaining::rehash(
        ...
        old_capacity_value = llvm_utils->CreateLoad(old_capacity);
        capacity = llvm_utils->CreateLoad(get_pointer_to_capacity(set));
        LLVM::CreateStore(*builder, llvm::ConstantInt::get(llvm::Type::getInt32Ty(context), llvm::APInt(32, 0)), idx_ptr);
        llvm::BasicBlock *loophead = llvm::BasicBlock::Create(context, "loop.head");
        llvm::BasicBlock *loopbody = llvm::BasicBlock::Create(context, "loop.body");
        llvm::BasicBlock *loopend = llvm::BasicBlock::Create(context, "loop.end");
    ...
    void LLVMSetLinearProbing::rehash_all_at_once_if_needed(
        ...
        llvm::Value* occupancy = llvm_utils->CreateLoad2(llvm::Type::getInt32Ty(context), 
                                                         get_pointer_to_occupancy_using_typecode(set_type, set));
        llvm::Value* capacity = llvm_utils->CreateLoad2(llvm::Type::getInt32Ty(context), 
                                                        get_pointer_to_capacity_using_typecode(el_type_code, set));
        // Threshold hash is chosen from https://en.wikipedia.org/wiki/Hash_table#Load_factor
        // occupancy / capacity >= 0.6 is same as 5 * occupancy >= 3 * capacity
    ...
    void LLVMSetLinearProbing::set_deepcopy(
        ...
        llvm::Value* src_occupancy = llvm_utils->CreateLoad2(llvm::Type::getInt32Ty(context), 
                                                         get_pointer_to_occupancy_using_typecode(set_type_, src));
        llvm::Value* dest_occupancy_ptr = get_pointer_to_occupancy(dest);
        LLVM::CreateStore(*builder, src_occupancy, dest_occupancy_ptr);
    ...
    void LLVMSetSeparateChaining::set_deepcopy(
        ...
        llvm::Value* src_capacity = llvm_utils->CreateLoad(get_pointer_to_capacity(src));
    ...
    void LLVMSetSeparateChaining::deepcopy_el_linked_list(
        ...
        llvm_utils->start_new_block(loopbody);
        {
            ...
            llvm_utils->deepcopy(src_el, dest_el_ptr, set_type->m_type, set_type->m_type, module, name2memidx);
