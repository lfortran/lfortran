                ...
                a_kind = ASRUtils::extract_kind_from_ttype_t(v_type->m_type);
                    ...
                    case ASR::array_physical_typeType::SIMDArray: {
                        ...
                        int64_t simd_len = ASRUtils::get_fixed_size_of_array(
                            v_type->m_dims, v_type->n_dims);
                        LCOMPILERS_ASSERT(simd_len >= 0);
                        if (simd_len == 0) {
                            llvm_type = llvm::ArrayType::get(el_type, 0);
                        } else {
#if LLVM_VERSION_MAJOR >= 11
                            llvm_type = llvm::VectorType::get(el_type,
                                static_cast<unsigned>(simd_len), false);
#else
                            llvm_type = llvm::VectorType::get(el_type,
                                static_cast<unsigned>(simd_len));
#endif
                        }
                        break;
                    }
