...
namespace LCompilers {
    ...
    bool LLVMUtils::is_proper_array_of_strings_llvm_var(ASR::ttype_t* type, llvm::Value* str){
        ...
        switch(ASRUtils::extract_physical_type(type)){
            case ASR::DescriptorArray:{ // A pointer to the Array Descriptor
    ...
    */
    ...
    void LLVMUtils::initialize_string_stack(ASR::string_physical_typeType str_physical_type,
        ...
        set_string_memory_on_heap(str_physical_type, str, len);
...
// TODO :: use the implementation below intead of using malloc
    ...
    llvm::Value* LLVMUtils::create_string_descriptor(llvm::Value* data, llvm::Value* len, std::string name){
        // TODO :: USE STRING DESCRIPTOR ON THE STACK.
    ...
    llvm::Value* LLVMUtils::get_string_element_in_array(ASR::String_t* str_type, llvm::Value* array_ptr/*PointerToDataArray*/, llvm::Value* arr_idx){
        llvm::Value* string_len = get_string_length(str_type, array_ptr);
        llvm::Value* string_len_plus_null_char = builder->CreateAdd(string_len, llvm::ConstantInt::get(context, llvm::APInt(64, 1)));
        llvm::Value* string_data = get_string_data(str_type, array_ptr);
        ...
        return create_string_descriptor(desired_element, string_len, "arr_element");
    }
    ...
    void LLVMUtils::allocate_allocatable_string(ASR::String_t* str_type, llvm::Value* str, llvm::Value* amount_to_allocate){
        switch (str_type->m_len_kind){
            ...
            case ASR::AssumedLength:{//stringDescriptor's length remains the same
                set_string_memory_on_heap(str_type->m_physical_type, 
                    str, get_string_length(str_type, str));
                break;
            }
            case ASR::DeferredLength:{
                llvm::Value* str_data{}; // Pointer to it (e.g `i8**`)
                llvm::Value* str_len{}; // Pointer to it (e.g `i64*`)
                std::tie(str_data, str_len) = get_string_length_data(str_type, str, true, true);
                LCOMPILERS_ASSERT(amount_to_allocate)
                set_string_memory_on_heap(str_type->m_physical_type, str_data, amount_to_allocate);
                builder->CreateStore(amount_to_allocate, str_len);
                break;
            }
            default:
    ...
    void LLVMUtils::set_array_of_strings_memory_on_stack(ASR::String_t* str_type,llvm::Value* str,
        ...
        set_array_of_strings_memory_on_heap(str_type, str, str_len, array_size);

        //TODO :: Use implementation below

        // llvm::Value* string_len = get_string_length(str_type, str);
        // llvm::Value* string_len_plus_null_char = builder->CreateAdd(string_len, llvm::ConstantInt::get(context, llvm::APInt(64, 1)));
        // llvm::Value* whole_memory_needed = builder->CreateMul(
        ...
        // builder->CreateMemSet(allocated_mem, llvm::ConstantInt::get(context, llvm::APInt(8,'\0')), whole_memory_needed, llvm::MaybeAlign());
        // builder->CreateStore(allocated_mem, llvm_utils->create_gep2(string_descriptor, str_desc, 0));
    }
