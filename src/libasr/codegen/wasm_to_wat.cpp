 ...
 public:
  ...
  void decode_imports_section(uint32_t offset) {
    ...
    for (uint32_t i = 0; i < no_of_imports; i++) {
      ...
      imports.p[i].name.resize(name_size);  // do not pass al to this resize as
                                            // it is std::string.resize()
  ...
  void decode_export_section(uint32_t offset) {
    ...
    for (uint32_t i = 0; i < no_of_exports; i++) {
      uint32_t name_size = read_u32(wasm_bytes, offset);
      exports.p[i].name.resize(name_size);  // do not pass al to this resize as
                                            // it is std::string.resize()
      for (uint32_t j = 0; j < name_size; j++) {
  ...
  std::string get_wat() {
    ...
    for (uint32_t i = 0; i < type_indices.size(); i++) {
      ...
      for (uint32_t j = 0; j < func_types[func_index].result_types.size();
           j++) {
        result +=
            " " + var_type_to_string[func_types[func_index].result_types.p[j]];
      }
      result += ")";
      result += indent + "    (local";
