    ...
    public:
        ...
        void decode_imports_section(uint32_t offset)
        {
            ...
            for (uint32_t i = 0; i < no_of_imports; i++) {
                ...
                imports.p[i].mod_name.resize(mod_name_size);  // do not pass al to this resize as it
                                                              // is std::string.resize()
                ...
                imports.p[i].name.resize(name_size);  // do not pass al to this resize as it is
                                                      // std::string.resize()
                ...
                switch (imports.p[i].kind) {
                    ...
                    default: {
                        throw CodeGenError("Only importing functions and memory are currently "
                                           "supported");
                    }
                }
            }
        ...
        void decode_export_section(uint32_t offset)
        {
            ...
            for (uint32_t i = 0; i < no_of_exports; i++) {
                uint32_t name_size = read_u32(wasm_bytes, offset);
                exports.p[i].name.resize(name_size);  // do not pass al to this resize as it is
                                                      // std::string.resize()
                for (uint32_t j = 0; j < name_size; j++) {
        ...
        void decode_data_section(uint32_t offset)
        {
            ...
            for (uint32_t i = 0; i < no_of_data_segments; i++) {
                ...
                if (num != 0) {
                    throw CodeGenError("Only active default memory (index = 0) is currently "
                                       "supported");
                }

                {
                    WASM_INSTS_VISITOR::WATVisitor v
                        = WASM_INSTS_VISITOR::WATVisitor(wasm_bytes, offset, "", "");
                ...
                data_segments.p[i].text.resize(text_size);  // do not pass al to this resize as it
                                                            // is std::string.resize()
        ...
        std::string get_wat()
        {
            ...
            for (uint32_t i = 0; i < imports.size(); i++) {
                ...
                if (imports[i].kind == 0x00) {
                    result += "(func (;" + std::to_string(imports[i].type_idx) + ";) (type "
                              + std::to_string(imports[i].type_idx) + ")))";
            ...
            for (uint32_t i = 0; i < type_indices.size(); i++) {
...

                {
                    WASM_INSTS_VISITOR::WATVisitor v = WASM_INSTS_VISITOR::WATVisitor(
                        wasm_bytes, codes.p[i].insts_start_index, "", indent + "    ");
                    v.decode_instructions();
                    result += v.src;
                }
            ...
            for (uint32_t i = 0; i < data_segments.size(); i++) {
                result += indent + "(data (;" + std::to_string(i) + ";) (" + data_segments[i].insts
                          + ") \"" + data_segments[i].text + "\")";
            }
...
Result<std::string>
wasm_to_wat(Vec<uint8_t>& wasm_bytes, Allocator& al, diag::Diagnostics& diagnostics)
{
