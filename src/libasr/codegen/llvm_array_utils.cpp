...
bool compile_time_dimensions_t(ASR::dimension_t* m_dims, int n_dims) {
  ...
  for (int r = 0; r < n_dims; r++) {
    if (m_dims[r].m_length == nullptr && m_dims[r].m_start == nullptr) {
      is_ok = false;
      break;
    }
  ...
  switch (v->m_type->type) {
    ...
    default: {
      throw LCompilersException(
          "Explicit shape checking supported only for integer, real, complex, "
...
std::unique_ptr<Descriptor> Descriptor::get_descriptor(
    llvm::LLVMContext& context, llvm::IRBuilder<>* builder,
    LLVMUtils* llvm_utils, DESCR_TYPE descr_type) {
  switch (descr_type) {
    case DESCR_TYPE::_SimpleCMODescriptor: {
      return std::make_unique<SimpleCMODescriptor>(context, builder,
                                                   llvm_utils);
    }
  }
  return nullptr;
...
bool SimpleCMODescriptor::is_array(llvm::Value* tmp) {
  ...
  if (tmp->getType()->isPointerTy()) {
    tmp_type =
        static_cast<llvm::PointerType*>(tmp->getType())->getElementType();
  } else {
    tmp_type = tmp->getType();
  }
  if (tmp_type->isStructTy()) {
    llvm::StructType* tmp_struct_type =
...
llvm::Value* SimpleCMODescriptor::convert_to_argument(llvm::Value* tmp,
                                                      llvm::Type* arg_type,
                                                      bool data_only) {
  if (data_only) {
  ...
  llvm::Value* third_ele_ptr = LLVM::CreateLoad(
      *builder, get_pointer_to_dimension_descriptor_array(tmp));
  llvm::Value* third_arg_ptr = llvm_utils->create_gep(arg_struct, 2);
  builder->CreateStore(third_ele_ptr, third_arg_ptr);
  return arg_struct;
}

llvm::Type* SimpleCMODescriptor::get_argument_type(
  ...
  if (type_struct->getElementType(0)->isArrayTy()) {
    llvm::ArrayType* arr_type =
        static_cast<llvm::ArrayType*>(type_struct->getElementType(0));
  ...
  if (arr_arg_type_cache.find(m_h) == arr_arg_type_cache.end() ||
      (arr_arg_type_cache.find(m_h) != arr_arg_type_cache.end() &&
       arr_arg_type_cache[m_h].find(std::string(arg_name)) ==
           arr_arg_type_cache[m_h].end())) {
...
llvm::Type* SimpleCMODescriptor::get_array_type(ASR::ttype_t* m_type_,
                                                int a_kind, int rank,
                                                llvm::Type* el_type,
                                                bool get_pointer) {
  ASR::ttypeType type_ = m_type_->type;
  std::pair<std::pair<int, int>, int> array_key =
      std::make_pair(std::make_pair((int)type_, a_kind), rank);
  ...
  array_type_vec = {el_type->getPointerTo(), llvm::Type::getInt32Ty(context),
                    dim_des_array, llvm::Type::getInt1Ty(context),
                    llvm::Type::getInt32Ty(context)};
  llvm::StructType* new_array_type =
      llvm::StructType::create(context, array_type_vec, "array");
...
llvm::Type* SimpleCMODescriptor::get_malloc_array_type(ASR::ttype_t* m_type_,
                                                       int a_kind, int rank,
                                                       llvm::Type* el_type,
                                                       bool get_pointer) {
  ASR::ttypeType type_ = m_type_->type;
  ...
  llvm::Type* dim_des_array = create_dimension_descriptor_array_type();
  std::vector<llvm::Type*> array_type_vec = {
      el_type->getPointerTo(), llvm::Type::getInt32Ty(context), dim_des_array,
...
llvm::Value* SimpleCMODescriptor::get_pointer_to_dimension_descriptor_array(
    llvm::Value* arr, bool load) {
...
llvm::Value* SimpleCMODescriptor::get_rank(llvm::Value* arr, bool get_pointer) {
...
void SimpleCMODescriptor::set_rank(llvm::Value* arr, llvm::Value* rank) {
  llvm::Value* rank_ptr = llvm_utils->create_gep(arr, 4);
  LLVM::CreateStore(*builder, rank, rank_ptr);
}

llvm::Value* SimpleCMODescriptor::get_dimension_size(llvm::Value* dim_des_arr,
                                                     llvm::Value* dim,
                                                     bool load) {
  llvm::Value* dim_size =
      llvm_utils->create_gep(llvm_utils->create_ptr_gep(dim_des_arr, dim), 2);
  if (!load) {
    return dim_size;
  }
...
void SimpleCMODescriptor::fill_array_details(
  ...
  llvm::Value* dim_des_first =
      builder->CreateAlloca(dim_des, LLVM::CreateLoad(*builder, llvm_ndims));
  builder->CreateStore(llvm::ConstantInt::get(context, llvm::APInt(32, n_dims)),
                       get_rank(arr, true));
  ...
  llvm::PointerType* first_ptr2ptr_type =
      static_cast<llvm::PointerType*>(first_ptr->getType());
  llvm::PointerType* first_ptr_type =
      static_cast<llvm::PointerType*>(first_ptr2ptr_type->getElementType());
  llvm::Value* arr_first = builder->CreateAlloca(
      first_ptr_type->getElementType(), LLVM::CreateLoad(*builder, llvm_size));
...
void SimpleCMODescriptor::fill_malloc_array_details(
  ...
  llvm::Value* dim_des_val =
      LLVM::CreateLoad(*builder, llvm_utils->create_gep(arr, 2));
  for (int r = 0; r < n_dims; r++) {
    llvm::Value* dim_val = llvm_utils->create_ptr_gep(dim_des_val, r);
    llvm::Value* s_val = llvm_utils->create_gep(dim_val, 0);
    llvm::Value* l_val = llvm_utils->create_gep(dim_val, 1);
  ...
  llvm::AllocaInst* arg_size =
      builder->CreateAlloca(llvm::Type::getInt32Ty(context), nullptr);
  llvm::DataLayout data_layout(module);
  llvm::Type* ptr2firstptr_type = ptr2firstptr->getType();
  llvm::Type* ptr_type =
      static_cast<llvm::PointerType*>(ptr2firstptr_type)->getElementType();
  uint64_t size = data_layout.getTypeAllocSize(
      static_cast<llvm::PointerType*>(ptr_type)->getElementType());
  llvm::Value* llvm_size =
  ...
  llvm::Value* ptr_as_char_ptr = lfortran_malloc(
      context, *module, *builder, LLVM::CreateLoad(*builder, arg_size));
  llvm::Value* first_ptr = builder->CreateBitCast(ptr_as_char_ptr, ptr_type);
  builder->CreateStore(first_ptr, ptr2firstptr);
}

void SimpleCMODescriptor::fill_dimension_descriptor(llvm::Value* arr,
                                                    int n_dims) {
  llvm::Value* dim_des_val = llvm_utils->create_gep(arr, 2);
  llvm::Value* llvm_ndims =
      builder->CreateAlloca(llvm::Type::getInt32Ty(context), nullptr);
...
llvm::Value* SimpleCMODescriptor::get_pointer_to_dimension_descriptor(
...
llvm::Value* SimpleCMODescriptor::get_upper_bound(llvm::Value* dim_des) {
  llvm::Value* lb =
      LLVM::CreateLoad(*builder, llvm_utils->create_gep(dim_des, 1));
  llvm::Value* dim_size =
      LLVM::CreateLoad(*builder, llvm_utils->create_gep(dim_des, 2));
  return builder->CreateSub(
      builder->CreateAdd(dim_size, lb),
      llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
}

llvm::Value* SimpleCMODescriptor::get_stride(llvm::Value*) { return nullptr; }
...
llvm::Value* SimpleCMODescriptor::cmo_convertor_single_element(
    llvm::Value* arr, std::vector<llvm::Value*>& m_args, int n_args,
    bool check_for_bounds) {
  llvm::Value* dim_des_arr_ptr =
      LLVM::CreateLoad(*builder, llvm_utils->create_gep(arr, 2));
  ...
  for (int r = 0; r < n_args; r++) {
    ...
    llvm::Value* lval =
        LLVM::CreateLoad(*builder, llvm_utils->create_gep(dim_des_ptr, 1));
    ...
    llvm::Value* dim_size =
        LLVM::CreateLoad(*builder, llvm_utils->create_gep(dim_des_ptr, 2));
    prod = builder->CreateMul(prod, dim_size);
  }
  return idx;
}
...
llvm::Value* SimpleCMODescriptor::get_single_element(
    llvm::Value* array, std::vector<llvm::Value*>& m_args, int n_args,
    bool data_only, llvm::Value** llvm_diminfo) {
  ...
  if (data_only) {
    ...
    idx = cmo_convertor_single_element_data_only(llvm_diminfo, m_args, n_args,
                                                 check_for_bounds);
    ...
    tmp =
        llvm_utils->create_ptr_gep(LLVM::CreateLoad(*builder, full_array), idx);
  }
  return tmp;
}
...
void SimpleCMODescriptor::set_is_allocated_flag(llvm::Value* array,
                                                uint64_t status) {
...
llvm::Value* SimpleCMODescriptor::get_array_size(llvm::Value* array,
                                                 llvm::Value* dim, int kind,
                                                 int dim_kind) {
  llvm::Value* dim_des_val =
      this->get_pointer_to_dimension_descriptor_array(array);
  llvm::Value* tmp = nullptr;
  if (dim) {
    tmp = builder->CreateSub(
        dim, llvm::ConstantInt::get(context, llvm::APInt(dim_kind * 8, 1)));
  ...
  llvm::Value* llvm_size =
      builder->CreateAlloca(llvm_utils->getIntType(kind), nullptr);
  builder->CreateStore(
      llvm::ConstantInt::get(context, llvm::APInt(kind * 8, 1)), llvm_size);
  ...
  llvm::Value* cond =
      builder->CreateICmpSLT(LLVM::CreateLoad(*builder, r), rank);
  ...
  r_val = builder->CreateAdd(
      r_val, llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
...
llvm::Value* SimpleCMODescriptor::reshape(llvm::Value* array,
                                          llvm::Value* shape,
                                          llvm::Module* module) {
  llvm::Value* reshaped = builder->CreateAlloca(
      array->getType()->getContainedType(0), nullptr, "reshaped");
  ...
  llvm::PointerType* first_ptr_type =
      static_cast<llvm::PointerType*>(first_ptr2ptr_type->getElementType());
  llvm::Value* arr_first =
      builder->CreateAlloca(first_ptr_type->getElementType(), num_elements);
  builder->CreateStore(arr_first, first_ptr);
  ...
  llvm::Value* llvm_size =
      llvm::ConstantInt::get(context, llvm::APInt(32, size));
  num_elements = builder->CreateMul(num_elements, llvm_size);
  builder->CreateMemCpy(LLVM::CreateLoad(*builder, first_ptr),
                        llvm::MaybeAlign(),
                        LLVM::CreateLoad(*builder, ptr2firstptr),
                        llvm::MaybeAlign(), num_elements);

  if (this->is_array(shape)) {
    llvm::Value* n_dims = this->get_array_size(shape, nullptr, 4);
    llvm::Value* shape_data =
        LLVM::CreateLoad(*builder, this->get_pointer_to_data(shape));
    ...
    llvm_utils->start_new_block(loophead);
    llvm::Value* cond =
        builder->CreateICmpSLT(LLVM::CreateLoad(*builder, r), n_dims);
    builder->CreateCondBr(cond, loopbody, loopend);
    ...
    builder->CreateStore(llvm::ConstantInt::get(context, llvm::APInt(32, 1)),
                         s_val);
    llvm::Value* dim_size = LLVM::CreateLoad(
        *builder, llvm_utils->create_ptr_gep(shape_data, r_val));
    builder->CreateStore(dim_size, dim_size_ptr);
    r_val = builder->CreateAdd(
        r_val, llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
    builder->CreateStore(r_val, r);
...
void SimpleCMODescriptor::copy_array(llvm::Value* src, llvm::Value* dest) {
  llvm::Value* src_data_ptr =
      LLVM::CreateLoad(*builder, this->get_pointer_to_data(src));
  builder->CreateStore(src_data_ptr, this->get_pointer_to_data(dest));
  llvm::Value* src_offset_ptr =
      LLVM::CreateLoad(*builder, llvm_utils->create_gep(src, 1));
  builder->CreateStore(src_offset_ptr, llvm_utils->create_gep(dest, 1));
  llvm::Value* src_dim_des_val =
      this->get_pointer_to_dimension_descriptor_array(src, true);
  llvm::Value* dest_dim_des_val =
      this->get_pointer_to_dimension_descriptor_array(dest, true);
  ...
  llvm::Value* dest_dim_val =
      llvm_utils->create_ptr_gep(dest_dim_des_val, r_val);
  ...
  builder->CreateStore(LLVM::CreateLoad(*builder, src_dim_size_ptr),
                       dest_dim_size_ptr);
  r_val = builder->CreateAdd(
      r_val, llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
  builder->CreateStore(r_val, r);
  ...
  llvm::Value* src_rank_ptr =
      LLVM::CreateLoad(*builder, llvm_utils->create_gep(src, 4));
