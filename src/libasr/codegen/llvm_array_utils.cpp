...
namespace LLVMArrUtils
{
        ...
        switch (v->m_type->type) {
            ...
            default: {
                throw LCompilersException(
                    "Explicit shape checking supported only for integer, real, complex, logical and derived types.");
            }
        }
        return compile_time_dimensions_t(m_dims, n_dims);
    }

    std::unique_ptr<Descriptor> Descriptor::get_descriptor(llvm::LLVMContext& context,
                                                           llvm::IRBuilder<>* builder,
                                                          ...
                                                          bool data_only)
    {
        ...
        llvm::Value* third_ele_ptr
            = LLVM::CreateLoad(*builder, get_pointer_to_dimension_descriptor_array(tmp));
        ...
        ASR::ttype_t* m_type_, int a_kind, int rank, llvm::Type* el_type, bool get_pointer)
    {
        ...
        llvm::Type* dim_des_array = create_dimension_descriptor_array_type();
        std::vector<llvm::Type*> array_type_vec = { el_type->getPointerTo(),
                                                    llvm::Type::getInt32Ty(context),
                                                    dim_des_array,
                                                    llvm::Type::getInt1Ty(context),
                                                    llvm::Type::getInt32Ty(context) };
    ...
    llvm::Type* SimpleCMODescriptor::get_dimension_descriptor_type(bool get_pointer)
    {
        if (!get_pointer) {
            return dim_des;
        }
    ...
    llvm::Value* SimpleCMODescriptor::get_pointer_to_dimension_descriptor_array(llvm::Value* arr,
                                                                                bool load)
    {
    ...
    void SimpleCMODescriptor::fill_array_details(
        llvm::Value* arr, int n_dims, std::vector<std::pair<llvm::Value*, llvm::Value*>>& llvm_dims)
    {
        ...
        llvm::Module* module)
    {
        ...
        llvm::Value* ptr_as_char_ptr
            = lfortran_malloc(context, *module, *builder, LLVM::CreateLoad(*builder, arg_size));
    ...
    llvm::Value* SimpleCMODescriptor::cmo_convertor_single_element(
        llvm::Value* arr, std::vector<llvm::Value*>& m_args, int n_args, bool check_for_bounds)
    {
                                                     ...
                                                     int dim_kind)
    {
        ...
        llvm::Value* tmp = nullptr;
        if (dim) {
            tmp = builder->CreateSub(dim,
                                     llvm::ConstantInt::get(context, llvm::APInt(dim_kind * 8, 1)));
    ...
    {
        llvm::Value* reshaped
            = builder->CreateAlloca(array->getType()->getContainedType(0), nullptr, "reshaped");
        ...
        if (this->is_array(shape)) {
            ...
            builder->CreateStore(llvm::ConstantInt::get(context, llvm::APInt(32, 1)), s_val);
            llvm::Value* dim_size
                = LLVM::CreateLoad(*builder, llvm_utils->create_ptr_gep(shape_data, r_val));
            builder->CreateStore(dim_size, dim_size_ptr);
            r_val = builder->CreateAdd(r_val, llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
            builder->CreateStore(r_val, r);
            builder->CreateBr(loophead);
