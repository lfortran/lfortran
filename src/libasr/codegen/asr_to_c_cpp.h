...
class BaseCCPPVisitor : public ASR::BaseVisitor<Derived> {
 ...
 public:
  ...
  BaseCCPPVisitor(diag::Diagnostics &diag, Platform &platform,
      ...
      : diag{diag},
        platform{platform},
        gen_stdstring{gen_stdstring},
        gen_stdcomplex{gen_stdcomplex},
        is_c{is_c},
...
#include <lfortran_intrinsics.h>
  ...
  void visit_ArraySize(const ASR::ArraySize_t &x) {
    ...
    if (x.m_dim == nullptr) {
      ...
      args +=
          std::to_string(ASRUtils::extract_kind_from_ttype_t(x.m_type)) + "-1";
    }
  ...
  std::string get_c_type_from_ttype_t(ASR::ttype_t *t) {
    ...
    switch (t->type) {
      ...
      case ASR::ttypeType::Derived: {
        ...
        type_src = std::string("struct ") +
                   ASRUtils::symbol_name(der_type->m_derived_type);
        break;
      }
      default: {
        throw CodeGenError("Type " + ASRUtils::type_to_str_python(t) +
                           " not supported yet.");
      }
    }
    return type_src;
  ...
  void visit_CPtrToPointer(const ASR::CPtrToPointer_t &x) {
    ...
    std::string type_src =
        get_c_type_from_ttype_t(ASRUtils::expr_type(x.m_ptr));
    std::string indent(indentation_level * indentation_spaces, ' ');
