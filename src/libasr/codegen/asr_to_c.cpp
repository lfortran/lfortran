...
std::string format_type_c(const std::string &dims, const std::string &type,
                          const std::string &name, bool use_ref,
                          bool /*dummy*/) {
  std::string fmt;
  std::string ref = "";
  if (use_ref) ref = "*";
...
class ASRToCVisitor : public BaseCCPPVisitor<ASRToCVisitor> {
 public:
  ...
  std::string get_array_type(std::string type_name,
                             std::string encoded_type_name, size_t n_dims,
                             bool make_ptr = true) {
    if (eltypedims2arraytype.find(encoded_type_name) !=
    ...
    std::string array_data =
        format_type_c("*", type_name, "data", false, false);
  ...
  void generate_array_decl(std::string &sub, std::string v_m_name,
                           ...
                           ASR::dimension_t *m_dims, int n_dims, bool use_ref,
                           bool dummy, bool declare_value,
    ...
    std::string type_name_without_ptr =
        get_array_type(type_name, encoded_type_name, n_dims, false);
    if (declare_value) {
      std::string variable_name = std::string(v_m_name) + "_value";
      sub = format_type_c("", type_name_without_ptr, variable_name, use_ref,
                          dummy) +
            ";\n";
      ...
      if (!is_pointer) {
        sub += indent +
               format_type_c(dims, type_name_copy,
                             std::string(v_m_name) + "_data", use_ref, dummy) +
               ";\n";
        sub += indent + std::string(v_m_name) +
               "->data = " + std::string(v_m_name) + "_data;\n";
      ...
      if (ASRUtils::is_integer(*t2)) {
        ...
        if (is_array) {
          ...
          generate_array_decl(sub, std::string(v.m_name), type_name, dims,
                              encoded_type_name, t->m_dims, t->n_dims, use_ref,
                              dummy,
        ...
        std::string dims = convert_dims_c(t->n_dims, t->m_dims);
        sub = format_type_c(dims, "struct " + der_type_name + "*", v.m_name,
                            use_ref, dummy);
      } else {
      ...
      if (ASRUtils::is_integer(*v.m_type)) {
        ...
        if (is_array) {
          ...
          generate_array_decl(sub, std::string(v.m_name), type_name, dims,
                   ...
                   pre_initialise_derived_type) {
          dims = convert_dims_c(t->n_dims, t->m_dims);
          std::string value_var_name = v.m_parent_symtab->get_unique_name(
              std::string(v.m_name) + "_value");
          sub = format_type_c(dims, "struct " + der_type_name, value_var_name,
                              use_ref, dummy);
          ...
          sub += ";\n";
          sub += indent + format_type_c("", "struct " + der_type_name + "*",
...
#define ASSERT_MSG(cond, msg)                                                  \
  ...
  void visit_DerivedType(const ASR::DerivedType_t &x) {
    ...
    std::string open_struct =
        indent + "struct " + std::string(x.m_name) + " {\n";
    std::string body = "";
    indent.push_back(' ');
    for (size_t i = 0; i < x.n_members; i++) {
      ...
      body += indent +
              convert_variable_decl(*ASR::down_cast<ASR::Variable_t>(member),
                                    false) +
              ";\n";
    }
    indentation_level -= 1;
  ...
  void visit_Print(const ASR::Print_t &x) {
    ...
    for (size_t i = 0; i < x.n_values; i++) {
      ...
      out += get_print_type(value_type,
                            ASR::is_a<ASR::ArrayItem_t>(*x.m_values[i]));
      v.push_back(src);
      if (i + 1 != x.n_values) {
        out += "\%s";
  ...
  void visit_StringItem(const ASR::StringItem_t &x) {
    ...
    src = "(char *)memcpy((char *)calloc(2U, sizeof(char)), " + str + " + " +
          idx + " - 1, 1U)";
  }
