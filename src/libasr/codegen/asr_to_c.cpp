...
std::string
format_type_c(const std::string& dims,
...
public:
    ...
    ASRToCVisitor(diag::Diagnostics& diag, Platform& platform, int64_t default_lower_bound)
    ...
    std::string convert_dims_c(size_t n_dims, ASR::dimension_t* m_dims, bool convert_to_1d = false)
    {
        std::string dims;
        size_t size = 1;
                             ...
                             bool is_pointer = false)
    {
        ...
        if (declare_value) {
            ...
            if (!is_pointer) {
                sub += indent
                       + format_type_c(
                           dims, type_name_copy, std::string(v_m_name) + "_data", use_ref, dummy)
                       + ";\n";
                sub += indent + std::string(v_m_name) + "->data = " + std::string(v_m_name)
                       + "_data;\n";
                                      ...
                                      bool use_static = true)
    {
        ...
        if (ASRUtils::is_pointer(v.m_type)) {
            ...
            if (ASRUtils::is_integer(*t2)) {
                ...
                std::string dims = convert_dims_c(t->n_dims, t->m_dims);
                sub = format_type_c(
                    dims, "struct " + der_type_name + "*", v.m_name, use_ref, dummy);
            } else {
            ...
            if (ASRUtils::is_integer(*v.m_type)) {
                ...
                if (is_array) {
                    ...
                    dims = convert_dims_c(t->n_dims, t->m_dims);
                    std::string value_var_name
                        = v.m_parent_symtab->get_unique_name(std::string(v.m_name) + "_value");
                    sub = format_type_c(
                        dims, "struct " + der_type_name, value_var_name, use_ref, dummy);
                    ...
                    sub += indent
                           + format_type_c(
                               "", "struct " + der_type_name + "*", v.m_name, use_ref, dummy);
                    if (t->n_dims != 0) {
                        sub += " = " + value_var_name;
                    } else {
                        sub += " = &" + value_var_name;
                    ...
                    dims = convert_dims_c(t->n_dims, t->m_dims);
                    if (v.m_intent == ASRUtils::intent_in || v.m_intent == ASRUtils::intent_inout) {
                        use_ref = false;
                        dims = "";
                    }
                    sub = format_type_c(
...
#define ASSERT_MSG(cond, msg)                                                  \
    ...
    void visit_ArrayItem(const ASR::ArrayItem_t& x)
    {
        ...
        for (size_t i = 0; i < x.n_args; i++) {
            ...
            current_index
                += "(" + src + " - " + array + "->dims[" + std::to_string(i) + "].lower_bound)";
    ...
    void visit_StringItem(const ASR::StringItem_t& x)
    {
        ...
        src = "(char *)memcpy((char *)calloc(2U, sizeof(char)), " + str + " + " + idx + " - 1, 1U)";
    }
