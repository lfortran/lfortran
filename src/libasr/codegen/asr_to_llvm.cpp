...
public:
    ...
    */
    ...
    // llvm::Value* create_and_setup_string(ASR::String_t* str, std::string name = ""){
    //     switch(str->m_physical_type){
    //         case ASR::DescriptorString:{
    //             llvm_utils->create_string(str, name);
    //             setup_string
    //             break;
    //         }
    //         default:
...
ptr_type[ptr_member] = llvm_utils->get_type_from_ttype_t_util(
    ...
    */
    void visit_expr_load_wrapper(ASR::expr_t* x, int desired_ptr_load){
        int ptr_loads_copy = ptr_loads;
    ...
    void visit_StringItem(const ASR::StringItem_t& x) {
        ...
        if( is_assignment_target ) { // Hackish
            ...
            // TODO : Create a stringDescriptor of length 1 using LLVM, instead of calling runtime C function
    ...
    void visit_ArrayConstantUtil(const ASR::ArrayConstant_t &x) {
        ...
        } else if (ASRUtils::is_character(*x_m_type)) { // SepcialCase.
            char* whole_array = (char*)x.m_data;
            ASR::String_t* str = ASRUtils::get_string_type(x_m_type);
            tmp = declare_global_constant_string(str->m_physical_type, whole_array, str->m_len);
            return;
        }
    ...
    llvm::Value* declare_global_constant_string(
        ASR::string_physical_typeType phys_type, char* data, ASR::expr_t* len_expr){
        int64_t len;
        {
            bool is_value_const = ASRUtils::is_value_constant(len_expr, len);
            LCOMPILERS_ASSERT(is_value_const);
        }
        llvm::Constant* string_constant = builder->CreateGlobalStringPtr(data,"str_const");
        switch(phys_type){
            case ASR::DescriptorString:{
    ...
    void visit_StringConstant(const ASR::StringConstant_t &x) {
        ...
        tmp = declare_global_constant_string(
            str->m_physical_type, x.m_s, str->m_len);
    }
            ...
            if (ASR::is_a<ASR::Var_t>(*x.m_args[i].m_value)) {
                ...
                *ASRUtils::type_get_past_pointer(
                ...
                switch(arg_type->type) {
                    ...
                    case ASR::String:{ // Non-Primitive, no need to create alloca and store.
                        tmp = value;
                        break;
                    }
                    default: {
