...
public:
    ...
    llvm::Value* CreateLoad2(ASR::ttype_t *type, llvm::Value *x) {
        llvm::Type* el_type = llvm_utils->get_type_from_ttype_t_util(type, module.get());
                    ...
                    if (llvm_symtab.find(h) != llvm_symtab.end()) {
                        ...
                        if( !ASRUtils::is_array(arg->m_type) ) {
                            ...
                            if (x_abi == ASR::abiType::BindC) {
                                llvm::Type* array_data_type = llvm_utils->get_el_type(ASRUtils::type_get_past_array(arg->m_type), module.get());
                                tmp = builder->CreateBitCast(tmp, array_data_type->getPointerTo());
                            } else if (orig_arg && !LLVM::is_llvm_pointer(*orig_arg->m_type) && LLVM::is_llvm_pointer(*arg->m_type)) {
                                tmp = LLVM::CreateLoad(*builder, tmp);
                            }
                        }
                    } else {
