        ...
        for (size_t i = 0; i < x.n_args; i++) {
            ...
            if (ASR::is_a<ASR::ArrayPhysicalCast_t>(*arg_expr)) {
                ...
                if (arr_cast->m_old == ASR::DescriptorArray && (arr_cast->m_new == ASR::PointerArray || arr_cast->m_new == ASR::FixedSizeArray)) {
                    ...
                    (ASR::is_a<ASR::Integer_t>(*ASRUtils::type_get_past_allocatable(arg_expr_type)) ||
                    ASR::is_a<ASR::Real_t>(*ASRUtils::type_get_past_allocatable(arg_expr_type)) ||
                    ASR::is_a<ASR::Complex_t>(*ASRUtils::type_get_past_allocatable(arg_expr_type)) ||
                    ASR::is_a<ASR::Logical_t>(*ASRUtils::type_get_past_allocatable(arg_expr_type)))) {
                int64_t ptr_loads_copy = ptr_loads;
                ptr_loads = 1;
                this->visit_expr_wrapper(arg_expr, false);
                ptr_loads = ptr_loads_copy;

                ASR::Variable_t *arg_variable = ASRUtils::expr_to_variable_or_null(arg_expr);
                ASR::FunctionType_t *ft = ASRUtils::get_FunctionType(function);
                if (!ASRUtils::is_allocatable(ft->m_arg_types[i]) &&
                    (arg_variable && arg_variable->m_presence != ASR::presenceType::WasOptional)) {
