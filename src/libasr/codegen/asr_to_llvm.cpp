    ...
    void visit_AllocateUtil(const T& x, ASR::expr_t* m_stat, bool realloc, ASR::expr_t* m_source = nullptr) {
        for( size_t i = 0; i < x.n_args; i++ ) {
            ...
            if( n_dims == 0 ) {
                ...
                if( ASR::is_a<ASR::StructType_t>(*ASRUtils::extract_type(ASRUtils::expr_type(tmp_expr))) && !ASRUtils::is_class_type(ASRUtils::expr_type(tmp_expr)) ) {
        ...
        for (auto &item : x.m_symtab->get_scope()) {
            if (is_a<ASR::Variable_t>(*item.second)) {
                ...
                if (x.m_parent_module && func_type->m_module) {
                    ASR::Module_t* parent_module = ASR::down_cast<ASR::Module_t>(ASRUtils::symbol_get_past_external(x.m_parent_module));
    ...
    void visit_Associate(const ASR::Associate_t& x) {
        if( ASR::is_a<ASR::ArraySection_t>(*x.m_value) ) {
            ...
            if (ASR::is_a<ASR::PointerNullConstant_t>(*x.m_value)) {
                ...
                llvm::Type* llvm_target_type = llvm_utils->get_type_from_ttype_t_util(target_type, module.get());
                ...
                llvm::Type* ptr_type = llvm_utils->getStructType(ASRUtils::extract_type(value_type), module.get(), true);
            ...
            if( orig_arg && !(ASR::is_a<ASR::Pointer_t>(*expr_type(x.m_args[i].m_value)) && ASRUtils::is_class_type(
                ASRUtils::type_get_past_array(
                    ASRUtils::type_get_past_allocatable(
                        ASRUtils::type_get_past_pointer(
                            ASRUtils::expr_type(x.m_args[i].m_value)))))) ) {
                tmp = convert_to_polymorphic_arg(tmp, ASRUtils::EXPR(ASR::make_Var_t(al, orig_arg->base.base.loc, &orig_arg->base)),
                    ASRUtils::type_get_past_allocatable(
    ...
    llvm::Value* convert_to_polymorphic_arg(llvm::Value* dt, ASR::expr_t* s_m_args0,
        ...
        if( ASRUtils::is_unlimited_polymorphic_type(s_m_args0) ) {
            ...
            if (struct_t->m_is_cstruct) {
                ...
                llvm::Type* _type = llvm_utils->get_type_from_ttype_t_util(ASRUtils::type_get_past_array(s_m_args0_type), module.get());
                ...
                builder->CreateStore(builder->CreateBitCast(dt, llvm_utils->getStructType(ASRUtils::type_get_past_array(s_m_args0_type), module.get(), true)), class_ptr);
                ...
                ASR::StructType_t* class_t = ASR::down_cast<ASR::StructType_t>(ASRUtils::extract_type(arg_type));
                ASR::symbol_t* struct_sym = ASRUtils::symbol_get_past_external(class_t->m_derived_type);
                if( type2vtab.find(struct_sym) == type2vtab.end() &&
                    type2vtab[struct_sym].find(current_scope) == type2vtab[struct_sym].end() ) {
                    create_vtab_for_struct_type(struct_sym, current_scope);
                }
                ...
                ASR::StructType_t* dest_class = ASR::down_cast<ASR::StructType_t>(ASRUtils::extract_type(s_m_args0_type));
                ASR::symbol_t* dest_struct_sym = ASRUtils::symbol_get_past_external(dest_class->m_derived_type);
    ...
    void visit_SubroutineCall(const ASR::SubroutineCall_t &x) {
        ...
        if (x.m_dt && (!is_nopass)) {
            ...
            if (ASR::is_a<ASR::Var_t>(*x.m_dt)) {
                ...
                dt = convert_to_polymorphic_arg(dt, s->m_args[0], s_m_args0_type, dt_type);
                args.push_back(dt);
                ...
                llvm::Value* dt_polymorphic = convert_to_polymorphic_arg(dt, s_m_args0, s_m_args0_type, caller_type);
                ...
                dt_polymorphic = convert_to_polymorphic_arg(dt, s->m_args[0], ASRUtils::expr_type(s->m_args[0]),
