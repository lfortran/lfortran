...
namespace
{
    ...
    public:
        ...
        CodeGenError(const std::string& msg, const Location& loc)
            : d{ diag::Diagnostic(
                msg, diag::Level::Error, diag::Stage::CodeGen, { diag::Label("", { loc }) }) }
        {
        }
    };
...
void
printf(llvm::LLVMContext& context,
       llvm::Module& module,
       ...
       const std::vector<llvm::Value*>& args)
{
    ...
    if (!fn_printf) {
        ...
        fn_printf = llvm::Function::Create(
            function_type, llvm::Function::ExternalLinkage, "_lfortran_printf", &module);
    }
    builder.CreateCall(fn_printf, args);
}

void
exit(llvm::LLVMContext& context,
     llvm::Module& module,
     llvm::IRBuilder<>& builder,
     llvm::Value* exit_code)
{
    ...
    if (!fn_exit) {
        ...
        fn_exit = llvm::Function::Create(
            function_type, llvm::Function::ExternalLinkage, "exit", &module);
    }
    builder.CreateCall(fn_exit, { exit_code });
}

void
string_init(llvm::LLVMContext& context,
            llvm::Module& module,
            llvm::IRBuilder<>& builder,
            llvm::Value* arg_size,
...
public:
    ...
    void fill_array_details(llvm::Value* arr, ASR::dimension_t* m_dims, int n_dims)
    {
    ...
    */
    inline void fill_malloc_array_details(llvm::Value* arr, ASR::dimension_t* m_dims, int n_dims)
    {
        std::vector<std::pair<llvm::Value*, llvm::Value*>> llvm_dims;
        for (int r = 0; r < n_dims; r++) {
            ASR::dimension_t m_dim = m_dims[r];
        ...
        switch (mem_type->type) {
            ...
            default:
                throw CodeGenError("Cannot identify the type of member, '"
                                       + std::string(member->m_name) + "' in derived type, '"
                                       + der_type_name + "'.",
    ...
    llvm::Value* lfortran_strop(llvm::Value* left_arg,
                                llvm::Value* right_arg,
                                std::string runtime_func_name)
    {
        llvm::Function* fn = module->getFunction(runtime_func_name);
    ...
    llvm::Value* lfortran_str_cmp(llvm::Value* left_arg,
                                  llvm::Value* right_arg,
                                  std::string runtime_func_name)
    {
        llvm::Function* fn = module->getFunction(runtime_func_name);
    ...
    llvm::Value* lfortran_str_len(llvm::Value* str)
    {
        ...
        if (!fn) {
            llvm::FunctionType* function_type = llvm::FunctionType::get(
                llvm::Type::getInt32Ty(context), { character_type->getPointerTo() }, false);
            fn = llvm::Function::Create(
                function_type, llvm::Function::ExternalLinkage, runtime_func_name, *module);
        }
    ...
    llvm::Value* lfortran_str_copy(llvm::Value* str, llvm::Value* idx1, llvm::Value* idx2)
    {
        ...
        if (!fn) {
            llvm::FunctionType* function_type
                = llvm::FunctionType::get(character_type,
                                          { character_type,
                                            llvm::Type::getInt32Ty(context),
                                            llvm::Type::getInt32Ty(context) },
                                          false);
    ...
    llvm::Value* lfortran_str_slice(llvm::Value* str,
                                    llvm::Value* idx1,
                                    llvm::Value* idx2,
                                    llvm::Value* step,
    ...
    llvm::Value* lfortran_type_to_str(llvm::Value* arg,
    ...
    llvm::Value* complex_re(llvm::Value* c, llvm::Type* complex_type = nullptr)
    {
        ...
        std::vector<llvm::Value*> idx = { llvm::ConstantInt::get(context, llvm::APInt(32, 0)),
    ...
    llvm::Value* complex_from_floats(llvm::Value* re,
                                     llvm::Value* im,
                                     llvm::Type* complex_type = nullptr)
    {
        if (complex_type == nullptr) {
        ...
        std::vector<llvm::Value*> idx1 = { llvm::ConstantInt::get(context, llvm::APInt(32, 0)),
                                           llvm::ConstantInt::get(context, llvm::APInt(32, 0)) };
        std::vector<llvm::Value*> idx2 = { llvm::ConstantInt::get(context, llvm::APInt(32, 0)),
                                           llvm::ConstantInt::get(context, llvm::APInt(32, 1)) };
    ...
    llvm::Value* nested_struct_rd(std::vector<llvm::Value*> vals, llvm::StructType* rd)
    {
    ...
    void visit_TranslationUnit(const ASR::TranslationUnit_t& x)
    {
        ...
        std::vector<llvm::Type*> els_4_ptr
            = { llvm::Type::getFloatPtrTy(context), llvm::Type::getFloatPtrTy(context) };
        std::vector<llvm::Type*> els_8_ptr
            = { llvm::Type::getDoublePtrTy(context), llvm::Type::getDoublePtrTy(context) };
        ...
        llvm::Type* bound_arg
            = static_cast<llvm::Type*>(arr_descr->get_dimension_descriptor_type(true));
    ...
    void visit_Nullify(const ASR::Nullify_t& x)
    {
        for (size_t i = 0; i < x.n_vars; i++) {
            ...
            llvm::Value* np
                = builder->CreateIntToPtr(llvm::ConstantInt::get(context, llvm::APInt(32, 0)), tp);
    ...
    void _Deallocate(const T& x)
    {
        ...
        if (!free_fn) {
            llvm::FunctionType* function_type
                = llvm::FunctionType::get(llvm::Type::getVoidTy(context), { character_type }, true);
    ...
    void visit_TupleConstant(const ASR::TupleConstant_t& x)
    {
        ...
        std::string type_code = ASRUtils::get_type_code(tuple_type->m_type, tuple_type->n_type);
    ...
    void visit_ArrayItem(const ASR::ArrayItem_t& x)
    {
        ...
        if (ASR::is_a<ASR::Character_t>(*x.m_type) && n_dims == 0) {
            ...
            // TODO: Currently the string starts at the right location, but goes to the end of the
            // original string. We have to allocate a new string, copy it and add null termination.
            ...
            tmp = arr_descr->get_single_element(
                array, indices, x.n_args, is_data_only, llvm_diminfo.p);
        }
    }
    ...
    void visit_ArraySection(const ASR::ArraySection_t& x)
    {
        ...
        ASR::dimension_t* m_dims;
        int n_dims = ASRUtils::extract_dimensions_from_ttype(ASRUtils::expr_type(x.m_v), m_dims);
        LFORTRAN_ASSERT(ASR::is_a<ASR::Character_t>(*ASRUtils::expr_type(x.m_v)) && n_dims == 0);
        // String indexing:
        if (x.n_args == 1) {
        ...
        llvm::Value* idx1 = tmp;
        // idx = builder->CreateSub(idx, llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
        // std::vector<llvm::Value*> idx_vec = {llvm::ConstantInt::get(context, llvm::APInt(32, 0)),
        // idx};
        // std::vector<llvm::Value*> idx_vec = {idx};
        llvm::Value* str = CreateLoad(array);
    ...
    void visit_Variable(const ASR::Variable_t& x)
    {
        ...
        LFORTRAN_ASSERT(x.m_intent == intent_local || x.m_abi == ASR::abiType::Interactive);
        ...
        if (x.m_type->type == ASR::ttypeType::Integer) {
            ...
            if (!external) {
                if (init_value) {
                    ...
                    module->getNamedGlobal(x.m_name)->setInitializer(
                        llvm::ConstantPointerNull::get(static_cast<llvm::PointerType*>(void_ptr)));
                }
            }
            llvm_symtab[h] = ptr;
            ...
            llvm::Type* x_ptr = get_type_from_ttype_t(x.m_type,
                                                      x.m_storage,
                                                      is_array_type,
                                                      is_malloc_array_type,
                                                      is_list,
            ...
            if (!external) {
                if (init_value) {
                    ...
                    module->getNamedGlobal(x.m_name)->setInitializer(
                        llvm::ConstantPointerNull::get(static_cast<llvm::PointerType*>(x_ptr)));
                }
            }
            llvm_symtab[h] = ptr;
    ...
    void start_module_init_function_prototype(const ASR::Module_t& x)
    {
        ...
        llvm::Function* F = llvm::Function::Create(
            function_type, llvm::Function::ExternalLinkage, module_fn_name, module.get());
                                      ...
                                      int& a_kind)
    {
        ...
        switch (asr_type->type) {
            case (ASR::ttypeType::Integer): {
                ...
                if (n_dims > 0) {
                    ...
                    if (m_storage == ASR::storage_typeType::Allocatable) {
                        ...
                        llvm_type
                            = arr_descr->get_malloc_array_type(asr_type, a_kind, n_dims, el_type);
                    } else {
            ...
            case (ASR::ttypeType::Pointer): {
                ...
                llvm_type = get_type_from_ttype_t(t2,
                                                  m_storage,
                                                  is_array_type,
                                                  is_malloc_array_type,
                                                  is_list,
            ...
            case (ASR::ttypeType::List): {
                ...
                llvm::Type* el_llvm_type = get_type_from_ttype_t(asr_list->m_type,
                                                                 m_storage,
                                                                 is_array_type,
                                                                 is_malloc_array_type,
                                                                 is_list,
        ...
        for (auto& item : x.m_symtab->get_scope()) {
            if (is_a<ASR::Variable_t>(*item.second)) {
                ...
                if (v->m_intent == intent_local || v->m_intent == intent_return_var
                    || !v->m_intent) {
                    type = get_type_from_ttype_t(v->m_type,
                                                 v->m_storage,
                                                 is_array_type,
                                                 is_malloc_array_type,
                                                 is_list,
                    ...
                    if (x.class_type == ASR::symbolType::Function) {
                        ...
                        if (is_array_type && !is_list) {
                            ...
                            if (abi_type == ASR::abiType::Source && is_v_arg) {
                                type = arr_descr->get_argument_type(
                                    type, m_h, v->m_name, arr_arg_type_cache);
                                is_array_type = false;
                    ...
                    llvm_symtab[h] = ptr;
                    if (is_malloc_array_type && v->m_type->type != ASR::ttypeType::Pointer
                        && !is_list) {
                    ...
                    if (v->m_symbolic_value != nullptr && !ASR::is_a<ASR::List_t>(*v->m_type)) {
                        target_var = ptr;
                        tmp = nullptr;
                        ...
                        if (finder != nested_globals.end()) {
                            ...
                            builder->CreateStore(target_var, llvm_utils->create_gep(ptr, idx));
                        }
                    } else {
                        if (is_a<ASR::Character_t>(*v->m_type) && !is_array_type && !is_list) {
                            ...
                            if (strlen >= 0) {
                                ...
                                arg_size = builder->CreateAdd(
                                    arg_size, llvm::ConstantInt::get(context, llvm::APInt(32, 1)));
                                          ...
                                          ASR::intentType arg_intent)
    {
        ...
        switch (asr_type->type) {
            case (ASR::ttypeType::Integer): {
                ...
                if (n_dims > 0) {
                    if (m_abi == ASR::abiType::BindC
                        ...
                        llvm::Type* el_type = get_el_type(asr_type);
                        if (m_storage == ASR::storage_typeType::Allocatable) {
                            type = arr_descr->get_malloc_array_type(
                                asr_type, a_kind, n_dims, el_type, true);
                        } else {
                            type = arr_descr->get_array_type(
                                asr_type, a_kind, n_dims, el_type, true);
                        }
                    }
                } else {
            ...
            case (ASR::ttypeType::Pointer): {
                ...
                type = get_arg_type_from_ttype_t(t2,
                                                 m_abi,
                                                 arg_m_abi,
                                                 m_storage,
                                                 arg_m_value_attr,
    ...
    std::vector<llvm::Type*> convert_args(const T& x)
    {
        ...
        for (size_t i = 0; i < x.n_args; i++) {
            if (is_a<ASR::Variable_t>(
                    *symbol_get_past_external(ASR::down_cast<ASR::Var_t>(x.m_args[i])->m_v))) {
                ...
                int n_dims = 0, a_kind = 4;
                bool is_array_type = false;
                type = get_arg_type_from_ttype_t(arg->m_type,
                                                 x.m_abi,
                                                 arg->m_abi,
                                                 arg->m_storage,
                                                 arg->m_value_attr,
                                                 ...
                                                 is_array_type,
                                                 arg->m_intent);
                if (arg->m_intent == ASRUtils::intent_out && ASR::is_a<ASR::CPtr_t>(*arg->m_type)) {
                    type = type->getPointerTo();
                }
                std::uint32_t m_h;
                std::string m_name = std::string(x.m_name);
                ...
                if (is_array_type && arg->m_type->type != ASR::ttypeType::Pointer) {
                    if (x.m_abi == ASR::abiType::Source) {
                        ...
                        type = arr_descr->get_argument_type(
                            orig_type, m_h, arg->m_name, arr_arg_type_cache);
                        is_array_type = false;
    ...
    {
        bool finder
            = std::find(nested_call_out.begin(), nested_call_out.end(), parent_function_hash)
              != nested_call_out.end();
        ...
        if (nested_func_types[parent_function_hash].size() > 0
            && parent_function_hash != calling_function_hash && finder && !is_nested_call) {
            llvm::Value* sp_loc
                = module->getOrInsertGlobal(nested_sp_name, llvm::Type::getInt32Ty(context));
            ...
            for (auto& item : x->m_symtab->get_scope()) {
                if (is_a<ASR::Variable_t>(*item.second)) {
                    ...
                    if (finder != nested_globals.end()) {
                        int idx = std::distance(nested_globals.begin(), finder);
                        llvm::Value* glob_struct
                            = module->getOrInsertGlobal(nested_desc_name, nested_global_struct);
                        llvm::Value* target = CreateLoad(llvm_utils->create_gep(glob_struct, idx));
                        llvm::Value* glob_stack
                            = module->getOrInsertGlobal(nested_stack_name, nested_global_stack);
                        llvm::Value* glob_stack_gep = llvm_utils->create_gep(glob_stack, sp_val);
                        llvm::Value* glob_stack_elem = llvm_utils->create_gep(glob_stack_gep, idx);
                        builder->CreateStore(CreateLoad(target), glob_stack_elem);
                        llvm::Value* glob_stack_val = CreateLoad(glob_stack_gep);
                        llvm::Value* glob_stack_sp = llvm_utils->create_gep(glob_stack, sp_val);
                        builder->CreateStore(glob_stack_val, glob_stack_sp);
                    }
                }
            }
            builder->CreateStore(builder->CreateAdd(builder->getInt32(1), sp_val), sp_loc);
        }
    }
    ...
    void pop_nested_stack(const T& x)
    {
        ...
        bool finder
            = std::find(nested_call_out.begin(), nested_call_out.end(), calling_function_hash)
              != nested_call_out.end();
        bool is_nested_call = std::find(nesting_map[parent_function_hash].begin(),
                                        nesting_map[parent_function_hash].end(),
                                        calling_function_hash)
        ...
        if (nested_func_types[calling_function_hash].size() > 0
            && calling_function_hash != parent_function_hash && finder && !is_nested_call) {
            llvm::Value* sp_loc
                = module->getOrInsertGlobal(nested_sp_name, llvm::Type::getInt32Ty(context));
            llvm::Value* sp_val = CreateLoad(sp_loc);
            llvm::BasicBlock* dec_sp = llvm::BasicBlock::Create(context, "decrement_sp", lfn);
            llvm::BasicBlock* norm_cont = llvm::BasicBlock::Create(context, "normal_continue", lfn);
            ...
            for (auto& item : x->m_symtab->get_scope()) {
                if (is_a<ASR::Variable_t>(*item.second)) {
                    ...
                    if (finder != nested_globals.end()) {
                        ...
                        llvm::Value* glob_struct_loc
                            = module->getOrInsertGlobal(nested_desc_name, nested_global_struct);
                        llvm::Value* target
                            = CreateLoad(llvm_utils->create_gep(glob_struct_loc, idx));
                        builder->CreateStore(glob_stack_elem, target);
                        builder->CreateStore(target, llvm_utils->create_gep(glob_struct_loc, idx));
                    }
                }
            }
            ...
            */
            ...
            if (llvm_symtab_fn_names.find(fn_name) == llvm_symtab_fn_names.end()) {
                ...
                F = llvm::Function::Create(
                    function_type, llvm::Function::ExternalLinkage, fn_name, module.get());
            } else {
    ...
    void declare_needed_global_types(T& x)
    {
        ...
        if (nested_func_types[h].size() > 0) {
            ...
            if (std::find(nested_call_out.begin(), nested_call_out.end(), h)
                ...
                nested_global_stack = llvm::ArrayType::get(nested_global_struct_vals, 1000);
    ...
    bool is_nested_pointer(llvm::Value* val)
    {
        ...
        return val->getType()->isPointerTy() && val->getType()->getContainedType(0)->isPointerTy();
    }
    ...
    void visit_CLoc(const ASR::CLoc_t& x)
    {
        ...
        tmp = builder->CreateBitCast(tmp, llvm::Type::getVoidTy(context)->getPointerTo());
    }
    ...
    void visit_CPtrToPointer(const ASR::CPtrToPointer_t& x)
    {
        ...
        if (ASRUtils::is_array(ASRUtils::expr_type(fptr))) {
            ...
            int fptr_rank
                = ASRUtils::extract_dimensions_from_ttype(ASRUtils::expr_type(fptr), fptr_dims);
            llvm::Value* llvm_rank = llvm::ConstantInt::get(context, llvm::APInt(32, fptr_rank));
            llvm::Value* dim_des
                = builder->CreateAlloca(arr_descr->get_dimension_descriptor_type(), llvm_rank);
            builder->CreateStore(
                dim_des, arr_descr->get_pointer_to_dimension_descriptor_array(fptr_array, false));
            ...
            llvm_cptr = builder->CreateBitCast(
                llvm_cptr, static_cast<llvm::PointerType*>(fptr_data->getType())->getElementType());
            ...
            for (int i = 0; i < fptr_rank; i++) {
                ...
                llvm::Value* new_ub
                    = shape_data ? CreateLoad(llvm_utils->create_ptr_gep(shape_data, i)) : i32_one;
                builder->CreateStore(new_lb, desi_lb);
                builder->CreateStore(
                    builder->CreateAdd(builder->CreateSub(new_ub, new_lb), i32_one), desi_size);
            }
        } else {
            uint64_t ptr_loads_copy = ptr_loads;
            ptr_loads = 1 - reduce_loads;
            ...
            ptr_loads = ptr_loads_copy;
            llvm_cptr = builder->CreateBitCast(
                llvm_cptr, static_cast<llvm::PointerType*>(llvm_fptr->getType())->getElementType());
    ...
    void visit_Assignment(const ASR::Assignment_t& x)
    {
        ...
        if (is_target_list && is_value_list) {
            ...
            if (ASR::is_a<ASR::TupleConstant_t>(*x.m_target)) {
                ...
                std::string type_code
                    = ASRUtils::get_type_code(value_tuple_type->m_type, value_tuple_type->n_type);
        ...
        if (finder != nested_globals.end()) {
            ...
            nested_04.f90 */
            llvm::Value* ptr = module->getOrInsertGlobal(nested_desc_name, nested_global_struct);
            int idx = std::distance(nested_globals.begin(), finder);
    ...
    {
        this->visit_expr(*x);
        if (x->type == ASR::exprType::ArrayItem || x->type == ASR::exprType::ArraySection
    ...
    void visit_IntegerCompare(const ASR::IntegerCompare_t& x)
    {
        ...
        switch (x.m_op) {
            ...
            default: {
                throw CodeGenError("Comparison operator not implemented", x.base.base.loc);
            }
        }
    }
    ...
    void visit_IntegerBinOp(const ASR::IntegerBinOp_t& x)
    {
        ...
        switch (x.m_op) {
            ...
            case ASR::binopType::Pow: {
                ...
                int a_kind;
                a_kind
                    = down_cast<ASR::Integer_t>(ASRUtils::type_get_past_pointer(x.m_type))->m_kind;
                type = getFPType(a_kind);
                ...
                if (!fn_pow) {
                    ...
                    fn_pow = llvm::Function::Create(
                        function_type, llvm::Function::ExternalLinkage, func_name, module.get());
                }
    ...
    void visit_RealBinOp(const ASR::RealBinOp_t& x)
    {
        ...
        switch (x.m_op) {
            ...
            default: {
                throw CodeGenError("Binary operator '" + ASRUtils::binop_to_str_python(x.m_op)
                                       + "' not supported",
                                   x.base.base.loc);
            }
        }
    ...
    void visit_IntegerConstant(const ASR::IntegerConstant_t& x)
    {
        ...
        switch (a_kind) {
            case 4: {
                tmp = llvm::ConstantInt::get(context,
                                             llvm::APInt(32, static_cast<int32_t>(val), true));
                break;
            }
            case 8: {
    ...
    inline void fetch_val(ASR::Variable_t* x)
    {
        ...
        if (llvm_symtab.find(x_h) == llvm_symtab.end()) {
            LFORTRAN_ASSERT(std::find(nested_globals.begin(), nested_globals.end(), x_h)
                            != nested_globals.end());
            auto finder = std::find(nested_globals.begin(), nested_globals.end(), x_h);
            llvm::Constant* ptr = module->getOrInsertGlobal(nested_desc_name, nested_global_struct);
            int idx = std::distance(nested_globals.begin(), finder);
            std::vector<llvm::Value*> idx_vec
                = { llvm::ConstantInt::get(context, llvm::APInt(32, 0)),
    ...
    {
        ASR::Variable_t* v = ASR::down_cast<ASR::Variable_t>(symbol_get_past_external(x.m_v));
        fetch_var(v);
    }
    ...
    void visit_FileRead(const ASR::FileRead_t& x)
    {
        if (x.m_fmt != nullptr) {
            diag.codegen_warning_label(
                "format string in read() is not implemented yet and it is currently treated as '*'",
                { x.m_fmt->base.loc },
                "treated as '*'");
        }
        if (x.m_unit != nullptr) {
            diag.codegen_error_label(
                "unit in read() is not implemented yet", { x.m_unit->base.loc }, "not implemented");
            throw CodeGenAbort();
        }
        diag.codegen_error_label(
            "The intrinsic function read() is not implemented yet in the LLVM backend",
            { x.base.base.loc },
            "not implemented");
        throw CodeGenAbort();
    }
    ...
    void visit_Print(const ASR::Print_t& x)
    {
        if (x.m_fmt != nullptr) {
            diag.codegen_warning_label(
                "format string in `print` is not implemented yet and it is currently treated as '*'",
                { x.m_fmt->base.loc },
                "treated as '*'");
        }
        handle_print(x);
    ...
    void visit_FileWrite(const ASR::FileWrite_t& x)
    {
        if (x.m_fmt != nullptr) {
            diag.codegen_warning_label(
                "format string in write() is not implemented yet and it is currently treated as '*'",
                { x.m_fmt->base.loc },
                "treated as '*'");
        }
        if (x.m_unit != nullptr) {
    ...
    void visit_Stop(const ASR::Stop_t& x)
    {
        ...
        if (x.m_code && ASRUtils::expr_type(x.m_code)->type == ASR::ttypeType::Integer) {
            ...
            int exit_code_int = 0;
            exit_code = llvm::ConstantInt::get(context, llvm::APInt(32, exit_code_int));
        }
        exit(context, *module, *builder, exit_code);
    }
    ...
    template <typename T>
    inline void set_func_subrout_params(T* func_subrout,
                                        ASR::abiType& x_abi,
                                        std::uint32_t& m_h,
                    ...
                    if (is_a<ASR::Variable_t>(*symbol_get_past_external(
                        ...
                        if (llvm_symtab.find(h) != llvm_symtab.end()) {
                            ...
                            if (func_subrout->type == ASR::symbolType::Function) {
                                ASR::Function_t* func = down_cast<ASR::Function_t>(func_subrout);
                                set_func_subrout_params(
                                    func, x_abi, m_h, orig_arg, orig_arg_name, orig_arg_intent, i);
                            ...
                            if (x_abi == ASR::abiType::Source && arr_descr->is_array(tmp)) {
                                ...
                                tmp = arr_descr->convert_to_argument(
                                    tmp, new_arr_type, (!ASRUtils::is_dimension_empty(dims, n)));
    ...
    void generate_flip_sign(ASR::call_arg_t* m_args)
    {
        ...
        builder->CreateStore(builder->CreateBitCast(tmp, variable_type->getElementType()),
                             variable);
    }
    ...
    void generate_fma(ASR::call_arg_t* m_args)
    {
        ...
        llvm::Value* c = tmp;
        tmp = builder->CreateIntrinsic(llvm::Intrinsic::fma, { a->getType() }, { b, c, a });
    }
    ...
    void visit_SubroutineCall(const ASR::SubroutineCall_t& x)
    {
        ...
        if (ASR::is_a<ASR::Function_t>(*proc_sym)) {
            ...
            ASR::ClassProcedure_t* clss_proc = ASR::down_cast<ASR::ClassProcedure_t>(proc_sym);
        ...
        if (llvm_symtab_fn_arg.find(h) != llvm_symtab_fn_arg.end()) {
            ...
            throw CodeGenError("Subroutine code not generated for '" + std::string(s->m_name)
                               + "'");
        } else {
    ...
    void handle_bitwise_args(const ASR::FunctionCall_t& x, llvm::Value*& arg1, llvm::Value*& arg2)
    {
    ...
    void visit_FunctionCall(const ASR::FunctionCall_t& x)
    {
        ...
        if (llvm_symtab_fn_arg.find(h) != llvm_symtab_fn_arg.end()) {
            ...
            throw CodeGenError("Function code not generated for '" + std::string(s->m_name) + "'");
        } else {
            llvm::Function* fn = llvm_symtab_fn[h];
            std::string m_name = std::string(((ASR::Function_t*) (&(x.m_name->base)))->m_name);
            std::vector<llvm::Value*> args2 = convert_call_args(x, m_name);
...
asr_to_llvm(ASR::TranslationUnit_t& asr,
            diag::Diagnostics& diagnostics,
            llvm::LLVMContext& context,
            Allocator& al,
            ...
            const std::string& run_fn)
{
    ...
    v.nested_func_types
        = pass_find_nested_vars(asr, context, v.nested_globals, v.nested_call_out, v.nesting_map);
    try {
