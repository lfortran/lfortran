...
LLVMModule::LLVMModule(std::unique_ptr<llvm::Module> m) { m_m = std::move(m); }
...
std::string LLVMModule::get_return_type(const std::string &fn_name) {
  ...
  if (type->isFloatTy()) {
    ...
    if (st->hasName()) {
      if (startswith(std::string(st->getName()), "complex_4")) {
        ...
        throw LCompilersException(
            "LLVMModule::get_return_type(): Struct return type `" +
            std::string(st->getName()) + "` not supported");
      }
    } else {
      throw LCompilersException(
          "LLVMModule::get_return_type(): Noname struct return type not "
          "supported");
    }
    ...
    throw LCompilersException(
        "LLVMModule::get_return_type(): Return type not supported");
  }
}
...
std::unique_ptr<llvm::Module> LLVMEvaluator::parse_module(
...
intptr_t LLVMEvaluator::get_symbol_address(const std::string &name) {
  ...
  if (!addr0) {
    ...
    throw LCompilersException("JITSymbol::getAddress() returned an error: " +
                              msg);
  }
...
void LLVMEvaluator::save_asm_file(llvm::Module &m,
                                  const std::string &filename) {
  write_file(filename, get_asm(m));
}

void LLVMEvaluator::save_object_file(llvm::Module &m,
                                     const std::string &filename) {
...
void LLVMEvaluator::print_version_message() { llvm::cl::PrintVersionMessage(); }

llvm::LLVMContext &LLVMEvaluator::get_context() { return *context; }
