...
uint64_t
APInt_getint(const llvm::APInt& i)
{
...
std::string
LLVMModule::get_return_type(const std::string& fn_name)
{
    ...
    if (type->isFloatTy()) {
        ...
        if (st->hasName()) {
            if (startswith(std::string(st->getName()), "complex_4")) {
                ...
                throw LCompilersException("LLVMModule::get_return_type(): Struct return type `"
                                          + std::string(st->getName()) + "` not supported");
            }
        } else {
            throw LCompilersException(
                "LLVMModule::get_return_type(): Noname struct return type not supported");
        }
...
LLVMEvaluator::parse_module(const std::string& source)
{
    llvm::SMDiagnostic err;
    std::unique_ptr<llvm::Module> module = llvm::parseAssemblyString(source, err, *context);
    if (!module) {
...
void
LLVMEvaluator::add_module(const std::string& source)
{
...
void
LLVMEvaluator::add_module(std::unique_ptr<llvm::Module> mod)
{
...
void
LLVMEvaluator::add_module(std::unique_ptr<LLVMModule> m)
{
    add_module(std::move(m->m_m));
}

intptr_t
LLVMEvaluator::get_symbol_address(const std::string& name)
{
#if LLVM_VERSION_MAJOR <= 11
...
#endif
    if (!s) {
        throw std::runtime_error("findSymbol() failed to find the symbol '" + name + "'");
    }
#if LLVM_VERSION_MAJOR <= 11
...
int32_t
LLVMEvaluator::int32fn(const std::string& name)
{
...
int64_t
LLVMEvaluator::int64fn(const std::string& name)
{
    intptr_t addr = get_symbol_address(name);
...
bool
LLVMEvaluator::boolfn(const std::string& name)
{
    intptr_t addr = get_symbol_address(name);
    bool (*f)() = (bool (*)()) addr;
    return f();
...
float
LLVMEvaluator::floatfn(const std::string& name)
{
    intptr_t addr = get_symbol_address(name);
    float (*f)() = (float (*)()) addr;
    return f();
...
double
LLVMEvaluator::doublefn(const std::string& name)
{
    intptr_t addr = get_symbol_address(name);
...
std::complex<float>
LLVMEvaluator::complex4fn(const std::string& name)
{
    intptr_t addr = get_symbol_address(name);
...
std::complex<double>
LLVMEvaluator::complex8fn(const std::string& name)
{
    intptr_t addr = get_symbol_address(name);
...
void
LLVMEvaluator::voidfn(const std::string& name)
{
    intptr_t addr = get_symbol_address(name);
    void (*f)() = (void (*)()) addr;
    f();
...
void
write_file(const std::string& filename, const std::string& contents)
{
    std::ofstream out;
    out.open(filename);
...
std::string
LLVMEvaluator::get_asm(llvm::Module& m)
{
...
void
LLVMEvaluator::save_asm_file(llvm::Module& m, const std::string& filename)
{
    write_file(filename, get_asm(m));
}

void
LLVMEvaluator::save_object_file(llvm::Module& m, const std::string& filename)
{
...
void
LLVMEvaluator::create_empty_object_file(const std::string& filename)
{
    std::string source;
    std::unique_ptr<llvm::Module> module = parse_module(source);
...
void
LLVMEvaluator::opt(llvm::Module& m)
{
    m.setTargetTriple(target_triple);
    m.setDataLayout(TM->createDataLayout());
...
std::string
LLVMEvaluator::module_to_string(llvm::Module& m)
{
    std::string buf;
...
void
LLVMEvaluator::print_version_message()
{
    llvm::cl::PrintVersionMessage();
}

llvm::LLVMContext&
LLVMEvaluator::get_context()
{
    return *context;
}

void
LLVMEvaluator::print_targets()
{
    llvm::InitializeNativeTarget();
#ifdef HAVE_TARGET_AARCH64
    LLVMInitializeAArch64TargetInfo();
...
std::string
LLVMEvaluator::get_default_target_triple()
{
