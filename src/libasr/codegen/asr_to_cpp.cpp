...
std::string
format_type(const std::string& dims,
            const std::string& type,
            const std::string& name,
            bool use_ref,
            bool dummy,
            bool use_kokko = true,
            std::string kokko_ref = "&",
            bool use_name = false,
            size_t size = 0)
{
    std::string fmt;
...
std::string
remove_characters(std::string s, char c)
{
...
public:
    ...
    ASRToCPPVisitor(diag::Diagnostics& diag, Platform& platform, int64_t default_lower_bound)
    ...
    bool is_array_type_available(std::string& encoded_type_name, std::string& dims, size_t n_dims)
    {
            ...
            if (!is_pointer) {
                for (int i = 0; i < n_dims; i++) {
                    if (m_dims[i].m_start) {
                        ...
                        sub += indent + std::string(v_m_name) + "->dims[" + std::to_string(i)
                               + "].lower_bound = " + src + ";\n";
                    } else {
                        sub += indent + std::string(v_m_name) + "->dims[" + std::to_string(i)
                               + "].lower_bound = 0" + ";\n";
