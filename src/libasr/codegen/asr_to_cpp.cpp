...
std::string format_type(const std::string &dims, const std::string &type,
                        ...
                        bool use_kokko = true, std::string kokko_ref = "&",
                        bool use_name = false, size_t size = 0) {
  std::string fmt;
  if (dims.size() == 0) {
    std::string ref;
    if (use_ref) ref = "&";
...
class ASRToCPPVisitor : public BaseCCPPVisitor<ASRToCPPVisitor> {
 public:
  ...
  std::map<std::string, std::map<std::string, std::map<size_t, std::string>>>
      eltypedims2arraytype;

  ASRToCPPVisitor(diag::Diagnostics &diag, Platform &platform,
                  int64_t default_lower_bound)
      : BaseCCPPVisitor(diag, platform, true, true, false, default_lower_bound),
        array_types_decls(
  ...
  bool is_array_type_available(std::string &encoded_type_name,
                               std::string &dims, size_t n_dims) {
    return eltypedims2arraytype.find(encoded_type_name) !=
               eltypedims2arraytype.end() &&
  ...
  std::string get_array_type(std::string type_name,
                             std::string encoded_type_name, std::string &dims,
    ...
    std::string name =
        encoded_type_name + "_" + dims_copy + "_" + std::to_string(n_dims);
    struct_name = "struct " + name;
    std::string array_data =
        format_type("*", type_name, "data", false, false, true, "*");
    new_array_type = struct_name + "\n{\n    " + array_data +
                     ";\n    dimension_descriptor dims[" +
                     std::to_string(n_dims) + "];\n    bool is_allocated;\n\n";
    std::string constructor =
        "    " + name + "(" + array_data + "_): data{data_} {};\n};\n";
  ...
  void generate_array_decl(std::string &sub, std::string v_m_name,
                           ...
                           bool use_ref, bool dummy, bool declare_value,
                           bool is_pointer = false) {
    std::string indent(indentation_level * indentation_spaces, ' ');
    std::string type_name_copy = type_name;
    type_name = get_array_type(type_name, encoded_type_name, dims, n_dims);
    std::string type_name_without_ptr =
        get_array_type(type_name, encoded_type_name, dims, n_dims, false);
    if (declare_value) {
      ...
      sub += format_type("", type_name_without_ptr, variable_name, use_ref,
                         dummy, false);
      if (!is_pointer) {
      ...
      sub +=
          indent + format_type("", type_name, v_m_name, use_ref, dummy, false);
      sub += " = &" + variable_name + ";\n";
      if (!is_pointer) {
        for (int i = 0; i < n_dims; i++) {
          if (m_dims[i].m_start) {
            this->visit_expr(*m_dims[i].m_start);
            sub += indent + std::string(v_m_name) + "->dims[" +
                   std::to_string(i) + "].lower_bound = " + src + ";\n";
          } else {
            sub += indent + std::string(v_m_name) + "->dims[" +
                   std::to_string(i) + "].lower_bound = 0" + ";\n";
          }
...
Kokkos::View<T*> from_std_vector(const std::vector<T> &v)
{
  ...
  void visit_DoConcurrentLoop(const ASR::DoConcurrentLoop_t &x) {
    ...
    out +=
        ", KOKKOS_LAMBDA(const long " + std::string(loop_var->m_name) + ") {\n";
    indentation_level += 1;
    for (size_t i = 0; i < x.n_body; i++) {
      this->visit_stmt(*x.m_body[i]);
      out += src;
