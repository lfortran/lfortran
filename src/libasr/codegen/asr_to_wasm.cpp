 ...
 public:
  ...
  void get_wasm(Vec<uint8_t> &code) {
    code.reserve(m_al, 8U /* preamble size */ +
                           ...
                           + m_type_section.size() + m_import_section.size() +
                           m_func_section.size() + m_export_section.size() +
                           m_code_section.size() + m_data_section.size());

    wasm::emit_header(code, m_al);  // emit header and version
    wasm::encode_section(code, m_type_section, m_al, 1U,
                         no_of_types);  // no_of_types indicates total (imported
                                        // + defined) no of functions
  ...
  ASR::asr_t *get_import_func_var_type(
    ...
    switch (type.first) {
      case ASR::ttypeType::Integer:
        return ASR::make_Integer_t(m_al, global_scope_loc, type.second, nullptr,
                                   0);
      case ASR::ttypeType::Real:
        return ASR::make_Real_t(m_al, global_scope_loc, type.second, nullptr,
                                0);
      default:
  ...
  void import_function(ImportFunc &import_func) {
    ...
    for (var_idx = 0; var_idx < import_func.param_types.size(); var_idx++) {
      ...
      auto variable = ASR::make_Variable_t(
          m_al, global_scope_loc, nullptr, s2c(m_al, std::to_string(var_idx)),
  ...
  void emit_imports() {
        ...
        for (auto &item : p->m_symtab->get_scope()) {
          if (ASR::is_a<ASR::Function_t>(*item.second)) {
            ...
            if (fn->m_abi == ASR::abiType::BindC &&
              ...
              wasm::emit_import_fn(m_import_section, m_al, "js", fn->m_name,
                                   no_of_types);
              no_of_imports++;
    ...
    wasm::emit_import_mem(m_import_section, m_al, "js", "memory", min_no_pages,
                          max_no_pages);
    no_of_imports++;
  }
    ...
    if (ASRUtils::is_pointer(v->m_type)) {
      ...
      if (ASRUtils::is_integer(*v->m_type)) {
        ...
        diag.codegen_warning_label(
            "Unsupported variable type: " + ASRUtils::type_to_str(v->m_type),
        ...
        if (v->m_intent == ASRUtils::intent_local ||
          ...
          if (v->m_symbolic_value) {
            ...
            LFORTRAN_ASSERT(m_var_name_idx_map.find(get_hash(
                                (ASR::asr_t *)v)) != m_var_name_idx_map.end())
            wasm::emit_set_local(m_code_section, m_al,
  ...
  void emit_function_body(const ASR::Function_t &x) {
    ...
    if (strcmp(x.m_name, "_lcompilers_main") == 0) {
      ...
      wasm::emit_call(
          m_code_section, m_al,
          m_func_name_idx_map[get_hash(m_import_func_asr_map["set_exit_code"])]
              ->index);
    }
    if ((x.n_body == 0) || ((x.n_body > 0) && !ASR::is_a<ASR::Return_t>(
                                                  *x.m_body[x.n_body - 1]))) {
      handle_return();
    }
  ...
  bool is_unsupported_function(const ASR::Function_t &x) {
    ...
    if (x.m_abi == ASR::abiType::BindC &&
      ...
      if (ASRUtils::is_intrinsic_function2(&x)) {
        diag.codegen_warning_label(
            "WASM: C Intrinsic Functions not yet spported", {x.base.base.loc},
            std::string(x.m_name));
      }
      return true;
    }
  ...
  int get_kind_from_operands(const T &x) {
    ...
    if (left_kind != right_kind) {
      diag.codegen_error_label("Operand kinds do not match", {x.base.base.loc},
  ...
  void handle_return() {
    if (cur_sym_info->return_var) {
      LFORTRAN_ASSERT(m_var_name_idx_map.find(
                          get_hash((ASR::asr_t *)cur_sym_info->return_var)) !=
                      m_var_name_idx_map.end());
      wasm::emit_get_local(
          m_code_section, m_al,
        ...
        if (arg_kind > 0 && dest_kind > 0) {
          if (arg_kind == 4 && dest_kind == 4) {
            ...
            std::string msg = "Conversion from kinds " +
                              ...
                              std::to_string(dest_kind) + " not supported";
            throw CodeGenError(msg);
          }
        }
        break;
          ...
          case 4: {
            ...
            wasm::emit_call(
                ...
                m_func_name_idx_map[get_hash(
                                        m_import_func_asr_map["print_i32"])]
                    ->index);
            break;
          }
          case 8: {
            ...
            wasm::emit_call(
                m_code_section, m_al,
                m_func_name_idx_map[get_hash(
                                        m_import_func_asr_map["print_i64"])]
                    ->index);
            break;
          }
          default: {
        ...
        switch (a_kind) {
          case 4: {
            ...
            wasm::emit_call(
                m_code_section, m_al,
                m_func_name_idx_map[get_hash(
                                        m_import_func_asr_map["print_f32"])]
                    ->index);
            break;
          }
          case 8: {
            ...
            wasm::emit_call(
                m_code_section, m_al,
                m_func_name_idx_map[get_hash(
                                        m_import_func_asr_map["print_f64"])]
                    ->index);
            break;
          }
          default: {
