...
unwind_callback(struct _Unwind_Context* context, void* vdata)
{
...
*/
int
shared_lib_callback(struct dl_phdr_info* info, size_t /* size */, void* _data)
{
...
void
get_local_address(StacktraceItem& item)
{
#ifdef HAVE_LFORTRAN_LINK
...
*/
std::string
demangle_function_name(std::string name)
{
    std::string s;
...
*/
void
process_section(bfd* abfd, asection* section, void* _data)
{
    ...
    data->line_found = bfd_find_nearest_line(
        abfd, section, data->symbol_table, offset, &filename, &function_name, &data->line);
...
int
load_symbol_table(bfd* abfd, line_data* data)
{
...
void
get_symbol_info_bfd(std::string binary_filename,
                    uintptr_t addr,
 ...
 */
std::string
remove_leading_whitespace(const std::string& str)
{
...
std::string
read_line_from_file(std::string filename, unsigned int line_number)
{
...
/* Returns a string of 2 lines for the function with address 'addr' in the file
   ...
   Teuchos::RCP<A>::assert_not_null() const throw_null_ptr_error(typeName(*this));
*/
std::string
addr2str(const StacktraceItem& i)
{
    std::ostringstream s;
    ...
    if (i.function_name == "") {
        ...
        if (i.local_pc == 0) {
            ...
            if (i.source_filename == "") {
                ...
                s << color(style::dim) << "  File \"" << color(style::reset) << color(style::bold)
                  << color(fg::magenta) << i.source_filename << color(fg::reset)
                  << color(style::reset) << color(style::dim) << "\", line " << i.line_number
                  << color(style::reset);
        ...
        s << color(style::dim) << "  Binary file \"" + color(style::reset) << color(style::bold)
          << color(fg::magenta) << i.binary_filename << color(fg::reset) << color(style::reset)
          << color(style::dim) << "\", in " << i.function_name << color(style::reset);
    } else {
        // Nicely format the filename + function name + line
        s << color(style::dim) << "  File \"" << color(style::reset) << color(style::bold)
          << color(fg::magenta) << i.source_filename << color(fg::reset) << color(style::reset)
          << color(style::dim) << "\", line " << i.line_number << ", in " << i.function_name
          << color(style::reset);
        const std::string line_text
            = remove_leading_whitespace(read_line_from_file(i.source_filename, i.line_number));
        if (line_text != "") {
            s << "\n    " << line_text;
        }
    }
...
*/
std::string
stacktrace2str(const std::vector<StacktraceItem>& d, int skip)
{
...
void
loc_segfault_callback_print_stack(int /* sig_num */)
{
...
void
loc_abort_callback_print_stack(int /* sig_num */)
{
    std::cerr << get_stacktrace(3);
...
void
get_local_info_dwarfdump(std::vector<StacktraceItem>& d)
{
    ...
    for (size_t i = 0; i < d.size(); i++) {
        address_to_line_number(
            filenames, addresses, d[i].local_pc, d[i].source_filename, d[i].line_number);
    }
}
...
get_local_info(std::vector<StacktraceItem>& d)
{
    ...
    for (size_t i = 0; i < d.size(); i++) {
#ifdef HAVE_LFORTRAN_BFD
        get_symbol_info_bfd(d[i].binary_filename,
                            d[i].local_pc,
                            d[i].source_filename,
                            d[i].function_name,
                            d[i].line_number);
