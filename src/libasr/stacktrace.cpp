...
*/
int shared_lib_callback(struct dl_phdr_info *info, size_t /* size */,
                        void *_data) {
...
void get_local_address(StacktraceItem &item) {
  ...
  if (dl_iterate_phdr(shared_lib_callback, &item) == 0) {
    ...
    // happen if the stacktrace is somehow corrupted. In that case, we simply
    // abort here.
    std::cout << "The stack address was not found in any shared library or the "
                 "main program, the stack is probably corrupted. Aborting."
              << std::endl;
    abort();
  }
#else
  ...
  for (uint32_t i = 0; i < _dyld_image_count(); i++) {
    ...
    if (header->magic == MH_MAGIC_64) {
      cmd = (struct load_command *)((char *)header +
...
bool is_whitespace_char(const char c) { return c == ' ' || c == '\t'; }
...
std::string read_line_from_file(std::string filename,
  ...
  std::string line;
  while (n < line_number) {
    if (in.eof()) return "Line not found";
    getline(in, line);
    n += 1;  // loop update
  }
  return line;
...
std::string addr2str(const StacktraceItem &i) {
  ...
  if (i.function_name == "") {
    ...
    if (i.local_pc == 0) {
      ...
      if (i.source_filename == "") {
        ...
        s << color(style::dim) << "  File \"" << color(style::reset)
          ...
          << color(fg::reset) << color(style::reset) << color(style::dim)
          << "\", line " << i.line_number << color(style::reset);
    ...
    s << color(style::dim) << "  File \"" << color(style::reset)
      << color(style::bold) << color(fg::magenta) << i.source_filename
      << color(fg::reset) << color(style::reset) << color(style::dim)
      << "\", line " << i.line_number << ", in " << i.function_name
      << color(style::reset);
    const std::string line_text = remove_leading_whitespace(
        read_line_from_file(i.source_filename, i.line_number));
    if (line_text != "") {
...
void show_stacktrace() { std::cout << get_stacktrace(3); }
...
void get_local_info(std::vector<StacktraceItem> &d) {
  ...
  for (size_t i = 0; i < d.size(); i++) {
    ...
    get_symbol_info_bfd(d[i].binary_filename, d[i].local_pc,
                        d[i].source_filename, d[i].function_name,
                        d[i].line_number);
#endif
  }
#endif
