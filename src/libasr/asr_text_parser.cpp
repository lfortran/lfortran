...
// Appends the UTF-8 encoding of a code point in [0, 0xFFFF] (the maximum
// range representable by a single `\uXXXX` EDN escape) to `out`.
        ...
        while (true) {
            ...
            if (c == '\\') {
                ...
                switch (e) {
                    ...
                    case 'b':  buffer += '\b'; pos++; break;
                    case 'f':  buffer += '\f'; pos++; break;
                    case 'u': {
                        pos++; // consume 'u'
                        if (pos + 4 > n) {
