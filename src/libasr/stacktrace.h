 ...
 */
std::string
get_stacktrace(int impl_stacktrace_depth = 0);
...
void
get_local_addresses(std::vector<StacktraceItem>& d);

// Fills in the `function_name` if available, and if so, also fills in
// `source_filename` and `line_number` if available
void
get_local_info(std::vector<StacktraceItem>& d);

// Converts the information stored in `d` into a string
std::string
stacktrace2str(const std::vector<LFortran::StacktraceItem>& d, int skip);

// Returns line number information from address
void
address_to_line_number(const std::vector<std::string>& filenames,
...
std::string
error_stacktrace(const std::vector<StacktraceItem>& stacktrace);
