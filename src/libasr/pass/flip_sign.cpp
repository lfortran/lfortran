...
public:
    FlipSignVisitor(Allocator& al_, ASR::TranslationUnit_t& unit_, const std::string& rl_path_)
    ...
    void set_flip_sign()
    {
        is_flip_sign_present = (is_if_present && is_compare_present && is_function_call_present
    ...
    void handle_Compare(const T& x)
    {
        ...
        if (potential_one && potential_one->type == ASR::exprType::IntegerConstant) {
...
void
pass_replace_flip_sign(Allocator& al,
                       ASR::TranslationUnit_t& unit,
