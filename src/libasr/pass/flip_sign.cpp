...
*/
class FlipSignVisitor
    : public PassUtils::SkipOptimizationFunctionVisitor<FlipSignVisitor> {
 private:
 ...
 public:
  FlipSignVisitor(Allocator& al_, ASR::TranslationUnit_t& unit_,
      ...
      : SkipOptimizationFunctionVisitor(al_), unit(unit_), rl_path(rl_path_) {
  ...
  void visit_If(const ASR::If_t& x) {
    ...
    if (is_flip_sign_present) {
      ...
      ASR::stmt_t* flip_sign_call = PassUtils::get_flipsign(
          flip_sign_signal_variable, flip_sign_variable, al, unit, rl_path,
          current_scope, [&](const std::string& msg, const Location&) {
  ...
  void set_flip_sign() {
    is_flip_sign_present =
        (is_if_present && is_compare_present && is_function_call_present &&
         is_function_modulo && is_divisor_2 && is_one_present &&
         is_unary_op_present && is_operand_same_as_input);
  }
  ...
  void visit_Assignment(const ASR::Assignment_t& x) {
    if (x.m_value->type == ASR::exprType::RealUnaryMinus) {
      ...
      ASR::RealUnaryMinus_t* negation =
          ASR::down_cast<ASR::RealUnaryMinus_t>(x.m_value);
  ...
  void handle_Compare(const T& x) {
    ...
    if (potential_one &&
      ...
      ASR::IntegerConstant_t* const_int =
          ASR::down_cast<ASR::IntegerConstant_t>(potential_one);
  ...
  void visit_FunctionCall(const ASR::FunctionCall_t& x) {
    ...
    if (func_name && func_name->type == ASR::symbolType::ExternalSymbol) {
      ASR::ExternalSymbol_t* ext_sym =
          ASR::down_cast<ASR::ExternalSymbol_t>(func_name);
    ...
    if (is_function_modulo && x.n_args == 2) {
      ...
      if (arg1->type == ASR::exprType::IntegerConstant) {
        ASR::IntegerConstant_t* const_int =
            ASR::down_cast<ASR::IntegerConstant_t>(arg1);
