...
public:
    ReplaceArrayDimIntrinsicCalls(Allocator& al_)
        : al(al_)
    {
    }
    ...
    void replace_ArraySize(ASR::ArraySize_t* x)
    {
        ...
        bool is_argument
            = v->m_intent == ASRUtils::intent_in || v->m_intent == ASRUtils::intent_out;
    ...
    {
        if (!ASR::is_a<ASR::Var_t>(*x->m_v) || !ASRUtils::is_value_constant(x->m_dim)) {
            return;
        }
        ...
        if (x->m_bound == ASR::arrayboundType::LBound) {
            ...
            *current_expr = ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
                al, x->base.base.loc, ub, ASR::binopType::Sub, const_1, x->m_type, nullptr));
        }
    }
};
...
class ArrayDimIntrinsicCallsVisitor
    : public ASR::CallReplacerOnExpressionsVisitor<ArrayDimIntrinsicCallsVisitor>
{
private:
    ReplaceArrayDimIntrinsicCalls replacer;

public:
    ArrayDimIntrinsicCallsVisitor(Allocator& al_)
        : replacer(al_)
    {
    }
...
void
pass_update_array_dim_intrinsic_calls(Allocator& al,
