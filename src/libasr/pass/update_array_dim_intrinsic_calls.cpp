...
*/
class ReplaceArrayDimIntrinsicCalls
    : public ASR::BaseExprReplacer<ReplaceArrayDimIntrinsicCalls> {
 private:
  Allocator& al;
 ...
 public:
  ...
  void replace_ArraySize(ASR::ArraySize_t* x) {
    ...
    ASR::expr_t* array_size = ASRUtils::EXPR(
        ASR::make_IntegerConstant_t(al, x->base.base.loc, 1, x->m_type));
    for (int i = 0; i < n; i++) {
      array_size = ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
          al, x->base.base.loc, array_size, ASR::binopType::Mul,
          dims[i].m_length, x->m_type, nullptr));
    }
  ...
  void replace_ArrayBound(ASR::ArrayBound_t* x) {
    ...
    if (x->m_bound == ASR::arrayboundType::LBound) {
      *current_expr = dims[dim - 1].m_start;
      ...
      ASR::expr_t* ub = ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
          al, x->base.base.loc, dims[dim - 1].m_length, ASR::binopType::Add,
          dims[dim - 1].m_start, x->m_type, nullptr));
      ASR::expr_t* const_1 = ASRUtils::EXPR(
          ASR::make_IntegerConstant_t(al, x->base.base.loc, 1, x->m_type));
      *current_expr = ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
          al, x->base.base.loc, ub, ASR::binopType::Sub, const_1, x->m_type,
          nullptr));
    }
  }
};
...
present in the ASR.
*/
class ArrayDimIntrinsicCallsVisitor
    : public ASR::CallReplacerOnExpressionsVisitor<
          ArrayDimIntrinsicCallsVisitor> {
 private:
...
void pass_update_array_dim_intrinsic_calls(
