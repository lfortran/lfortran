...
class InlineFunctionCallVisitor
    : public PassUtils::PassVisitor<InlineFunctionCallVisitor> {
 private:
  std::string rl_path;
 ...
 public:
  ...
  void set_empty_block(SymbolTable* scope, const Location& loc) {
    ...
    if (empty_block_name != "~empty_block") {
      ...
      empty_block = ASR::down_cast<ASR::symbol_t>(ASR::make_Block_t(
          al, loc, empty_symtab, s2c(al, empty_block_name), nullptr, 0));
  ...
  void visit_FunctionCall(const ASR::FunctionCall_t& x) {
    ...
    if (!from_inline_function_call || inlining_function) {
      ...
      if (ASR::is_a<ASR::ExternalSymbol_t>(*x.m_name)) {
        ASR::ExternalSymbol_t* called_sym_ext =
            ASR::down_cast<ASR::ExternalSymbol_t>(x.m_name);
        ASR::symbol_t* f_sym =
            ASRUtils::symbol_get_past_external(called_sym_ext->m_external);
        ...
        std::string new_sym_name_str =
            current_scope->get_unique_name(called_sym_name);
        char* new_sym_name = s2c(al, new_sym_name_str);
        if (current_scope->get_symbol(new_sym_name_str) == nullptr) {
          ASR::Module_t* m =
              ASR::down_cast2<ASR::Module_t>(f->m_symtab->parent->asr_owner);
          char* modname = m->m_name;
          ASR::symbol_t* new_sym =
              ASR::down_cast<ASR::symbol_t>(ASR::make_ExternalSymbol_t(
                  al, called_sym->base.loc, current_scope, new_sym_name, f_sym,
                  modname, nullptr, 0, f->m_name, ASR::accessType::Private));
    ...
    for (size_t i = 0; i < func->n_args + 1; i++) {
      ...
      // TODO: Expand to other symbol types, Function, Subroutine,
      // ExternalSymbol
      if (!ASR::is_a<ASR::Variable_t>(*(arg_var->m_v))) {
        arg2value.clear();
        return;
      }
      ASR::Variable_t* arg_variable =
          ASR::down_cast<ASR::Variable_t>(arg_var->m_v);
      std::string arg_variable_name = std::string(arg_variable->m_name);
      std::string arg_name = current_scope->get_unique_name(
          arg_variable_name + "_" + std::string(func->m_name));
      ASR::stmt_t* assign_stmt = nullptr;
      ASR::expr_t* call_arg_var = nullptr;
      if (x_m_args_i) {
        call_arg_var = PassUtils::create_auxiliary_variable_for_expr(
            x_m_args_i, arg_name, al, current_scope, assign_stmt);
      } else {
        call_arg_var = PassUtils::create_auxiliary_variable(
            func_margs_i->base.loc, arg_name, al, current_scope,
            ASRUtils::expr_type(func_margs_i));
        return_var = call_arg_var;
      }
      if (assign_stmt) {
        pass_result_local.push_back(al, assign_stmt);
      }
      arg2value[arg_variable_name] =
          ASR::down_cast<ASR::Var_t>(call_arg_var)->m_v;
    }
    ...
    // exprs_to_be_visited temporarily stores the initilisation expression as
    // well.
    for (auto& itr : func->m_symtab->get_scope()) {
      ...
      if (arg2value.find(func_var_name) == arg2value.end()) {
        std::string local_var_name = current_scope->get_unique_name(
            func_var_name + "_" + std::string(func->m_name));
        node_duplicator.success = true;
        ASR::expr_t* m_symbolic_value =
            node_duplicator.duplicate_expr(func_var->m_symbolic_value);
        ...
        node_duplicator.success = true;
        ASR::expr_t* m_value =
            node_duplicator.duplicate_expr(func_var->m_value);
        if (!node_duplicator.success) {
          success = false;
          break;
        }
        ASR::symbol_t* local_var = (ASR::symbol_t*)ASR::make_Variable_t(
            al, func_var->base.base.loc, current_scope, s2c(al, local_var_name),
            ASR::intentType::Local, nullptr, nullptr,
            ASR::storage_typeType::Default, func_var->m_type,
            ASR::abiType::Source, ASR::accessType::Public,
            ASR::presenceType::Required, false);
        current_scope->add_symbol(local_var_name, local_var);
        arg2value[func_var_name] = local_var;
        if (m_symbolic_value) {
          exprs_to_be_visited.push_back(
              std::make_pair(m_symbolic_value, local_var));
        }
        if (m_value) {
    ...
    for (size_t i = 0; i < exprs_to_be_visited.size() && success; i++) {
      ...
      ASR::expr_t* var = LFortran::ASRUtils::EXPR(
          ASR::make_Var_t(al, variable->base.loc, variable));
      ASR::stmt_t* assign_stmt = ASRUtils::STMT(
          ASR::make_Assignment_t(al, var->base.loc, var, value, nullptr));
      pass_result_local.push_back(al, assign_stmt);
    }
    ...
    for (size_t i = 0; i < func->n_body && success; i++) {
      node_duplicator.success = true;
      ASR::stmt_t* m_body_copy =
          node_duplicator.duplicate_stmt(func->m_body[i]);
      if (node_duplicator.success) {
    ...
    if (success) {
      ...
      if (success) {
        ...
        for (size_t i = 0; i < func->n_body; i++) {
          ...
          is_goto_added =
              is_goto_added || return_replacer.has_replacement_happened;
