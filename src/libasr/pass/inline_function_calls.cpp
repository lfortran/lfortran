...
public:
    ...
    InlineFunctionCallVisitor(Allocator& al_,
                              const std::string& rl_path_,
                              bool inline_external_symbol_calls_)
    ...
    void visit_Var(const ASR::Var_t& x)
    {
        ...
        if (current_routine_scope && current_routine_scope->get_symbol(x_var_name) == nullptr) {
            fixed_duplicated_expr_stmt = false;
            return;
        }
    ...
    void set_empty_block(SymbolTable* scope, const Location& loc)
    {
        ...
        if (empty_block_name != "~empty_block") {
            ...
            empty_block = ASR::down_cast<ASR::symbol_t>(
                ASR::make_Block_t(al, loc, empty_symtab, s2c(al, empty_block_name), nullptr, 0));
    ...
    void visit_FunctionCall(const ASR::FunctionCall_t& x)
    {
        ...
        if (!from_inline_function_call || inlining_function) {
            ...
            if (ASR::is_a<ASR::ExternalSymbol_t>(*x.m_name)) {
                ...
                if (current_scope->get_symbol(new_sym_name_str) == nullptr) {
                    ...
                    ASR::symbol_t* new_sym = ASR::down_cast<ASR::symbol_t>(
        ...
        if (!ASR::is_a<ASR::Function_t>(*routine)) {
            if (ASR::is_a<ASR::ExternalSymbol_t>(*routine) && inline_external_symbol_calls) {
        ...
        for (size_t i = 0; i < func->n_args + 1; i++) {
            ...
            std::string arg_name = current_scope->get_unique_name(arg_variable_name + "_"
                                                                  + std::string(func->m_name));
            ...
            if (x_m_args_i) {
                ...
                call_arg_var
                    = PassUtils::create_auxiliary_variable(func_margs_i->base.loc,
                                                           arg_name,
                                                           al,
                                                           current_scope,
        ...
        for (auto& itr : func->m_symtab->get_scope()) {
            ...
            if (arg2value.find(func_var_name) == arg2value.end()) {
                ...
                ASR::symbol_t* local_var
                    = (ASR::symbol_t*) ASR::make_Variable_t(al,
                                                            func_var->base.base.loc,
                                                            current_scope,
                                                            s2c(al, local_var_name),
        ...
        for (size_t i = 0; i < exprs_to_be_visited.size() && success; i++) {
            ...
            ASR::expr_t* var
                = LFortran::ASRUtils::EXPR(ASR::make_Var_t(al, variable->base.loc, variable));
            ASR::stmt_t* assign_stmt
                = ASRUtils::STMT(ASR::make_Assignment_t(al, var->base.loc, var, value, nullptr));
            pass_result_local.push_back(al, assign_stmt);
        }
        ...
        if (success) {
            ...
            if (success) {
                ...
                ASR::stmt_t* block_call = ASRUtils::STMT(
                    ASR::make_BlockCall_t(al, x.base.base.loc, block_call_label, empty_block));
...
void
pass_inline_function_calls(Allocator& al,
