...
class DeadCodeRemovalVisitor
    : public PassUtils::PassVisitor<DeadCodeRemovalVisitor> {
 private:
  std::string rl_path;
 ...
 public:
  ...
  DeadCodeRemovalVisitor(Allocator& al_, const std::string& rl_path_)
      : PassVisitor(al_, nullptr), rl_path(rl_path_), dead_code_removed(false) {
    ...
    for (size_t i = 0; i < x.n_body; i++) {
      ...
      switch (case_body->type) {
        case ASR::case_stmtType::CaseStmt: {
          ASR::CaseStmt_t* casestmt =
              ASR::down_cast<ASR::CaseStmt_t>(case_body);
        ...
        case ASR::case_stmtType::CaseStmt_Range: {
          ASR::CaseStmt_Range_t* casestmt_range =
              ASR::down_cast<ASR::CaseStmt_Range_t>(case_body);
          transform_stmts(casestmt_range->m_body, casestmt_range->n_body);
          xx.m_body[i] = (ASR::case_stmt_t*)(&(casestmt_range->base));
          if (ASRUtils::is_value_in_range(casestmt_range->m_start,
                                          casestmt_range->m_end, x.m_test)) {
