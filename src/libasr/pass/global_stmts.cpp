  ...
  if (unit.n_items > 0) {
    ...
    for (size_t i = 0; i < unit.n_items; i++) {
      if (unit.m_items[i]->type == ASR::asrType::expr) {
        ...
        if (LFortran::ASRUtils::expr_type(value)->type ==
            ASR::ttypeType::Integer) {
          s.from_str(al, fn_name_s + std::to_string(idx));
          var_name = s.c_str(al);

          int a_kind =
              down_cast<ASR::Integer_t>(LFortran::ASRUtils::expr_type(value))
                  ->m_kind;

          type = LFortran::ASRUtils::TYPE(
              ASR::make_Integer_t(al, loc, a_kind, nullptr, 0));
          return_var = ASR::make_Variable_t(
              al, loc, fn_scope, var_name, LFortran::ASRUtils::intent_local,
              nullptr, nullptr, ASR::storage_typeType::Default, type,
              ASR::abiType::BindC, ASR::Public, ASR::presenceType::Required,
              false);
          return_var_ref = EXPR(
              ASR::make_Var_t(al, loc, down_cast<ASR::symbol_t>(return_var)));
          fn_scope->add_symbol(std::string(var_name),
                               down_cast<ASR::symbol_t>(return_var));
          target = return_var_ref;
          idx++;
        } else if (LFortran::ASRUtils::expr_type(value)->type ==
                   ASR::ttypeType::Real) {
          s.from_str(al, fn_name_s + std::to_string(idx));
          var_name = s.c_str(al);
          ...
          throw LCompilersException(
              "Return type not supported in interactive mode");
        }
        ASR::stmt_t *asr_stmt = LFortran::ASRUtils::STMT(
            ASR::make_Assignment_t(al, loc, target, value, nullptr));
    ...
    if (return_var) {
      ...
      ASR::asr_t *fn = ASR::make_Function_t(
          ...
          /* a_body */ body.p,
          /* n_body */ body.size(),
          /* a_return_var */ return_var_ref, ASR::abiType::BindC, ASR::Public,
          ASR::Implementation, nullptr, false, false, false, false);
      ...
      ASR::asr_t *fn = ASR::make_Function_t(
          ...
          /* n_args */ 0, nullptr, 0,
          /* a_body */ body.p,
          /* n_body */ body.size(), nullptr, ASR::abiType::Source, ASR::Public,
          ASR::Implementation, nullptr, false, false, false, false);
      std::string sym_name = fn_name;
      if (unit.m_global_scope->get_symbol(sym_name) != nullptr) {
        throw LCompilersException("Function already defined");
