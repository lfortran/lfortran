...
/*
This ASR pass replaces array slice with do loops and array expression
assignments. The function `pass_replace_arr_slice` transforms the ASR tree
in-place.
...
class ArrSliceVisitor : public PassUtils::PassVisitor<ArrSliceVisitor> {
 ...
 public:
  ArrSliceVisitor(Allocator& al, const std::string& rl_path)
  ...
  ASR::ttype_t* get_array_from_slice(const ASR::ArraySection_t& x,
                                     ASR::expr_t* arr_var) {
    ...
    for (size_t i = 0; i < x.n_args; i++) {
      if (x.m_args[i].m_step != nullptr) {
        ...
        ASR::expr_t* gap = LFortran::ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
            al, x.base.base.loc, end, ASR::binopType::Sub, start, int32_type,
            nullptr));
        ASR::expr_t* slice_size =
            LFortran::ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
                al, x.base.base.loc, gap, ASR::binopType::Div, step, int32_type,
                nullptr));
        ASR::expr_t* actual_size =
            LFortran::ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
                al, x.base.base.loc, slice_size, ASR::binopType::Add, const_1,
                int32_type, nullptr));
    ...
    switch (t2->type) {
      case ASR::ttypeType::Integer: {
        ...
        new_type = ASRUtils::TYPE(ASR::make_Integer_t(
            al, x.base.base.loc, curr_type->m_kind, m_dims.p, m_dims.size()));
        break;
      }
      case ASR::ttypeType::Real: {
        ASR::Real_t* curr_type = down_cast<ASR::Real_t>(t2);
        new_type = ASRUtils::TYPE(ASR::make_Real_t(
            al, x.base.base.loc, curr_type->m_kind, m_dims.p, m_dims.size()));
        break;
      }
      case ASR::ttypeType::Complex: {
        ASR::Complex_t* curr_type = down_cast<ASR::Complex_t>(t2);
        new_type = ASRUtils::TYPE(ASR::make_Complex_t(
            al, x.base.base.loc, curr_type->m_kind, m_dims.p, m_dims.size()));
        break;
      }
      case ASR::ttypeType::Logical: {
        ASR::Logical_t* curr_type = down_cast<ASR::Logical_t>(t2);
        new_type = ASRUtils::TYPE(ASR::make_Logical_t(
            al, x.base.base.loc, curr_type->m_kind, m_dims.p, m_dims.size()));
        break;
      }
      case ASR::ttypeType::Derived: {
        ASR::Derived_t* curr_type = down_cast<ASR::Derived_t>(t2);
        new_type = ASRUtils::TYPE(ASR::make_Derived_t(al, x.base.base.loc,
                                                      curr_type->m_derived_type,
    ...
    if (ASR::is_a<ASR::Pointer_t>(*x.m_type)) {
      new_type =
          ASRUtils::TYPE(ASR::make_Pointer_t(al, x.base.base.loc, new_type));
    }
  ...
  void visit_ArraySection(const ASR::ArraySection_t& x) {
    if (create_slice_var) {
      ...
      ASR::asr_t* slice_asr = ASR::make_Variable_t(
          al, x.base.base.loc, current_scope, new_var_name,
          ASR::intentType::Local, nullptr, nullptr,
          ASR::storage_typeType::Default, get_array_from_slice(x, x_arr_var),
      ...
      slice_var = LFortran::ASRUtils::EXPR(
          ASR::make_Var_t(al, x.base.base.loc, slice_sym));
      Vec<ASR::expr_t*> idx_vars_target, idx_vars_value;
      PassUtils::create_idx_vars(idx_vars_target, x.n_args, x.base.base.loc, al,
                                 current_scope, "_t");
      PassUtils::create_idx_vars(idx_vars_value, x.n_args, x.base.base.loc, al,
                                 current_scope, "_v");
      ASR::stmt_t* doloop = nullptr;
      int a_kind =
          ASRUtils::extract_kind_from_ttype_t(ASRUtils::expr_type(x.m_v));
      ASR::ttype_t* int_type = LFortran::ASRUtils::TYPE(
      ...
      for (int i = (int)x.n_args - 1; i >= 0; i--) {
        ...
        if (doloop == nullptr) {
          ASR::expr_t* target_ref = PassUtils::create_array_ref(
              slice_sym, idx_vars_target, al, x.base.base.loc, x.m_type);
          ASR::expr_t* value_ref =
              PassUtils::create_array_ref(x.m_v, idx_vars_value, al);
          ASR::stmt_t* assign_stmt =
              LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
                  al, x.base.base.loc, target_ref, value_ref, nullptr));
          doloop_body.push_back(al, assign_stmt);
        } else {
          ASR::stmt_t* set_to_one = LFortran::ASRUtils::STMT(
              ASR::make_Assignment_t(al, x.base.base.loc,
                                     idx_vars_target[i + 1], const_1, nullptr));
          doloop_body.push_back(al, set_to_one);
          doloop_body.push_back(al, doloop);
        }
        ASR::expr_t* inc_expr =
            LFortran::ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
                al, x.base.base.loc, idx_vars_target[i], ASR::binopType::Add,
                const_1, int_type, nullptr));
        ASR::stmt_t* assign_stmt =
            LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
                al, x.base.base.loc, idx_vars_target[i], inc_expr, nullptr));
        doloop_body.push_back(al, assign_stmt);
      ...
      ASR::stmt_t* set_to_one = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
          al, x.base.base.loc, idx_vars_target[0], const_1, nullptr));
  ...
  void visit_IntegerBinOp(const ASR::IntegerBinOp_t& x) { handle_BinOp(x); }
  ...
  void visit_LogicalBinOp(const ASR::LogicalBinOp_t& x) { handle_BinOp(x); }
