...
public:
    ...
    ASR::symbol_t* create_subroutine_from_function(ASR::Function_t* s)
    {
        ...
        ASR::asr_t* s_sub_asr = ASR::make_Function_t(al,
                                                     s->base.base.loc,
                                                     s->m_symtab,
                                                     s->m_name,
                                                     a_args.p,
                            ...
                            ASR::ttype_t* var_type)
    {
        ...
        if (current_scope->get_symbol(std::string(idx_var_name)) == nullptr) {
            ASR::asr_t* idx_sym = ASR::make_Variable_t(al,
                                                       loc,
                                                       current_scope,
                                                       idx_var_name,
    ...
    void visit_Cast(const ASR::Cast_t& x)
    {
        ...
        if (tmp_val != nullptr && PassUtils::is_array(tmp_val)) {
            if (result_var == nullptr) {
                ...
                result_var = create_var(result_var_num,
                                        std::string("_implicit_cast_res"),
                                        x.base.base.loc,
            ...
            for (int i = n_dims - 1; i >= 0; i--) {
                ...
                if (doloop == nullptr) {
                    ...
                    ASR::expr_t* impl_cast_el_wise = LFortran::ASRUtils::EXPR(
                        ASR::make_Cast_t(al, x.base.base.loc, ref, x.m_kind, x.m_type, nullptr));
    ...
    void visit_Var(const ASR::Var_t& x)
    {
        ...
        if (result_var != nullptr && PassUtils::is_array(result_var)) {
            ...
            ASR::stmt_t* doloop = nullptr;
            for (int i = n_dims - 1; i >= 0; i--) {
                // TODO: Add an If debug node to check if the lower and upper bounds of both the
                // arrays are same.
                ASR::do_loop_head_t head;
                head.m_v = idx_vars[i];
                head.m_start = PassUtils::get_bound(result_var, i + 1, "lbound", al);
    ...
    void handle_UnaryOp(const T& x, int unary_type)
    {
        ...
        if (rank_operand > 0) {
            result_var = result_var_copy;
            if (result_var == nullptr) {
                result_var = create_var(result_var_num, res_prefix, x.base.base.loc, operand);
                result_var_num += 1;
            }
            tmp_val = result_var;
            ...
            for (int i = n_dims - 1; i >= 0; i--) {
                ...
                if (doloop == nullptr) {
                    ...
                    ASR::stmt_t* assign = LFortran::ASRUtils::STMT(
                        ASR::make_Assignment_t(al, x.base.base.loc, res, op_el_wise, nullptr));
                    doloop_body.push_back(al, assign);
                } else {
                    doloop_body.push_back(al, doloop);
                }
    ...
    void visit_ArrayOpCommon(const T& x, std::string res_prefix)
    {
        ...
        if (rank_left > 0 && rank_right > 0) {
            ...
            PassUtils::create_idx_vars(
                idx_vars_value, n_dims, x.base.base.loc, al, current_scope, "_v");
            ...
            for (int i = n_dims - 1; i >= 0; i--) {
                ...
                if (doloop == nullptr) {
                    ...
                    switch (x.class_type) {
                        case ASR::exprType::IntegerBinOp:
                            op_el_wise = LFortran::ASRUtils::EXPR(
                                ASR::make_IntegerBinOp_t(al,
                                                         x.base.base.loc,
                                                         ref_1,
                                                         (ASR::binopType) x.m_op,
                                                         ref_2,
                                                         x.m_type,
                                                         nullptr));
                            break;
                        ...
                        case ASR::exprType::IntegerCompare:
                            op_el_wise = LFortran::ASRUtils::EXPR(
                                ASR::make_IntegerCompare_t(al,
                                                           x.base.base.loc,
                                                           ref_1,
                                                           (ASR::cmpopType) x.m_op,
                                                           ref_2,
                                                           x.m_type,
                                                           nullptr));
                            break;
                ...
                ASR::expr_t* inc_expr
                    = LFortran::ASRUtils::EXPR(ASR::make_IntegerBinOp_t(al,
                                                                        x.base.base.loc,
                                                                        idx_vars_value[i],
                                                                        ASR::binopType::Add,
                                                                        const_1,
                                                                        int32_type,
                                                                        nullptr));
                ASR::stmt_t* assign_stmt = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
            ...
            ASR::stmt_t* set_to_one = LFortran::ASRUtils::STMT(
                ASR::make_Assignment_t(al, x.base.base.loc, idx_vars_value[0], idx_lb, nullptr));
            pass_result.push_back(al, set_to_one);
            pass_result.push_back(al, doloop);
        } else if ((rank_left == 0 && rank_right > 0) || (rank_right == 0 && rank_left > 0)) {
            result_var = result_var_copy;
    ...
    void visit_FunctionCall(const ASR::FunctionCall_t& x)
    {
        ...
        if (sub && ASR::is_a<ASR::Function_t>(*sub)
            ...
            if (result_var == nullptr) {
                result_var
                    = create_var(result_var_num, "_func_call_res", x.base.base.loc, x.m_type);
                result_var_num += 1;
            }
            ...
            tmp_val = result_var;
            ASR::stmt_t* subrout_call = LFortran::ASRUtils::STMT(ASR::make_SubroutineCall_t(
                al, x.base.base.loc, sub, nullptr, s_args.p, s_args.size(), nullptr));
            ...
            if (rank_operand > 0) {
                ...
                for (int i = n_dims - 1; i >= 0; i--) {
                    ...
                    if (doloop == nullptr) {
                        ...
                        op_el_wise
                            = LFortran::ASRUtils::EXPR(ASR::make_FunctionCall_t(al,
                                                                                x.base.base.loc,
                                                                                x.m_name,
                                                                                x.m_original_name,
                                                                                ref_args.p,
...
void
pass_replace_array_op(Allocator& al,
