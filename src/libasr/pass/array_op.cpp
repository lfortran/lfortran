...
In this case a new variable will not be created to store the result of RHS, just
Var1 will be used as the final destination and a do loop pass will be added as
follows, do i = lbound(Var1), ubound(Var1)
...
class ArrayOpVisitor : public PassUtils::PassVisitor<ArrayOpVisitor> {
 ...
 public:
  ArrayOpVisitor(Allocator& al, const std::string& rl_path)
  ...
  ASR::symbol_t* create_subroutine_from_function(ASR::Function_t* s) {
    ...
    ASR::asr_t* s_sub_asr = ASR::make_Function_t(
        al, s->base.base.loc, s->m_symtab, s->m_name, a_args.p, a_args.size(),
        nullptr, 0, s->m_body, s->n_body, nullptr, s->m_abi, s->m_access,
        s->m_deftype, nullptr, false, false, false, s->m_inline);
  ...
  ASR::ttype_t* get_matching_type(ASR::expr_t* sibling) {
    ...
    for (int i = 0; i < ndims; i++) {
      if (m_dims[i].m_start != nullptr || m_dims[i].m_length != nullptr) {
        return sibling_type;
      }
    }
    ...
    for (int i = 0; i < ndims; i++) {
      ...
      new_m_dim.m_length = ASRUtils::compute_length_from_start_end(
          al, new_m_dim.m_start,
          PassUtils::get_bound(sibling, i + 1, "ubound", al));
      new_m_dims.push_back(al, new_m_dim);
    }
    return PassUtils::set_dim_rank(sibling_type, new_m_dims.p, ndims, true,
                                   &al);
  }
  ...
  ASR::expr_t* create_var(int counter, std::string suffix, const Location& loc,
    ...
    if (current_scope->get_symbol(std::string(idx_var_name)) == nullptr) {
      ASR::asr_t* idx_sym = ASR::make_Variable_t(
          al, loc, current_scope, idx_var_name, ASR::intentType::Local, nullptr,
          nullptr, ASR::storage_typeType::Default, var_type,
          ASR::abiType::Source, ASR::accessType::Public,
          ASR::presenceType::Required, false);
      current_scope->add_symbol(std::string(idx_var_name),
                                ASR::down_cast<ASR::symbol_t>(idx_sym));
      idx_var = LFortran::ASRUtils::EXPR(
          ASR::make_Var_t(al, loc, ASR::down_cast<ASR::symbol_t>(idx_sym)));
    } else {
      ASR::symbol_t* idx_sym =
          current_scope->get_symbol(std::string(idx_var_name));
  ...
  void visit_Cast(const ASR::Cast_t& x) {
    ...
    if (tmp_val != nullptr && PassUtils::is_array(tmp_val)) {
      if (result_var == nullptr) {
        ...
        result_var =
            create_var(result_var_num, std::string("_implicit_cast_res"),
                       x.base.base.loc, const_cast<ASR::expr_t*>(&(x.base)));
        result_var_num += 1;
      }
      int n_dims = PassUtils::get_rank(result_var);
      Vec<ASR::expr_t*> idx_vars;
      PassUtils::create_idx_vars(idx_vars, n_dims, x.base.base.loc, al,
                                 current_scope);
      ...
      for (int i = n_dims - 1; i >= 0; i--) {
        ...
        if (doloop == nullptr) {
          ...
          ASR::expr_t* res =
              PassUtils::create_array_ref(result_var, idx_vars, al);
          ASR::expr_t* impl_cast_el_wise =
              LFortran::ASRUtils::EXPR(ASR::make_Cast_t(
                  al, x.base.base.loc, ref, x.m_kind, x.m_type, nullptr));
          ASR::stmt_t* assign = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
              al, x.base.base.loc, res, impl_cast_el_wise, nullptr));
          doloop_body.push_back(al, assign);
        } else {
          doloop_body.push_back(al, doloop);
        }
        doloop = LFortran::ASRUtils::STMT(ASR::make_DoLoop_t(
            al, x.base.base.loc, head, doloop_body.p, doloop_body.size()));
      }
  ...
  void visit_Var(const ASR::Var_t& x) {
    ...
    if (result_var != nullptr && PassUtils::is_array(result_var)) {
      ...
      ASR::stmt_t* doloop = nullptr;
      for (int i = n_dims - 1; i >= 0; i--) {
        // TODO: Add an If debug node to check if the lower and upper bounds of
        ...
        if (doloop == nullptr) {
          ...
          ASR::expr_t* res =
              PassUtils::create_array_ref(result_var, idx_vars, al);
          ASR::stmt_t* assign = LFortran::ASRUtils::STMT(
              ASR::make_Assignment_t(al, x.base.base.loc, res, ref, nullptr));
          doloop_body.push_back(al, assign);
        } else {
          doloop_body.push_back(al, doloop);
        }
  ...
  void visit_LogicalNot(const ASR::LogicalNot_t& x) { handle_UnaryOp(x, 4); }
  ...
  void handle_UnaryOp(const T& x, int unary_type) {
    ...
    if (rank_operand > 0) {
      ...
      if (result_var == nullptr) {
        result_var =
            create_var(result_var_num, res_prefix, x.base.base.loc, operand);
        result_var_num += 1;
      }
      tmp_val = result_var;
      ...
      for (int i = n_dims - 1; i >= 0; i--) {
        ...
        if (doloop == nullptr) {
          ...
          if (unary_type == 0) {
            op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_IntegerUnaryMinus_t(
                al, x.base.base.loc, ref, x.m_type, nullptr));
          } else if (unary_type == 1) {
            op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_RealUnaryMinus_t(
                al, x.base.base.loc, ref, x.m_type, nullptr));
          } else if (unary_type == 2) {
            op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_ComplexUnaryMinus_t(
                al, x.base.base.loc, ref, x.m_type, nullptr));
          } else if (unary_type == 3) {
            op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_IntegerBitNot_t(
                al, x.base.base.loc, ref, x.m_type, nullptr));
          } else if (unary_type == 4) {
            op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_LogicalNot_t(
                al, x.base.base.loc, ref, x.m_type, nullptr));
          }
          ASR::stmt_t* assign = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
              al, x.base.base.loc, res, op_el_wise, nullptr));
          doloop_body.push_back(al, assign);
        } else {
          doloop_body.push_back(al, doloop);
        }
    ...
    if (rank_left > 0 && rank_right > 0) {
      if (rank_left != rank_right) {
        ...
        throw LCompilersException(
            "Cannot generate loop for operands of different shapes");
      }
      result_var = result_var_copy;
      if (result_var == nullptr) {
        result_var =
            create_var(result_var_num, res_prefix, x.base.base.loc, left);
        result_var_num += 1;
      }
      tmp_val = result_var;
      ...
      PassUtils::create_idx_vars(idx_vars, n_dims, x.base.base.loc, al,
                                 current_scope, "_t");
      PassUtils::create_idx_vars(idx_vars_value, n_dims, x.base.base.loc, al,
                                 current_scope, "_v");
      ...
      for (int i = n_dims - 1; i >= 0; i--) {
        ...
        if (doloop == nullptr) {
          ASR::expr_t* ref_1 =
              PassUtils::create_array_ref(left, idx_vars_value, al);
          ASR::expr_t* ref_2 =
              PassUtils::create_array_ref(right, idx_vars_value, al);
          ASR::expr_t* res =
              PassUtils::create_array_ref(result_var, idx_vars, al);
          ASR::expr_t* op_el_wise = nullptr;
          switch (x.class_type) {
            case ASR::exprType::IntegerBinOp:
              op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
                  al, x.base.base.loc, ref_1, (ASR::binopType)x.m_op, ref_2,
                  x.m_type, nullptr));
              break;
            ...
            case ASR::exprType::LogicalBinOp:
              op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_LogicalBinOp_t(
                  al, x.base.base.loc, ref_1, (ASR::logicalbinopType)x.m_op,
                  ref_2, x.m_type, nullptr));
              break;
            case ASR::exprType::IntegerCompare:
              op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_IntegerCompare_t(
                  al, x.base.base.loc, ref_1, (ASR::cmpopType)x.m_op, ref_2,
                  x.m_type, nullptr));
              break;
            ...
            default:
              throw LCompilersException(
                  "The desired operation is not supported yet for arrays.");
          }
          ASR::stmt_t* assign = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
              al, x.base.base.loc, res, op_el_wise, nullptr));
          doloop_body.push_back(al, assign);
          ...
          ASR::stmt_t* set_to_one =
              LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
                  al, x.base.base.loc, idx_vars_value[i + 1], idx_lb, nullptr));
          doloop_body.push_back(al, set_to_one);
          doloop_body.push_back(al, doloop);
        }
        ASR::expr_t* inc_expr =
            LFortran::ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
                al, x.base.base.loc, idx_vars_value[i], ASR::binopType::Add,
                const_1, int32_type, nullptr));
        ASR::stmt_t* assign_stmt =
            LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
                al, x.base.base.loc, idx_vars_value[i], inc_expr, nullptr));
        doloop_body.push_back(al, assign_stmt);
        doloop = LFortran::ASRUtils::STMT(ASR::make_DoLoop_t(
            al, x.base.base.loc, head, doloop_body.p, doloop_body.size()));
      }
      ASR::expr_t* idx_lb = PassUtils::get_bound(right, 1, "lbound", al);
      ASR::stmt_t* set_to_one = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
          al, x.base.base.loc, idx_vars_value[0], idx_lb, nullptr));
      ...
      if (result_var == nullptr) {
        result_var =
            create_var(result_var_num, res_prefix, x.base.base.loc, arr_expr);
        result_var_num += 1;
      }
      tmp_val = result_var;
      ...
      for (int i = n_dims - 1; i >= 0; i--) {
        ...
        if (doloop == nullptr) {
          ASR::expr_t* ref =
              PassUtils::create_array_ref(arr_expr, idx_vars_value, al);
          ASR::expr_t* res =
              PassUtils::create_array_ref(result_var, idx_vars, al);
          ...
          switch (x.class_type) {
            case ASR::exprType::IntegerBinOp:
              op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
                  al, x.base.base.loc, lexpr, (ASR::binopType)x.m_op, rexpr,
                  x.m_type, nullptr));
              break;
            case ASR::exprType::RealBinOp:
              op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_RealBinOp_t(
            ...
            case ASR::exprType::LogicalBinOp:
              op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_LogicalBinOp_t(
                  al, x.base.base.loc, lexpr, (ASR::logicalbinopType)x.m_op,
                  rexpr, x.m_type, nullptr));
              break;
            case ASR::exprType::IntegerCompare:
              op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_IntegerCompare_t(
                  al, x.base.base.loc, lexpr, (ASR::cmpopType)x.m_op, rexpr,
                  x.m_type, nullptr));
              break;
            case ASR::exprType::RealCompare:
              op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_RealCompare_t(
          ...
          ASR::expr_t* idx_lb =
              PassUtils::get_bound(op_expr, i + 2, "lbound", al);
          ASR::stmt_t* set_to_one =
              LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
                  al, x.base.base.loc, idx_vars_value[i + 1], idx_lb, nullptr));
          doloop_body.push_back(al, set_to_one);
  ...
  void visit_FunctionCall(const ASR::FunctionCall_t& x) {
    ...
    if (sub && ASR::is_a<ASR::Function_t>(*sub) &&
      ...
      tmp_val = result_var;
      ASR::stmt_t* subrout_call = LFortran::ASRUtils::STMT(
          ASR::make_SubroutineCall_t(al, x.base.base.loc, sub, nullptr,
      ...
      for (size_t iarg = 0; iarg < x.n_args; iarg++) {
        ...
        if (common_rank != rank_operand && rank_operand > 0) {
      ...
      if (!are_all_rank_same) {
        throw LCompilersException(
            "Broadcasting support not yet available "
      ...
      for (int i = n_dims - 1; i >= 0; i--) {
        ...
        if (doloop == nullptr) {
          ...
          ASR::ttype_t* dim_less_type =
              ASRUtils::duplicate_type(al, x.m_type, &empty_dim);
          ASR::expr_t* op_el_wise = nullptr;
          op_el_wise = LFortran::ASRUtils::EXPR(ASR::make_FunctionCall_t(
              al, x.base.base.loc, x.m_name, x.m_original_name, ref_args.p,
              ref_args.size(), dim_less_type, nullptr, x.m_dt));
          ASR::expr_t* res =
              PassUtils::create_array_ref(result_var, idx_vars, al);
          ASR::stmt_t* assign = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
              al, x.base.base.loc, res, op_el_wise, nullptr));
