...
class PassManager {
 private:
  ...
  std::map<std::string, pass_function> _passes_db = {
      ...
      {"array_dim_intrinsics_update",
       &LFortran::pass_update_array_dim_intrinsic_calls},
 ...
 public:
  PassManager() : is_fast{false}, apply_default_passes{false} {
    _passes = {"global_stmts",     "class_constructor",
               "implied_do_loops", "pass_array_by_data",
               "arr_slice",        "array_op",
               "print_arr",        "array_dim_intrinsics_update",
               "do_loops",         "forall",
               "select_case",      "inline_function_calls",
               "unused_functions"};

    _with_optimization_passes = {"global_stmts",
                                 "class_constructor",
                                 "implied_do_loops",
                                 "pass_array_by_data",
                                 "arr_slice",
    ...
    for (size_t i = 0; i < arg_pass.size(); i++) {
      ...
      if (ch == ',' || i == arg_pass.size() - 1) {
        ...
        if (_passes_db.find(current_pass) == _passes_db.end()) {
          ...
          std::cerr << " Only the following passes are supported:- "
                    << std::endl;
  ...
  void use_optimization_passes() { is_fast = true; }

  void do_not_use_optimization_passes() { is_fast = false; }

  void use_default_passes() { apply_default_passes = true; }

  void do_not_use_default_passes() { apply_default_passes = false; }
};
