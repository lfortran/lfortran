...
public:
    ...
    void visit_Program(const ASR::Program_t& x) {
        ...
        SymbolTable* old_scope = current_scope;
        current_scope = x.m_symtab;
        transform_stmts_in_body(xx.m_body, xx.n_body);
        for (auto& item : x.m_symtab->get_scope()) {
            visit_symbol(*item.second);
        }
        current_scope = old_scope;
    ...
    void replace_stmts_in_body(ASR::stmt_t**& body, size_t& n_body) {
        Vec<ASR::stmt_t*> new_body;
        new_body.reserve(al, n_body);
        bool saved_transform = transform_stmts;
        Vec<ASR::stmt_t*> saved_result;
        saved_result.from_pointer_n_copy(al, pass_result.p, pass_result.size());
        for (size_t i = 0; i < n_body; i++) {
            pass_result.n = 0;
            pass_result.reserve(al, 1);
            transform_stmts = false;
        ...
        body = new_body.p;
        n_body = new_body.size();
        transform_stmts = saved_transform;
        pass_result.from_pointer_n_copy(al, saved_result.p, saved_result.size());
    }

    void visit_WhileLoop(const ASR::WhileLoop_t& x) {
        ASR::WhileLoop_t& xx = const_cast<ASR::WhileLoop_t&>(x);
        replace_stmts_in_body(xx.m_body, xx.n_body);
    }
    ...
    void visit_If(const ASR::If_t& x) {
        ...
        replace_stmts_in_body(xx.m_body, xx.n_body);
        if (x.n_orelse > 0) {
            replace_stmts_in_body(xx.m_orelse, xx.n_orelse);
        }
    }
