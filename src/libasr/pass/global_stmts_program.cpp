...
void pass_wrap_global_stmts_into_program(
  ...
  if (unit.n_items > 0) {
    ...
    if (ASR::is_a<ASR::Function_t>(*fn) &&
      ...
      ASR::asr_t *stmt = ASR::make_SubroutineCall_t(
          al, unit.base.base.loc, fn, nullptr, nullptr, 0, nullptr);
  ...
  ASR::asr_t *prog =
      ASR::make_Program_t(al, unit.base.base.loc,
                          /* a_symtab */ current_scope,
                          /* a_name */ s2c(al, prog_name), nullptr, 0,
                          /* a_body */ prog_body.p,
                          /* n_body */ prog_body.n);
  unit.m_global_scope->add_symbol(prog_name,
                                  ASR::down_cast<ASR::symbol_t>(prog));
