...
class ReplaceArrayConstant: public ASR::BaseExprReplacer<ReplaceArrayConstant> {
    ...
    void replace_ImpliedDoLoop(ASR::ImpliedDoLoop_t* x) {
        ...
        args.push_back(al, ASRUtils::EXPR((ASR::asr_t*)x));
        ...
        dim.m_length = ASRUtils::EXPR(ASR::make_IntegerConstant_t(al, x->base.base.loc, 1, ASRUtils::TYPE(ASR::make_Integer_t(al, x->base.base.loc, 4))));

        dims.push_back(al, dim);
        ASR::ttype_t* array_type = ASRUtils::TYPE(ASR::make_Array_t(al, x->base.base.loc,
                                    x->m_type, dims.p, dims.n, ASR::array_physical_typeType::FixedSizeArray));
        ASR::expr_t* array_constant_expr = ASRUtils::EXPR(ASR::make_ArrayConstant_t(al, x->base.base.loc, args.p, args.n,
                                            array_type, ASR::arraystorageType::ColMajor));

        // result_var = nullptr;
        replace_ArrayConstant(ASR::down_cast<ASR::ArrayConstant_t>(array_constant_expr));
    }
    ...
    public:
        ...
        void visit_Print(const ASR::Print_t &x) {
            for (size_t i = 0; i < x.n_values; i++) {
                replacer.result_var = nullptr;
                resultvar2value[replacer.result_var] = x.m_values[i];
                ASR::expr_t** current_expr_copy_9 = current_expr;
...
void pass_replace_array_constant(Allocator &al,
