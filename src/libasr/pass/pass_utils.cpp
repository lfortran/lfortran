...
namespace PassUtils
{
                               ...
                               Allocator* al)
    {
        ...
        switch (t2->type) {
            ...
            case ASR::ttypeType::Character: {
                ASR::Character_t* x_type_ref = ASR::down_cast<ASR::Character_t>(t2);
                if (create_new) {
                    new_type = LFortran::ASRUtils::TYPE(ASR::make_Character_t(*al,
                                                                              x_type->base.loc,
                                                                              x_type_ref->m_kind,
                                                                              x_type_ref->m_len,
                                                                              nullptr,
    ...
    int get_rank(ASR::expr_t* x)
    {
        ...
        if (x->type == ASR::exprType::Var) {
            const ASR::symbol_t* x_sym
                = LFortran::ASRUtils::symbol_get_past_external(ASR::down_cast<ASR::Var_t>(x)->m_v);
                                  ...
                                  ASR::ttype_t* _type)
    {
        ...
        ASR::expr_t* array_ref = LFortran::ASRUtils::EXPR(
            ASR::make_ArrayItem_t(al, loc, arr_var, args.p, args.size(), _type, nullptr));
        return array_ref;
    }
    ...
    ASR::symbol_t* import_function(std::string func_name,
                                   std::string module_name,
                                   Allocator& al,
                                   ASR::TranslationUnit_t& unit,
                                   const std::string& rl_path,
    ...
    ASR::symbol_t* import_function2(std::string func_name,
                                    std::string module_name,
                                    Allocator& al,
                                    ASR::TranslationUnit_t& unit,
                                    SymbolTable*& current_scope)
    ...
    {
        ASR::ttype_t* int32_type
            = LFortran::ASRUtils::TYPE(ASR::make_Integer_t(al, arr_expr->base.loc, 4, nullptr, 0));
        ...
        return LFortran::ASRUtils::EXPR(ASR::make_ArrayBound_t(
    ...
    {
        ASR::symbol_t* v = import_generic_procedure("flipsign",
        ...
        return ASRUtils::STMT(ASRUtils::symbol_resolve_external_generic_procedure_without_eval(
    ...
    {
        ASR::asr_t* expr_sym = ASR::make_Variable_t(al,
                                                    expr->base.loc,
                                                    current_scope,
                                                    s2c(al, name),
        ...
        if (current_scope->get_symbol(name) == nullptr) {
            ...
            throw LCompilersException("Symbol with " + name + " is already present in "
                                      + std::to_string(current_scope->counter));
        }
        ASR::expr_t* var = LFortran::ASRUtils::EXPR(
            ASR::make_Var_t(al, expr->base.loc, ASR::down_cast<ASR::symbol_t>(expr_sym)));
    ...
    ASR::expr_t* get_fma(ASR::expr_t* arg0,
                         ...
                         const std::function<void(const std::string&, const Location&)> err)
    {
        ASR::symbol_t* v = import_generic_procedure("fma",
                                                    "lfortran_intrinsic_optimization",
                                                    al,
                                                    unit,
                                                    rl_path,
        ...
        return ASRUtils::EXPR(ASRUtils::symbol_resolve_external_generic_procedure_without_eval(
                                               ...
                                               std::string prefix)
    {
        ...
        for (int i = 0; i < num_args; i++) {
            ...
            ASR::symbol_t* arg = ASR::down_cast<ASR::symbol_t>(
                ASR::make_Variable_t(al,
                                     ...
                                     nullptr,
                                     ASR::storage_typeType::Default,
                                     types[std::min(i, (int) types.size() - 1)],
                                     ASR::abiType::Source,
                                     ASR::accessType::Public,
                                     ASR::presenceType::Required,
                                     false));
        ...
        ASR::stmt_t* copy_stmt
            = ASRUtils::STMT(ASR::make_Assignment_t(al, target->base.loc, target, value, nullptr));
        loop_body.push_back(al, copy_stmt);
        ASR::stmt_t* fallback_loop = ASRUtils::STMT(
            ASR::make_DoLoop_t(al, do_loop_head.loc, do_loop_head, loop_body.p, loop_body.size()));
        Vec<ASR::stmt_t*> fallback_while_loop = replace_doloop(
            al, *ASR::down_cast<ASR::DoLoop_t>(fallback_loop), (int) ASR::cmpopType::Lt);
                                 ...
                                 Location& loc)
    {
        ...
        ASR::symbol_t* v
            = insert_fallback_vector_copy(al, unit, global_scope, types, "vector_copy_");
        Vec<ASR::call_arg_t> args;
        args.reserve(al, 6);
        ...
        return ASRUtils::STMT(
            ASR::make_SubroutineCall_t(al, loc, v, nullptr, args.p, args.size(), nullptr));
    }
    ...
    ASR::expr_t* get_sign_from_value(
        ...
        const std::function<void(const std::string&, const Location&)> err)
    {
        ASR::symbol_t* v = import_generic_procedure("sign_from_value",
                                                    "lfortran_intrinsic_optimization",
                                                    al,
                                                    unit,
                                                    rl_path,
    ...
    Vec<ASR::stmt_t*> replace_doloop(Allocator& al, const ASR::DoLoop_t& loop, int comp)
    {
        ...
        if (!a && !b && !c) {
            ...
            ASR::ttype_t* cond_type
                = LFortran::ASRUtils::TYPE(ASR::make_Logical_t(al, loc, a_kind, nullptr, 0));
            ...
            if (!c) {
                int a_kind
                    = ASRUtils::extract_kind_from_ttype_t(ASRUtils::expr_type(loop.m_head.m_v));
                ASR::ttype_t* type
                    = LFortran::ASRUtils::TYPE(ASR::make_Integer_t(al, loc, a_kind, nullptr, 0));
            ...
            inc_stmt = LFortran::ASRUtils::STMT(
                ASR::make_Assignment_t(al,
                                       loc,
                                       target,
                                       LFortran::ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
        ...
        ASR::stmt_t* stmt2
            = LFortran::ASRUtils::STMT(ASR::make_WhileLoop_t(al, loc, cond, body.p, body.size()));
