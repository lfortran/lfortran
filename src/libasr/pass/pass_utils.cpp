...
void get_dim_rank(ASR::ttype_t* x_type, ASR::dimension_t*& m_dims,
                  int& n_dims) {
...
ASR::ttype_t* set_dim_rank(ASR::ttype_t* x_type, ASR::dimension_t*& m_dims,
                           int& n_dims, bool create_new, Allocator* al) {
  ...
  switch (t2->type) {
    case ASR::ttypeType::Integer: {
      ASR::Integer_t* x_type_ref = ASR::down_cast<ASR::Integer_t>(t2);
      if (create_new) {
        new_type = LFortran::ASRUtils::TYPE(ASR::make_Integer_t(
            *al, x_type->base.loc, x_type_ref->m_kind, m_dims, n_dims));
      } else {
    ...
    case ASR::ttypeType::Real: {
      ASR::Real_t* x_type_ref = ASR::down_cast<ASR::Real_t>(t2);
      if (create_new) {
        new_type = LFortran::ASRUtils::TYPE(ASR::make_Real_t(
            *al, x_type->base.loc, x_type_ref->m_kind, m_dims, n_dims));
      } else {
        x_type_ref->n_dims = n_dims;
        x_type_ref->m_dims = m_dims;
    ...
    case ASR::ttypeType::Complex: {
      ASR::Complex_t* x_type_ref = ASR::down_cast<ASR::Complex_t>(t2);
      if (create_new) {
        new_type = LFortran::ASRUtils::TYPE(ASR::make_Complex_t(
            *al, x_type->base.loc, x_type_ref->m_kind, m_dims, n_dims));
      } else {
        x_type_ref->n_dims = n_dims;
        x_type_ref->m_dims = m_dims;
    ...
    case ASR::ttypeType::Derived: {
      ASR::Derived_t* x_type_ref = ASR::down_cast<ASR::Derived_t>(t2);
      if (create_new) {
        new_type = LFortran::ASRUtils::TYPE(ASR::make_Derived_t(
            *al, x_type->base.loc, x_type_ref->m_derived_type, m_dims, n_dims));
      } else {
        x_type_ref->n_dims = n_dims;
        x_type_ref->m_dims = m_dims;
      }
    ...
    case ASR::ttypeType::Logical: {
      ASR::Logical_t* x_type_ref = ASR::down_cast<ASR::Logical_t>(t2);
      if (create_new) {
        new_type = LFortran::ASRUtils::TYPE(ASR::make_Logical_t(
            *al, x_type->base.loc, x_type_ref->m_kind, m_dims, n_dims));
      } else {
        x_type_ref->n_dims = n_dims;
        x_type_ref->m_dims = m_dims;
    ...
    case ASR::ttypeType::Character: {
      ASR::Character_t* x_type_ref = ASR::down_cast<ASR::Character_t>(t2);
      if (create_new) {
        new_type = LFortran::ASRUtils::TYPE(
            ASR::make_Character_t(*al, x_type->base.loc, x_type_ref->m_kind,
                                  x_type_ref->m_len, nullptr, m_dims, n_dims));
      } else {
        x_type_ref->n_dims = n_dims;
        x_type_ref->m_dims = m_dims;
      }
  ...
  if (ASR::is_a<ASR::Pointer_t>(*x_type)) {
    new_type =
        ASRUtils::TYPE(ASR::make_Pointer_t(*al, x_type->base.loc, new_type));
  }
...
bool is_array(ASR::expr_t* x) { return get_rank(x) > 0; }
...
ASR::expr_t* create_array_ref(ASR::expr_t* arr_expr,
  ...
  ASR::expr_t* array_ref = LFortran::ASRUtils::EXPR(ASR::make_ArrayItem_t(
      al, arr_expr->base.loc, arr_expr, args.p, args.size(), _type, nullptr));
  return array_ref;
}
...
ASR::expr_t* create_array_ref(ASR::symbol_t* arr, Vec<ASR::expr_t*>& idx_vars,
  ...
  ASR::expr_t* array_ref = LFortran::ASRUtils::EXPR(ASR::make_ArrayItem_t(
      al, loc, arr_var, args.p, args.size(), _type, nullptr));
  return array_ref;
}
  ...
  for (int i = 1; i <= n_vars; i++) {
    ...
    if (current_scope->get_symbol(std::string(var_name)) == nullptr) {
      ASR::asr_t* idx_sym = ASR::make_Variable_t(
          al, loc, current_scope, var_name, intent, nullptr, nullptr,
          ASR::storage_typeType::Default, int32_type, ASR::abiType::Source,
          ASR::accessType::Public, ASR::presenceType::Required, false);
      current_scope->add_symbol(std::string(var_name),
...
ASR::symbol_t* import_generic_procedure(std::string func_name,
                                        std::string module_name, Allocator& al,
  ...
  ASR::Module_t* m = LFortran::ASRUtils::load_module(
      al, current_scope, module_name, loc, true, rl_path, false,
  ...
  ASR::asr_t* fn = ASR::make_ExternalSymbol_t(
      al, t->base.loc, current_scope, s2c(al, sym), t, s2c(al, module_name),
      nullptr, 0, s2c(al, remote_sym), ASR::accessType::Private);
...
ASR::symbol_t* import_function(std::string func_name, std::string module_name,
  ...
  ASR::asr_t* fn = ASR::make_ExternalSymbol_t(
      al, mfn->base.base.loc, current_scope, mfn->m_name, (ASR::symbol_t*)mfn,
...
ASR::symbol_t* import_function2(std::string func_name, std::string module_name,
  ...
  ASR::Function_t* mfn = ASR::down_cast<ASR::Function_t>(t);
  ASR::asr_t* fn = ASR::make_ExternalSymbol_t(
      al, mfn->base.base.loc, current_scope2, mfn->m_name, (ASR::symbol_t*)mfn,
      m->m_name, nullptr, 0, mfn->m_name, ASR::accessType::Private);
  std::string sym = mfn->m_name;
...
ASR::expr_t* create_compare_helper(Allocator& al, const Location& loc,
  ...
  if (ASRUtils::is_integer(*type)) {
    return ASRUtils::EXPR(
        ASR::make_IntegerCompare_t(al, loc, left, op, right, type, nullptr));
  } else if (ASRUtils::is_real(*type)) {
    return ASRUtils::EXPR(
        ASR::make_RealCompare_t(al, loc, left, op, right, type, nullptr));
  } else if (ASRUtils::is_complex(*type)) {
    return ASRUtils::EXPR(
        ASR::make_ComplexCompare_t(al, loc, left, op, right, type, nullptr));
  } else if (ASRUtils::is_logical(*type)) {
    return ASRUtils::EXPR(
        ASR::make_LogicalCompare_t(al, loc, left, op, right, type, nullptr));
  } else if (ASRUtils::is_character(*type)) {
    return ASRUtils::EXPR(
        ASR::make_StringCompare_t(al, loc, left, op, right, type, nullptr));
  } else {
...
ASR::expr_t* create_binop_helper(Allocator& al, const Location& loc,
  ...
  if (ASRUtils::is_integer(*type)) {
    return ASRUtils::EXPR(
        ASR::make_IntegerBinOp_t(al, loc, left, op, right, type, nullptr));
  } else if (ASRUtils::is_real(*type)) {
    return ASRUtils::EXPR(
        ASR::make_RealBinOp_t(al, loc, left, op, right, type, nullptr));
  } else if (ASRUtils::is_complex(*type)) {
    return ASRUtils::EXPR(
        ASR::make_ComplexBinOp_t(al, loc, left, op, right, type, nullptr));
  } else {
    throw LCompilersException("Type not supported");
  }
}
...
ASR::expr_t* get_bound(ASR::expr_t* arr_expr, int dim, std::string bound,
  ...
  ASR::expr_t* dim_expr = LFortran::ASRUtils::EXPR(
...
ASR::stmt_t* get_flipsign(
  ...
  ASR::symbol_t* v = import_generic_procedure(
      "flipsign", "lfortran_intrinsic_optimization", al, unit, rl_path,
      current_scope, arg0->base.loc);
  ...
  return ASRUtils::STMT(
      ASRUtils::symbol_resolve_external_generic_procedure_without_eval(
          arg0->base.loc, v, args, current_scope, al, err));
}
...
ASR::expr_t* to_int32(ASR::expr_t* x, ASR::ttype_t* int64type, Allocator& al) {
  ...
  if (cast_kind > 0) {
    return LFortran::ASRUtils::EXPR(ASR::make_Cast_t(
        al, x->base.loc, x, (ASR::cast_kindType)cast_kind, int64type, nullptr));
  } else {
    throw LCompilersException(
        "Array indices can only be of type real or integer.");
  }
  return nullptr;
}
...
ASR::expr_t* create_auxiliary_variable_for_expr(ASR::expr_t* expr,
  ...
  ASR::asr_t* expr_sym = ASR::make_Variable_t(
      al, expr->base.loc, current_scope, s2c(al, name), ASR::intentType::Local,
  ...
  if (current_scope->get_symbol(name) == nullptr) {
    ...
    throw LCompilersException("Symbol with " + name +
                              " is already present in " +
                              std::to_string(current_scope->counter));
  }
  ASR::expr_t* var = LFortran::ASRUtils::EXPR(ASR::make_Var_t(
      al, expr->base.loc, ASR::down_cast<ASR::symbol_t>(expr_sym)));
  assign_stmt = ASRUtils::STMT(
      ASR::make_Assignment_t(al, var->base.loc, var, expr, nullptr));
  return var;
}
...
ASR::expr_t* create_auxiliary_variable(Location& loc, std::string& name,
  ...
  ASR::asr_t* expr_sym = ASR::make_Variable_t(
      al, loc, current_scope, s2c(al, name), ASR::intentType::Local, nullptr,
      nullptr, ASR::storage_typeType::Default, var_type, ASR::abiType::Source,
      ASR::accessType::Public, ASR::presenceType::Required, false);
  if (current_scope->get_symbol(name) == nullptr) {
    current_scope->add_symbol(name, ASR::down_cast<ASR::symbol_t>(expr_sym));
  } else {
  ...
  ASR::expr_t* var = LFortran::ASRUtils::EXPR(
      ASR::make_Var_t(al, loc, ASR::down_cast<ASR::symbol_t>(expr_sym)));
  return var;
}
...
ASR::expr_t* get_fma(
    ...
    const std::function<void(const std::string&, const Location&)> err) {
  ASR::symbol_t* v =
      import_generic_procedure("fma", "lfortran_intrinsic_optimization", al,
                               unit, rl_path, current_scope, arg0->base.loc);
  Vec<ASR::call_arg_t> args;
  args.reserve(al, 3);
...
ASR::symbol_t* insert_fallback_vector_copy(Allocator& al,
                                           ASR::TranslationUnit_t& unit,
  ...
  for (int i = 0; i < num_args; i++) {
    ...
    ASR::symbol_t* arg = ASR::down_cast<ASR::symbol_t>(ASR::make_Variable_t(
        al, unit.base.base.loc, vector_copy_symtab, s2c(al, arg_name),
        ASR::intentType::In, nullptr, nullptr, ASR::storage_typeType::Default,
        types[std::min(i, (int)types.size() - 1)], ASR::abiType::Source,
        ASR::accessType::Public, ASR::presenceType::Required, false));
    ASR::expr_t* arg_expr =
        ASRUtils::EXPR(ASR::make_Var_t(al, arg->base.loc, arg));
  ...
  create_idx_vars(idx_vars, 1, arg_exprs[2]->base.loc, al, vector_copy_symtab);
  ...
  ASR::stmt_t* copy_stmt = ASRUtils::STMT(
      ASR::make_Assignment_t(al, target->base.loc, target, value, nullptr));
  loop_body.push_back(al, copy_stmt);
  ASR::stmt_t* fallback_loop = ASRUtils::STMT(ASR::make_DoLoop_t(
      al, do_loop_head.loc, do_loop_head, loop_body.p, loop_body.size()));
  Vec<ASR::stmt_t*> fallback_while_loop =
      replace_doloop(al, *ASR::down_cast<ASR::DoLoop_t>(fallback_loop),
                     (int)ASR::cmpopType::Lt);
  for (size_t i = 0; i < fallback_while_loop.size(); i++) {
    body.push_back(al, fallback_while_loop[i]);
  }
  ASR::asr_t* vector_copy_asr = ASR::make_Function_t(
      al, unit.base.base.loc, vector_copy_symtab, s2c(al, vector_copy_name),
      arg_exprs.p, arg_exprs.size(), nullptr, 0, body.p, body.size(), nullptr,
      ASR::abiType::Source, ASR::accessType::Public,
      ASR::deftypeType::Implementation, nullptr, false, false, false, false);
  global_scope->add_symbol(vector_copy_name,
                           ASR::down_cast<ASR::symbol_t>(vector_copy_asr));
...
ASR::stmt_t* get_vector_copy(ASR::expr_t* array0, ASR::expr_t* array1,
  ...
  ASR::symbol_t* v = insert_fallback_vector_copy(al, unit, global_scope, types,
                                                 "vector_copy_");
  Vec<ASR::call_arg_t> args;
  args.reserve(al, 6);
  ...
  return ASRUtils::STMT(ASR::make_SubroutineCall_t(al, loc, v, nullptr, args.p,
                                                   args.size(), nullptr));
}
...
ASR::expr_t* get_sign_from_value(
    ...
    const std::function<void(const std::string&, const Location&)> err) {
  ASR::symbol_t* v = import_generic_procedure(
      "sign_from_value", "lfortran_intrinsic_optimization", al, unit, rl_path,
      current_scope, arg0->base.loc);
  Vec<ASR::call_arg_t> args;
  args.reserve(al, 2);
  ASR::call_arg_t arg0_, arg1_;
...
Vec<ASR::stmt_t*> replace_doloop(Allocator& al, const ASR::DoLoop_t& loop,
  ...
  if (!a && !b && !c) {
    int a_kind = 4;
    if (loop.m_head.m_v) {
      a_kind = ASRUtils::extract_kind_from_ttype_t(
          ASRUtils::expr_type(loop.m_head.m_v));
    }
    ASR::ttype_t* cond_type = LFortran::ASRUtils::TYPE(
        ASR::make_Logical_t(al, loc, a_kind, nullptr, 0));
    cond = LFortran::ASRUtils::EXPR(
        ASR::make_LogicalConstant_t(al, loc, true, cond_type));
  } else {
    LFORTRAN_ASSERT(a);
    LFORTRAN_ASSERT(b);
    if (!c) {
    ...
    ASR::ttype_t* type = LFortran::ASRUtils::TYPE(
        ASR::make_Integer_t(al, loc, a_kind, nullptr, 0));
    stmt1 = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
        al, loc, target,
        LFortran::ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
            al, loc, a, ASR::binopType::Sub, c, type, nullptr)),
        nullptr));

    cond = LFortran::ASRUtils::EXPR(ASR::make_IntegerCompare_t(
        al, loc,
        LFortran::ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
            al, loc, target, ASR::binopType::Add, c, type, nullptr)),
        cmp_op, b, type, nullptr));

    inc_stmt = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
        al, loc, target,
        LFortran::ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
            al, loc, target, ASR::binopType::Add, c, type, nullptr)),
        nullptr));
  ...
  ASR::stmt_t* stmt2 = LFortran::ASRUtils::STMT(
      ASR::make_WhileLoop_t(al, loc, cond, body.p, body.size()));
