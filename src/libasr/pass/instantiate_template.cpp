...
public:
    ...
    ASR::asr_t* instantiate_Function(ASR::Function_t& x)
    {
        ...
        std::string func_name = new_function_name;

        Vec<ASR::expr_t*> args;
        args.reserve(al, x.n_args);
        for (size_t i = 0; i < x.n_args; i++) {
            ASR::Variable_t* param_var
                = ASR::down_cast<ASR::Variable_t>((ASR::down_cast<ASR::Var_t>(x.m_args[i]))->m_v);
        ...
        ASR::Variable_t* return_var
            = ASR::down_cast<ASR::Variable_t>((ASR::down_cast<ASR::Var_t>(x.m_return_var))->m_v);
        std::string return_var_name = return_var->m_name;
        ASR::ttype_t* return_param_type = ASRUtils::expr_type(x.m_return_var);
        ASR::ttype_t* return_type
            = ASR::is_a<ASR::TypeParameter_t>(*return_param_type)
                  ? subs[ASR::down_cast<ASR::TypeParameter_t>(return_param_type)->m_param]
                  : return_param_type;
        ASR::asr_t* new_return_var = ASR::make_Variable_t(al,
        ...
        for (auto const& sym_pair : x.m_symtab->get_scope()) {
            if (current_scope->resolve_symbol(sym_pair.first) == nullptr) {
                ...
                if (ASR::is_a<ASR::Variable_t>(*sym)) {
                    ...
                    ASR::asr_t* new_var = ASR::make_Variable_t(al,
                                                               var_sym->base.base.loc,
                                                               current_scope,
                                                               s2c(al, var_sym_name),
                                                               var_sym->m_intent,
                                                               nullptr,
        ...
        ASR::asr_t* result = ASR::make_Function_t(al,
                                                  x.base.base.loc,
                                                  current_scope,
                                                  s2c(al, func_name),
                                                  args.p,
        ...
        if (ASR::is_a<ASR::TypeParameter_t>(*param_type)) {
            ...
            switch (t->type) {
                ...
                case ASR::ttypeType::Character: {
                    ...
                    return ASRUtils::TYPE(ASR::make_Character_t(al,
                                                                t->base.loc,
                                                                tnew->m_kind,
                                                                tnew->m_len,
                                                                tnew->m_len_expr,
    ...
    ASR::asr_t* make_BinOp_helper(ASR::expr_t* left,
                                  ASR::expr_t* right,
                                  ASR::binopType op,
                                  const Location& loc)
    {
        ...
        // bool right_is_int = ASRUtils::is_character(*left_type) &&
        // ASRUtils::is_integer(*right_type); bool left_is_int = ASRUtils::is_integer(*left_type) &&
        ...
        if (ASRUtils::is_integer(*dest_type)) {
                    ...
                    case (ASR::binopType::BitLShift): {
                        if (right_value < 0) {
                            throw LCompilersException(
                                "ICE: failure in instantiation: Negative shift count not allowed.");
                        }
                        result = left_value << right_value;
                        break;
                    }
            ...
            if (op == ASR::binopType::BitAnd || op == ASR::binopType::BitOr
                || op == ASR::binopType::BitXor || op == ASR::binopType::BitLShift
                || op == ASR::binopType::BitRShift) {
                throw LCompilersException(
                    "ICE: failure in instantiation: Unsupported binary operation on floats: '"
    ...
    ASR::expr_t* cast_helper(ASR::ttype_t* left_type, ASR::expr_t* right, bool is_assign = false)
    {
        ...
        if (ASRUtils::is_integer(*left_type) && ASRUtils::is_integer(*right_type)) {
            ...
            if ((is_assign && (lkind != rkind)) || (lkind > rkind)) {
                return ASR::down_cast<ASR::expr_t>(ASRUtils::make_Cast_t_value(
                    al, right->base.loc, right, ASR::cast_kindType::IntegerToInteger, left_type));
            }
        }
        return right;
    }
...
pass_instantiate_generic_function(Allocator& al,
                                  ...
                                  ASR::Function_t& func)
{
    FunctionInstantiator tf(al, subs, current_scope, new_function_name);
