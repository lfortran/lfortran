 ...
 public:
  ...
  void visit_DoLoop(const ASR::DoLoop_t& x) {
    ...
    if (x_head.m_increment) {
      ...
      x_inc = ASRUtils::EXPR(ASR::make_IntegerConstant_t(
          al, x_head.m_end->base.loc, 1, int32_type));
    }
    ...
    int64_t new_end = _start + (groups - 1) * _inc * unroll_factor_ +
                      (unroll_factor_ - 1) * _inc;
    int64_t remaining_part = loop_size % unroll_factor_;
    ASR::ttype_t* int32_type = LFortran::ASRUtils::TYPE(
        ASR::make_Integer_t(al, x.base.base.loc, 4, nullptr, 0));
    xx.m_head.m_end = ASRUtils::EXPR(
        ASR::make_IntegerConstant_t(al, x_end->base.loc, new_end, int32_type));

    Vec<ASR::stmt_t*> init_and_whileloop = PassUtils::replace_doloop(al, x);
    ASR::stmt_t* whileloop_stmt = init_and_whileloop[1];
    ASR::WhileLoop_t* whileloop =
        ASR::down_cast<ASR::WhileLoop_t>(whileloop_stmt);
    ...
    for (int64_t j = 1; j < unroll_factor_; j++) {
      for (size_t i = 0; i < whileloop->n_body; i++) {
        ...
        ASR::stmt_t* m_body_copy =
            node_duplicator.duplicate_stmt(whileloop->m_body[i]);
    ...
    ASR::stmt_t* unrolled_whileloop = ASRUtils::STMT(
        ASR::make_WhileLoop_t(al, x.base.base.loc, whileloop->m_test,
                              unrolled_loop.p, unrolled_loop.size()));
