        ...
        case ASR::exprType::IntrinsicArrayFunction: {
            ...
            switch (intrinsic_array_function->m_arr_intrinsic_id) {
                ...
                case static_cast<int64_t>(ASRUtils::IntrinsicArrayFunctions::MinLoc): {
                    if ((intrinsic_array_function->m_arr_intrinsic_id == static_cast<int64_t>(ASRUtils::IntrinsicArrayFunctions::MaxLoc) ||
                         intrinsic_array_function->m_arr_intrinsic_id == static_cast<int64_t>(ASRUtils::IntrinsicArrayFunctions::MinLoc)) &&
                        intrinsic_array_function->m_args[1] == nullptr) {
                        // MaxLoc/MinLoc without dim return a 1D array of size rank(array)
                        allocate_dims.reserve(al, 1);
                        ASR::dimension_t allocate_dim;
                        allocate_dim.loc = loc;
                        allocate_dim.m_start = int32_one;
                        int rank = ASRUtils::extract_n_dims_from_ttype(ASRUtils::expr_type(intrinsic_array_function->m_args[0]));
                        allocate_dim.m_length = ASRUtils::EXPR(ASR::make_IntegerConstant_t(al, loc, rank, ASRUtils::expr_type(int32_one)));
                        allocate_dims.push_back(al, allocate_dim);
                    } else {
