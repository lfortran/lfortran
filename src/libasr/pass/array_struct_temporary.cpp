            ...
            if (ASR::is_a<ASR::StructInstanceMember_t>(
                ...
                if (ASRUtils::is_array(
                    ...
                    for (size_t i = 0; i < rank; i++) {
                        ...
                        allocate_dim.loc = loc;
                        allocate_dim.m_start = int32_one;
                        ASR::expr_t* dim = ASRUtils::EXPR(
                            ASR::make_IntegerConstant_t(
                                al, loc, i + 1,
                                ASRUtils::expr_type(int32_one)));
                        allocate_dim.m_length = ASRUtils::EXPR(
                            ASR::make_ArraySize_t(
                                al, loc, member->m_v, dim,
                                ASRUtils::expr_type(int32_one), nullptr));
                        allocate_dims.push_back(al, allocate_dim);
                    }
                    break;
    ...
    switch( value->type ) {
        ...
        case ASR::exprType::ArrayItem: {
            return !ASRUtils::is_array(ASRUtils::expr_type(value)) &&
                   ...
                   !ASRUtils::is_pointer(ASRUtils::expr_type(value));
        }
        case ASR::exprType::StructInstanceMember: {
        ...
        case ASR::exprType::ComplexRe: {
            return is_directly_addressable_expr(
...
class ArgSimplifier: public ASR::CallReplacerOnExpressionsVisitor<ArgSimplifier>
{
    ...
    bool is_component_array_section(ASR::expr_t* expr) {
        ...
        ASR::StructInstanceMember_t* member =
            ASR::down_cast<ASR::StructInstanceMember_t>(value);
        return ASRUtils::is_array(ASRUtils::expr_type(member->m_v));
    }


    ASR::expr_t* call_create_and_allocate_temporary_variable(
        ASR::expr_t*& expr, Allocator &al,
        Vec<ASR::stmt_t*>*& current_body, const std::string& name_hint,
        SymbolTable* current_scope, ExprsWithTargetType& exprs_with_target,
        bool materialize_component_section=false) {
        ...
        for( size_t i = 0; i < x_n_args; i++ ) {
            ...
            if( is_temporary_needed(x_m_args[i]) ) {
                ...
                x_m_args_vec.push_back(al, array_var_temporary);
                       ...
                       !is_directly_addressable_expr(x_m_args[i]) ) {
                ASR::expr_t* struct_var_temporary = create_and_allocate_temporary_variable_for_struct(
    ...
    void traverse_call_args(Vec<ASR::call_arg_t>& x_m_args_vec, ASR::call_arg_t* x_m_args,
        ...
        /* For other frontends, we might need to traverse the arguments
           in reverse order. */
        for( size_t i = 0; i < x_n_args; i++ ) {
            bool is_writable_dummy = false;
            if (orig_args) {
                ASR::Variable_t* orig_variable =
                    ASRUtils::expr_to_variable_or_null(orig_args[i]);
                is_writable_dummy = orig_variable &&
            ...
            if( is_temporary_needed(x_m_args[i].m_value) ) {
                ...
                ASR::expr_t* array_var_temporary =
                    call_create_and_allocate_temporary_variable(
                        x_m_args[i].m_value, al, current_body, name_hint,
                        current_scope, exprs_with_target, !is_writable_dummy);
    ...
    void visit_ComplexRe(const ASR::ComplexRe_t& x) {
        ...
        if (!is_directly_addressable_expr(xx.m_arg)) {
...
class ReplaceExprWithTemporary: public ASR::BaseExprReplacer<ReplaceExprWithTemporary> {
    ...
    void replace_ArraySection(ASR::ArraySection_t* x) {
        ...
        if( is_simd_expression ) {
            ...
            current_body->push_back(al, ASRUtils::STMT(ASRUtils::make_Assignment_t_util(
                    al, loc, array_expr_ptr, *current_expr, nullptr, false, false)));
            *current_expr = array_expr_ptr;
            return ;
        }
    ...
    void replace_ComplexRe(ASR::ComplexRe_t* x) {
        if (is_directly_addressable_expr(x->m_arg)) {
            return;
        }
...
class ReplaceExprWithTemporaryVisitor:
    ...
    void transform_stmts(ASR::stmt_t **&m_body, size_t &n_body) {
        ...
        for (size_t i = 0; i < n_body; i++) {
            ...
            visit_stmt(*m_body[i]);
            if (forall_replacement) {
                current_body->push_back(al, forall_replacement);
                forall_replacement = nullptr;
            } else {
                current_body->push_back(al, m_body[i]);
            }
    ...
    void visit_ForAllSingle(const ASR::ForAllSingle_t &x) {
        // Convert ForAllSingle to DoConcurrentLoop so that any temporary
        // variables (e.g., Associate for array sections) generated by this
        // pass are placed inside the loop body, not hoisted outside where
        // loop variables are uninitialized.
        ...
        while (ASR::is_a<ASR::ForAllSingle_t>(*inner)) {
            ASR::ForAllSingle_t &nested =
                *ASR::down_cast<ASR::ForAllSingle_t>(inner);
        ...
        for (size_t i = 0; i < dc.n_head; i++) {
            visit_do_loop_head(dc.m_head[i]);
        }
        transform_stmts(dc.m_body, dc.n_body);
        forall_replacement = do_concurrent;
    }
