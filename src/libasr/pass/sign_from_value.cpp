...
*/
class SignFromValueVisitor
    : public PassUtils::SkipOptimizationFunctionVisitor<SignFromValueVisitor> {
 private:
 ...
 public:
  ...
  ASR::expr_t* is_extract_sign(ASR::expr_t* expr) {
    ...
    ASR::expr_t *arg0 = func_call->m_args[0].m_value,
                *arg1 = func_call->m_args[1].m_value;
  ...
  void handle_BinOp(const T& x_const) {
    ...
    sign_from_value_var = PassUtils::get_sign_from_value(
        first_arg, second_arg, al, unit, rl_path, current_scope,
        x.base.base.loc, [&](const std::string& msg, const Location&) {
