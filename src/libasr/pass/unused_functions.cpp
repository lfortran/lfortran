...
class CollectUnusedFunctionsVisitor
 ...
 public:
  ...
  void visit_FunctionCall(const ASR::FunctionCall_t &x) {
    ...
    if (x.m_value) visit_expr(*x.m_value);
    if (x.m_dt) visit_expr(*x.m_dt);
  }
...
std::map<uint64_t, std::string> collect_unused_functions(
...
class ProgramVisitor : public ASR::BaseWalkVisitor<ProgramVisitor> {
 public:
  bool program_present = false;

  void visit_Program(const ASR::Program_t & /*x*/) { program_present = true; }
};
...
class UnusedFunctionsVisitor
    : public ASR::BaseWalkVisitor<UnusedFunctionsVisitor> {
 private:
  Allocator &al;
 ...
 public:
  ...
  void remove_unused_fn(SymbolTable *symtab) {
    ...
    for (auto it = symtab->get_scope().begin(); it != symtab->get_scope().end();
         ++it) {
  ...
  void visit_Program(const ASR::Program_t &x) { remove_unused_fn(x.m_symtab); }
  void visit_Module(const ASR::Module_t &x) { remove_unused_fn(x.m_symtab); }
  void visit_Function(const ASR::Function_t &x) {
  ...
  void visit_GenericProcedure(const ASR::GenericProcedure_t &x) {
    ...
    for (size_t i = 0; i < x.n_procs; i++) {
      ...
      if (fn_unused.find(h) != fn_unused.end()) {
        // std::cout << "GP: erase proc #" << i << " / " << x.n_procs <<
        // std::endl;
      } else {
