...
namespace GetEnvironmentVariable {
    ...
    static inline void verify_args(const ASR::IntrinsicImpureSubroutine_t& x, diag::Diagnostics& diagnostics) {
        if (x.n_args == 5) {
            ASRUtils::require_impl(x.m_overload_id == 0, "Overload Id for get_environment_variable expected to be 0, found " + std::to_string(x.m_overload_id), x.base.base.loc, diagnostics);
            ASRUtils::require_impl(ASRUtils::is_character(*ASRUtils::expr_type(x.m_args[0])), "First argument must be of character type", x.base.base.loc, diagnostics);
            ...
            ASRUtils::require_impl(ASRUtils::is_logical(*ASRUtils::expr_type(x.m_args[4])), "Fifth argument must be of logical type", x.base.base.loc, diagnostics);
        } else {
            ASRUtils::require_impl(false, "Unexpected number of args, get_environment_variable takes 5 arguments, found " + std::to_string(x.n_args), x.base.base.loc, diagnostics);
        }
    }
    ...
    static inline ASR::stmt_t* instantiate_GetEnvironmentVariable(Allocator &al, const Location &loc,
        ...
        std::string c_func_name_1 = "_lfortran_get_environment_variable_name";
        std::string c_func_name_2 = "_lfortran_get_environment_variable_value";
        std::string c_func_name_3 = "_lfortran_get_environment_variable_length";
        std::string c_func_name_4 = "_lfortran_get_environment_variable_status";
        std::string c_func_name_5 = "_lfortran_get_environment_variable_trimname";
        ...
        if(overload_id == 5){
            ...
            fill_func_arg_sub("length", arg_types[2], InOut);
            fill_func_arg_sub("status", arg_types[3], InOut);
            fill_func_arg_sub("trimname", arg_types[4], InOut);
        }
        ...
        ASR::symbol_t *s_5 = b.create_c_func_subroutines(c_func_name_5, fn_symtab, 0, arg_types[4]);

        fn_symtab->add_symbol(c_func_name_1, s_1);
        fn_symtab->add_symbol(c_func_name_2, s_2);
        fn_symtab->add_symbol(c_func_name_3, s_3);
        fn_symtab->add_symbol(c_func_name_4, s_4);
        fn_symtab->add_symbol(c_func_name_5, s_5);

        dep.push_back(al, s2c(al, c_func_name_1));
        dep.push_back(al, s2c(al, c_func_name_2));
        dep.push_back(al, s2c(al, c_func_name_3));
        dep.push_back(al, s2c(al, c_func_name_4));
        dep.push_back(al, s2c(al, c_func_name_5));
        ...
        body.push_back(al, b.Assignment(args[4], b.Call(s_5, call_args, arg_types[4])));
