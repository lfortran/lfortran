...
class DoConcurrentStatementVisitor : public ASR::CallReplacerOnExpressionsVisitor<DoConcurrentStatementVisitor> {
    ...
    void visit_Call(const T &x) {
        ...
        ASR::asr_t* mo = ASR::make_Module_t(
            al, x.base.base.loc, module_scope, s2c(al, module_name), nullptr, 0, false, false);
        ...
        ASR::symbol_t* fun_sym_for_module = duplicator.duplicate_Function(
            ASR::down_cast<ASR::Function_t>(x_copy->m_name), module_scope);
        module->m_symtab->add_symbol(ASRUtils::symbol_name(x_copy->m_name), fun_sym_for_module);
        ...
        if (ASR::is_a<ASR::Program_t>(*ASR::down_cast<ASR::symbol_t>(asr_owner))) {
            ...
            program->m_symtab->add_or_overwrite_symbol(ASRUtils::symbol_name(x_copy->m_name),
            ...
            if (ASR::is_a<ASR::Program_t>(*ASR::down_cast<ASR::symbol_t>(asr_owner))) {
                ASR::asr_t* ext_fn = ASR::make_ExternalSymbol_t(
                                        ...
                                        x_copy->m_original_name
                                            ...
                                            : ASRUtils::symbol_name(x_copy->m_name),
                                        ASR::accessType::Public);
                current_scope->add_or_overwrite_symbol(ASRUtils::symbol_name(x_copy->m_name),
                                                       ASR::down_cast<ASR::symbol_t>(ext_fn));
                func_sym = current_scope->get_symbol(ASRUtils::symbol_name(x_copy->m_name));
            } else if (ASR::is_a<ASR::Module_t>(*ASR::down_cast<ASR::symbol_t>(asr_owner))) {
                func_sym = current_scope->resolve_symbol(ASRUtils::symbol_name(x.m_name));
            }
        }
