...
public:
    ...
    ASR::expr_t* get_first_scalar_expr(ASR::expr_t* expr)
    {
        ...
        public:
            ...
            ReplaceLoopVarWithStart(Allocator& al_,
                                    ASR::symbol_t* loop_var_sym_,
                                    ASR::expr_t* start_value_)
                : al(al_)
                , loop_var_sym(loop_var_sym_)
    ...
    {
        // Derive a conservative element length without evaluating indices (e.g., trim(lines(iii))
        // uses declared length).
        if (!expr)
            return nullptr;
        if (ASR::is_a<ASR::IntrinsicElementalFunction_t>(*expr)) {
            ...
            } else if (f->m_intrinsic_id
                           == static_cast<int64_t>(
                               ASRUtils::IntrinsicElementalFunctions::StringConcat)
                       && f->n_args > 1) {
                ...
                for (size_t i = 0; i < f->n_args; i++) {
                    ...
                    if (total_len == nullptr) {
                        ...
                        ASR::ttype_t* int_type
                            = ASRUtils::TYPE(ASR::make_Integer_t(al, expr->base.loc, 4));
                        total_len = ASRUtils::EXPR(ASR::make_IntegerBinOp_t(al,
                                                                            expr->base.loc,
                                                                            total_len,
                                                                            ASR::binopType::Add,
                                                                            arg_len,
        ...
        if (ASR::is_a<ASR::String_t>(*t)) {
            ...
            if (st->m_len && ASRUtils::is_value_constant(st->m_len)) {
                ...
                return ASRUtils::EXPR(ASR::make_StringLen_t(
                    al,
                    expr->base.loc,
                    expr,
                    ASRUtils::TYPE(ASR::make_Integer_t(al, expr->base.loc, 4)),
    ...
    ASR::expr_t* get_ImpliedDoLoop_size(ASR::ImpliedDoLoop_t* implied_doloop)
    {
        ...
        if (d == nullptr) {
            ...
            implied_doloop_size = builder.Add(
                builder.Div(builder.Sub(end, start), d),
            ...
            if (ASR::is_a<ASR::ImpliedDoLoop_t>(*implied_doloop->m_values[i])) {
                if (implied_doloop_size_ == nullptr) {
                    ...
                    implied_doloop_size_
                        = builder.Add(get_ImpliedDoLoop_size(ASR::down_cast<ASR::ImpliedDoLoop_t>(
        ...
        bool inner_depends_on_var
            = (loop_var_sym != nullptr && implied_doloop_size_ != nullptr && d == nullptr
               && ASRUtils::expr_references_symbol(implied_doloop_size_, loop_var_sym));

        if (inner_depends_on_var) {
            ASR::expr_t* two
                = make_ConstantWithKind(make_IntegerConstant_t, make_Integer_t, 2, kind, loc);
            ASR::expr_t* arith_sum
                = builder.Div(builder.Mul(implied_doloop_size, builder.Add(start, end)), two);
            if (const_elements > 0) {
                ASR::expr_t* const_total = builder.Mul(
                    make_ConstantWithKind(
                        make_IntegerConstant_t, make_Integer_t, const_elements, kind, loc),
                    implied_doloop_size);
            ...
            if (const_elements > 0) {
                if (implied_doloop_size_ == nullptr) {
                    implied_doloop_size_ = make_ConstantWithKind(
                        make_IntegerConstant_t, make_Integer_t, const_elements, kind, loc);
                } else {
    ...
    {
        ASR::ttype_t* int_type
            = ASRUtils::TYPE(ASR::make_Integer_t(al, x->base.base.loc, get_index_kind()));
            ...
            if (ASR::is_a<ASR::ArrayConstant_t>(*element)) {
                if (ASRUtils::is_value_constant(element)) {
                    ...
                    ASR::expr_t* element_array_size
                        = get_ArrayConstant_size(ASR::down_cast<ASR::ArrayConstant_t>(element));
                ...
                if (ASR::is_a<ASR::Array_t>(*array_item->m_type)) {
                    if (ASRUtils::is_fixed_size_array(array_item->m_type)) {
                        ...
                        constant_size += ASRUtils::get_fixed_size_of_array(array_type->m_dims,
                                                                           array_type->n_dims);
                    } else {
                ...
                ASR::ttype_t* element_type
                    = ASRUtils::type_get_past_allocatable(ASRUtils::expr_type(element));
                if (ASRUtils::is_array(element_type)) {
                    if (ASRUtils::is_fixed_size_array(element_type)) {
                        ASR::dimension_t* m_dims = nullptr;
                        size_t n_dims
                            = ASRUtils::extract_dimensions_from_ttype(element_type, m_dims);
                ...
                ASR::expr_t* implied_doloop_size
                    = get_ImpliedDoLoop_size(ASR::down_cast<ASR::ImpliedDoLoop_t>(element));
                if (array_size) {
                ...
                ASR::IntrinsicElementalFunction_t* intrinsic_element_t
                    = ASR::down_cast<ASR::IntrinsicElementalFunction_t>(element);
                ...
                if (ASRUtils::is_fixed_size_array(fc_element_t->m_type)) {
                    ...
                    if (array_size == nullptr) {
                       ...
                       && ASRUtils::is_array(ASRUtils::expr_type(element))) {
                ASR::IntrinsicArrayFunction_t* intrinsic_element_t
                    = ASR::down_cast<ASR::IntrinsicArrayFunction_t>(element);
                if (ASRUtils::is_fixed_size_array(intrinsic_element_t->m_type)) {
                    constant_size += ASRUtils::get_fixed_size_of_array(intrinsic_element_t->m_type);
                } else {
                    ASR::expr_t* element_array_size = ASRUtils::get_size(element, al, false);
                    if (array_size == nullptr) {
                        ...
                        || ASR::is_a<ASR::IntegerBinOp_t>(*element)
                        || ASR::is_a<ASR::RealUnaryMinus_t>(*element)
                        || ASR::is_a<ASR::IntegerUnaryMinus_t>(*element))
                       && ASRUtils::is_array(ASRUtils::expr_type(element))) {
        ...
        if (constant_size > 0) {
            constant_size_asr = make_ConstantWithType(
                make_IntegerConstant_t, constant_size, int_type, x->base.base.loc);
            if (array_size == nullptr) {
        ...
        is_allocatable = true;
        if (array_size == nullptr) {
            array_size = make_ConstantWithKind(
                make_IntegerConstant_t, make_Integer_t, 0, get_index_kind(), x->base.base.loc);
        }
        return array_size;
    }
    ...
    void replace_ArrayConstructor(ASR::ArrayConstructor_t* x)
    {
        ...
        if (result_var != nullptr && resultvar2value.find(result_var) != resultvar2value.end()
            && resultvar2value[result_var] == &(x->base)) {
            is_result_var_fixed_size
                = ASRUtils::is_fixed_size_array(ASRUtils::expr_type(result_var));
        }
        ...
        if (has_array_valued_idoloop) {
            ...
            ASR::expr_t* total_size_var = PassUtils::create_var(
                idl_size_counter++, "_idl_total_size_", loc, int_type, al, current_scope);
                ...
                if (ASR::is_a<ASR::ImpliedDoLoop_t>(*x->m_args[k])) {
                    ...
                    for (size_t v = 0; v < idl->n_values; v++) {
                        ...
                        ASR::expr_t* increment
                            = ASRUtils::EXPR(ASR::make_IntegerBinOp_t(al,
                                                                      loc,
                                                                      total_size_var,
                                                                      ASR::binopType::Add,
                                                                      add_val,
                    ...
                    ASR::stmt_t* size_loop = ASRUtils::STMT(ASR::make_DoLoop_t(
                        al, loc, nullptr, head, loop_body.p, loop_body.size(), nullptr, 0));
        ...
        // For runtime-dependent string lengths, defer evaluation and use allocatable results with
        // per-element runtime allocation.
        ...
        dim.m_start = ASRUtils::EXPR(ASR::make_IntegerConstant_t(
            ...
            ASRUtils::type_get_past_pointer(
                ASRUtils::type_get_past_allocatable(ASRUtils::expr_type(array_constructor)))));
        ...
        if (is_result_var_fixed_size) {
            ...
            if (is_allocatable) {
                result_type_ = ASRUtils::TYPE(ASR::make_Allocatable_t(
                    al,
                    x->m_type->base.loc,
                ...
                if (element_type && ASRUtils::is_character(*element_type)) {
                    ASR::down_cast<ASR::String_t>(ASRUtils::extract_type(result_type_))->m_len
                        = nullptr;
                    ASR::down_cast<ASR::String_t>(ASRUtils::extract_type(result_type_))->m_len_kind
                        = ASR::string_length_kindType::DeferredLength;
                }
            } else {
                result_type_ = ASRUtils::duplicate_type(
                    al, ASRUtils::type_get_past_allocatable_pointer(x->m_type), &dims);
        ...
        *current_expr = result_var;
        ...
        if (element_type && ASRUtils::is_character(*element_type)) {
            // Default to declared length if available; override with a derived source length when
            // safe.
            ...
            if (!declared_len_expr && x->n_args > 0) {
                ...
                if (derived_len)
                    elem_len_expr_for_alloc = derived_len;
            }
        }
        ...
        if (is_allocatable && element_type && ASRUtils::is_character(*element_type)
            ...
            is_allocatable = false;
            result_type_ = ASRUtils::duplicate_type(
                al, ASRUtils::type_get_past_allocatable(x->m_type), &dims);
        }
        arg.m_type = nullptr;
        ...
        for (size_t i = 0; i < x->n_args; i++) {
            if (ASR::is_a<ASR::ArrayItem_t>(*x->m_args[i])
                || ASR::is_a<ASR::ArraySection_t>(*x->m_args[i])) {
        ...
        PassUtils::ReplacerUtils::replace_ArrayConstructor_(
            al, x, result_var, result_vec, current_scope);
    ...
    void replace_ArrayConstant(ASR::ArrayConstant_t* x)
    {
        ...
        dim.m_start = ASRUtils::EXPR(ASR::make_IntegerConstant_t(
            ...
            1,
            ASRUtils::type_get_past_pointer(
                ASRUtils::type_get_past_allocatable(ASRUtils::expr_type(array_constant_size)))));
        ...
        if (is_result_var_fixed_size) {
            ...
            if (is_allocatable) {
                result_type_ = ASRUtils::TYPE(ASRUtils::make_Allocatable_t_util(
                    al,
                    x->m_type->base.loc,
        ...
        result_var = PassUtils::create_var(
            result_counter, "_array_constant_", loc, result_type_, al, current_scope);
        result_counter += 1;
        *current_expr = result_var;
        ...
        LCOMPILERS_ASSERT(result_var != nullptr);
        Vec<ASR::stmt_t*>* result_vec = &pass_result;
        PassUtils::ReplacerUtils::replace_ArrayConstant(
            x, this, remove_original_statement, result_vec);
        result_var = result_var_copy;
    }
    ...
    void replace_ArrayPhysicalCast(ASR::ArrayPhysicalCast_t* x)
    {
        ...
        if ((is_arr_construct_arg
             && ASRUtils::is_fixed_size_array(ASRUtils::expr_type(x->m_arg)))) {
            *current_expr = x->m_arg;
            return;
        }

        if ((x->m_old == x->m_new && x->m_old != ASR::array_physical_typeType::DescriptorArray)
...
public:
    ...
    ArrayConstantVisitor(Allocator& al_, const LCompilers::PassOptions& pass_options_)
        ...
        , replacer(al_,
                   ...
                   resultvar2value,
                   pass_options_.realloc_lhs_arrays,
                   allocate_target,
                   pass_options_)
        , parent_body(nullptr)
        , pass_options(pass_options_)
    ...
    void visit_Assignment(const ASR::Assignment_t& x)
    {
        ...
        if (ASRUtils::is_allocatable(x.m_target) && ASR::is_a<ASR::ArrayConstant_t>(*x.m_value)) {
    ...
    ASR::asr_t* create_array_constant(const T& x, ASR::expr_t* value)
    {
        ...
        ASR::ttype_t* index_type
            = ASRUtils::TYPE(ASR::make_Integer_t(al, x.base.base.loc, get_index_kind()));
        ASR::expr_t* one
            = ASRUtils::EXPR(ASR::make_IntegerConstant_t(al, x.base.base.loc, 1, index_type));
        ...
        ASR::asr_t* array_constant = ASRUtils::make_ArrayConstructor_t_util(
            al, value->base.loc, args.p, args.n, array_type, ASR::arraystorageType::ColMajor);
    ...
    bool expand_implied_do_loop_flat(ASR::ImpliedDoLoop_t* idl, Vec<ASR::expr_t*>& result)
    {
        ...
        for (int64_t i = start_val; (step_val > 0) ? (i <= end_val) : (i >= end_val);
             i += step_val) {
            ASR::expr_t* loop_var_val = ASRUtils::EXPR(ASR::make_IntegerConstant_t(
                al, idl->base.base.loc, i, ASRUtils::expr_type(idl->m_var)));
    ...
    ASR::stmt_t* create_do_loop_form_idl(ASR::ImpliedDoLoop_t* x, ASR::expr_t* format_string)
    {
        ...
        do_loop_head.loc = x->base.base.loc;
        do_loop_head.m_v = x->m_var;
        do_loop_head.m_start = x->m_start;
        do_loop_head.m_end = x->m_end;
        ...
        for (size_t i = 0; i < x->n_values; i++) {
            if (ASR::is_a<ASR::ImpliedDoLoop_t>(*x->m_values[i])) {
                do_loop_body.push_back(
                    al,
                    create_do_loop_form_idl(ASR::down_cast<ASR::ImpliedDoLoop_t>(x->m_values[i]),
                                            format_string));
            } else {
        ...
        if (args.size() > 0) {
            ASR::expr_t* fmt_val = ASRUtils::EXPR(ASR::make_StringFormat_t(
                al,
                x->base.base.loc,
                format_string,
                args.p,
            ...
            if (print) {
                stmt = ASRUtils::STMT(ASRUtils::make_print_t_util(
                    al, x->base.base.loc, print_values.p, print_values.size()));
            } else {
                // this will be file_write
                LCOMPILERS_ASSERT(file_write);
                stmt = ASRUtils::STMT(ASR::make_FileWrite_t(al,
                                                            x->base.base.loc,
                                                            0,
                                                            m_unit,
                                                            nullptr,
        ...
        do_loop = ASRUtils::STMT(ASR::make_DoLoop_t(al,
                                                    x->base.base.loc,
                                                    nullptr,
                                                    do_loop_head,
                                                    do_loop_body.p,
        ...
        */
        ...
        for (size_t i = 0; i < x.n_args; i++) {
            ...
            if (ASR::is_a<ASR::ImpliedDoLoop_t>(*value)) {
                ...
                // (to preserve variable-length trimmed strings instead of storing in fixed-length
                // array)
...
void
pass_replace_implied_do_loops(Allocator& al,
