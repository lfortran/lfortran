...
*/
class FMAVisitor
    : public PassUtils::SkipOptimizationFunctionVisitor<FMAVisitor> {
 private:
 ...
 public:
  ...
  void visit_RealBinOp(const ASR::RealBinOp_t& x_const) {
    ...
    if (is_other_expr_negative) {
      other_expr = ASRUtils::EXPR(
          ASR::make_RealUnaryMinus_t(al, other_expr->base.loc, other_expr,
                                     ASRUtils::expr_type(other_expr), nullptr));
    }

    ASR::RealBinOp_t* mul_binop = ASR::down_cast<ASR::RealBinOp_t>(mul_expr);
    ASR::expr_t *first_arg = mul_binop->m_left,
                *second_arg = mul_binop->m_right;

    if (is_mul_expr_negative) {
      first_arg = ASRUtils::EXPR(
          ASR::make_RealUnaryMinus_t(al, first_arg->base.loc, first_arg,
                                     ASRUtils::expr_type(first_arg), nullptr));
    }

    fma_var = PassUtils::get_fma(other_expr, first_arg, second_arg, al, unit,
                                 rl_path, current_scope, x.base.base.loc,
                                 [&](const std::string& msg, const Location&) {
  ...
  void visit_IntegerBitNot(const ASR::IntegerBitNot_t& x) { visit_UnaryOp(x); }
  void visit_LogicalNot(const ASR::LogicalNot_t& x) { visit_UnaryOp(x); }
