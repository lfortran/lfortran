...
public:
    NestedVarVisitor(llvm::LLVMContext& context,
                     std::vector<uint64_t>& needed_globals,
                     std::vector<uint64_t>& nested_call_out,
                     std::map<uint64_t, std::vector<uint64_t>>& nesting_map)
        : context{ context }
        , needed_globals{ needed_globals }
        , nested_call_out{ nested_call_out }
    ...
    void visit_procedure(const T& x)
    {
        ...
        if (nesting_depth == 1) {
            ...
            if (calls_out && x.m_deftype == LFortran::ASR::Implementation) {
                if (std::find(calls_to.begin(), calls_to.end(), cur_func_hash) == calls_to.end()
                    && calls_to.size() >= 1) {
                ...
                } else if (std::find(calls_to.begin(), calls_to.end(), cur_func_hash)
                               != calls_to.end()
                           && calls_to.size() >= 2) {
    ...
    void visit_FunctionCall(const ASR::FunctionCall_t& x)
    {
        if (nesting_depth == 1) {
            ...
            if (std::find(calls_to.begin(), calls_to.end(), call_hash) == calls_to.end()) {
    ...
    void visit_Var(const ASR::Var_t& x)
    {
        ...
        if (nesting_depth > 1) {
            ...
            // nested procedure.
            if (current_scope->get_symbol(v->m_name) == nullptr) {
                uint32_t h = get_hash((ASR::asr_t*) v);
                llvm::Type* rel_type;
                auto finder = std::find(needed_globals.begin(), needed_globals.end(), h);
                ...
                if (finder == needed_globals.end()) {
                    ...
                    switch (v->m_type->type) {
                        ...
                        case ASR::ttypeType::Logical: {
                            int a_kind = down_cast<ASR::Logical_t>(v->m_type)->m_kind;
                            rel_type = getLogicalType(a_kind)->getPointerTo();
                            break;
                        }
                        default: {
                            throw LCompilersException(
                                "Variable type not supported in nested functions");
                            break;
                        }
                    }
...
std::map<uint64_t, std::vector<llvm::Type*>>
pass_find_nested_vars(ASR::TranslationUnit_t& unit,
                      llvm::LLVMContext& context,
                      std::vector<uint64_t>& needed_globals,
                      std::vector<uint64_t>& nested_call_out,
                      std::map<uint64_t, std::vector<uint64_t>>& nesting_map)
