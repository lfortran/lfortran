...
inline ASR::expr_t*
gen_test_expr_CaseStmt(Allocator& al,
                       const Location& loc,
                       ...
                       ASR::expr_t* a_test)
{
    ...
    if (Case_Stmt->n_test == 1) {
        ...
        for (std::uint32_t j = 2; j < Case_Stmt->n_test; j++) {
            ...
            test_expr
                = PassUtils::create_binop_helper(al, loc, test_expr, newExpr, ASR::binopType::Add);
        }
    }
    return test_expr;
}
...
inline ASR::expr_t*
gen_test_expr_CaseStmt_Range(Allocator& al,
                             const Location& loc,
...
void
case_to_if(Allocator& al, const ASR::Select_t& x, ASR::expr_t* a_test, Vec<ASR::stmt_t*>& body)
{
    ...
    switch (case_body->type) {
        case ASR::case_stmtType::CaseStmt: {
            ...
            last_if_else = ASRUtils::STMT(ASR::make_If_t(al,
                                                         x.base.base.loc,
                                                         test_expr,
                                                         Case_Stmt->m_body,
                                                         Case_Stmt->n_body,
    ...
    for (idx = (int) x.n_body - 2; idx >= 0; idx--) {
        ...
        last_if_else = ASRUtils::STMT(ASR::make_If_t(
            al, x.base.base.loc, test_expr, m_body, n_body, if_body_vec.p, if_body_vec.size()));
    }
    body.reserve(al, 1);
...
Vec<ASR::stmt_t*>
replace_selectcase(Allocator& al, const ASR::Select_t& select_case)
{
...
{
public:
    SelectCaseVisitor(Allocator& al)
