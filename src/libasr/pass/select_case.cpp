...
void case_to_if(Allocator& al, const ASR::Select_t& x, ASR::expr_t* a_test, Vec<ASR::stmt_t*>& body) {
    ...
    switch(case_body->type) {
        case ASR::case_stmtType::CaseStmt: {
            ...
            last_if_else = ASRUtils::STMT(ASR::make_If_t(al, x.base.base.loc, nullptr, test_expr, Case_Stmt->m_body, Case_Stmt->n_body, x.m_default, x.n_default));
            break;
        }
    ...
    for( idx = (int) x.n_body - 2; idx >= 0; idx-- ) {
        ...
        last_if_else = ASRUtils::STMT(ASR::make_If_t(al, x.base.base.loc, nullptr, test_expr, m_body, n_body, if_body_vec.p, if_body_vec.size()));
    }
    body.reserve(al, 1);
    ...
    for( size_t idx = 0; idx < x.n_body; idx++ ) {
        ...
        switch(case_body->type) {
            case ASR::case_stmtType::CaseStmt: {
                ...
                body.push_back(al, ASRUtils::STMT(ASR::make_If_t(al, loc, nullptr,
