...
public:
    ...
    void visit_Function(const ASR::Function_t &x) {
        ...
        if (xx.m_return_var && ASR::is_a<ASR::Var_t>(*xx.m_return_var)) {
            ASR::symbol_t* rv_sym = ASR::down_cast<ASR::Var_t>(
                xx.m_return_var)->m_v;
            ASR::symbol_t* rv_sym_deref =
                ASRUtils::symbol_get_past_external(rv_sym);
            bool will_be_converted =
                PassUtils::is_aggregate_or_array_or_nonPrimitive_type(
                    xx.m_return_var) ||
                PassUtils::is_symbolic_list_type(xx.m_return_var);
            if (will_be_converted &&
                ASR::is_a<ASR::Variable_t>(*rv_sym_deref)) {
                ASR::Variable_t* rv_var = ASR::down_cast<ASR::Variable_t>(
                    rv_sym_deref);
                bool rv_is_array_of_struct =
                    !ASRUtils::is_allocatable(rv_var->m_type) &&
                    ASRUtils::is_array(rv_var->m_type) &&
                    ASR::is_a<ASR::StructType_t>(
                        *ASRUtils::type_get_past_array(rv_var->m_type));
                Location loc = rv_var->base.base.loc;
                ASR::ttype_t* logical_type = ASRUtils::TYPE(
                    ASR::make_Logical_t(al, loc, 4));

                if (ASRUtils::is_allocatable(rv_var->m_type)) {
                    ASR::expr_t* var_expr = ASRUtils::EXPR(
                        ASR::make_Var_t(al, loc, rv_sym));
                    Vec<ASR::expr_t*> allocated_args;
                    allocated_args.reserve(al, 1);
                    allocated_args.push_back(al, var_expr);
                    ...
                    if_body.push_back(al, dealloc_stmt);
                    ASR::stmt_t* if_stmt = ASRUtils::STMT(ASR::make_If_t(
                        al, loc, nullptr, is_allocated, if_body.p,
                        if_body.n, nullptr, 0));
                    dealloc_stmts.push_back(al, if_stmt);
                } else if (ASR::is_a<ASR::StructType_t>(*rv_var->m_type) &&
                        rv_var->m_type_declaration) {
                    ASR::Struct_t* struct_type = ASR::down_cast<ASR::Struct_t>(
                    ...
                    if (struct_hierarchy_has_finalizer(struct_type)) {
                        ...
                        emit_struct_default_init_stmts(var_expr, struct_type,
                            xx.m_symtab, loc, init_stmts);
                        for (size_t k = 0; k < init_stmts.size(); k++) {
                            dealloc_stmts.push_back(al, init_stmts[k]);
                        }
                    }

                    Vec<ASR::stmt_t*> cleanup;
                    cleanup.reserve(al, 1);
                    emit_struct_cleanup_stmts(var_expr, struct_type,
                        xx.m_symtab, loc, logical_type, cleanup);
                    for (size_t k = 0; k < cleanup.size(); k++) {
                        dealloc_stmts.push_back(al, cleanup[k]);
                    }
                } else if (rv_is_array_of_struct &&
                        rv_var->m_type_declaration) {
                    ASR::Struct_t* struct_type = ASR::down_cast<ASR::Struct_t>(
                        ASRUtils::symbol_get_past_external(
                            rv_var->m_type_declaration));
                    ...
                    Vec<ASR::stmt_t*> cleanup;
                    cleanup.reserve(al, 1);
                    emit_array_of_struct_cleanup_stmts(var_expr_full,
                        struct_type, n_dims, xx.m_symtab, loc,
                        logical_type, cleanup);
                    if (cleanup.size() > 0) {
                        ASR::stmt_t* wrapper_block = nullptr;
                        if (cleanup.size() == 1) {
                            wrapper_block = cleanup[0];
                            ...
                            ASR::expr_t* true_cond = ASRUtils::EXPR(
                                ASR::make_LogicalConstant_t(al, loc, true,
                                    logical_type));
                            wrapper_block = ASRUtils::STMT(ASR::make_If_t(
                                al, loc, nullptr, true_cond, cleanup.p,
                                cleanup.size(), nullptr, 0));
                        }
                        dealloc_stmts.push_back(al, wrapper_block);
                    }
                }
            }
        }
