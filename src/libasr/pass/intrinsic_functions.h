...
namespace Sind {
    ...
    static ASR::expr_t *eval_Sind(Allocator& al, const Location& loc,
            ASR::ttype_t* t1, Vec<ASR::expr_t*>& args, diag::Diagnostics& /*diag*/) {
        double degrees = ASR::down_cast<ASR::RealConstant_t>(args[0])->m_r;
        double PI = 3.14159265358979323846;
    ...
    static inline ASR::expr_t* instantiate_Sind(Allocator &al, const Location &loc,
            SymbolTable *scope, Vec<ASR::ttype_t*>& arg_types, ASR::ttype_t *return_type,
            Vec<ASR::call_arg_t>& new_args, int64_t /*overload_id*/) {
        std::string c_func_name;
        if (ASRUtils::extract_kind_from_ttype_t(arg_types[0]) == 4) {
            c_func_name = "_lfortran_ssind";
        } else {
            c_func_name = "_lfortran_dsind";
        }
        std::string new_name = "_lcompilers_asind_"+ type_to_str_python(arg_types[0]);
