...
public:
    ...
    ASR::symbol_t* insert_new_procedure(ASR::Function_t* x, std::vector<size_t>& indices)
    {
        ...
        for (size_t i = 0; i < x->n_args + 1; i++) {
            ...
            ASR::expr_t* new_arg = ASRUtils::EXPR(ASR::make_Var_t(
                al, arg->base.base.loc, new_symtab->get_symbol(std::string(arg->m_name))));
            if (i < x->n_args) {
        ...
        if (ASR::is_a<ASR::Function_t>(*((ASR::symbol_t*) x))) {
            ...
            ASR::asr_t* new_subrout = ASR::make_Function_t(al,
                                                           x->base.base.loc,
                                                           new_symtab,
                                                           s2c(al, new_name),
                                                           new_args.p,
    ...
    void edit_new_procedure(ASR::Function_t* x, std::vector<size_t>& indices)
    {
        ...
        for (size_t i = 0; i < x->n_args; i++) {
            ...
            if (std::find(indices.begin(), indices.end(), i) != indices.end()) {
                ...
                PassUtils::create_vars(dim_variables,
                                       2 * n_dims,
                                       arg->base.base.loc,
                                       al,
                                       x->m_symtab,
...
public:
    ReplaceSubroutineCallsVisitor(Allocator& al_, PassArrayByDataProcedureVisitor& v_)
        : PassVisitor(al_, nullptr)
        , v(v_)
    {
...
class ReplaceFunctionCallsVisitor
    : public ASR::CallReplacerOnExpressionsVisitor<ReplaceFunctionCallsVisitor>
{
private:
    ReplaceFunctionCalls replacer;

public:
    ReplaceFunctionCallsVisitor(Allocator& al_, PassArrayByDataProcedureVisitor& v_)
        : replacer(al_, v_)
    {
    }
...
void
pass_array_by_data(Allocator& al,
