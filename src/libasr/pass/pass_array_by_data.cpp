 ...
 */
class PassArrayByDataProcedureVisitor
    : public PassUtils::PassVisitor<PassArrayByDataProcedureVisitor> {
 private:
 ...
 public:
  std::map<ASR::symbol_t*, std::pair<ASR::symbol_t*, std::vector<size_t>>>
      proc2newproc;

  PassArrayByDataProcedureVisitor(Allocator& al_)
  ...
  ASR::symbol_t* insert_new_procedure(ASR::Function_t* x,
    ...
    for (auto& item : x->m_symtab->get_scope()) {
      ...
      if (ASR::is_a<ASR::Variable_t>(*item.second)) {
        ...
        new_arg = ASR::down_cast<ASR::symbol_t>(ASR::make_Variable_t(
            al, arg->base.base.loc, new_symtab, s2c(al, item.first),
            arg->m_intent, arg->m_symbolic_value, arg->m_value, arg->m_storage,
            arg->m_type, arg->m_abi, arg->m_access, arg->m_presence,
            arg->m_value_attr));
      } else if (ASR::is_a<ASR::ExternalSymbol_t>(*item.second)) {
        ASR::ExternalSymbol_t* arg =
            ASR::down_cast<ASR::ExternalSymbol_t>(item.second);
        new_arg = ASR::down_cast<ASR::symbol_t>(ASR::make_ExternalSymbol_t(
            al, arg->base.base.loc, new_symtab, s2c(al, item.first),
            arg->m_external, arg->m_module_name, arg->m_scope_names,
            arg->n_scope_names, arg->m_original_name, arg->m_access));
      }
    ...
    for (size_t i = 0; i < x->n_args + 1; i++) {
      ...
      ASR::expr_t* new_arg = ASRUtils::EXPR(
          ASR::make_Var_t(al, arg->base.base.loc,
                          new_symtab->get_symbol(std::string(arg->m_name))));
      if (i < x->n_args) {
    ...
    if (ASR::is_a<ASR::Function_t>(*((ASR::symbol_t*)x))) {
      ...
      ASR::asr_t* new_subrout = ASR::make_Function_t(
          al, x->base.base.loc, new_symtab, s2c(al, new_name), new_args.p,
          new_args.size(), nullptr, 0, new_body.p, new_body.size(), return_var,
          x->m_abi, x->m_access, x->m_deftype, s2c(al, new_bindc_name),
          x->m_elemental, x->m_pure, x->m_module, x->m_inline);
  ...
  void edit_new_procedure(ASR::Function_t* x, std::vector<size_t>& indices) {
    ...
    for (size_t i = 0; i < x->n_args; i++) {
      ...
      if (std::find(indices.begin(), indices.end(), i) != indices.end()) {
        ...
        PassUtils::create_vars(dim_variables, 2 * n_dims, arg->base.base.loc,
                               al, x->m_symtab, arg_name, ASR::intentType::In);
        ...
        ASR::ttype_t* new_type =
            ASRUtils::duplicate_type(al, arg->m_type, &new_dims);
        arg->m_type = new_type;
...
/*
    ...
    to subroutine calls having dimensional information passed as arguments. See
   example below,
 ...
 */
class ReplaceSubroutineCallsVisitor
    : public PassUtils::PassVisitor<ReplaceSubroutineCallsVisitor> {
 private:
  PassArrayByDataProcedureVisitor& v;

 public:
  ReplaceSubroutineCallsVisitor(Allocator& al_,
  ...
  void visit_SubroutineCall(const ASR::SubroutineCall_t& x) {
    ...
    ASR::stmt_t* new_call = ASRUtils::STMT(ASR::make_SubroutineCall_t(
        al, x.base.base.loc, new_subrout_sym, new_subrout_sym, new_args.p,
...
*/
class ReplaceFunctionCalls
    : public ASR::BaseExprReplacer<ReplaceFunctionCalls> {
 private:
  Allocator& al;
  PassArrayByDataProcedureVisitor& v;

 public:
  ReplaceFunctionCalls(Allocator& al_, PassArrayByDataProcedureVisitor& v_)
      : al(al_), v(v_) {}
  ...
  void replace_FunctionCall(ASR::FunctionCall_t* x) {
    ...
    ASR::expr_t* new_call = ASRUtils::EXPR(ASR::make_FunctionCall_t(
        al, x->base.base.loc, new_func_sym, new_func_sym, new_args.p,
        new_args.size(), x->m_type, nullptr, x->m_dt));
...
*/
class ReplaceFunctionCallsVisitor
    : public ASR::CallReplacerOnExpressionsVisitor<
          ReplaceFunctionCallsVisitor> {
 private:
...
Therefore, they cannot generate code for such functions. To avoid backends like
WASM from failing we remove those functions by implementing and calling the
following visitor.
*/
class RemoveArrayByDescriptorProceduresVisitor
    : public PassUtils::PassVisitor<RemoveArrayByDescriptorProceduresVisitor> {
 private:
  PassArrayByDataProcedureVisitor& v;

 public:
  RemoveArrayByDescriptorProceduresVisitor(Allocator& al_,
                                           PassArrayByDataProcedureVisitor& v_)
      : PassVisitor(al_, nullptr), v(v_) {}
