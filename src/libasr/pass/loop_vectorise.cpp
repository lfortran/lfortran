...
*/
class LoopVectoriseVisitor
    : public PassUtils::SkipOptimizationFunctionVisitor<LoopVectoriseVisitor> {
 private:
 ...
 public:
  ...
  bool is_loop(const ASR::stmt_t& x) {
    return (ASR::is_a<ASR::DoLoop_t>(x) || ASR::is_a<ASR::WhileLoop_t>(x));
  }
  ...
  bool is_vector_copy(ASR::stmt_t* x, Vec<ASR::expr_t*>& arrays) {
    ...
    ASR::ArrayItem_t* target_array_ref =
        ASR::down_cast<ASR::ArrayItem_t>(target);
    ASR::ArrayItem_t* value_array_ref = ASR::down_cast<ASR::ArrayItem_t>(value);
    if (target_array_ref->m_args->m_left || target_array_ref->m_args->m_step ||
        value_array_ref->m_args->m_left || value_array_ref->m_args->m_step) {
      return false;
    }
  ...
  int64_t select_vector_instruction_size() { return avx512; }
  ...
  void get_vector_intrinsic(ASR::stmt_t* loop_stmt, ASR::expr_t* index,
    ...
    if (is_vector_copy(loop_stmt, arrays)) {
      ...
      vector_length = ASRUtils::EXPR(ASR::make_IntegerConstant_t(
          al, loop_stmt->base.loc, vector_length_int,
          ASRUtils::expr_type(index)));
      ASR::expr_t* start = ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
          al, loop_stmt->base.loc, index, ASR::binopType::Mul, vector_length,
          ASRUtils::expr_type(index), nullptr));
      ASR::expr_t* one = ASRUtils::EXPR(ASR::make_IntegerConstant_t(
          al, loop_stmt->base.loc, 1, ASRUtils::expr_type(index)));
      ASR::expr_t* next_index = ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
          al, loop_stmt->base.loc, index, ASR::binopType::Add, one,
          ASRUtils::expr_type(index), nullptr));
      ASR::expr_t* end = ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
          al, loop_stmt->base.loc, next_index, ASR::binopType::Mul,
          vector_length, ASRUtils::expr_type(index), nullptr));
      vectorised_loop_body.push_back(
          al, PassUtils::get_vector_copy(target_sym, value_sym, start, end, one,
                                         vector_length, al, unit, current_scope,
                                         loop_stmt->base.loc));
      return;
    }
  }
  ...
  void vectorise_loop(const ASR::DoLoop_t& x) {
    ...
    get_vector_intrinsic(loop_stmt, x.m_head.m_v, vector_length,
                         vectorised_loop_body);
    if (!vector_length) {
      return;
    }
    ...
    vectorised_loop_head.m_end = ASRUtils::EXPR(ASR::make_IntegerConstant_t(
        al, x.m_head.m_v->base.loc, loop_size / vector_length_int - 1,
        ASRUtils::expr_type(x.m_head.m_v)));
    // TODO: Update this for increments other 1.
    ASR::expr_t* one = ASRUtils::EXPR(ASR::make_IntegerConstant_t(
        al, loop_stmt->base.loc, 1, ASRUtils::expr_type(x.m_head.m_v)));
    vectorised_loop_head.m_increment = one;
    vectorised_loop_head.m_v = x.m_head.m_v;
    vectorised_loop_head.loc = x.m_head.loc;

    ASR::stmt_t* vectorised_loop = ASRUtils::STMT(ASR::make_DoLoop_t(
        al, x.base.base.loc, vectorised_loop_head, vectorised_loop_body.p,
