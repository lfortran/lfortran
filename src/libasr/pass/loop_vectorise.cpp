...
public:
    LoopVectoriseVisitor(Allocator& al_, ASR::TranslationUnit_t& unit_, const std::string& rl_path_)
    ...
    bool is_vector_copy(ASR::stmt_t* x, Vec<ASR::expr_t*>& arrays)
    {
        ...
        if (!ASR::is_a<ASR::ArrayItem_t>(*target) || !ASR::is_a<ASR::ArrayItem_t>(*value)) {
            return false;
        }
    ...
    void get_vector_intrinsic(ASR::stmt_t* loop_stmt,
                              ASR::expr_t* index,
                              ...
                              Vec<ASR::stmt_t*>& vectorised_loop_body)
    {
        ...
        if (is_vector_copy(loop_stmt, arrays)) {
            ...
            vector_length = ASRUtils::EXPR(ASR::make_IntegerConstant_t(
                al, loop_stmt->base.loc, vector_length_int, ASRUtils::expr_type(index)));
            ASR::expr_t* start = ASRUtils::EXPR(ASR::make_IntegerBinOp_t(al,
                                                                         loop_stmt->base.loc,
                                                                         index,
                                                                         ASR::binopType::Mul,
                                                                         vector_length,
            ...
            ASR::expr_t* next_index
                = ASRUtils::EXPR(ASR::make_IntegerBinOp_t(al,
                                                          ...
                                                          ASRUtils::expr_type(index),
                                                          nullptr));
            ASR::expr_t* end = ASRUtils::EXPR(ASR::make_IntegerBinOp_t(al,
                                                                       loop_stmt->base.loc,
                                                                       next_index,
                                                                       ASR::binopType::Mul,
                                                                       vector_length,
                                                                       ASRUtils::expr_type(index),
                                                                       nullptr));
            vectorised_loop_body.push_back(al,
                                           PassUtils::get_vector_copy(target_sym,
                                                                      value_sym,
                                                                      start,
                                                                      end,
                                                                      one,
    ...
    void vectorise_loop(const ASR::DoLoop_t& x)
    {
        ...
        get_vector_intrinsic(loop_stmt, x.m_head.m_v, vector_length, vectorised_loop_body);
        if (!vector_length) {
            return;
        }
        ...
        vectorised_loop_head.m_end
            = ASRUtils::EXPR(ASR::make_IntegerConstant_t(al,
                                                         x.m_head.m_v->base.loc,
                                                         loop_size / vector_length_int - 1,
        ...
        ASR::stmt_t* vectorised_loop
            = ASRUtils::STMT(ASR::make_DoLoop_t(al,
                                                x.base.base.loc,
                                                vectorised_loop_head,
                                                vectorised_loop_body.p,
...
void
pass_loop_vectorise(Allocator& al,
                    ASR::TranslationUnit_t& unit,
