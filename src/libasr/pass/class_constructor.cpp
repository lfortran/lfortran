...
class ClassConstructorVisitor
    : public PassUtils::PassVisitor<ClassConstructorVisitor> {
 private:
 ...
 public:
  ...
  ClassConstructorVisitor(Allocator& al)
  ...
  void visit_DerivedTypeConstructor(const ASR::DerivedTypeConstructor_t& x) {
    ...
    for (size_t i = 0; i < std::min(dt_dertype->n_members, x.n_args); i++) {
      ASR::symbol_t* member = dt_dertype->m_symtab->resolve_symbol(std::string(
          dt_dertype->m_members[i], strlen(dt_dertype->m_members[i])));
      ASR::expr_t* derived_ref =
          LFortran::ASRUtils::EXPR(ASRUtils::getDerivedRef_t(
              al, x.base.base.loc, (ASR::asr_t*)result_var, member,
              current_scope));
      ASR::stmt_t* assign = LFortran::ASRUtils::STMT(ASR::make_Assignment_t(
          al, x.base.base.loc, derived_ref, x.m_args[i], nullptr));
      pass_result.push_back(al, assign);
    }
  }
};

void pass_replace_class_constructor(
