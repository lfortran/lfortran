...
assignments. The function `pass_replace_print_arr` transforms the ASR tree
in-place.
  ...
  void visit_Print(const ASR::Print_t& x) {
    if (x.n_values == 1 && PassUtils::is_array(x.m_values[0])) {
      ...
      for (int i = n_dims - 1; i >= 0; i--) {
        ...
        if (doloop == nullptr) {
          ASR::expr_t* ref =
              PassUtils::create_array_ref(arr_expr, idx_vars, al);
          Vec<ASR::expr_t*> print_args;
          print_args.reserve(al, 1);
          print_args.push_back(al, ref);
          ASR::stmt_t* print_stmt = LFortran::ASRUtils::STMT(
              ASR::make_Print_t(al, x.base.base.loc, nullptr, print_args.p,
