...
std::vector<std::string> split(const std::string &s) {
  ...
  size_t old_pos = 0;
  size_t new_pos;
  while ((new_pos = s.find_first_of(split_chars, old_pos)) !=
         std::string::npos) {
...
std::vector<std::string> slice(const std::vector<std::string> &v, int start,
                               int end) {
  int oldlen = v.size();
  int newlen;
...
std::string replace(const std::string &s, const std::string &regex,
