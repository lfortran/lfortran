  ...
  ASR::symbol_t *read_symbol() {
    ...
    if (symtab->get_symbol(symbol_name) == nullptr) {
      ...
      switch (ty) {
        ...
        default:
          throw LCompilersException("Symbol type not supported");
      }
...
class FixExternalSymbolsVisitor
    : public BaseWalkVisitor<FixExternalSymbolsVisitor> {
 private:
  ...
  void visit_ExternalSymbol(const ExternalSymbol_t &x) {
    ...
    if (x.m_module_name == nullptr) {
      throw LCompilersException(
          "The ExternalSymbol was referenced in some ASR node, but it was not "
          "loaded as part of the SymbolTable");
    }
    ...
    if (global_symtab->get_symbol(module_name) != nullptr) {
      ...
      throw LCompilersException(
          "ExternalSymbol cannot be resolved, the module '" + module_name +
          "' was not found, so the symbol '" + original_name +
