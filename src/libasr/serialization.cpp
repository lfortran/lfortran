...
{
public:
    ASRDeserializationVisitor(Allocator& al, const std::string& s, bool load_symtab_id)
        :
#ifdef WITH_LFORTRAN_BINARY_MODFILES
        BinaryReader(s)
        ,
...
#define INSERT_SYMBOL_CASE(x)                                                                      \
    ...
    void symtab_insert_symbol(SymbolTable& symtab, const std::string& name, ASR::symbol_t* sym)
    {
...
namespace ASR
{
    ...
    public:
        FixExternalSymbolsVisitor(SymbolTable& symtab)
            : external_symtab{ &symtab }
        {
        }
