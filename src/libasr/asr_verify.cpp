...
namespace ASR
{
    ...
    public:
        ...
        {
            std::string msg
                = std::to_string(loc.first) + ":" + std::to_string(loc.last) + ": " + error_msg;
            /*
        ...
        void visit_TranslationUnit(const TranslationUnit_t& x)
        {
            ...
            require(down_cast2<TranslationUnit_t>(current_symtab->asr_owner)->m_global_scope
                        == current_symtab,
        ...
        void visit_Program(const Program_t& x)
        {
            ...
            require(ASRUtils::symbol_symtab(down_cast<symbol_t>(current_symtab->asr_owner))
                        == current_symtab,
                    "The asr_owner invariant failed");
        ...
        void visit_AssociateBlock(const AssociateBlock_t& x)
        {
            SymbolTable* parent_symtab = current_symtab;
            current_symtab = x.m_symtab;
            require(x.m_symtab != nullptr, "The AssociateBlock::m_symtab cannot be nullptr");
            require(x.m_symtab->parent == parent_symtab,
        ...
        void visit_DerivedType(const DerivedType_t& x)
        {
            SymbolTable* parent_symtab = current_symtab;
            current_symtab = x.m_symtab;
            require(x.m_symtab != nullptr, "The DerivedType::m_symtab cannot be nullptr");
            require(x.m_symtab->parent == parent_symtab,
        ...
        void visit_Variable(const Variable_t& x)
        {
            ...
            require(id_symtab_map.find(symtab->counter) != id_symtab_map.end(),
                    "Variable::m_parent_symtab must be present in the ASR (" + std::string(x.m_name)
                        + ")");
        ...
        {
            if (check_external) {
                require(x.m_external != nullptr, "ExternalSymbol::m_external cannot be nullptr");
                ...
                require(s != nullptr,
                        "ExternalSymbol::m_original_name ('" + std::string(x.m_original_name)
                            + "') + scope_names not found in a module '" + std::string(m->m_name)
                            + "'");
...
bool
asr_verify(const ASR::TranslationUnit_t& unit, bool check_external)
{
