    ...
    void visit_Variable(const Variable_t &x) {
        ...
        if(ASRUtils::is_character(*x.m_type)){
            ...
            if(str->m_len_kind == ASR::DeferredLength){
                /* 
                    String type Varaible + DeferredLength ==> Must be allocatable or pointer
                    String type Expressions + DeferredLength + Not allocatable or pointer ==> Allowed.
                */ 
            ...
            if(x.m_abi == abiType::BindC){
                require(str->m_physical_type == CChar,
                    "Cbind character variable must be of physical type \"CChar\"");
            }
