...
class VerifyVisitor : public BaseWalkVisitor<VerifyVisitor> {
 ...
 public:
  ...
  void require(bool cond, const std::string &error_msg, const Location &loc) {
  ...
  // case when the symtab is in a different module or if the `sym`'s symbol
  // table does not actually contain it.
  ...
  void visit_TranslationUnit(const TranslationUnit_t &x) {
    ...
    require(
        id_symtab_map.find(x.m_global_scope->counter) == id_symtab_map.end(),
        "TranslationUnit::m_global_scope->counter must be unique");
    require(
        x.m_global_scope->asr_owner == (ASR::asr_t *)&x,
        "The TranslationUnit::m_global_scope::asr_owner must point to itself");
    require(down_cast2<TranslationUnit_t>(current_symtab->asr_owner)
                    ->m_global_scope == current_symtab,
  ...
  void visit_Program(const Program_t &x) {
    ...
    require(x.m_symtab != nullptr, "The Program::m_symtab cannot be nullptr");
    ...
    require(ASRUtils::symbol_symtab(down_cast<symbol_t>(
                current_symtab->asr_owner)) == current_symtab,
            "The asr_owner invariant failed");
  ...
  void visit_Module(const Module_t &x) {
    SymbolTable *parent_symtab = current_symtab;
    current_symtab = x.m_symtab;
    require(x.m_symtab != nullptr, "The Module::m_symtab cannot be nullptr");
    require(x.m_symtab->parent == parent_symtab,
    ...
    for (size_t i = 0; i < x.n_dependencies; i++) {
      require(x.m_dependencies[i] != nullptr,
              "A module dependency must not be a nullptr", x.base.base.loc);
      ...
      require(valid_name(x.m_dependencies[i]),
              "A module dependency must be a valid string", x.base.base.loc);
    }
    current_symtab = parent_symtab;
  }
  ...
  void visit_Function(const Function_t &x) {
    SymbolTable *parent_symtab = current_symtab;
    current_symtab = x.m_symtab;
    require(x.m_symtab != nullptr, "The Function::m_symtab cannot be nullptr");
    require(x.m_symtab->parent == parent_symtab,
  ...
  void visit_Variable(const Variable_t &x) {
    ...
    require(symtab != nullptr, "Variable::m_parent_symtab cannot be nullptr");
    ...
    if (x.m_symbolic_value) visit_expr(*x.m_symbolic_value);
  ...
  void visit_ExternalSymbol(const ExternalSymbol_t &x) {
    if (check_external) {
      ...
      require(m, "ExternalSymbol::m_external is not in a module");
      require(std::string(x.m_module_name) == std::string(m->m_name),
              "ExternalSymbol::m_module_name `" + std::string(x.m_module_name) +
                  "` must match external's module name `" +
                  std::string(m->m_name) + "`");
      ASR::symbol_t *s = m->m_symtab->find_scoped_symbol(
          x.m_original_name, x.n_scope_names, x.m_scope_names);
      ...
      require(s == x.m_external,
              "ExternalSymbol::m_name + scope_names found but not equal to "
              "m_external");
    }
  }
  ...
  void visit_Var(const Var_t &x) {
    require(x.m_v != nullptr, "Var_t::m_v cannot be nullptr");
    require(is_a<Variable_t>(*x.m_v) || is_a<ExternalSymbol_t>(*x.m_v) ||
                is_a<Function_t>(*x.m_v),
            "Var_t::m_v " + std::string(ASRUtils::symbol_name(x.m_v)) +
                " does not point to a Variable_t, ExternalSymbol_t,"
                "Function_t, or Subroutine_t");
    require(symtab_in_scope(current_symtab, x.m_v),
            "Var::m_v `" + std::string(ASRUtils::symbol_name(x.m_v)) +
  ...
  void visit_ArrayItem(const ArrayItem_t &x) { visit_ArrayItemSection(x); }
  ...
  void visit_SubroutineCall(const SubroutineCall_t &x) {
    if (x.m_dt) {
      ...
      require(symtab_in_scope(current_symtab, x.m_name),
              "SubroutineCall::m_name '" + std::string(symbol_name(x.m_name)) +
                  "' cannot point outside of its symbol table",
              x.base.base.loc);
    }
    for (size_t i = 0; i < x.n_args; i++) {
  ...
  SymbolTable *get_dt_symtab(ASR::symbol_t *dt, const Location &loc) {
    LFORTRAN_ASSERT(dt)
    SymbolTable *symtab =
        ASRUtils::symbol_symtab(ASRUtils::symbol_get_past_external(dt));
    require(symtab,
            "m_dt::m_v::m_type::class/derived_type must point to a symbol with "
            "a symbol table",
            loc);
    return symtab;
  }
  ...
  SymbolTable *get_dt_symtab(ASR::expr_t *dt, const Location &loc) {
    require(ASR::is_a<ASR::Var_t>(*dt), "m_dt must point to a Var", loc);
    ...
    switch (t2->type) {
      ...
      default:
        require(false,
                "m_dt::m_v::m_type must point to a type with a symbol table "
                "(Derived or Class)",
                loc);
    }
  ...
  ASR::symbol_t *get_parent_type_dt(ASR::symbol_t *dt, const Location &loc) {
    ...
    switch (dt->type) {
      ...
      default:
        require(false, "m_dt::m_v::m_type must point to a Derived type", loc);
    }
    return parent;
  }
    ...
    switch (t2->type) {
      case (ASR::ttypeType::Derived): {
        ...
        ASR::DerivedType_t *der_type =
            ASR::down_cast<ASR::DerivedType_t>(type_sym);
        parent = der_type->m_parent;
        break;
      }
      case (ASR::ttypeType::Class): {
  ...
  void visit_FunctionCall(const FunctionCall_t &x) {
    require(x.m_name, "FunctionCall::m_name must be present", x.base.base.loc);
    if (x.m_dt) {
      SymbolTable *symtab = get_dt_symtab(x.m_dt, x.base.base.loc);
      require(symtab_in_scope(symtab, x.m_name),
      ...
      if (check_external) {
        ...
        require(ASR::is_a<ASR::Function_t>(*fn),
                "FunctionCall::m_name must be a Function", x.base.base.loc);
      }
    }
    for (size_t i = 0; i < x.n_args; i++) {
      if (x.m_args[i].m_value) {
