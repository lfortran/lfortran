...
namespace ASRUtils
{
    ...
    static inline ASR::ttype_t* symbol_type(const ASR::symbol_t* f)
    {
        switch (f->type) {
            ...
            default: {
                throw LCompilersException("Cannot return type of, " + std::to_string(f->type)
                                          + " symbol.");
            }
        }
        return nullptr;
    ...
    static inline std::string type_to_str(const ASR::ttype_t* t)
    {
        switch (t->type) {
            ...
            case ASR::ttypeType::Pointer: {
                return type_to_str(ASRUtils::type_get_past_pointer(const_cast<ASR::ttype_t*>(t)))
                       + " pointer";
            }
            default:
    ...
    static inline bool is_intrinsic_procedure(const T* fn)
    {
        ...
        if (m != nullptr) {
            if (startswith(m->m_name, "lfortran_intrinsic"))
                return true;
        }
        return false;
    }
    ...
    static inline bool is_intrinsic_optimization(const T* routine)
    {
        ...
        if (ASR::is_a<ASR::ExternalSymbol_t>(*sym)) {
            ...
            return (std::string(ext_sym->m_module_name).find("lfortran_intrinsic_optimization")
                    != std::string::npos);
        }
        ASR::Module_t* m = get_sym_module0(sym);
        if (m != nullptr) {
    ...
    static inline Vec<ASR::call_arg_t> get_arg_values(Allocator& al,
    ...
    static inline Vec<ASR::expr_t*> call_arg2expr(Allocator& al,
                                                  const Vec<ASR::call_arg_t>& call_args)
    {
        Vec<ASR::expr_t*> args;
    ...
    {
        return intent == intent_in || intent == intent_out || intent == intent_inout
    ...
    std::vector<std::string> determine_module_dependencies(const ASR::TranslationUnit_t& unit);
    ...
    ASR::asr_t* getDerivedRef_t(Allocator& al,
                                const Location& loc,
                                ASR::asr_t* v_var,
                                ASR::symbol_t* member,
                                SymbolTable* current_scope);

    bool use_overloaded(ASR::expr_t* left,
                        ASR::expr_t* right,
                        ASR::binopType op,
    ...
    bool is_op_overloaded(ASR::binopType op,
                          std::string& intrinsic_op_name,
                          SymbolTable* curr_scope);
    ...
    bool is_op_overloaded(ASR::cmpopType op,
                          std::string& intrinsic_op_name,
                          SymbolTable* curr_scope);
    ...
    static inline std::string get_parameter_name(const ASR::ttype_t* t)
    {
        switch (t->type) {
            ...
            default:
                throw LCompilersException("Cannot obtain type parameter from this type");
        }
    }
    ...
    inline int extract_dimensions_from_ttype(ASR::ttype_t* x, ASR::dimension_t*& m_dims)
    {
        int n_dims = 0;
        switch (x->type) {
            case ASR::ttypeType::Integer: {
            ...
            case ASR::ttypeType::Pointer: {
                n_dims = extract_dimensions_from_ttype(ASR::down_cast<ASR::Pointer_t>(x)->m_type,
                                                       m_dims);
                break;
            }
            case ASR::ttypeType::List: {
    ...
    inline bool ttype_set_dimensions(ASR::ttype_t* x, ASR::dimension_t* m_dims, int64_t n_dims)
    {
        switch (x->type) {
            case ASR::ttypeType::Integer: {
                ASR::Integer_t* Integer_type = ASR::down_cast<ASR::Integer_t>(x);
    ...
    static inline ASR::ttype_t* duplicate_type(Allocator& al,
                                               const ASR::ttype_t* t,
        ...
        switch (t->type) {
            ...
            case ASR::ttypeType::Character: {
                ...
                return ASRUtils::TYPE(ASR::make_Character_t(
                    al, t->base.loc, tnew->m_kind, tnew->m_len, tnew->m_len_expr, dimsp, dimsn));
            }
            case ASR::ttypeType::Derived: {
                ...
                size_t dimsn = dims ? dims->n : tnew->n_dims;
                return ASRUtils::TYPE(
                    ASR::make_Derived_t(al, t->base.loc, tnew->m_derived_type, dimsp, dimsn));
            }
            case ASR::ttypeType::Pointer: {
                ASR::Pointer_t* ptr = ASR::down_cast<ASR::Pointer_t>(t);
                ASR::ttype_t* dup_type = duplicate_type(al, ptr->m_type, dims);
                return ASRUtils::TYPE(ASR::make_Pointer_t(al, ptr->base.base.loc, dup_type));
            }
            case ASR::ttypeType::TypeParameter: {
                ASR::TypeParameter_t* tp = ASR::down_cast<ASR::TypeParameter_t>(t);
                ...
                return ASRUtils::TYPE(
                    ASR::make_TypeParameter_t(al, t->base.loc, tp->m_param, dimsp, dimsn));
            }
            default:
                throw LCompilersException("Not implemented " + std::to_string(t->type));
        }
    ...
    inline bool is_same_type_pointer(ASR::ttype_t* source, ASR::ttype_t* dest)
    {
        ...
        if ((!is_source_pointer && !is_dest_pointer) || (is_source_pointer && is_dest_pointer)) {
            return false;
        }
        ...
        switch (kind_expr->type) {
            ...
            case ASR::exprType::Var: {
                ...
                ASR::Variable_t* kind_variable
                    = ASR::down_cast<ASR::Variable_t>(symbol_get_past_external(kind_var->m_v));
                if (kind_variable->m_storage == ASR::storage_typeType::Parameter) {
                    if (kind_variable->m_type->type == ASR::ttypeType::Integer) {
                        ...
                        a_kind
                            = ASR::down_cast<ASR::IntegerConstant_t>(kind_variable->m_value)->m_n;
                    } else {
        ...
        switch (len_expr->type) {
            ...
            case ASR::exprType::Var: {
                ...
                ASR::Variable_t* len_variable
                    = ASR::down_cast<ASR::Variable_t>(symbol_get_past_external(len_var->m_v));
            ...
            default: {
                throw SemanticError(
                    "Only Integers or variables implemented so far for `len` expressions", loc);
            }
        }
    ...
    int select_generic_procedure(const Vec<ASR::call_arg_t>& args,
                                 const ASR::GenericProcedure_t& p,
                                 Location loc,
                                 const std::function<void(const std::string&, const Location&)> err,
    ...
    public:
        ReplaceArgVisitor(Allocator& al_,
        ...
        void replace_FunctionCall(ASR::FunctionCall_t* x)
        {
            ...
            if (maybe_modname == std::string(modname) && f_sym == maybe_f_actual) {
                ...
                new_es = ASR::down_cast<ASR::symbol_t>(
    ...
    {
    public:
        ExprStmtDuplicator(Allocator& al)
    ...
    ASR::asr_t* make_Cast_t_value(Allocator& al,
                                  const Location& a_loc,
                                  ASR::expr_t* a_arg,
                                  ASR::cast_kindType a_kind,
                                  ASR::ttype_t* a_type);
                                                             ...
                                                             ASR::expr_t* end)
    {
        ...
        if (start_value && end_value) {
            ...
            return ASRUtils::EXPR(ASR::make_IntegerConstant_t(
                al, start->base.loc, end_int - start_int + 1, ASRUtils::expr_type(start)));
        }
            ...
            if (ASR::is_a<ASR::IntegerBinOp_t>(*end)) {
                ...
                if (end_binop->m_op == ASR::binopType::Add
                    ...
                    if (end_leftv) {
                        ...
                        if (remaining_portion == 0 && end_binop->m_op == ASR::binopType::Add) {
                            return end_right;
                        }
        ...
        ASR::expr_t* diff = ASRUtils::EXPR(ASR::make_IntegerBinOp_t(
            al, end->base.loc, end, ASR::binopType::Sub, start, ASRUtils::expr_type(end), nullptr));
        ASR::expr_t* constant_one = ASR::down_cast<ASR::expr_t>(
            ASR::make_IntegerConstant_t(al, diff->base.loc, 1, ASRUtils::expr_type(diff)));
        return ASRUtils::EXPR(ASR::make_IntegerBinOp_t(al,
                                                       end->base.loc,
                                                       diff,
                                                       ASR::binopType::Add,
    ...
    static inline bool is_pass_array_by_data_possible(ASR::Function_t* x, std::vector<size_t>& v)
    {
        ...
        for (size_t i = 0; i < x->n_args; i++) {
            ...
            if (ASRUtils::is_dimension_empty(dims, n_dims)
                && (argi->m_intent == ASRUtils::intent_in || argi->m_intent == ASRUtils::intent_out)
    ...
    static inline ASR::expr_t* get_bound(ASR::expr_t* arr_expr,
                                         int dim,
                                         std::string bound,
                                         Allocator& al)
    {
        ASR::ttype_t* int32_type
            = ASRUtils::TYPE(ASR::make_Integer_t(al, arr_expr->base.loc, 4, nullptr, 0));
        ASR::expr_t* dim_expr
            = ASRUtils::EXPR(ASR::make_IntegerConstant_t(al, arr_expr->base.loc, dim, int32_type));
        ASR::arrayboundType bound_type = ASR::arrayboundType::LBound;
        if (bound == "ubound") {
            bound_type = ASR::arrayboundType::UBound;
        }
        return ASRUtils::EXPR(ASR::make_ArrayBound_t(
            al, arr_expr->base.loc, arr_expr, dim_expr, int32_type, bound_type, nullptr));
    }

    static inline ASR::expr_t* get_size(ASR::expr_t* arr_expr, int dim, Allocator& al)
    {
        ASR::ttype_t* int32_type
            = ASRUtils::TYPE(ASR::make_Integer_t(al, arr_expr->base.loc, 4, nullptr, 0));
        ASR::expr_t* dim_expr
        ...
        return ASRUtils::EXPR(
            ASR::make_ArraySize_t(al, arr_expr->base.loc, arr_expr, dim_expr, int32_type, nullptr));
    }

    static inline void get_dimensions(ASR::expr_t* array, Vec<ASR::expr_t*>& dims, Allocator& al)
    {
