...
class JsonVisitorVisitor(ASDLVisitor):
    ...
    def visitModule(self, mod):
        ...
        self.emit(      's.append("}");', 2)
        self.emit(  '}', 1)
        self.emit(  """std::string get_escaped_str(const std::string &s) {""", 1)
        self.emit(      R"""std::ostringstream o;""", 2)
        self.emit(      R"""for (auto c = s.cbegin(); c != s.cend(); c++) {""", 2)
        self.emit(      R"""    switch (*c) {""", 2)
        self.emit(      R"""    case '"': o << "\\\""; break;""", 2)
        self.emit(      R"""    case '\\': o << "\\\\"; break;""", 2)
        self.emit(      R"""    case '\b': o << "\\b"; break;""", 2)
        self.emit(      R"""    case '\f': o << "\\f"; break;""", 2)
        self.emit(      R"""    case '\n': o << "\\n"; break;""", 2)
        self.emit(      R"""    case '\r': o << "\\r"; break;""", 2)
        self.emit(      R"""    case '\t': o << "\\t"; break;""", 2)
        self.emit(      R"""    default:""", 2)
        self.emit(      R"""        if ('\x00' <= *c && *c <= '\x1f') {""", 2)
        self.emit(      R"""            o << "\\u"<< std::hex << std::setw(4) << std::setfill('0') << static_cast<int>(*c);""", 2)
        self.emit(      R"""        } else {""", 2)
        self.emit(      R"""            o << *c;""", 2)
        self.emit(      R"""        }""", 2)
        self.emit(      R"""    }""", 2)
        self.emit(      R"""}""", 2)
        self.emit(      R"""return o.str();""", 2)
        self.emit(  """}""", 1)
