...
uint32_t
murmur_hash_str(const std::string& s, uint32_t seed)
{
    return murmur_hash(&s[0], s.length(), seed);
}

uint32_t
murmur_hash_int(uint64_t i, uint32_t seed)
{
...
void
SymbolTable::mark_all_variables_external(Allocator& /*al*/)
{
    for (auto& a : scope) {
...
ASR::symbol_t*
SymbolTable::find_scoped_symbol(const std::string& name, size_t n_scope_names, char** m_scope_names)
{
...
std::string
SymbolTable::get_unique_name(const std::string& name)
{
