...
//
// Thread-local arena provides fast bump-pointer allocation for local arrays.
// This avoids stack overflow for programs with many/large local arrays while
// being nearly as fast as stack allocation.
//
// Usage pattern (generated by LLVM codegen):
//   void* saved = _lfortran_arena_save();    // at function entry
//   arr1 = _lfortran_arena_alloc(size1);     // allocate local arrays
//   arr2 = _lfortran_arena_alloc(size2);
//   ...
//   _lfortran_arena_restore(saved);          // at function exit (all paths)

#define LFORTRAN_ARENA_SIZE (64 * 1024 * 1024)  // 64 MB per thread

// Exported thread-local arena pointer for inline access from generated code
LFORTRAN_API LFORTRAN_THREAD_LOCAL char* _lfortran_arena_ptr = NULL;

// Initialize arena - called at program start
LFORTRAN_API void _lfortran_arena_init(void) {
    if (_lfortran_arena_ptr == NULL) {
        _lfortran_arena_ptr = (char*)malloc(LFORTRAN_ARENA_SIZE);
        if (_lfortran_arena_ptr == NULL) {
            fprintf(stderr, "Fatal: Failed to allocate arena (%d bytes)\n",
                    LFORTRAN_ARENA_SIZE);
            exit(1);
        }
    }
...
static void ensure_arena_initialized(void) {
    if (_lfortran_arena_ptr == NULL) {
        _lfortran_arena_init();
    }
}
...
LFORTRAN_API void* _lfortran_arena_alloc(int64_t size) {
    ...
    void* ptr = _lfortran_arena_ptr;
    _lfortran_arena_ptr += size;
    return ptr;
}
               ...
               int64_t delim_len)
{
    ...
    FILE* already_open = get_file_pointer_from_unit(unit_num, NULL, NULL, NULL, NULL, NULL);
...
LFORTRAN_API void _lfortran_inquire(const fchar* f_name_data, int64_t f_name_len, bool *exists, int32_t unit_num,
    ...
    if (unit_num != -1) {
        ...
        bool write_access;
        int delim;
        FILE *fp = get_file_pointer_from_unit(unit_num, &unit_file_bin, &access_id, &read_access, &write_access, &delim);
        if (write != NULL) {
            if (write_access) {
...
{
    bool unit_file_bin;
    FILE* fd = get_file_pointer_from_unit(unit_num, &unit_file_bin, NULL, NULL, NULL, NULL);
    if (fd == NULL) {
