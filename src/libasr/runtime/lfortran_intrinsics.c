...
typedef struct serialization_info{
    ...
    int32_t current_stop; // current index in the serialization_string.
    Stack* array_sizes_stack; // holds the sizes of the arrays.
...
// Set the currentType by parsing through the serialization string.
// TODO : maybe change function's name + pass only needed parameters not the whole struct.
void set_current_element_info(Serialization_Info* s_info){
...
bool move_to_next_element(struct serialization_info* s_info, bool peek){
    ...
    while(true){
        ...
        if(isdigit(cur)){ // ArraySize --> `50[I4]`
            ...
            if(zero_size) {
                set_current_element_info(s_info);/*Keep deserializing*/
                continue;
            }
...
void default_formatting(char** result, struct serialization_info* s_info){
    ...
    *result = realloc(*result, result_capacity + 1 /*Null Character*/ );
    ...
    while(move_to_next_element(s_info, false)){
        ...
        char* temp_result = (char*)malloc(size_to_allocate);
        temp_result[0] = '\0';
        if(result_size > 0){strcpy(temp_result, default_spacing);}
        print_into_string(s_info,  temp_result + ((result_size>0)?default_spacing_len:0));
        int64_t printed_arg_size = strlen(temp_result);
        int64_t old_capacity = result_capacity;
        while(result_capacity <= printed_arg_size + result_size){ // Check if string extension is needed.
            if(result_size + printed_arg_size > result_capacity*2){
                result_capacity = printed_arg_size + result_size +1 ;
            } else {
                result_capacity *=2;
            }
        }
        if(result_capacity != old_capacity){*result = (char*)realloc(*result, result_capacity + 1);}
        for(int i = 0 ;i <= printed_arg_size;i++){ /*meant for optimization instead of strcat*/ 
            (*result)[result_size+i] = temp_result[i];
        }
    ...
    int32_t array_sizes_cnt, ...)
{
    ...
    {fprintf(stderr,"Internal Error : default formatting error");exit(1);}
