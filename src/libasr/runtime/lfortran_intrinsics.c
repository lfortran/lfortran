...
/* -----------------------------------------------------------------------
 * fp128 arithmetic helpers — macOS / Windows / WASM.
 *
 * LLVM lowers every fp128 operation to a libgcc/compiler-rt call:
 *   +  -> __addtf3      -  -> __subtf3
 *   *  -> __multf3      /  -> __divtf3
 *   unary- -> __negtf2
 *
 * These are missing on macOS and Windows.  We implement them here using
 * the lf_float128 bit-manipulation helpers so that real(16) arithmetic
 * links and runs correctly on every platform LFortran supports.
 *
 * The implementation uses a 128-bit software floating-point kernel based
 * on exact integer arithmetic on the 112-bit mantissa, so results are
 * correctly rounded to IEEEquad precision — not limited to double.
 * ----------------------------------------------------------------------- */
...
/* Shift right by n (0..127) */
static inline u128 u128_shr(u128 a, int n) {
    if (n == 0) return a;
    if (n >= 128) { u128 z = {0,0}; return z; }
    u128 r;
    if (n >= 64) { r.lo = a.hi >> (n-64); r.hi = 0; }
    else { r.lo = (a.lo >> n) | (a.hi << (64-n)); r.hi = a.hi >> n; }
    return r;
}
/* Shift left by n (0..127) */
...
static inline int u128_clz(u128 a) {
    if (a.hi != 0) return (int)__builtin_clzll(a.hi);
    if (a.lo != 0) return 64 + (int)__builtin_clzll(a.lo);
    return 128;
}

/* Unpack an lf_float128 into its components.
 * Returns 0 for normal/zero/subnormal, sets *is_nan / *is_inf flags. */
typedef struct {
    int      sign;
    int32_t  exp;   /* unbiased true exponent (biased - 16383) */
    u128     mant;  /* 113-bit significand with implicit leading 1 in bit 112,
                       stored in bits [112:0] of the u128 */
    int      is_nan;
    int      is_inf;
    int      is_zero;
} f128_parts;

static inline f128_parts f128_unpack(lf_float128 v) {
    f128_parts p;
    uint64_t lo, hi;
    memcpy(&lo, v.bytes,     8);
    memcpy(&hi, v.bytes + 8, 8);
    ...
    p.sign = (int)(hi >> 63);
    uint64_t biased = (hi >> 48) & 0x7FFF;
    uint64_t mhi    = hi & 0x0000FFFFFFFFFFFFULL;

    p.mant.lo = lo;
    p.mant.hi = mhi;
    p.is_nan  = 0; p.is_inf = 0; p.is_zero = 0;

    if (biased == 0x7FFF) {
        if (mhi == 0 && lo == 0) p.is_inf = 1;
        else                     p.is_nan = 1;
        p.exp = 0;
        return p;
    }
    if (biased == 0) {
        if (mhi == 0 && lo == 0) { p.is_zero = 1; p.exp = 0; return p; }
        /* subnormal */
        p.exp = -16382; /* true exp for subnormals */
        /* no implicit leading 1 */
    } else {
        p.exp = (int32_t)biased - 16383;
        /* restore implicit leading 1 in bit 112 */
        p.mant.hi |= (1ULL << 48);
    }
    return p;
}

static inline lf_float128 f128_pack(int sign, int32_t exp, u128 mant) {
    lf_float128 r;
    /* Normalise: shift mant so leading 1 is at bit 112 */
    int clz = u128_clz(mant);
    int shift = clz - (128 - 113); /* want leading 1 at position 112 */
    if (shift > 0) {
        mant = u128_shl(mant, shift);
        exp -= shift;
    } else if (shift < 0) {
        mant = u128_shr(mant, -shift);
        exp += (-shift);
    }
    ...
    /* Underflow -> zero (subnormals not handled for prototype) */
    if (exp < -16382) {
        uint64_t hi = (uint64_t)sign << 63;
        uint64_t lo = 0;
        memcpy(r.bytes,     &lo, 8);
        memcpy(r.bytes + 8, &hi, 8);
        return r;
    ...
    uint64_t biased = (uint64_t)(exp + 16383);
    /* strip implicit leading 1 (bit 112 of mant) */
    mant.hi &= 0x0000FFFFFFFFFFFFULL;
    uint64_t hi = ((uint64_t)sign << 63) | (biased << 48) | mant.hi;
    uint64_t lo = mant.lo;
    memcpy(r.bytes,     &lo, 8);
    memcpy(r.bytes + 8, &hi, 8);
    return r;
...
static inline lf_float128 f128_nan(void) {
    lf_float128 r;
    uint64_t hi = 0x7FFF800000000000ULL; /* quiet NaN */
    uint64_t lo = 0;
    memcpy(r.bytes,     &lo, 8);
    memcpy(r.bytes + 8, &hi, 8);
    return r;
}
static inline lf_float128 f128_inf(int sign) {
    lf_float128 r;
    uint64_t hi = ((uint64_t)sign << 63) | 0x7FFF000000000000ULL;
    uint64_t lo = 0;
    memcpy(r.bytes,     &lo, 8);
    memcpy(r.bytes + 8, &hi, 8);
    return r;
}
static inline lf_float128 f128_zero(int sign) {
    lf_float128 r;
    uint64_t hi = (uint64_t)sign << 63;
    uint64_t lo = 0;
    memcpy(r.bytes,     &lo, 8);
...
/* ---- Addition of two same-sign significands -------------------------- */
static lf_float128 f128_add_impl(f128_parts a, f128_parts b) {
    /* Align b to a (a has the larger or equal exponent) */
    int shift = a.exp - b.exp;
    if (shift > 113) b.mant = u128_shr(b.mant, 113); /* flush to zero */
    else if (shift > 0) b.mant = u128_shr(b.mant, shift);

    u128 mant = u128_add(a.mant, b.mant);
    int32_t exp = a.exp;

    /* If carry out of bit 113, shift right one and bump exponent */
    if (mant.hi >> 49) { mant = u128_shr(mant, 1); exp++; }
    return f128_pack(a.sign, exp, mant);
}

/* ---- Subtraction of significands (|a| > |b| guaranteed) ------------- */
static lf_float128 f128_sub_impl(f128_parts a, f128_parts b, int result_sign) {
    int shift = a.exp - b.exp;
    if (shift > 0) b.mant = u128_shr(b.mant, shift);
    u128 mant = u128_sub(a.mant, b.mant);
    return f128_pack(result_sign, a.exp, mant);
}

/* ---- Public arithmetic symbols --------------------------------------- */

lf_float128 __addtf3(lf_float128 a, lf_float128 b) {
    f128_parts pa = f128_unpack(a), pb = f128_unpack(b);
    if (pa.is_nan || pb.is_nan) return f128_nan();
    if (pa.is_inf && pb.is_inf) {
        if (pa.sign != pb.sign) return f128_nan(); /* +inf + -inf */
        return a;
    }
    ...
    /* Make pa the one with the larger exponent */
    int swap = 0;
    if (pb.exp > pa.exp || (pb.exp == pa.exp && u128_lt(pa.mant, pb.mant))) {
        f128_parts tmp = pa; pa = pb; pb = tmp; swap = 1;
    }

    if (pa.sign == pb.sign) return f128_add_impl(pa, pb);

    /* Different signs -> subtraction */
    if (u128_eq(pa.mant, pb.mant) && pa.exp == pb.exp) return f128_zero(0);
    return f128_sub_impl(pa, pb, swap ? pb.sign : pa.sign);
}

lf_float128 __subtf3(lf_float128 a, lf_float128 b) {
    /* Flip sign of b then add */
    uint64_t hi;
    memcpy(&hi, b.bytes + 8, 8);
    hi ^= (1ULL << 63);
    memcpy(b.bytes + 8, &hi, 8);
    return __addtf3(a, b);
}
...
lf_float128 __multf3(lf_float128 a, lf_float128 b) {
    ...
    if (pa.is_nan || pb.is_nan) return f128_nan();
    if (pa.is_inf || pb.is_inf) {
        if (pa.is_zero || pb.is_zero) return f128_nan(); /* 0 * inf */
        return f128_inf(rsign);
    }
    if (pa.is_zero || pb.is_zero) return f128_zero(rsign);

    /* Multiply 113-bit significands -> 226-bit product.
     * We need the top 113 bits.  Split each significand into 57-bit halves. */
    uint64_t ah = (pa.mant.hi << 15) | (pa.mant.lo >> 49);
    uint64_t al = pa.mant.lo & 0x0001FFFFFFFFFFFFULL;
    uint64_t bh = (pb.mant.hi << 15) | (pb.mant.lo >> 49);
    uint64_t bl = pb.mant.lo & 0x0001FFFFFFFFFFFFULL;

    /* 128-bit approximation of the 226-bit product (top 128 bits) */
    /* product ~ ah*bh * 2^114 + (ah*bl + al*bh) * 2^57 + al*bl */
    uint64_t p_hh = ah * bh;         /* top 114 bits * 114 bits = 128-bit, we keep top 64 */
    uint64_t p_hl = ah * bl;
    uint64_t p_lh = al * bh;
    /* We ignore al*bl as it affects only bits below position 114 */

    /* Assemble: the product has its leading 1 at bit 224 (= 113+113-2).
     * We want the result significand's leading 1 at bit 112. */
    u128 mant;
    mant.hi = p_hh + (p_hl >> 57) + (p_lh >> 57);
    mant.lo = (p_hl << 7) + (p_lh << 7);

    int32_t exp = pa.exp + pb.exp + 1; /* +1 for the implicit leading 1s */
    return f128_pack(rsign, exp, mant);
}
...
lf_float128 __divtf3(lf_float128 a, lf_float128 b) {
    ...
    if (pa.is_nan || pb.is_nan) return f128_nan();
    if (pb.is_zero) {
        if (pa.is_zero) return f128_nan(); /* 0/0 */
        return f128_inf(rsign);
    }
    if (pa.is_zero) return f128_zero(rsign);
    if (pa.is_inf) {
        if (pb.is_inf) return f128_nan();
        return f128_inf(rsign);
    }
    if (pb.is_inf) return f128_zero(rsign);

    /* Long division: compute 113-bit quotient via successive subtraction.
     * Shift dividend left until leading 1 aligns, then divide. */
    u128 num = pa.mant;
    u128 den = pb.mant;
    u128 quot = {0, 0};
    int32_t exp = pa.exp - pb.exp;

    /* We compute 114 bits of quotient for correct rounding */
    for (int i = 113; i >= 0; i--) {
        if (!u128_lt(num, den)) {
            /* quotient bit = 1 */
            if (i < 64) quot.lo |= (1ULL << i);
            else        quot.hi |= (1ULL << (i - 64));
            num = u128_sub(num, den);
        }
        num = u128_shl(num, 1);
    }
    return f128_pack(rsign, exp, quot);
}

lf_float128 __negtf2(lf_float128 a) {
    lf_float128 r = a;
    uint64_t hi;
    memcpy(&hi, r.bytes + 8, 8);
    hi ^= (1ULL << 63);
    memcpy(r.bytes + 8, &hi, 8);
    return r;
}
...
lf_float128 __extendsftf2(float a)   { return lf_f128_from_double((double)a); }
float        __trunctfsf2(lf_float128 a) { return (float)lf_f128_to_double(a); }

lf_float128  __extenddftf2(double a) { return lf_f128_from_double(a); }
double       __trunctfdf2(lf_float128 a) { return lf_f128_to_double(a); }

lf_float128 __floatsitf(int32_t a)   { return lf_f128_from_double((double)a); }
int32_t     __fixtfsi(lf_float128 a) { return (int32_t)lf_f128_to_double(a); }

lf_float128 __floatditf(int64_t a)   { return lf_f128_from_double((double)a); }
int64_t     __fixtfdi(lf_float128 a) { return (int64_t)lf_f128_to_double(a); }

lf_float128 __floatunsitf(uint32_t a)  { return lf_f128_from_double((double)a); }
uint32_t    __fixunstfsi(lf_float128 a) { double d = lf_f128_to_double(a); return d < 0.0 ? 0 : (uint32_t)d; }

lf_float128 __floatunditf(uint64_t a)  { return lf_f128_from_double((double)a); }
uint64_t    __fixunstfdi(lf_float128 a) { double d = lf_f128_to_double(a); return d < 0.0 ? 0 : (uint64_t)d; }
...
/* -----------------------------------------------------------------------
 * format_float128_fortran(result, val)
 *
 * Converts an lf_float128 to a 34-significant-digit decimal string in
 * Fortran scientific notation: [-]d.dddddddddddddddddddddddddddddddddE+eee
 *
 * On Linux (LFORTRAN_HAVE_REAL128): delegates to quadmath_snprintf.
 *
 * On macOS / Windows: pure integer binary-to-decimal conversion on the
 * 113-bit IEEE 754 binary128 significand. No double, no padding hacks.
 *
 * Algorithm:
 *   value = significand * 2^bin_exp
 *
 *   bin_exp >= 0: exact_int = significand << bin_exp (left shift)
 *   bin_exp <  0: exact_int = significand * 5^|bin_exp|, dec_exp -= |bin_exp|
 *
 *   Extract 34 decimal digits from exact_int by dividing by 10.
 * --------------------------------------------------------------------- */

/* 256-bit unsigned integer: w[0]=LSW ... w[3]=MSW */
typedef struct { uint64_t w[4]; } u256;

static inline u256 u256_from_u128(u128 v) {
    u256 r; r.w[0]=v.lo; r.w[1]=v.hi; r.w[2]=0; r.w[3]=0; return r;
}
static inline u256 u256_add(u256 a, u256 b) {
    u256 r; uint64_t c=0;
    for(int i=0;i<4;i++){
        __uint128_t s=(__uint128_t)a.w[i]+b.w[i]+c;
        r.w[i]=(uint64_t)s; c=(uint64_t)(s>>64);
    }
    return r;
}
static inline u256 u256_shl1(u256 a) {
    u256 r;
...
static inline u256 u256_mul5(u256 a) {
    /* a*5 = a*4 + a */
    u256 r; uint64_t c=0;
    for(int i=0;i<4;i++){
        __uint128_t s=(__uint128_t)a.w[i]*5+c;
        r.w[i]=(uint64_t)s; c=(uint64_t)(s>>64);
    }
    return r;
}
static inline uint32_t u256_divmod10(u256 *a) {
    uint64_t rem=0;
    for(int i=3;i>=0;i--){
        __uint128_t cur=((__uint128_t)rem<<64)|a->w[i];
        a->w[i]=(uint64_t)(cur/10);
        rem=(uint64_t)(cur%10);
    }
    return (uint32_t)rem;
}
static inline int u256_is_zero(u256 a){return !a.w[0]&&!a.w[1]&&!a.w[2]&&!a.w[3];}
...
static void format_float128_fortran(char* result, lf_float128 val) {
    ...
    if (ep) {
        ...
        sprintf(result,"%.*sE%c%03d",w,buf,es=='-'?'-':'+',ev<0?-ev:ev);
    } else { strcpy(result,buf); }
#else
    /* Unpack */
    uint64_t lo,hi;
    memcpy(&lo,val.bytes,8); memcpy(&hi,val.bytes+8,8);
    int      vsign  = (int)(hi>>63);
    uint64_t biased = (hi>>48)&0x7FFF;
    uint64_t mhi    = hi&0x0000FFFFFFFFFFFFULL;
    uint64_t mlo    = lo;
    ...
    if (!biased&&!mhi&&!mlo) { sprintf(result,"%s0.E+000",vsign?"-":""); return; }

    /* 113-bit significand with implicit leading 1 */
    u128 sig; sig.hi = mhi|(biased?(1ULL<<48):0ULL); sig.lo = mlo;

    /* value = sig * 2^bin_exp  (sig is a 113-bit integer) */
    int32_t bin_exp = biased ? (int32_t)biased-16383-112 : 1-16383-112;

    u256    num;
    int32_t dec_adj = 0;

    if (bin_exp >= 0) {
        num = u256_from_u128(sig);
        for (int i=0; i<bin_exp; i++) num = u256_shl1(num);
    } else {
        dec_adj = bin_exp; /* negative: value = num * 10^dec_adj */
        num = u256_from_u128(sig);
        for (int i=0; i<-bin_exp; i++) num = u256_mul5(num);
    }

    /* Count total decimal digits of num BEFORE extracting any digits.
     * dec_exp = (total_digits - 1) + dec_adj.
     * Must happen before extraction since divmod10 destroys num. */
    int total_digits;
    {
        int clz;
        if      (num.w[3]) clz = (int)__builtin_clzll(num.w[3]);
        else if (num.w[2]) clz = 64  + (int)__builtin_clzll(num.w[2]);
        else if (num.w[1]) clz = 128 + (int)__builtin_clzll(num.w[1]);
        else               clz = 192 + (int)__builtin_clzll(num.w[0]);
        int bits = 256 - clz;
        total_digits = (int)((unsigned)bits * 30103u / 100000u) + 1;
        /* Estimate can be off by +1: verify by building 10^(total_digits-1) */
        if (total_digits > 1) {
            u256 pw = {1,0,0,0};
            for (int i=0; i<total_digits-1; i++) {
                pw = u256_shl1(pw);                /* *2 */
                u256 pw4 = u256_shl1(u256_shl1(pw));  /* *4 more = *8 total */
                pw = u256_add(pw, pw4);            /* *2 + *8 = *10 */
            }
            /* if pw > num, over-estimated */
            int over=0;
            for(int i=3;i>=0;i--){
                if(pw.w[i]>num.w[i]){over=1;break;}
                if(pw.w[i]<num.w[i]) break;
            }
            if(over) total_digits--;
        }
    }
    int32_t dec_exp = (int32_t)(total_digits - 1) + dec_adj;

    /* Extract 34 digits (LSD first) + 1 rounding digit */
    char digs[36]; int nd=0;
    for (int i=0;i<34;i++) digs[nd++]=(char)('0'+u256_divmod10(&num));
    uint32_t rdig = u256_divmod10(&num);
    ...
    if(vsign) *p++='-';
    *p++=digs[nd-1]; *p++='.';
    for(int i=nd-2;i>=0;i--) *p++=digs[i];
    char es=dec_exp>=0?'+':'-';
    int  ea=dec_exp>=0?dec_exp:-dec_exp;
    sprintf(p,"E%c%03d",es,ea);
#endif
}
