...
std::string highlight_line(const std::string &line, const size_t first_column,
                           const size_t last_column, bool use_colors) {
...
std::string Diagnostics::render(const std::string &input,
  ...
  for (auto &d : this->diagnostics) {
    ...
    if (compiler_options.error_format == "human") {
      out += render_diagnostic_human(d, input, lm, compiler_options.use_colors,
  ...
  if (compiler_options.error_format == "human") {
    if (this->diagnostics.size() > 0 && !compiler_options.no_error_banner) {
      if (!compiler_options.no_warnings || has_error()) {
        ...
        out += "\n\n";
        out += bold + "Note" + reset +
               ": if any of the above error or warning messages are not clear "
               "or are lacking\n";
        out +=
            "context please report it to us (we consider that a bug that needs "
            "to be fixed).\n";
      }
    }
  }
...
void populate_span(diag::Span &s, const LocationManager &lm,
  ...
  lm.pos_to_linecol(lm.output_to_input_pos(s.loc.first, false), s.first_line,
                    s.first_column);
  lm.pos_to_linecol(lm.output_to_input_pos(s.loc.last, true), s.last_line,
                    s.last_column);
...
std::string render_diagnostic_human(Diagnostic &d, const std::string &input,
                                    const LocationManager &lm, bool use_colors,
...
std::string render_diagnostic_human(const Diagnostic &d, bool use_colors) {
  ...
  out << type_color << message_type << reset << bold << ": " << d.message
      << reset << std::endl;
  ...
  if (d.labels.size() > 0) {
    ...
    out << std::string(line_num_width, ' ') << blue_bold << "-->" << reset
        << " " << s.filename << ":" << s.first_line << ":" << s.first_column;
      ...
      for (size_t i = 0; i < l.spans.size(); i++) {
        ...
        if (i >= 1) {
          if (s0.first_line == s0.last_line) {
            ...
            if (s2.first_line == s2.last_line &&
                s2.first_line == s0.first_line) {
        ...
        if (s0.first_line == s0.last_line) {
          ...
          out << blue_bold << std::setw(line_num_width)
              << std::to_string(s0.first_line) << " |" << reset << " " << line
              << std::endl;
          out << std::string(line_num_width + 1, ' ') << blue_bold << "|"
              << reset << " ";
          out << std::string(s0.first_column - 1, ' ');
          out << color
              << std::string(s0.last_column - s0.first_column + 1, symbol);
        } else {
          if (s0.first_line < s0.last_line) {
            out << std::string(line_num_width + 1, ' ') << blue_bold << "|"
                << reset << std::endl;
            ...
            out << "   " + std::string(s0.first_column - 1, ' ');
            int64_t repeat =
                (int64_t)line.size() - (int64_t)s0.first_column + 1;
            if (repeat > 0) {
