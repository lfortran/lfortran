...
inline void load_serialised_asr(const std::string &s, std::string& asr_binary, const std::string &filename) {
#ifdef WITH_LFORTRAN_BINARY_MODFILES
    BinaryReader b(s);
#else
    TextReader b(s);
    ...
    if (file_type != lfortran_modfile_type_string) {
        // Print the error and communicate the issue back to the main program
        std::cerr << "Error: LCompilers Modfile format not recognized for file: " << filename << std::endl;
        throw LCompilersException("LCompilers Modfile format not recognized for file: " + filename);
    }
    
    std::string version = b.read_string();
    if (version != LFORTRAN_VERSION) {
        // Print the error and communicate the issue back to the main program
        std::cerr << "Error: Incompatible format for file: " << filename 
                  << ". LFortran Modfile was generated using version '" 
                  << version << "', but current LFortran version is '" 
                  << LFORTRAN_VERSION << "'" << std::endl;
        throw LCompilersException("Incompatible format for file: " + filename + ". LFortran Modfile was generated using version '" + version + "', but current LFortran version is '" + LFORTRAN_VERSION + "'");
    }
    
    // Read the binary string and set the output
