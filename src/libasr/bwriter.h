...
class BinaryWriter {
 ...
 public:
  ...
  void write_int32(uint32_t i) { s.append(uint32_to_string(i)); }

  void write_int64(uint64_t i) { s.append(uint64_to_string(i)); }
...
class BinaryReader {
 ...
 public:
  ...
  uint8_t read_int8() {
    if (pos + 1 > s.size()) {
      throw LCompilersException(
          "read_int8: String is too short for deserialization.");
    }
    uint8_t n = s[pos];
    pos += 1;
    return n;
  ...
  uint32_t read_int32() {
    if (pos + 4 > s.size()) {
      throw LCompilersException(
          "read_int32: String is too short for deserialization.");
    }
  ...
  uint64_t read_int64() {
    if (pos + 8 > s.size()) {
      throw LCompilersException(
          "read_int64: String is too short for deserialization.");
    }
  ...
  std::string read_string() {
    ...
    if (pos + n > s.size()) {
      throw LCompilersException(
          "read_string: String is too short for deserialization.");
    }
 ...
 public:
  ...
  double read_float64() {
    ...
    while (s[pos] != ' ') {
      ...
      if (pos >= s.size()) {
        throw LCompilersException(
            "read_float64: String is too short for deserialization.");
      }
    }
    pos++;
