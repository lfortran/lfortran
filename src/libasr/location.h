...
struct LocationManager {
  ...
  //       gets expanded to:
  //           sin(1.5*x)+cos(3.5*y)
  //       So the single input code interval `X(1.5, 3.5)` corresponds to an
  //       interval `sin(1.5*x)+cos(3.5*y)`, but we do not specify any more
  //       detailed correspondence on a character basis. If for example `x` is
  //       defined but `y` is undefined, then the compiler error message can for
