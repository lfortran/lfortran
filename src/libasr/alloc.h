...
inline size_t align(size_t n) { return (n + ALIGNMENT - 1) & ~(ALIGNMENT - 1); }
...
class Allocator {
 ...
 public:
  ...
  size_t size_current() { return current_pos - (size_t)start; }

  size_t size_total() { return size; }

  size_t num_chunks() { return blocks.size(); }
};
