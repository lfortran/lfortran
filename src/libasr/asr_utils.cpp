...
void extract_module_python(
  ...
  for (auto &a : m.m_global_scope->get_scope()) {
    if (ASR::is_a<ASR::Module_t>(*a.second)) {
      if (a.first == "__main__") {
        module_found = true;
        children_modules.push_back(std::make_pair(
            module_name, ASR::down_cast<ASR::Module_t>(a.second)));
      } else {
        children_modules.push_back(
            std::make_pair(a.first, ASR::down_cast<ASR::Module_t>(a.second)));
      }
    }
  }
  if (!module_found) {
...
ASR::Module_t *load_module(
  ...
  ASR::TranslationUnit_t *mod1 =
      find_and_load_module(al, module_name, *symtab, intrinsic, rl_path);
  if (mod1 == nullptr && !intrinsic) {
    // Module not found as a regular module. Try intrinsic module
    if (module_name == "iso_c_binding" || module_name == "iso_fortran_env" ||
  ...
  if (mod1 == nullptr) {
    err("Module '" + module_name +
            "' not declared in the current source and the modfile was not "
            "found",
        loc);
  }
  ...
  ASR::TranslationUnit_t *tu = ASR::down_cast2<ASR::TranslationUnit_t>(
      ASR::make_TranslationUnit_t(al, loc, symtab, nullptr, 0));

  // Load any dependent modules recursively
  bool rerun = true;
  while (rerun) {
    rerun = false;
    std::vector<std::string> modules_list = determine_module_dependencies(*tu);
    for (auto &item : modules_list) {
      if (symtab->get_symbol(item) == nullptr) {
        ...
        ASR::TranslationUnit_t *mod1 =
            find_and_load_module(al, item, *symtab, is_intrinsic, rl_path);
        if (mod1 == nullptr && !is_intrinsic) {
          // Module not found as a regular module. Try intrinsic module
          if (item == "iso_c_binding" || item == "iso_fortran_env") {
  ...
  for (auto &item : modules_list) {
    if (symtab->get_symbol(item) == nullptr) {
      err("ICE: Module '" + item + "' modfile was not found, but should have",
          loc);
    }
  }
...
void set_intrinsic(ASR::symbol_t *sym) {
  switch (sym->type) {
    ...
    case ASR::symbolType::DerivedType: {
      ASR::DerivedType_t *derived_type_sym =
          ASR::down_cast<ASR::DerivedType_t>(sym);
...
ASR::asr_t *getDerivedRef_t(Allocator &al, const Location &loc,
  ...
  switch (member_type->type) {
    case ASR::ttypeType::Derived: {
      ...
      ASR::symbol_t *der_type_sym =
          current_scope->resolve_symbol(der_type_name);
      if (der_type_sym == nullptr) {
        ...
        if (m_external->type == ASR::symbolType::ExternalSymbol) {
          ASR::ExternalSymbol_t *m_ext =
              (ASR::ExternalSymbol_t *)(&(m_external->base));
          m_external = m_ext->m_external;
          module_name = m_ext->m_module_name;
        }
        Str mangled_name;
        mangled_name.from_str(al, "1_" + std::string(module_name) + "_" +
        ...
        if (current_scope->get_symbol(mangled_name.str()) == nullptr) {
          ...
          if (current_scope->get_symbol(std::string(der_type_name)) !=
              nullptr) {
            der_tmp = current_scope->get_symbol(std::string(der_type_name));
            if (der_tmp->type == ASR::symbolType::ExternalSymbol) {
              ASR::ExternalSymbol_t *der_ext_tmp =
                  (ASR::ExternalSymbol_t *)(&(der_tmp->base));
          ...
          if (make_new_ext_sym) {
            der_ext = (ASR::symbol_t *)ASR::make_ExternalSymbol_t(
        ...
        ASR::asr_t *der_new =
            ASR::make_Derived_t(al, loc, der_ext, der->m_dims, der->n_dims);
        member_type = ASRUtils::TYPE(der_new);
      } else if (ASR::is_a<ASR::ExternalSymbol_t>(*der_type_sym)) {
        member_type = ASRUtils::TYPE(ASR::make_Derived_t(
            al, loc, der_type_sym, der->m_dims, der->n_dims));
      }
      break;
    }
    default:
  ...
  return ASR::make_DerivedRef_t(al, loc, LFortran::ASRUtils::EXPR(v_var),
        ...
        case ASR::symbolType::Function: {
          ...
          if (func->n_args == 2) {
            ...
            if (left_arg_type->type == left_type->type &&
              ...
              ASR::symbol_t *a_name =
                  curr_scope->resolve_symbol(matched_func_name);
              if (a_name == nullptr) {
                err("Unable to resolve matched function for operator "
                    "overloading, " +
                        matched_func_name,
                    loc);
              }
              ...
              asr = ASR::make_FunctionCall_t(al, loc, a_name, sym, a_args.p, 2,
...
bool is_op_overloaded(ASR::binopType op, std::string &intrinsic_op_name,
  ...
  switch (op) {
    ...
    default: {
      throw LCompilersException("Binary operator '" +
    ...
    for (size_t i = 0; i < gen_proc->n_procs && !found; i++) {
      ...
      if (subrout->n_args == 2) {
        ...
        if (target_arg_type->type == target_type->type &&
          ...
          if (a_name == nullptr) {
            err("Unable to resolve matched subroutine for assignment "
                "overloading, " +
                    matched_subrout_name,
                loc);
          }
          asr = ASR::make_SubroutineCall_t(al, loc, a_name, sym, a_args.p, 2,
                                           nullptr);
        }
      }
    }
...
bool is_parent(ASR::DerivedType_t *a, ASR::DerivedType_t *b) {
  ...
  while (current_parent) {
    ...
    current_parent =
        ASR::down_cast<ASR::DerivedType_t>(current_parent)->m_parent;
  }
  return false;
}
    ...
    switch (a.type) {
      ...
      case (ASR::ttypeType::Derived): {
        ...
        ASR::DerivedType_t *a2_type = ASR::down_cast<ASR::DerivedType_t>(
            ASRUtils::symbol_get_past_external(a2->m_derived_type));
        ASR::DerivedType_t *b2_type = ASR::down_cast<ASR::DerivedType_t>(
            ASRUtils::symbol_get_past_external(b2->m_derived_type));
        return a2_type == b2_type;
      }
      case (ASR::ttypeType::Class): {
        ASR::Class_t *a2 = ASR::down_cast<ASR::Class_t>(&a);
        ASR::Class_t *b2 = ASR::down_cast<ASR::Class_t>(&b);
        ASR::symbol_t *a2_typesym =
            ASRUtils::symbol_get_past_external(a2->m_class_type);
        ASR::symbol_t *b2_typesym =
            ASRUtils::symbol_get_past_external(b2->m_class_type);
        if (a2_typesym->type != b2_typesym->type) {
          return false;
        }
        if (a2_typesym->type == ASR::symbolType::ClassType) {
          ASR::ClassType_t *a2_type =
              ASR::down_cast<ASR::ClassType_t>(a2_typesym);
          ASR::ClassType_t *b2_type =
              ASR::down_cast<ASR::ClassType_t>(b2_typesym);
          return a2_type == b2_type;
        } else if (a2_typesym->type == ASR::symbolType::DerivedType) {
          ASR::DerivedType_t *a2_type =
              ASR::down_cast<ASR::DerivedType_t>(a2_typesym);
          ASR::DerivedType_t *b2_type =
              ASR::down_cast<ASR::DerivedType_t>(b2_typesym);
...
template <typename T>
bool argument_types_match(const Vec<ASR::call_arg_t> &args, const T &sub) {
...
bool select_func_subrout(
    const ASR::symbol_t *proc, const Vec<ASR::call_arg_t> &args, Location &loc,
    const std::function<void(const std::string &, const Location &)> err) {
  bool result = false;
...
int select_generic_procedure(
  ...
  for (size_t i = 0; i < p.n_procs; i++) {
    if (ASR::is_a<ASR::ClassProcedure_t>(*p.m_procs[i])) {
      ...
      const ASR::symbol_t *proc =
          ASRUtils::symbol_get_past_external(clss_fn->m_proc);
  ...
  if (raise_error) {
    err("Arguments do not match for any generic procedure, " +
            std::string(p.m_name),
        loc);
  }
  return -1;
...
ASR::asr_t *symbol_resolve_external_generic_procedure_without_eval(
  ...
  bool is_subroutine =
      ASR::down_cast<ASR::Function_t>(final_sym)->m_return_var == nullptr;
  ASR::ttype_t *return_type = nullptr;
  ...
  if (is_subroutine) {
    return ASR::make_SubroutineCall_t(al, loc, final_sym, v, args.p,
                                      args.size(), nullptr);
  } else {
...
ASR::asr_t *make_Cast_t_value(Allocator &al, const Location &a_loc,
  ...
  if (ASRUtils::expr_value(a_arg)) {
    ...
    if (a_kind == ASR::cast_kindType::RealToInteger) {
      int64_t v =
          ASR::down_cast<ASR::RealConstant_t>(ASRUtils::expr_value(a_arg))->m_r;
      ...
      double double_value =
          ASR::down_cast<ASR::RealConstant_t>(ASRUtils::expr_value(a_arg))->m_r;
      value = ASR::down_cast<ASR::expr_t>(
          ASR::make_ComplexConstant_t(al, a_loc, double_value, 0, a_type));
    } else if (a_kind == ASR::cast_kindType::IntegerToReal) {
      // TODO: Clashes with the pow functions
      // int64_t value =
      // ASR::down_cast<ASR::ConstantInteger_t>(ASRUtils::expr_value(a_arg))->m_n;
      // value = ASR::down_cast<ASR::expr_t>(ASR::make_ConstantReal_t(al, a_loc,
      // (double)v, a_type));
    } else if (a_kind == ASR::cast_kindType::IntegerToComplex) {
      int64_t int_value =
          ASR::down_cast<ASR::IntegerConstant_t>(ASRUtils::expr_value(a_arg))
              ->m_n;
      value = ASR::down_cast<ASR::expr_t>(
          ASR::make_ComplexConstant_t(al, a_loc, (double)int_value, 0, a_type));
    } else if (a_kind == ASR::cast_kindType::IntegerToInteger) {
      // TODO: implement
      // int64_t v =
      // ASR::down_cast<ASR::ConstantInteger_t>(ASRUtils::expr_value(a_arg))->m_n;
      // value = ASR::down_cast<ASR::expr_t>(ASR::make_ConstantInteger_t(al,
      // a_loc, v, a_type));
    } else if (a_kind == ASR::cast_kindType::IntegerToLogical) {
      // TODO: implement
    } else if (a_kind == ASR::cast_kindType::ComplexToComplex) {
      ASR::ComplexConstant_t *value_complex =
          ASR::down_cast<ASR::ComplexConstant_t>(ASRUtils::expr_value(a_arg));
