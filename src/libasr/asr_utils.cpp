...
namespace ASRUtils
{
            ...
            if (ASR::is_a<ASR::Module_t>(*a.second)) {
                if (a.first == "__main__") {
                    ...
                    children_modules.push_back(
                        std::make_pair(module_name, ASR::down_cast<ASR::Module_t>(a.second)));
                } else {
                    children_modules.push_back(
                               ...
                               const std::function<void(const std::string&, const Location&)> err)
    {
        ...
        if (mod1 == nullptr && !intrinsic) {
            ...
            if (module_name == "iso_c_binding" || module_name == "iso_fortran_env"
                ...
                mod1 = find_and_load_module(
                    al, "lfortran_intrinsic_" + module_name, *symtab, true, rl_path);
            }
        }
        if (mod1 == nullptr) {
            err("Module '" + module_name
                    + "' not declared in the current source and the modfile was not found",
                loc);
        }
                ...
                if (symtab->get_symbol(item) == nullptr) {
                    ...
                    if (mod1 == nullptr && !is_intrinsic) {
                        ...
                        if (item == "iso_c_binding" || item == "iso_fortran_env") {
                            mod1 = find_and_load_module(
                                al, "lfortran_intrinsic_" + item, *symtab, true, rl_path);
                        }
                    }
                                                 ...
                                                 const std::string& rl_path)
    {
        ...
        std::string modfile;
        if (!read_file(modfilename, modfile))
            return nullptr;
                                ...
                                SymbolTable* current_scope)
    {
        ...
        switch (member_type->type) {
            case ASR::ttypeType::Derived: {
                ...
                if (der_type_sym == nullptr) {
                    ...
                    Str mangled_name;
                    mangled_name.from_str(
                        al, "1_" + std::string(module_name) + "_" + std::string(der_type_name));
                    ...
                    member_type = ASRUtils::TYPE(
                        ASR::make_Derived_t(al, loc, der_type_sym, der->m_dims, der->n_dims));
                }
                break;
            }
            default:
        ...
        return ASR::make_DerivedRef_t(
            al, loc, LFortran::ASRUtils::EXPR(v_var), member, member_type, nullptr);
    }
                    ...
                    case ASR::symbolType::Function: {
                        ...
                        if (func->n_args == 2) {
                            ...
                            if (left_arg_type->type == left_type->type
                                ...
                                if (a_name == nullptr) {
                                    err("Unable to resolve matched function for operator overloading, "
                                            + matched_func_name,
                                        loc);
                                }
                                ...
                                asr = ASR::make_FunctionCall_t(al,
                                                               loc,
                                                               a_name,
                                                               sym,
                                                               a_args.p,
                          ...
                          SymbolTable* curr_scope)
    {
        ...
        switch (op) {
            ...
            default: {
                throw LCompilersException("Binary operator '" + ASRUtils::binop_to_str_python(op)
                                          + "' not supported yet");
            }
        }
            ...
            for (size_t i = 0; i < gen_proc->n_procs && !found; i++) {
                ...
                if (subrout->n_args == 2) {
                    ...
                    if (target_arg_type->type == target_type->type
                        ...
                        if (a_name == nullptr) {
                            err("Unable to resolve matched subroutine for assignment overloading, "
                                    + matched_subrout_name,
                                loc);
                        }
                        asr = ASR::make_SubroutineCall_t(
                            al, loc, a_name, sym, a_args.p, 2, nullptr);
                    }
                }
            }
        }
    ...
    bool types_equal(const ASR::ttype_t& a, const ASR::ttype_t& b)
    {
        ...
        } else if (a.type == ASR::ttypeType::Derived && b.type == ASR::ttypeType::Class) {
            ASR::Derived_t* a2 = ASR::down_cast<ASR::Derived_t>(&a);
            ASR::Class_t* b2 = ASR::down_cast<ASR::Class_t>(&b);
        ...
        } else if (a.type == ASR::ttypeType::Class && b.type == ASR::ttypeType::Derived) {
            ASR::Class_t* a2 = ASR::down_cast<ASR::Class_t>(&a);
            ASR::Derived_t* b2 = ASR::down_cast<ASR::Derived_t>(&b);
    ...
    bool select_func_subrout(const ASR::symbol_t* proc,
        ...
        const std::function<void(const std::string&, const Location&)> err)
    {
        ...
        if (is_subroutine) {
            return ASR::make_SubroutineCall_t(al, loc, final_sym, v, args.p, args.size(), nullptr);
        } else {
            return ASR::make_FunctionCall_t(
                al, loc, final_sym, v, args.p, args.size(), return_type, nullptr, nullptr);
        }
    }
                                  ...
                                  ASR::ttype_t* a_type)
    {
        ...
        if (ASRUtils::expr_value(a_arg)) {
            ...
            if (a_kind == ASR::cast_kindType::RealToInteger) {
                int64_t v = ASR::down_cast<ASR::RealConstant_t>(ASRUtils::expr_value(a_arg))->m_r;
                value = ASR::down_cast<ASR::expr_t>(
                    ASR::make_IntegerConstant_t(al, a_loc, v, a_type));
            } else if (a_kind == ASR::cast_kindType::RealToReal) {
                double v = ASR::down_cast<ASR::RealConstant_t>(ASRUtils::expr_value(a_arg))->m_r;
                value = ASR::down_cast<ASR::expr_t>(ASR::make_RealConstant_t(al, a_loc, v, a_type));
                ...
                // int64_t value =
                // ASR::down_cast<ASR::ConstantInteger_t>(ASRUtils::expr_value(a_arg))->m_n; value =
                // ASR::down_cast<ASR::expr_t>(ASR::make_ConstantReal_t(al, a_loc, (double)v,
                // a_type));
            } else if (a_kind == ASR::cast_kindType::IntegerToComplex) {
                int64_t int_value
                    = ASR::down_cast<ASR::IntegerConstant_t>(ASRUtils::expr_value(a_arg))->m_n;
                value = ASR::down_cast<ASR::expr_t>(
                ...
                // int64_t v =
                // ASR::down_cast<ASR::ConstantInteger_t>(ASRUtils::expr_value(a_arg))->m_n; value =
                // ASR::down_cast<ASR::expr_t>(ASR::make_ConstantInteger_t(al, a_loc, v, a_type));
