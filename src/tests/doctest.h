...
DOCTEST_INTERFACE String
toString(int short in);
DOCTEST_INTERFACE String
toString(int short unsigned in);
DOCTEST_INTERFACE String
toString(int in);
DOCTEST_INTERFACE String
toString(int unsigned in);
DOCTEST_INTERFACE String
toString(int long in);
DOCTEST_INTERFACE String
toString(int long unsigned in);
DOCTEST_INTERFACE String
toString(int long long in);
DOCTEST_INTERFACE String
toString(int long long unsigned in);
DOCTEST_INTERFACE String
toString(std::nullptr_t in);
...
public:
    ...
    explicit Approx(const T& value,
                    typename detail::enable_if<
                        std::is_constructible<double, T>::value>::type* = static_cast<T*>(nullptr))
    {
...
#define DOCTEST_DO_BINARY_EXPRESSION_COMPARISON(op, op_str, op_macro)                              \
    ...
    template <typename R,                                                                          \
              typename enable_if<!doctest::detail::is_rvalue_reference<R>::value,                  \
                                 void>::type* = nullptr>                                           \
    DOCTEST_NOINLINE SFINAE_OP(Result, op) operator op(const R& rhs)                               \
    {                                                                                              \
...
#define DOCTEST_CMP_LE(l, r) le(l, r)
    ...
    struct Expression_lhs {
        ...
        DOCTEST_FORBIT_EXPRESSION(Expression_lhs, ^=)
        DOCTEST_FORBIT_EXPRESSION(Expression_lhs, |=)
        // these 2 are unfortunate because they should be allowed - they have higher precedence over
        // the comparisons, but the ExpressionDecomposer class uses the left shift operator to
...
#define DOCTEST_TEST_CASE_FIXTURE(x, name)                                                         \
    DOCTEST_IMPLEMENT_FIXTURE(                                                                     \
        DOCTEST_ANONYMOUS(DOCTEST_ANON_CLASS_), x, DOCTEST_ANONYMOUS(DOCTEST_ANON_FUNC_), name)
...
namespace
{
    ...
    struct FatalConditionHandler {
        ...
        {
            // Multiple threads may enter this filter/handler at once. We want the error message to
            // be printed on the console just once no matter how many threads have crashed.
    ...
    struct ConsoleReporter : public IReporter {
        ...
        void test_run_end(const TestRunStats& p) override
        {
            ...
            auto totwidth = int(std::ceil(
                log10((std::max(p.numTestCasesPassingFilters, static_cast<unsigned>(p.numAsserts)))
                      + 1)));
            auto passwidth = int(
