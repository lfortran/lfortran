...
public:
    ...
    std::map<std::string, std::map<std::string, std::map<std::string, ClassProcInfo>>>
        class_procedures;
    std::map<std::string, std::map<std::string, std::map<std::string, Location>>>
        class_deferred_procedures;
    ...
    void visit_TranslationUnit(const AST::TranslationUnit_t& x)
    {
        ...
        // for asr_owner.
        ASR::asr_t* tmp0
            = ASR::make_TranslationUnit_t(al, x.base.base.loc, current_scope, nullptr, 0);
        ...
        if (pre_declared_array_dims.size() > 0) {
            ...
            if (!sym_name.empty()) {
                diag.add(diag::Diagnostic(sym_name + " is/are used as dimensions but not declared",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
                                          { diag::Label("", { x.base.base.loc }) }));
                throw SemanticAbort();
        ...
        for (TypeMissingData* data : type_info) {
            ...
            if (expr) {
                switch (data->type->type) {
                        ...
                        if (expr->type == ASR::exprType::FunctionCall) {
                            ...
                            for (size_t i = 0; i < call->n_args; i++) {
                                if (ASR::is_a<ASR::Var_t>(*call->m_args[i].m_value)) {
                                    ASR::Variable_t* v
                                        = ASRUtils::EXPR2VAR(call->m_args[i].m_value);
                            ...
                            if (data->sym_type == (int64_t) ASR::symbolType::Function) {
                                ...
                                if (func_type->m_return_var_type) {
                                    ASRUtils::ReplaceWithFunctionParamVisitor replacer(
                                        al, func->m_args, func->n_args);
                                    func_type->m_return_var_type
                                        = replacer.replace_args_with_FunctionParam(data->type,
                                                                                   data->scope);
                                }
                            }
                        }
                    ...
                    default: {
                        diag.add(diag::Diagnostic("Only String type is supported as of now.",
                                                  diag::Level::Error,
                                                  diag::Stage::Semantic,
            ...
            if (sym && ASR::is_a<ASR::Variable_t>(*sym)) {
                ...
                ASRUtils::collect_variable_dependencies(al,
                                                        ...
                                                        sym_variable->m_type,
                                                        sym_variable->m_symbolic_value,
                                                        sym_variable->m_value);
        ...
        for (auto& itr : node2deps) {
            if (ASR::is_a<ASR::symbol_t>(*itr.first)) {
                ...
                if (ASR::is_a<ASR::Function_t>(*asr_owner_sym)) {
                    SetChar func_deps;
                    ASR::Function_t* asr_owner_func
                        = ASR::down_cast<ASR::Function_t>(asr_owner_sym);
                    func_deps.from_pointer_n_copy(
                        al, asr_owner_func->m_dependencies, asr_owner_func->n_dependencies);
    ...
    void fix_struct_type(SymbolTable* symtab)
    {
        for (auto& itr : symtab->get_scope()) {
            ...
            if (!ASR::is_a<ASR::Variable_t>(*sym) && !ASR::is_a<ASR::Struct_t>(*sym)) {
                continue;
            }
            ...
            ASR::ttype_t* sym_type = ASRUtils::type_get_past_pointer(ASRUtils::symbol_type(sym));
            if (ASR::is_a<ASR::StructType_t>(*sym_type)) {
                ...
                if (ASR::is_a<ASR::ExternalSymbol_t>(*der_sym)
                    ...
                    std::string derived_type_name
                        = ASR::down_cast<ASR::ExternalSymbol_t>(der_sym)->m_name;
                    ASR::symbol_t* sym_ = symtab->resolve_symbol(derived_type_name);
                    if (!sym_) {
                        diag.add(diag::Diagnostic("Derived type '" + derived_type_name
                                                      + "' not declared",
                                                  diag::Level::Error,
                                                  diag::Stage::Semantic,
    ...
    void populate_implicit_dictionary(Location& a_loc,
                                      std::map<std::string, ASR::ttype_t*>& implicit_dictionary)
    {
        for (char ch = 'i'; ch <= 'n'; ch++) {
            implicit_dictionary[std::string(1, ch)] = ASRUtils::TYPE(
                ASR::make_Integer_t(al, a_loc, compiler_options.po.default_integer_kind));
        }

        for (char ch = 'o'; ch <= 'z'; ch++) {
            implicit_dictionary[std::string(1, ch)]
                = ASRUtils::TYPE(ASR::make_Real_t(al, a_loc, 4));
        }
    ...
    template <typename T>
    void process_implicit_statements(const T& x,
                                     std::map<std::string, ASR::ttype_t*>& implicit_dictionary)
    {
        if (implicit_stack.size() > 0 && x.n_implicit == 0) {
            // We are inside a module and visiting a function / subroutine with no implicit
            // statement
            if (!is_interface) {
        ...
        for (size_t i = 0; i < x.n_implicit; i++) {
            ...
            if (AST::is_a<AST::ImplicitNone_t>(*x.m_implicit[i])) {
                ...
                for (size_t si = 0; si < implicit->n_specs; ++si) {
                    AST::ImplicitSpec_t* spec
                        = AST::down_cast<AST::ImplicitSpec_t>(implicit->m_specs[si]);
                    ...
                    if (attr_type->m_kind != nullptr) {
                        if (attr_type->n_kind == 1) {
                            ...
                            diag.add(diag::Diagnostic("Only one kind item supported for now",
                                                      diag::Level::Error,
                                                      diag::Stage::Semantic,
                                                      { diag::Label("", { x.base.base.loc }) }));
                            throw SemanticAbort();
                    ...
                    switch (ast_type) {
                        ...
                        case (AST::decl_typeType::TypeLogical): {
                            type = ASRUtils::TYPE(ASR::make_Logical_t(
                        ...
                        default:
                            diag.add(diag::Diagnostic("Return type not supported",
                                                      diag::Level::Error,
                                                      diag::Stage::Semantic,
                                                      { diag::Label("", { x.base.base.loc }) }));
                            throw SemanticAbort();
                    ...
                    for (size_t j = 0; j < spec->n_specs; j++) {
                        ...
                        AST::LetterSpec_t* letter_spec
                            = AST::down_cast<AST::LetterSpec_t>(spec->m_specs[j]);
    ...
    void visit_ModuleSubmoduleCommon(const T& x, std::string parent_name = "")
    {
        ...
        if (x.class_type == AST::modType::Submodule) {
            ASR::symbol_t* submod_parent = (ASR::symbol_t*) (ASRUtils::load_module(
                al,
                global_scope,
                parent_name,
                x.base.base.loc,
            ...
            if (!unsupported_sym_name.empty()) {
                throw LCompilersException("'" + unsupported_sym_name
                                          + "' is not supported yet for declaring with use.");
            }
        }
        ...
        if (parent_scope->get_symbol(sym_name) != nullptr) {
            diag.add(diag::Diagnostic("Module already defined",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { tmp->loc }) }));
            throw SemanticAbort();
    ...
    void visit_Program(const AST::Program_t& x)
    {
        ...
        generic_procedures.clear();
        current_module_dependencies.reserve(al, 4);
        Vec<size_t> procedure_decl_indices;
        procedure_decl_indices.reserve(al, 0);
        if (compiler_options.implicit_typing) {
            Location a_loc = x.base.base.loc;
            populate_implicit_dictionary(a_loc, implicit_dictionary);
            process_implicit_statements(x, implicit_dictionary);
        ...
        std::string sym_name = to_lower(x.m_name);
        if (parent_scope->get_symbol(sym_name) != nullptr) {
            diag.add(diag::Diagnostic("Program already defined",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { tmp->loc }) }));
            throw SemanticAbort();
    ...
    bool subroutine_contains_entry_function(std::string subroutine_name,
                                            AST::stmt_t** body,
                                            size_t n_body)
    {
    ...
    void update_duplicated_nodes(Allocator& al, SymbolTable* current_scope)
    {
        ...
        public:
            ...
            UpdateDuplicatedNodes(Allocator& al)
                : PassVisitor(al, nullptr)
            {
            }
            ...
            void visit_FunctionCall(const ASR::FunctionCall_t& x)
            {
                ...
                if (scope->counter == correct_scope->counter) {
                    ...
                    std::string func_call_origin_name
                        = ASRUtils::symbol_name(func_call->m_original_name);
                    sym = correct_scope->resolve_symbol(func_call_origin_name);
                    if (sym) {
                        func_call->m_original_name
                            = correct_scope->resolve_symbol(func_call_origin_name);
                    }
                }
    ...
    void create_template_entry_function(const Location& loc,
                                        std::string function_name,
                                        std::vector<AST::arg_t>& vector_args,
                                        ...
                                        std::string parent_function_name = "")
    {
        ...
        if (is_master) {
            ...
            ASR::symbol_t* entry_lcompilers_sym = ASR::down_cast<ASR::symbol_t>(
                ASRUtils::make_Variable_t_util(al,
                                               loc,
                                               current_scope,
                                               s2c(al, "entry__lcompilers"),
                                               nullptr,
                                               0,
                                               ASR::intentType::In,
                                               nullptr,
        ...
        for (auto it : vector_args) {
            ...
            if (arg) {
                ...
                diag.add(diag::Diagnostic("Alternate returns are not implemented yet",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
                                          { diag::Label("", { it.loc }) }));
                throw SemanticAbort();
        ...
        for (auto it : vector_args) {
            ...
            if (current_scope->get_symbol(arg_s) == nullptr) {
                if (compiler_options.implicit_typing) {
                    ...
                    if (t == nullptr) {
                        diag.add(diag::Diagnostic("Dummy argument '" + arg_s + "' not defined",
                                                  diag::Level::Error,
                                                  diag::Stage::Semantic,
                                                  { diag::Label("", { it.loc }) }));
                        throw SemanticAbort();
                    }
                    declare_implicit_variable2(it.loc, arg_s, ASRUtils::intent_unspecified, t);
                } else {
                    diag.add(diag::Diagnostic("Dummy argument '" + arg_s + "' not defined",
                                              diag::Level::Error,
                                              diag::Stage::Semantic,
            ...
            args.push_back(al, ASRUtils::EXPR(ASR::make_Var_t(al, it.loc, var)));
        }
        ...
        if (!return_var && is_function) {
            ...
            ASR::symbol_t* return_var_sym = ASR::down_cast<ASR::symbol_t>(
                ASRUtils::make_Variable_t_util(al,
                                               loc,
                                               current_scope,
                                               s2c(al, function_name),
                                               nullptr,
                                               0,
        ...
        for (auto& item : current_scope->get_scope()) {
            if (ASR::is_a<ASR::Function_t>(*item.second)) {
                ASR::Function_t* func
                    = ASR::down_cast<ASR::Function_t>(current_scope->resolve_symbol(item.first));
                update_duplicated_nodes(al, func->m_symtab);
            } else if (ASR::is_a<ASR::Variable_t>(*item.second)) {
                ASR::Variable_t* var
                    = ASR::down_cast<ASR::Variable_t>(current_scope->resolve_symbol(item.first));
                ...
                if (var_type->type == ASR::ttypeType::Array) {
                    ...
                    for (size_t i = 0; i < arr_type->n_dims; i++) {
                        ...
                        if (dim_length && ASR::is_a<ASR::Var_t>(*dim_length)) {
                            ...
                            ASR::symbol_t* dim_length_sym_in_scope
                                = current_scope->resolve_symbol(dim_length_sym_name);
                ...
                if (std::find(
                        current_procedure_args.begin(), current_procedure_args.end(), item.first)
    ...
    std::vector<AST::arg_t> perform_argument_mapping(const T& x, std::string sym_name)
    {
        ...
        for (size_t i = 0; i < x.n_args; i++) {
            ...
            if (it != arg_names_unique.end()) {
                ...
                if (entry_function_arguments_mapping.find(sym_name)
                    == entry_function_arguments_mapping.end()) {
        ...
        for (auto& it : entry_functions[sym_name]) {
            for (auto& arg : entry_function_args[it.first]) {
                ...
                if (it2 != arg_names_unique.end()) {
                    ...
                    if (entry_function_arguments_mapping.find(it.first)
                        == entry_function_arguments_mapping.end()) {
        ...
        for (auto& arg : arg_names_unique) {
            AST::arg_t a;
            a.loc = x.base.base.loc;
            a.m_arg = s2c(al, arg);
            master_args.push_back(a);
        }
    ...
    void visit_Subroutine(const AST::Subroutine_t& x)
    {
        ...
        if (x.n_temp_args > 0) {
            ...
            ASR::asr_t* temp = ASR::make_Template_t(al,
                                                    x.base.base.loc,
                                                    current_scope,
                                                    s2c(al, sym_name),
                                                    temp_args.p,
            ...
            if (current_scope->get_symbol(arg_s) == nullptr) {
                if (compiler_options.implicit_typing) {
                    ...
                    declare_implicit_variable2(
                        x.base.base.loc, arg_s, ASRUtils::intent_unspecified, t);
                } else {
                    diag.add(diag::Diagnostic("Dummy argument '" + arg_s + "' not defined",
                                              diag::Level::Error,
                                              diag::Stage::Semantic,
        ...
        if (parent_scope->get_symbol(sym_name) != nullptr) {
            ...
            if (ASR::is_a<ASR::Function_t>(*f1)) {
                ...
                if (ASRUtils::get_FunctionType(f2)->m_abi == ASR::abiType::Interactive
                    ...
                    diag.add(diag::Diagnostic("Subroutine already defined " + sym_name,
                                              diag::Level::Error,
                                              diag::Stage::Semantic,
                                              { diag::Label("", { tmp->loc }) }));
                    throw SemanticAbort();
                ...
                if (!ASR::is_a<ASR::GenericProcedure_t>(*f1_)) {
                    ...
                    f1_ = ASR::down_cast<ASR::symbol_t>(
                        ASR::make_GenericProcedure_t(al,
                                                     f1->base.loc,
                                                     parent_scope,
                                                     gp->m_name,
                                                     gp_procs.p,
                ...
                for (size_t igp = 0; igp < gp->n_procs; igp++) {
                    if (ASRUtils::symbol_get_past_external(gp->m_procs[igp])
                        == ASRUtils::symbol_get_past_external(
                            parent_scope->resolve_symbol(sym_name))) {
        ...
        if (subroutine_contains_entry_function(sym_name, x.m_body, x.n_body)) {
            ...
            */
            for (auto& entry_function : entry_functions[sym_name]) {
                create_template_entry_function(x.base.base.loc,
                                               entry_function.first,
                                               entry_function_args[entry_function.first],
                                               false,
                                               false,
                                               sym_name);
            }
                                      ...
                                      std::string& return_var_name)
    {
        ...
        if (n == 0 && compiler_options.implicit_interface && compiler_options.implicit_typing) {
            ...
            if (t == nullptr) {
                diag.add(diag::Diagnostic("No implicit return type available for `"
                                              + return_var_name + "`",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
                                          { diag::Label("", { loc }) }));
            ...
            switch (t->type) {
                ...
                default: {
                    diag.add(diag::Diagnostic("Implicit return type not supported yet",
                                              diag::Level::Error,
                                              diag::Stage::Semantic,
                                              { diag::Label("", { loc }) }));
                    throw SemanticAbort();
            ...
            AST::ast_t* r_ast = AST::make_AttrType_t(
                al, loc, ttype, nullptr, 0, nullptr, nullptr, AST::symbolType::None);
        ...
        for (size_t i = 0; i < n; i++) {
            if (AST::is_a<AST::AttrType_t>(*attributes[i])) {
                if (found) {
                    diag.add(diag::Diagnostic("Return type declared twice",
                                              diag::Level::Error,
                                              diag::Stage::Semantic,
                                              { diag::Label("", { loc }) }));
                    throw SemanticAbort();
    ...
    void visit_Function(const AST::Function_t& x)
    {
        ...
        if (x.m_return_var) {
            if (x.m_return_var->type == AST::exprType::Name) {
                ...
                diag.add(diag::Diagnostic("Return variable must be an identifier",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
        ...
        AST::AttrType_t* return_type
            = find_return_type(x.m_attributes, x.n_attributes, x.base.base.loc, return_var_name);
        if (current_scope->get_symbol(return_var_name) == nullptr) {
            ...
            if (!return_type) {
                diag.add(diag::Diagnostic("Return type not specified",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
                                          { diag::Label("", { x.base.base.loc }) }));
                throw SemanticAbort();
            ...
            if (return_type
                && !(x.n_attributes == 0 && compiler_options.implicit_typing
                     && compiler_options.implicit_interface)) {
                diag.add(diag::Diagnostic("Cannot specify the return type twice",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
                                          { diag::Label("", { x.base.base.loc }) }));
                throw SemanticAbort();
        ...
        if (parent_scope->get_symbol(sym_name) != nullptr) {
            ...
            if (ASR::is_a<ASR::ExternalSymbol_t>(*f1) && in_submodule) {
                ...
                if (ASRUtils::get_FunctionType(f2)->m_abi == ASR::abiType::Interactive) {
                    ...
                    diag.add(diag::Diagnostic("Function already defined",
                                              diag::Level::Error,
                                              diag::Stage::Semantic,
                                              { diag::Label("", { tmp->loc }) }));
                    throw SemanticAbort();
        ...
        if (subroutine_contains_entry_function(sym_name, x.m_body, x.n_body)) {
            ...
            */
            ...
            create_template_entry_function(
                x.base.base.loc, sym_name + "_main__lcompilers", master_args, true, true, sym_name);
        }
        if (x.n_temp_args > 0) {
            add_overloaded_procedures();
    ...
    void visit_Declaration(const AST::Declaration_t& x)
    {
        ...
        if (x.is_array) {
            for (auto& bound : x.array_bounds) {
                if (!is_compile_time_constant(bound)) {
                    diag.add(diag::Diagnostic("Explicit shaped array with nonconstant bounds",
                                              diag::Level::Error,
                                              diag::Stage::Semantic,
                                              { diag::Label("", { bound.loc }) }));
                    throw SemanticAbort();
                }
            }
        }
        ...
        if (x.m_type == AST::LFortranPragma) {
            ...
            if (startswith(t, "attributes ")) {
                ...
                if (startswith(t, "simd :: ")) {
                    ...
                    diag.add(diag::Diagnostic("Only `simd` attribute supported",
                                              diag::Level::Error,
                                              diag::Stage::Semantic,
                                              { diag::Label("", { x.base.base.loc }) }));
                    throw SemanticAbort();
                ...
                diag.add(diag::Diagnostic("Unsupported LFortran pragma type",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
                                          { diag::Label("", { x.base.base.loc }) }));
                throw SemanticAbort();
            ...
            diag.add(diag::Diagnostic("The pragma type not supported yet",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { x.base.base.loc }) }));
            throw SemanticAbort();
        ...
        for (auto& var : simd_variables) {
            ...
            if (s) {
                ...
                if (ASR::is_a<ASR::Array_t>(*t)) {
                    ...
                    diag.add(
                        diag::Diagnostic("The SIMD variable `" + var.first + "` must be an array",
                                         diag::Level::Error,
                                         diag::Stage::Semantic,
                ...
                diag.add(diag::Diagnostic("The SIMD variable `" + var.first + "` not declared",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
    ...
    void visit_DerivedType(const AST::DerivedType_t& x)
    {
            ...
            switch (x.m_attrtype[i]->type) {
                case AST::decl_attributeType::AttrExtends: {
                    if (attr_extend != nullptr) {
                        diag.add(
                            diag::Diagnostic("DerivedType can only extend one another DerivedType",
                                             diag::Level::Error,
                                             diag::Stage::Semantic,
                                             { diag::Label("", { x.base.base.loc }) }));
                        throw SemanticAbort();
        ...
        if ((is_requirement || is_template) && is_deferred) {
            ...
            tmp = ASRUtils::make_Variable_t_util(al,
                                                 x.base.base.loc,
                                                 current_scope,
                                                 s2c(al, dt_name),
                                                 nullptr,
        ...
        if (current_scope->get_symbol(sym_name) != nullptr) {
            diag.add(diag::Diagnostic("DerivedType already defined",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { x.base.base.loc }) }));
            throw SemanticAbort();
        ...
        if (attr_extend != nullptr) {
            ...
            if (parent_scope->get_symbol(parent_sym_name) == nullptr) {
                diag.add(diag::Diagnostic(parent_sym_name + " is not defined.",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
                                          { diag::Label("", { x.base.base.loc }) }));
                throw SemanticAbort();
        ...
        for (auto& item : current_scope->get_scope()) {
            ...
            if (item.first != "~abstract_type") {
                ASR::ttype_t* var_type
                    = ASRUtils::type_get_past_pointer(ASRUtils::symbol_type(item.second));
        ...
        tmp = ASR::make_Struct_t(al,
                                 x.base.base.loc,
                                 current_scope,
                                 s2c(al, to_lower(x.m_name)),
                                 struct_dependencies.p,
                                 struct_dependencies.size(),
                                 data_member_names.p,
    ...
    void visit_DerivedTypeProc(const AST::DerivedTypeProc_t& x)
    {
        for (size_t i = 0; i < x.n_symbols; i++) {
            AST::UseSymbol_t* use_sym = AST::down_cast<AST::UseSymbol_t>(x.m_symbols[i]);
            ...
            for (size_t i = 0; i < x.n_attr; i++) {
                switch (x.m_attr[i]->type) {
                    case AST::decl_attributeType::AttrPass: {
                        ...
                        LCOMPILERS_ASSERT(class_procedures[dt_name][use_sym_name].find("pass")
                                          == class_procedures[dt_name][use_sym_name].end());
                        class_procedures[dt_name][use_sym_name]["pass"].name
                            = (attr_pass->m_name) ? std::string(attr_pass->m_name) : "";
                        class_procedures[dt_name][use_sym_name]["pass"].loc
                            = attr_pass->base.base.loc;
                        break;
                    }
                    case AST::decl_attributeType::SimpleAttribute: {
                        auto& cdf = class_deferred_procedures;
                        AST::SimpleAttribute_t* attr
                            = AST::down_cast<AST::SimpleAttribute_t>(x.m_attr[i]);
                        if (attr->m_attr == AST::simple_attributeType::AttrDeferred) {
                            LCOMPILERS_ASSERT(cdf[dt_name][use_sym_name].find("deferred")
                                              == cdf[dt_name][use_sym_name].end());
                            cdf[dt_name][use_sym_name]["deferred"] = attr->base.base.loc;
                        } else if (attr->m_attr == AST::simple_attributeType::AttrNoPass) {
                            LCOMPILERS_ASSERT(cdf[dt_name][use_sym_name].find("nopass")
                                              == cdf[dt_name][use_sym_name].end());
    ...
    void fill_interface_proc_names(const AST::Interface_t& x, std::vector<std::string>& proc_names)
    {
        for (size_t i = 0; i < x.n_items; i++) {
            ...
            if (AST::is_a<AST::InterfaceModuleProcedure_t>(*item)) {
                ...
                AST::InterfaceProc_t* proc = AST::down_cast<AST::InterfaceProc_t>(item);
                switch (proc->m_proc->type) {
                    case AST::program_unitType::Subroutine: {
                        AST::Subroutine_t* subrout
                            = AST::down_cast<AST::Subroutine_t>(proc->m_proc);
                ...
                diag.add(diag::Diagnostic("Interface procedure type not imlemented yet",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
    ...
    void visit_Interface(const AST::Interface_t& x)
    {
        if (AST::is_a<AST::InterfaceHeaderName_t>(*x.m_header)) {
            std::string generic_name
                = to_lower(AST::down_cast<AST::InterfaceHeaderName_t>(x.m_header)->m_name);
            ...
            if (generic_procedures.find(generic_name) != generic_procedures.end()) {
                generic_procedures[generic_name].insert(
                    generic_procedures[generic_name].end(), proc_names.begin(), proc_names.end());
            } else {
            ...
            AST::intrinsicopType opType
                = AST::down_cast<AST::InterfaceHeaderOperator_t>(x.m_header)->m_op;
            std::vector<std::string> proc_names;
            fill_interface_proc_names(x, proc_names);
            ...
            std::string op_name = to_lower(
                AST::down_cast<AST::InterfaceHeaderDefinedOperator_t>(x.m_header)->m_operator_name);
            std::vector<std::string> proc_names;
            fill_interface_proc_names(x, proc_names);
            ...
            std::string op_name
                = to_lower(AST::down_cast<AST::InterfaceHeaderWrite_t>(x.m_header)->m_id);
            if (op_name != "formatted" && op_name != "unformatted") {
                diag.add(diag::Diagnostic("Can only be `formatted` or `unformatted`",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
            ...
            std::string op_name
                = to_lower(AST::down_cast<AST::InterfaceHeaderRead_t>(x.m_header)->m_id);
            if (op_name != "formatted" && op_name != "unformatted") {
                diag.add(diag::Diagnostic("Can only be `formatted` or `unformatted`",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
            ...
            diag.add(diag::Diagnostic("Interface type not imlemented yet",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { x.base.base.loc }) }));
            throw SemanticAbort();
    ...
    void add_custom_operator(std::pair<const std::string, std::vector<std::string>>& proc,
                             ASR::accessType access)
    {
        ...
        ASR::asr_t* v = ASR::make_CustomOperator_t(
            al, loc, current_scope, generic_name, symbols.p, symbols.size(), access);
    ...
    {
        for (auto& proc : overloaded_op_procs) {
            std::pair<const std::string, std::vector<std::string>> proc_
                = { intrinsic2str[proc.first], proc.second };
    ...
    void add_generic_procedures()
    {
        for (auto& proc : generic_procedures) {
            ...
            if (current_scope->resolve_symbol(generic_name)) {
                ...
                if (ASR::is_a<ASR::GenericProcedure_t>(*sym)) {
                    ...
                    for (size_t i = 0; i < gp->n_procs; i++) {
                        ASR::symbol_t* s
                            = current_scope->get_symbol(ASRUtils::symbol_name(gp->m_procs[i]));
                        if (s != nullptr) {
                            ...
                            s = m->m_symtab->get_symbol(ASRUtils::symbol_name(gp->m_procs[i]));
                            if (ASR::is_a<ASR::Function_t>(*s)) {
                                ASR::Function_t* fn = ASR::down_cast<ASR::Function_t>(s);
                                ASR::symbol_t* ep_s
                                    = (ASR::symbol_t*) ASR::make_ExternalSymbol_t(al,
                                                                                  fn->base.base.loc,
                                                                                  current_scope,
                                                                                  fn->m_name,
                                                                                  s,
            ...
            ASR::asr_t* v = ASR::make_GenericProcedure_t(
                al, loc, current_scope, generic_name, symbols.p, symbols.size(), ASR::Public);
    ...
    void add_generic_class_procedures()
    {
        for (auto& proc : generic_class_procedures) {
            ...
            loc.last = 1;
            ASR::Struct_t* clss
                = ASR::down_cast<ASR::Struct_t>(current_scope->get_symbol(proc.first));
            for (auto& pname : proc.second) {
                ...
                for (std::string& cand_proc : pname.second) {
                    if (clss->m_symtab->get_symbol(cand_proc) != nullptr) {
                        ...
                        diag.add(diag::Diagnostic(cand_proc + " doesn't exist inside " + proc.first
                                                      + " type",
                                                  diag::Level::Error,
                                                  diag::Stage::Semantic,
                                                  { diag::Label("", { loc }) }));
                ...
                if (operator_found || startswith(pname.first, "~def_op~")) {
                    ...
                    LCOMPILERS_ASSERT(strlen(generic_name) > 0);
                    v = ASR::make_CustomOperator_t(al,
                                                   loc,
                                                   clss->m_symtab,
                                                   generic_name,
                                                   cand_procs.p,
                    ...
                    v = ASR::make_GenericProcedure_t(
                        ...
                        cand_procs.p,
                        cand_procs.size(),
                        ASR::accessType::Public);  // Update the access as per the input Fortran
                                                   // code
                }
    ...
    void ensure_matching_types_for_pass_obj_dum_arg(ASR::Function_t* func,
                                                    char* pass_arg_name,
                                                    ASR::symbol_t* clss_sym,
                                                    Location& loc)
    {
            ...
            for (size_t i = 0; i < func->n_args && !is_pass_arg_name_found; i++) {
                ...
                if (strcmp(v->m_name, pass_arg_name) == 0) {
                    if (!arg_type_equal_to_class(v->m_type, clss_sym)) {
                        diag.add(diag::Diagnostic("Passed object dummy argument "
                                                      + std::string(pass_arg_name)
    ...
    void add_class_procedures()
    {
        for (auto& proc : class_procedures) {
            ASR::symbol_t* clss_sym
                = ASRUtils::symbol_get_past_external(current_scope->resolve_symbol(proc.first));
            ...
            for (auto& pname : proc.second) {
                ...
                bool is_nopass = (cdf.count(proc.first) && cdf[proc.first].count(pname.first)
                                  && cdf[proc.first][pname.first].count("nopass"));
                if (is_pass && is_nopass) {
                    diag.add(diag::Diagnostic(
                        ...
                        { diag::Label("pass specified here", { pname.second["pass"].loc }),
                          diag::Label("nopass specified here",
                                      { cdf[proc.first][pname.first]["nopass"] }) }));
                    throw SemanticAbort();
                }

                ASR::symbol_t* proc_sym
                    = proc_scope->resolve_symbol(pname.second["procedure"].name);
                if (proc_sym == nullptr) {
                    if (is_deferred) {
                        diag.add(diag::Diagnostic(
                        ...
                        diag.add(diag::Diagnostic(
                            "'" + pname.second["procedure"].name
                                + "' must be a module procedure"
                ...
                if (!is_deferred
                    ...
                    diag.add(diag::Diagnostic("PROCEDURE(interface) should be declared DEFERRED",
                                              diag::Level::Error,
                                              diag::Stage::Semantic,
                                              { diag::Label("", { loc }) }));
                    throw SemanticAbort();
                ...
                ASR::asr_t* v = ASR::make_ClassProcedure_t(al,
                                                           loc,
                                                           clss->m_symtab,
                                                           name,
                                                           pass_arg_name,
    ...
    std::string import_all(const ASR::Module_t* m, bool to_submodule = false)
    {
        ...
        for (auto& item : m->m_symtab->get_scope()) {
            if (current_scope->get_symbol(item.first) != nullptr && !in_submodule) {
                continue;
            }
            ...
            if (ASR::is_a<ASR::Function_t>(*item.second)) {
                ...
                ASR::asr_t* ep = ASR::make_ExternalSymbol_t(al,
                                                            gp->base.base.loc,
                                                            current_scope,
                                                            /* a_name */ gp->m_name,
                                                            (ASR::symbol_t*) gp,
                ...
                ASR::CustomOperator_t* gp = ASR::down_cast<ASR::CustomOperator_t>(item.second);
                ASR::asr_t* ep = ASR::make_ExternalSymbol_t(al,
                                                            gp->base.base.loc,
                                                            current_scope,
                                                            /* a_name */ gp->m_name,
                ...
                if (mvar->m_access == ASR::accessType::Public || to_submodule) {
                    ASR::asr_t* var = ASR::make_ExternalSymbol_t(al,
                                                                 mvar->base.base.loc,
                                                                 /* a_symtab */ current_scope,
                                                                 /* a_name */ mvar->m_name,
                                                                 (ASR::symbol_t*) mvar,
                ...
                ASR::asr_t* es = ASR::make_ExternalSymbol_t(al,
                                                            es0->base.base.loc,
                                                            /* a_symtab */ current_scope,
                                                            ...
                                                            es0->m_original_name,
                                                            dflt_access);
                current_scope->add_or_overwrite_symbol(item.first,
                                                       ASR::down_cast<ASR::symbol_t>(es));
                ...
                ASR::asr_t* v = ASR::make_ExternalSymbol_t(al,
                                                           mv->base.base.loc,
                                                           /* a_symtab */ current_scope,
                                                           /* a_name */ cname,
                                                           (ASR::symbol_t*) mv,
        ...
        if (current_scope->get_symbol(local_sym) != nullptr) {
            ...
            if (ASR::is_a<ASR::ExternalSymbol_t>(*gp_sym)) {
                ...
                for (size_t i = 0; i < gp->n_procs; i++) {
                    ...
                    ASR::symbol_t* m_proc = current_scope->resolve_symbol(gp_proc_name);
                    if (m_proc == nullptr) {
                        ...
                        if (m_proc == nullptr) {
                            ...
                            import_symbols_util(
                                m_, m__name, gp_proc_name, local_sym_, to_be_imported_later, loc);
                            m_proc = current_scope->resolve_symbol(local_sym_);
                        }
                    }
                ...
                for (size_t i = 0; i < gp_ext->n_procs; i++) {
                    ...
                    ASR::symbol_t* m_proc = current_scope->resolve_symbol(gp_ext_proc_name);
                    if (m_proc == nullptr) {
                        ...
                        if (m_proc == nullptr) {
                            ...
                            std::string m__name = std::string(m_->m_name);
                            import_symbols_util(m_,
                                                m__name,
                                                gp_ext_proc_name,
                                                local_sym_,
                                                to_be_imported_later,
                                                loc);
                            m_proc = current_scope->resolve_symbol(local_sym_);
                        }
                    }
            ...
            for (size_t i = 0; i < gp_ext->n_procs; i++) {
                ASR::symbol_t* m_proc
                    = current_scope->resolve_symbol(ASRUtils::symbol_name(gp_ext->m_procs[i]));
                if (m_proc == nullptr) {
                    are_all_present = false;
                    std::string proc_name = ASRUtils::symbol_name(gp_ext->m_procs[i]);
                    to_be_imported_later.push(
                        std::make_pair(proc_name, proc_name + "@" + local_sym));
                }
                gp_procs.push_back(al, m_proc);
            }
            ...
            if (are_all_present) {
                ...
                ep = ASR::make_ExternalSymbol_t(al,
                                                t->base.loc,
                                                current_scope,
                                                s2c(al, local_sym),
                                                t,
    ...
    void import_symbols_util(ASR::Module_t* m,
                             std::string& msym,
                             ...
                             const Location& loc)
    {
        ...
        if (!t) {
            diag.add(diag::Diagnostic("The symbol '" + remote_sym + "' not found in the module '"
                                          + msym + "'",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { loc }) }));
        ...
        if (ASR::is_a<ASR::Function_t>(*t)
            ...
            if (current_scope->get_symbol(local_sym) != nullptr) {
                diag.add(Diagnostic("Symbol '" + local_sym + "' from module '" + m->m_name
                                        + "' shadows '" + local_sym + "' in the current scope",
                                    Level::Warning,
                                    Stage::Semantic,
                                    { Label("", { loc }) }));
            ...
            ASR::asr_t* sub = ASR::make_ExternalSymbol_t(al,
                                                         loc,
                                                         /* a_symtab */ current_scope,
            ...
            process_generic_proc_custom_op<ASR::GenericProcedure_t>(
                local_sym, t, to_be_imported_later, loc, m, &ASR::make_GenericProcedure_t, nullptr);
        } else if (ASR::is_a<ASR::CustomOperator_t>(*t)) {
            process_generic_proc_custom_op<ASR::CustomOperator_t>(
                local_sym, t, to_be_imported_later, loc, m, &ASR::make_CustomOperator_t, nullptr);
            ...
            if (imported_func_sym != nullptr) {
                ASR::ExternalSymbol_t* ext_sym
                    = ASR::down_cast<ASR::ExternalSymbol_t>(imported_func_sym);
            ...
            if (mv->m_access == ASR::accessType::Private) {
                diag.add(diag::Diagnostic("Private variable `" + local_sym + "` cannot be imported",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
                                          { diag::Label("", { loc }) }));
                throw SemanticAbort();
            ...
            ASR::asr_t* req = ASR::make_ExternalSymbol_t(al,
                                                         loc,
                                                         current_scope,
                                                         s2c(al, local_sym),
                                                         (ASR::symbol_t*) mreq,
            ...
            ASR::asr_t* temp = ASR::make_ExternalSymbol_t(al,
                                                          loc,
                                                          current_scope,
                                                          s2c(al, local_sym),
                                                          (ASR::symbol_t*) mtemp,
            ...
            if (ASR::is_a<ASR::GenericProcedure_t>(*ext_sym->m_external)) {
                ...
                process_generic_proc_custom_op<ASR::CustomOperator_t>(local_sym,
                                                                      ...
                                                                      loc,
                                                                      m,
                                                                      &ASR::make_CustomOperator_t,
                                                                      nullptr);
            }
        } else {
            throw LCompilersException(
                "Only Subroutines, Functions, Variables and Derived supported in 'use', found: "
    ...
    void visit_Use(const AST::Use_t& x)
    {
        ...
        if (!t) {
            ...
            t = (ASR::symbol_t*) (ASRUtils::load_module(
                al,
                tu_symtab,
                msym,
                x.base.base.loc,
        ...
        if (!ASR::is_a<ASR::Module_t>(*t)) {
            diag.add(diag::Diagnostic("The symbol '" + msym + "' must be a module",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { x.base.base.loc }) }));
            throw SemanticAbort();
        ...
        if (x.n_symbols == 0) {
            ...
            for (size_t i = 0; i < x.n_symbols; i++) {
                ...
                switch (x.m_symbols[i]->type) {
                    case AST::use_symbolType::UseSymbol: {
                        remote_sym = to_lower(
                            AST::down_cast<AST::UseSymbol_t>(x.m_symbols[i])->m_remote_sym);
                        break;
                    }
                    ...
                    case AST::use_symbolType::IntrinsicOperator: {
                        AST::intrinsicopType op_type
                            = AST::down_cast<AST::IntrinsicOperator_t>(x.m_symbols[i])->m_op;
                        remote_sym = intrinsic2str[op_type];
                        break;
                    }
                    case AST::use_symbolType::DefinedOperator: {
                        remote_sym
                            = AST::down_cast<AST::DefinedOperator_t>(x.m_symbols[i])->m_opName;
                        break;
                    }
                    case AST::use_symbolType::UseWrite: {
                        remote_sym = AST::down_cast<AST::UseWrite_t>(x.m_symbols[i])->m_id;
                    ...
                    case AST::use_symbolType::UseRead: {
                        remote_sym = AST::down_cast<AST::UseRead_t>(x.m_symbols[i])->m_id;
                        if (remote_sym != "formatted" && remote_sym != "unformatted") {
                            diag.add(diag::Diagnostic(
                                "Can only be `formatted` or `unformatted`",
                                diag::Level::Error,
                    ...
                    default:
                        diag.add(diag::Diagnostic("Symbol with use not supported yet "
                                                      + std::to_string(x.m_symbols[i]->type),
                                                  diag::Level::Error,
                                                  diag::Stage::Semantic,
                                                  { diag::Label("", { x.base.base.loc }) }));
                        throw SemanticAbort();
                ...
                import_symbols_util(
                    m, msym, remote_sym, local_sym, to_be_imported_later, x.m_symbols[i]->base.loc);
            }
            ...
            while (!to_be_imported_later.empty()) {
                ...
                if (current_scope->resolve_symbol(local_sym) == nullptr) {
                    import_symbols_util(
                        m, msym, remote_sym, local_sym, to_be_imported_later, x.base.base.loc);
                }
            }
        }
    }
    ...
    void visit_Requirement(const AST::Requirement_t& x)
    {
        ...
        for (size_t i = 0; i < x.n_namelist; i++) {
            ...
            if (!current_scope->get_symbol(arg)) {
                diag.add(Diagnostic("Parameter " + arg + " is unused in " + x.m_name,
                                    Level::Warning,
                                    Stage::Semantic,
        ...
        for (auto& item : current_scope->get_scope()) {
            ...
            if (!defined) {
                diag.add(diag::Diagnostic("Symbol " + sym + " is not declared in "
        ...
        ASR::asr_t* req = ASR::make_Requirement_t(al,
                                                  x.base.base.loc,
                                                  current_scope,
                                                  s2c(al, to_lower(x.m_name)),
                                                  args.p,
    ...
    void visit_UnitRequire(const AST::UnitRequire_t& x)
    {
        ...
        ASR::symbol_t* req0
            = ASRUtils::symbol_get_past_external(current_scope->resolve_symbol(require_name));

        if (!req0 || !ASR::is_a<ASR::Requirement_t>(*req0)) {
            diag.add(diag::Diagnostic("No requirement '" + require_name + "' is defined",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { x.base.base.loc }) }));
            throw SemanticAbort();
        ...
        if (x.n_namelist != req->n_args) {
            diag.add(diag::Diagnostic("The number of parameters passed to '" + require_name
                                          + "' is not correct",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { x.base.base.loc }) }));
        ...
        for (size_t i = 0; i < x.n_namelist; i++) {
            ...
            if (AST::is_a<AST::AttrNamelist_t>(*attr)) {
                ...
                if (std::find(current_procedure_args.begin(), current_procedure_args.end(), req_arg)
                        == current_procedure_args.end()
                    && !current_scope->get_symbol(req_arg)) {
                    diag.add(diag::Diagnostic("Parameter '" + req_arg + "' was not declared",
                                              diag::Level::Error,
                                              diag::Stage::Semantic,
                                              { diag::Label("", { x.base.base.loc }) }));
                    throw SemanticAbort();
                ...
                ASR::ttype_t* ttype = determine_type(attr->base.loc,
                                                     req_param,
                                                     attr,
                                                     false,
                                                     false,
                ...
                diag.add(diag::Diagnostic("Unsupported decl_attribute for require statements.",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
        ...
        for (auto& item : req->m_symtab->get_scope()) {
            if (ASR::is_a<ASR::CustomOperator_t>(*item.second)) {
                ...
                // may not need to add new custom operators if another requires already got an
                // interface
                Vec<ASR::symbol_t*> symbols;
                ...
                ASR::symbol_t* new_c_op = ASR::down_cast<ASR::symbol_t>(
                    ASR::make_CustomOperator_t(al,
                                               c_op->base.base.loc,
                                               current_scope,
                                               s2c(al, c_op->m_name),
                                               symbols.p,
        ...
        tmp = ASR::make_Require_t(al, x.base.base.loc, s2c(al, require_name), args.p, args.size());
    ...
    void visit_Template(const AST::Template_t& x)
    {
        ...
        for (size_t i = 0; i < x.n_namelist; i++) {
            ...
            if (!s) {
                diag.add(diag::Diagnostic("Template argument " + arg
                                              + " has not been"
        ...
        ASR::asr_t* temp = ASR::make_Template_t(
            al, x.base.base.loc, current_scope, x.m_name, args.p, args.size(), reqs.p, reqs.size());
    ...
    void visit_Instantiate(const AST::Instantiate_t& x)
    {
        ...
        ASR::symbol_t* sym0
            = ASRUtils::symbol_get_past_external(current_scope->resolve_symbol(template_name));
        if (!sym0) {
            diag.add(diag::Diagnostic("Use of an unspecified template '" + template_name + "'",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { x.base.base.loc }) }));
            throw SemanticAbort();
        ...
        if (!ASR::is_a<ASR::Template_t>(*sym)) {
            diag.add(diag::Diagnostic("Cannot instantiate a non-template '" + template_name + "'",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { x.base.base.loc }) }));
            throw SemanticAbort();
        ...
        if (temp->n_args != x.n_args) {
            diag.add(diag::Diagnostic("Number of template arguments don't match",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { x.base.base.loc }) }));
            throw SemanticAbort();
        ...
        for (size_t i = 0; i < x.n_args; i++) {
            ...
            if (AST::is_a<AST::AttrType_t>(*x.m_args[i])) {
                ...
                dims.reserve(al, 0);
                ASR::symbol_t* type_declaration;
                ASR::ttype_t* arg_type = determine_type(x.m_args[i]->base.loc,
                                                        param,
                                                        x.m_args[i],
                                                        false,
                                                        false,
                ...
                if (!ASRUtils::is_type_parameter(*param_type)) {
                    diag.add(diag::Diagnostic("The type " + ASRUtils::type_to_str(arg_type)
                                                  + " cannot be applied to non-type parameter "
                                                  + param,
                                              diag::Level::Error,
                                              diag::Stage::Semantic,
                ...
                if (ASR::is_a<ASR::Function_t>(*param_sym)) {
                    ...
                    if (!f_arg0) {
                        diag.add(diag::Diagnostic("The function argument " + arg + " is not found",
                                                  diag::Level::Error,
                                                  diag::Stage::Semantic,
                                                  { diag::Label("", { x.m_args[i]->base.loc }) }));
                        throw SemanticAbort();
                    ...
                    if (!ASR::is_a<ASR::Function_t>(*f_arg)) {
                        diag.add(
                            diag::Diagnostic("The argument for " + param + " must be a function",
                                             diag::Level::Error,
                                             diag::Stage::Semantic,
                                             { diag::Label("", { x.m_args[i]->base.loc }) }));
                        throw SemanticAbort();
                    ...
                    check_restriction(
                        type_subs, symbol_subs, f, f_arg0, x.m_args[i]->base.loc, diag, []() {
                            throw SemanticAbort();
                        });
                } else {
                    ASR::ttype_t* param_type = ASRUtils::symbol_type(param_sym);
                    if (ASRUtils::is_type_parameter(*param_type)) {
                        ...
                        if (ASR::is_a<ASR::Struct_t>(*arg_sym)) {
                            arg_type = ASRUtils::TYPE(
                                ASR::make_StructType_t(al, x.m_args[i]->base.loc, arg_sym0));
                        } else {
                ...
                switch (intrinsic_op->m_op) {
                    ...
                    case (AST::GTE):
                        ...
                        break;
                    default:
                        diag.add(diag::Diagnostic("Unsupported binary operator",
                                                  diag::Level::Error,
                                                  diag::Stage::Semantic,
                                                  { diag::Label("", { x.m_args[i]->base.loc }) }));
                        throw SemanticAbort();
                ...
                if (is_binop) {
                    ...
                    diag.add(diag::Diagnostic("Must be binop or cmop",
                                              diag::Level::Error,
                                              diag::Stage::Semantic,
                                              { diag::Label("", { x.m_args[i]->base.loc }) }));
                    throw SemanticAbort();
                ...
                if (!found) {
                    if (f->n_args != 2) {
                        diag.add(diag::Diagnostic("The restriction " + f_name
                    ...
                    for (size_t i = 0; i < 2; i++) {
                        ...
                        args.push_back(al,
                                       ASRUtils::EXPR(ASR::make_Var_t(al, x.base.base.loc, var)));
                    }

                    // std::string func_name = op_name + "_intrinsic_" +
                    // ASRUtils::type_to_str(ltype);
                    std::string func_name = parent_scope->get_unique_name(op_name + "_intrinsic");

                    ASR::ttype_t* return_type = nullptr;
                    ASR::expr_t* value = nullptr;
                    ASR::expr_t* left = ASRUtils::EXPR(
                        ASR::make_Var_t(al, x.base.base.loc, current_scope->get_symbol("arg0")));
                    ASR::expr_t* right = ASRUtils::EXPR(
                        ASR::make_Var_t(al, x.base.base.loc, current_scope->get_symbol("arg1")));
                    ...
                    if (is_binop) {
                        ...
                        value = ASRUtils::EXPR(ASRUtils::make_Binop_util(
                            al, x.base.base.loc, binop, left, right, dest_type));
                        if (!ASRUtils::check_equal_type(dest_type, return_type)) {
                            diag.add(diag::Diagnostic("Unapplicable types for intrinsic operator "
                                                          + op_name,
                                                      diag::Level::Error,
                                                      diag::Stage::Semantic,
                                                      { diag::Label("", { x.base.base.loc }) }));
                        ...
                        return_type = ASRUtils::TYPE(ASR::make_Logical_t(
                            al, x.base.base.loc, compiler_options.po.default_integer_kind));
                        value = ASRUtils::EXPR(ASRUtils::make_Cmpop_util(
                            al, x.base.base.loc, cmpop, left, right, left_type));
                    }
                    ...
                    ASR::expr_t* return_expr = ASRUtils::EXPR(
                        ASR::make_Var_t(al, x.base.base.loc, current_scope->get_symbol("ret")));

                    Vec<ASR::stmt_t*> body;
                    body.reserve(al, 1);
                    ASR::symbol_t* return_sym = current_scope->get_symbol("ret");
                    ASR::expr_t* target
                        = ASRUtils::EXPR(ASR::make_Var_t(al, x.base.base.loc, return_sym));
                    ASRUtils::make_ArrayBroadcast_t_util(al, x.base.base.loc, target, value);
                    ASR::stmt_t* assignment = ASRUtils::STMT(
                        ASR::make_Assignment_t(al, x.base.base.loc, target, value, nullptr));
                ...
                diag.add(diag::Diagnostic("Unsupported template argument",
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
                                          { diag::Label("", { x.m_args[i]->base.loc }) }));
                throw SemanticAbort();
        ...
        if (x.n_symbols == 0) {
            ...
            for (size_t i = 0; i < x.n_symbols; i++) {
                ...
                if (s == nullptr) {
                    diag.add(diag::Diagnostic("Symbol " + generic_name + " was not found",
                                              diag::Level::Error,
                                              diag::Stage::Semantic,
                                              { diag::Label("", { x.base.base.loc }) }));
                    throw SemanticAbort();
                ...
                ASR::symbol_t* new_sym = instantiate_symbol(
                    al, current_scope, type_subs, symbol_subs, new_sym_name, s);
    ...
    ASR::symbol_t* replace_symbol(ASR::symbol_t* s, std::string name)
    {
        switch (s->type) {
            case ASR::symbolType::Variable: {
                ...
                if (ASR::is_a<ASR::TypeParameter_t>(*t)) {
                    ...
                    if (name.compare("integer") == 0) {
                        t = ASRUtils::TYPE(ASR::make_Integer_t(
                            al, tp->base.base.loc, compiler_options.po.default_integer_kind));
                    } else {
                        t = ASRUtils::TYPE(
                            ASR::make_TypeParameter_t(al, tp->base.base.loc, s2c(al, name)));
                    }
                    t = ASRUtils::make_Array_t_util(al, tp->base.base.loc, t, tp_m_dims, tp_n_dims);
                }
                ASR::asr_t* new_v = ASRUtils::make_Variable_t_util(al,
                                                                   v->base.base.loc,
                                                                   current_scope,
                                                                   s2c(al, name),
                                                                   v->m_dependencies,
            ...
            case ASR::symbolType::Function: {
                ...
                for (size_t i = 0; i < f->n_args; i++) {
                    ...
                    if (ASR::is_a<ASR::TypeParameter_t>(*param_type)) {
                        ...
                        if (context_map.find(tp->m_param) != context_map.end()) {
                            ...
                            if (tp_n_dims > 0) {
                                param_type = ASRUtils::make_Array_t_util(
                                    al, tp->base.base.loc, param_type, tp_m_dims, tp_n_dims);
                            }
                        }
                    }
                ...
                if (f->m_return_var != nullptr) {
                    ...
                    if (ASR::is_a<ASR::TypeParameter_t>(*return_type)) {
                        ASR::TypeParameter_t* tp
                            = ASR::down_cast<ASR::TypeParameter_t>(return_type);
                        if (context_map.find(tp->m_param) != context_map.end()) {
                            std::string pt = context_map[tp->m_param];
                            if (pt.compare("integer") == 0) {
                            ...
                            if (tp_n_dims > 0) {
                                return_type = ASRUtils::make_Array_t_util(
                                    al, tp->base.base.loc, return_type, tp_m_dims, tp_n_dims);
                            }
                        }
                    }
                    SetChar variable_dependencies_vec;
                    variable_dependencies_vec.reserve(al, 1);
                    ASRUtils::collect_variable_dependencies(
                        al, variable_dependencies_vec, return_type);
                    ASR::asr_t* new_return_var
                        = ASRUtils::make_Variable_t_util(al,
                                                         ...
                                                         nullptr,
                                                         nullptr,
                                                         return_var->m_storage,
                                                         return_type,
                                                         return_var->m_type_declaration,
                                                         return_var->m_abi,
                                                         return_var->m_access,
                                                         return_var->m_presence,
                                                         return_var->m_value_attr);
                    new_scope->add_symbol(return_var_name,
                                          ASR::down_cast<ASR::symbol_t>(new_return_var));
                    new_return_var_ref = ASRUtils::EXPR(ASR::make_Var_t(
                        al, f->base.base.loc, new_scope->get_symbol(return_var_name)));
                }

                ASR::asr_t* new_f = ASRUtils::make_Function_t_util(al,
                                                                   f->base.base.loc,
                                                                   new_scope,
                                                                   s2c(al, name),
                                                                   f->m_dependencies,
                                                                   ...
                                                                   ftype->m_inline,
                                                                   ftype->m_static,
                                                                   ftype->m_restrictions,
                                                                   ftype->n_restrictions,
                                                                   ftype->m_is_restriction,
                                                                   f->m_deterministic,
                                                                   f->m_side_effect_free);
            ...
            default: {
                ...
                diag.add(diag::Diagnostic("Symbol not found " + sym_name,
                                          diag::Level::Error,
                                          diag::Stage::Semantic,
    ...
    void visit_Enum(const AST::Enum_t& x)
    {
        ...
        if (x.n_attr == 1) {
            if (AST::is_a<AST::AttrBind_t>(*x.m_attr[0])) {
                ...
                if (bind->n_args == 1 && AST::is_a<AST::Name_t>(*bind->m_args[0])) {
                    ...
                    diag.add(diag::Diagnostic("Language name must be specified in "
                ...
                diag.add(diag::Diagnostic("Unsupported attribute type in enum, "
            ...
            diag.add(diag::Diagnostic("Only one attribute is allowed in enum",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { x.base.base.loc }) }));
            throw SemanticAbort();
        ...
        tmp = ASR::make_Enum_t(al,
                               ...
                               nullptr);
        parent_scope->add_symbol(sym_name, ASR::down_cast<ASR::symbol_t>(tmp));
        // Expose all enumerators into the parent scope as ExternalSymbols pointing into the
        // enumeration, which is the semantics of Fortran enums. That way `resolve_variable()` can
        // resolve them automatically. In ASR->Fortran we do not create any Fortran code for these
        // ExternalSymbols, since they are implicit. But in ASR we need to represent them
        // explicitly.
        for (auto it : current_scope->get_scope()) {
            ASR::Variable_t* var = ASR::down_cast<ASR::Variable_t>(it.second);
            parent_scope->add_symbol(
                var->m_name,
                ASR::down_cast<ASR::symbol_t>(ASR::make_ExternalSymbol_t(al,
                                                                         var->base.base.loc,
                                                                         parent_scope,
                                                                         s2c(al, var->m_name),
                                                                         it.second,
     ...
     * @return true if the expression can be evaluated at compile time
     * @return false if the expression must be evaluated at runtime
     */
    bool is_compile_time_constant(const AST::expr_t& expr)
    {
        if (AST::is_a<AST::Constant_t>(expr)) {
            return true;
        }

        // Handle unary operations on constants
        if (AST::is_a<AST::UnaryOp_t>(expr)) {
            AST::UnaryOp_t* op = AST::down_cast<AST::UnaryOp_t>(&expr);
            return is_compile_time_constant(*op->m_operand);
        }

        // Handle binary operations between constants
        if (AST::is_a<AST::BinOp_t>(expr)) {
            AST::BinOp_t* op = AST::down_cast<AST::BinOp_t>(&expr);
            return is_compile_time_constant(*op->m_left) && is_compile_time_constant(*op->m_right);
        }

        // Handle function calls
        if (AST::is_a<AST::FunctionCall_t>(expr)) {
            AST::FunctionCall_t* call = AST::down_cast<AST::FunctionCall_t>(&expr);

            // First check if all arguments are compile-time constants
            for (size_t i = 0; i < call->n_args; i++) {
                if (!is_compile_time_constant(*call->m_args[i].m_value)) {
                    return false;
                }
            }

            // Even if all arguments are constants, the function itself
            // must be a compile-time evaluable intrinsic
            std::string func_name = AST::down_cast<AST::Name_t>(call->m_name)->m_id;
            func_name = to_lower(func_name);

            // List of intrinsic functions that can be evaluated at compile time
            static const std::set<std::string> compile_time_functions
                = { "abs",   "min",    "max",    "minval", "maxval", "size",
                    "shape", "lbound", "ubound", "len",    "trim" };

            return compile_time_functions.find(func_name) != compile_time_functions.end();
        }

        return false;
    }

    void handle_array_bounds(const Location& loc, AST::expr_t* start, AST::expr_t* end)
    {
        if (start && !is_compile_time_constant(*start)) {
            diag.add(diag::Diagnostic("Array bound must be a compile-time constant",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { loc }) }));
            throw SemanticAbort();
        ...
        if (end && !is_compile_time_constant(*end)) {
            diag.add(diag::Diagnostic("Array bound must be a compile-time constant",
                                      diag::Level::Error,
                                      diag::Stage::Semantic,
                                      { diag::Label("", { loc }) }));
    ...
    // 在处理数组声明的地方调用这个函数
    // 例如在处理 dimension 属性时:
    if (AST::is_a<AST::AttrDimension_t>(*attr)) {
        AST::AttrDimension_t* dim = AST::down_cast<AST::AttrDimension_t>(attr);
        for (size_t i = 0; i < dim->n_dim; i++) {
            AST::dimension_t dim_i = dim->m_dim[i];
            handle_array_bounds(attr->base.loc, dim_i.m_start, dim_i.m_end);
            // ... rest of the array dimension handling code ...
        }
    }
};
