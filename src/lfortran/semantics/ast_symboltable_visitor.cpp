...
template <typename T>
void
extract_bind(T& x, ASR::abiType& abi_type, char*& bindc_name)
{
    if (x.m_bind) {
        ...
        if (bind->n_args == 1) {
            if (AST::is_a<AST::Name_t>(*bind->m_args[0])) {
                ...
                if (to_lower(std::string(name->m_id)) == "c") {
                    ...
                    throw SemanticError("Unsupported language in bind()", x.base.base.loc);
                }
            } else {
            ...
            throw SemanticError("At least one argument needed in bind()", x.base.base.loc);
        }
        if (bind->n_kwargs == 1) {
            ...
            if (to_lower(std::string(arg)) == "name") {
                ...
                throw SemanticError("Unsupported keyword argument in bind()", x.base.base.loc);
            }
        }
    }
}
...
public:
    ...
    SymbolTableVisitor(Allocator& al,
        ...
        for (size_t i = 0; i < x.n_args; i++) {
            ...
            if (current_scope->get_symbol(arg_s) == nullptr) {
                if (compiler_options.implicit_typing) {
                    declare_implicit_variable(x.base.base.loc, arg_s, ASRUtils::intent_unspecified);
                } else {
    ...
    void visit_Function(const AST::Function_t& x)
    {
        ...
        if (current_scope->get_symbol(return_var_name) == nullptr) {
            ...
            switch (return_type->m_type) {
                ...
                case (AST::decl_typeType::TypeCharacter): {
                    type = LFortran::ASRUtils::TYPE(
                        ASR::make_Character_t(al, x.base.base.loc, 1, a_len, nullptr, nullptr, 0));
                    break;
                }
                case (AST::decl_typeType::TypeType): {
                    ...
                    if (type_param)
                        type = LFortran::ASRUtils::TYPE(
                            ASR::make_TypeParameter_t(al, x.base.base.loc, nullptr, nullptr, 0));
                    else
                        type = LFortran::ASRUtils::TYPE(
            ...
            return_var = ASR::make_Variable_t(al,
                                              x.base.base.loc,
                                              current_scope,
            ...
            if (return_type) {
                throw SemanticError("Cannot specify the return type twice", x.base.base.loc);
            }
        ...
        for (size_t i = 0; i < x.n_attributes && !is_elemental; i++) {
            ...
            if (AST::is_a<AST::SimpleAttribute_t>(*func_attr)) {
                ...
                is_elemental
                    = is_elemental
                      || simple_func_attr->m_attr == AST::simple_attributeType::AttrElemental;
            }
        }
        ...
        for (auto i = current_procedure_used_type_parameter_indices.begin();
             i != current_procedure_used_type_parameter_indices.end();
             i++) {
    ...
    void add_assignment_procedures()
    {
        ...
        ASR::asr_t* v = ASR::make_CustomOperator_t(
            al, loc, current_scope, generic_name, symbols.p, symbols.size(), assgn[current_scope]);
    ...
    void add_generic_class_procedures()
    {
        for (auto& proc : generic_class_procedures) {
            ...
            loc.first = 1;
            loc.last = 1;
            ASR::DerivedType_t* clss
                = ASR::down_cast<ASR::DerivedType_t>(current_scope->get_symbol(proc.first));
            for (auto& pname : proc.second) {
                ...
                for (std::string& cand_proc : pname.second) {
                    if (clss->m_symtab->get_symbol(cand_proc) != nullptr) {
                        ...
                        throw SemanticError(
                            cand_proc + " doesn't exist inside " + proc.first + " type", loc);
                    }
                }
                Str s;
    ...
    void add_class_procedures()
    {
        for (auto& proc : class_procedures) {
            ...
            for (auto& pname : proc.second) {
                ...
                ASR::asr_t* v = ASR::make_ClassProcedure_t(
                    al, loc, clss->m_symtab, name, proc_name, proc_sym, ASR::abiType::Source);
                ASR::symbol_t* cls_proc_sym = ASR::down_cast<ASR::symbol_t>(v);
                clss->m_symtab->add_symbol(pname.first, cls_proc_sym);
            }
        }
                             ...
                             const Location& loc)
    {
        ...
        if (!t) {
            throw SemanticError(
                "The symbol '" + remote_sym + "' not found in the module '" + msym + "'", loc);
        }
        if (ASR::is_a<ASR::Function_t>(*t)
            ...
            throw LCompilersException(
                "Only Subroutines, Functions, Variables and Derived supported in 'use'");
        }
    }
    ...
    void visit_Use(const AST::Use_t& x)
    {
        ...
        if (!ASR::is_a<ASR::Module_t>(*t)) {
            throw SemanticError("The symbol '" + msym + "' must be a module", x.base.base.loc);
        }
...
Result<ASR::asr_t*>
symbol_table_visitor(Allocator& al,
