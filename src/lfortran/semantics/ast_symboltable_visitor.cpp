    ...
    if (bind->n_kwargs == 1) {
      ...
      if (to_lower(std::string(arg)) == "name") {
        if (AST::is_a<AST::String_t>(*value)) {
          ...
          throw SemanticError(
              "The value of the 'name' keyword argument in bind(c) must be a "
              "string",
              x.base.base.loc);
        }
      } else {
 ...
 public:
      ...
      if (expr) {
        switch (data->type->type) {
          case ASR::ttypeType::Character: {
            ASR::Character_t *char_type =
                ASR::down_cast<ASR::Character_t>(data->type);
          ...
          default: {
            throw SemanticError("Only Character type is supported as of now.",
                                data->type->base.loc);
          }
        }
      }
  ...
  void visit_ModuleSubmoduleCommon(const T &x, std::string parent_name = "") {
    ...
    ASR::asr_t *tmp0 = ASR::make_Module_t(
        ...
        /* a_name */ s2c(al, to_lower(x.m_name)), nullptr, 0, false, false);
    ...
    if (x.class_type == AST::modType::Submodule) {
      ...
      ASR::symbol_t *submod_parent = (ASR::symbol_t *)(ASRUtils::load_module(
          al, global_scope, parent_name, x.base.base.loc, false, rl_path, true,
  ...
  void visit_Submodule(const AST::Submodule_t &x) {
    in_submodule = true;
    visit_ModuleSubmoduleCommon<AST::Submodule_t, ASR::Module_t>(
        x, std::string(x.m_id));
    in_submodule = false;
  }
  ...
  void visit_Program(const AST::Program_t &x) {
    ...
    tmp = ASR::make_Program_t(al, x.base.base.loc,
                              /* a_symtab */ current_scope,
  ...
  void visit_Subroutine(const AST::Subroutine_t &x) {
    ...
    for (size_t i = 0; i < x.n_args; i++) {
      ...
      if (current_scope->get_symbol(arg_s) == nullptr) {
        if (compiler_options.implicit_typing) {
          ...
          throw SemanticError("Dummy argument '" + arg_s + "' not defined",
                              x.base.base.loc);
        }
      }
      ASR::symbol_t *var = current_scope->get_symbol(arg_s);
      args.push_back(al, LFortran::ASRUtils::EXPR(
                             ASR::make_Var_t(al, x.base.base.loc, var)));
    }
    std::string sym_name = to_lower(x.m_name);
    ...
    tmp = ASR::make_Function_t(al, x.base.base.loc,
                               ...
                               /* a_args */ args.p,
                               /* n_args */ args.size(), nullptr, 0,
                               /* a_body */ nullptr,
                               /* n_body */ 0, nullptr,
                               current_procedure_abi_type, s_access, deftype,
                               bindc_name, is_pure, is_module, false, false);
    parent_scope->add_symbol(sym_name, ASR::down_cast<ASR::symbol_t>(tmp));
    current_scope = parent_scope;
  ...
  void visit_Function(const AST::Function_t &x) {
    ...
    AST::AttrType_t *return_type =
        find_return_type(x.m_attributes, x.n_attributes, x.base.base.loc);
    if (current_scope->get_symbol(return_var_name) == nullptr) {
      ...
      if (!return_type) {
        throw SemanticError("Return type not specified", x.base.base.loc);
      }
      ASR::ttype_t *type;
      int a_kind = 4;
      int a_len = -10;
      if (return_type->m_kind != nullptr) {
        if (return_type->n_kind == 1) {
          ...
          if (return_type->m_type == AST::decl_typeType::TypeCharacter) {
            a_len = ASRUtils::extract_len<SemanticError>(kind_expr,
                                                         x.base.base.loc);
          } else {
            a_kind = ASRUtils::extract_kind<SemanticError>(kind_expr,
                                                           x.base.base.loc);
          }
        } else {
          throw SemanticError("Only one kind item supported for now",
                              x.base.base.loc);
        }
      }
      switch (return_type->m_type) {
        case (AST::decl_typeType::TypeInteger): {
          type = LFortran::ASRUtils::TYPE(
              ASR::make_Integer_t(al, x.base.base.loc, a_kind, nullptr, 0));
          break;
        }
        case (AST::decl_typeType::TypeReal): {
          type = LFortran::ASRUtils::TYPE(
              ASR::make_Real_t(al, x.base.base.loc, a_kind, nullptr, 0));
          break;
        }
        ...
        case (AST::decl_typeType::TypeComplex): {
          type = LFortran::ASRUtils::TYPE(
              ASR::make_Complex_t(al, x.base.base.loc, a_kind, nullptr, 0));
          break;
        }
        ...
        case (AST::decl_typeType::TypeCharacter): {
          type = LFortran::ASRUtils::TYPE(ASR::make_Character_t(
              al, x.base.base.loc, 1, a_len, nullptr, nullptr, 0));
          break;
        }
        case (AST::decl_typeType::TypeType): {
          ...
          if (is_template) {
            for (size_t i = 0; i < current_template_type_parameters.size();
                 i++) {
              ASR::TypeParameter_t *param =
                  ASR::down_cast2<ASR::TypeParameter_t>(
          ...
          if (type_param)
            type = LFortran::ASRUtils::TYPE(ASR::make_TypeParameter_t(
                al, x.base.base.loc, nullptr, nullptr, 0, nullptr, 0));
          else
            type = LFortran::ASRUtils::TYPE(
                ASR::make_Derived_t(al, x.base.base.loc, v, nullptr, 0));
          break;
        }
        default:
          throw SemanticError("Return type not supported", x.base.base.loc);
      }
      // Add it as a local variable:
      return_var = ASR::make_Variable_t(
      ...
      ASR::down_cast2<ASR::Variable_t>(return_var)->m_intent =
          LFortran::ASRUtils::intent_return_var;
    }
    ...
    for (size_t i = 0; i < x.n_attributes && !is_elemental; i++) {
      ...
      if (AST::is_a<AST::SimpleAttribute_t>(*func_attr)) {
        AST::SimpleAttribute_t *simple_func_attr =
            AST::down_cast<AST::SimpleAttribute_t>(func_attr);
        is_elemental =
            is_elemental || simple_func_attr->m_attr ==
                                AST::simple_attributeType::AttrElemental;
      }
    }
    Vec<ASR::ttype_t *> params;
    params.reserve(al, current_procedure_used_type_parameter_indices.size());
    for (auto i = current_procedure_used_type_parameter_indices.begin();
         i != current_procedure_used_type_parameter_indices.end(); i++) {
    ...
    tmp = ASR::make_Function_t(
        ...
        current_procedure_abi_type, s_access, deftype, bindc_name, is_elemental,
        false, false, false);
    parent_scope->add_symbol(sym_name, ASR::down_cast<ASR::symbol_t>(tmp));
    current_scope = parent_scope;
    current_procedure_args.clear();
  ...
  void visit_DerivedType(const AST::DerivedType_t &x) {
    ...
    if (attr_extend != nullptr) {
      ...
      if (parent_scope->get_symbol(parent_sym_name) == nullptr) {
        throw SemanticError(parent_sym_name + " is not defined.",
                            x.base.base.loc);
      }
      parent_sym = parent_scope->get_symbol(parent_sym_name);
    }
    if (is_template && data_member_names.size() == 0) {
      current_template_type_parameters.push_back(ASR::make_TypeParameter_t(
          al, x.base.base.loc, s2c(al, to_lower(x.m_name)), nullptr, 0, nullptr,
          0));
    }
    tmp = ASR::make_DerivedType_t(
        al, x.base.base.loc, current_scope, s2c(al, to_lower(x.m_name)),
        data_member_names.p, data_member_names.size(), ASR::abiType::Source,
        dflt_access, parent_sym);
    parent_scope->add_symbol(sym_name, ASR::down_cast<ASR::symbol_t>(tmp));
    current_scope = parent_scope;
  ...
  void visit_DerivedTypeProc(const AST::DerivedTypeProc_t &x) {
    for (size_t i = 0; i < x.n_symbols; i++) {
      ...
      if (use_sym->m_local_rename) {
        class_procedures[dt_name][to_lower(use_sym->m_local_rename)] =
            remote_sym_str;
      } else {
        class_procedures[dt_name][to_lower(use_sym->m_remote_sym)] =
            remote_sym_str;
      }
    }
  }
      ...
      if (AST::is_a<AST::InterfaceModuleProcedure_t>(*item)) {
        ...
        switch (proc->m_proc->type) {
          ...
          case AST::program_unitType::Function: {
            AST::Function_t *subrout =
                AST::down_cast<AST::Function_t>(proc->m_proc);
            char *proc_name = subrout->m_name;
            proc_names.push_back(std::string(proc_name));
            break;
          }
        ...
        throw SemanticError("Interface procedure type not imlemented yet",
                            item->base.loc);
      }
    }
  }
  ...
  void visit_Interface(const AST::Interface_t &x) {
    if (AST::is_a<AST::InterfaceHeaderName_t>(*x.m_header)) {
      std::string generic_name = to_lower(
          AST::down_cast<AST::InterfaceHeaderName_t>(x.m_header)->m_name);
      ...
      std::string op_name = to_lower(
          AST::down_cast<AST::InterfaceHeaderDefinedOperator_t>(x.m_header)
              ->m_operator_name);
      std::vector<std::string> proc_names;
      fill_interface_proc_names(x, proc_names);
      ...
      std::string op_name = to_lower(
          AST::down_cast<AST::InterfaceHeaderWrite_t>(x.m_header)->m_id);
      std::vector<std::string> proc_names;
      fill_interface_proc_names(x, proc_names);
      defined_op_procs[op_name] = proc_names;
    } else if (AST::is_a<AST::InterfaceHeaderRead_t>(*x.m_header)) {
      std::string op_name = to_lower(
          AST::down_cast<AST::InterfaceHeaderRead_t>(x.m_header)->m_id);
      std::vector<std::string> proc_names;
      fill_interface_proc_names(x, proc_names);
      defined_op_procs[op_name] = proc_names;
    } else {
  ...
  void add_overloaded_procedures() {
    for (auto &proc : overloaded_op_procs) {
      ...
      ASR::asr_t *v =
          ASR::make_CustomOperator_t(al, loc, current_scope, generic_name,
                                     symbols.p, symbols.size(), ASR::Public);
      current_scope->add_symbol(intrinsic2str[proc.first],
  ...
  void add_assignment_procedures() {
    ...
    ASR::asr_t *v = ASR::make_CustomOperator_t(
        al, loc, current_scope, generic_name, symbols.p, symbols.size(),
        assgn[current_scope]);
  ...
  void add_generic_procedures() {
    for (auto &proc : generic_procedures) {
      ...
      char *generic_name = s.c_str(al);
      ASR::asr_t *v =
          ASR::make_GenericProcedure_t(al, loc, current_scope, generic_name,
                                       symbols.p, symbols.size(), ASR::Public);
    ...
    for (auto &proc : generic_class_procedures) {
      ...
      for (auto &pname : proc.second) {
        ...
        ASR::asr_t *v = ASR::make_GenericProcedure_t(
            al, loc, clss->m_symtab, generic_name, cand_procs.p,
            cand_procs.size(),
            ASR::accessType::Public);  // Update the access as per the input
                                       // Fortran code
  ...
  void add_class_procedures() {
    for (auto &proc : class_procedures) {
      ...
      for (auto &pname : proc.second) {
        ...
        ASR::asr_t *v =
            ASR::make_ClassProcedure_t(al, loc, clss->m_symtab, name, proc_name,
                                       proc_sym, ASR::abiType::Source);
        ASR::symbol_t *cls_proc_sym = ASR::down_cast<ASR::symbol_t>(v);
        clss->m_symtab->add_symbol(pname.first, cls_proc_sym);
      }
    }
    ...
    for (auto &item : m->m_symtab->get_scope()) {
      ...
      if (ASR::is_a<ASR::Function_t>(*item.second)) {
        ...
        ASR::asr_t *fn = ASR::make_ExternalSymbol_t(
            ...
            /* a_symtab */ current_scope,
            /* a_name */ mfn->m_name, (ASR::symbol_t *)mfn, m->m_name, nullptr,
            0, mfn->m_name, dflt_access);
        ...
        ASR::GenericProcedure_t *gp =
            ASR::down_cast<ASR::GenericProcedure_t>(item.second);
        ASR::asr_t *ep = ASR::make_ExternalSymbol_t(
            al, gp->base.base.loc, current_scope,
            /* a_name */ gp->m_name, (ASR::symbol_t *)gp, m->m_name, nullptr, 0,
            gp->m_name, dflt_access);
        ...
        ASR::CustomOperator_t *gp =
            ASR::down_cast<ASR::CustomOperator_t>(item.second);
        ASR::asr_t *ep = ASR::make_ExternalSymbol_t(
            al, gp->base.base.loc, current_scope,
            /* a_name */ gp->m_name, (ASR::symbol_t *)gp, m->m_name, nullptr, 0,
            gp->m_name, dflt_access);
        ...
        ASR::asr_t *var = ASR::make_ExternalSymbol_t(
            ...
            /* a_symtab */ current_scope,
            /* a_name */ mvar->m_name, (ASR::symbol_t *)mvar, m->m_name,
            nullptr, 0, mvar->m_name, dflt_access);
        ...
        ASR::asr_t *es = ASR::make_ExternalSymbol_t(
            ...
            /* a_symtab */ current_scope,
            /* a_name */ s2c(al, sym), es0->m_external, es0->m_module_name,
            nullptr, 0, es0->m_original_name, dflt_access);
        current_scope->add_symbol(sym, ASR::down_cast<ASR::symbol_t>(es));
      } else if (ASR::is_a<ASR::DerivedType_t>(*item.second)) {
        ASR::DerivedType_t *mv =
            ASR::down_cast<ASR::DerivedType_t>(item.second);
        ...
        ASR::asr_t *v = ASR::make_ExternalSymbol_t(
            al, mv->base.base.loc,
            /* a_symtab */ current_scope,
            /* a_name */ cname, (ASR::symbol_t *)mv, m->m_name, nullptr, 0,
            mv->m_name, dflt_access);
  ...
  void import_symbols_util(
      ASR::Module_t *m, std::string &msym, std::string &remote_sym,
    ...
    if (ASR::is_a<ASR::Function_t>(*t) &&
      ...
      ASR::asr_t *sub = ASR::make_ExternalSymbol_t(
          ...
          /* a_symtab */ current_scope,
          /* a_name */ name.c_str(al), (ASR::symbol_t *)msub, m->m_name,
          nullptr, 0, msub->m_name, dflt_access);
      ...
      ASR::asr_t *ep =
          ASR::make_ExternalSymbol_t(al, t->base.loc, current_scope,
                                     /* a_name */ cname, t, m->m_name, nullptr,
                                     0, gp->m_name, dflt_access);
      ...
      ASR::asr_t *ep = ASR::make_ExternalSymbol_t(
          al, es->base.base.loc, current_scope,
          /* a_name */ es->m_name, es->m_external, es->m_module_name,
          es->m_scope_names, es->n_scope_names, es->m_original_name,
          es->m_access);
      ...
      if (ASR::is_a<ASR::CustomOperator_t>(*orig_sym)) {
        ...
        for (size_t igp = 0; igp < gp->n_procs; igp++) {
          ...
          ASR::symbol_t *mangled_sym =
              m->m_symtab->resolve_symbol(mangled_name);
      ...
      ASR::asr_t *fn = ASR::make_ExternalSymbol_t(
          al, mfn->base.base.loc,
          /* a_symtab */ current_scope,
          /* a_name */ cname, (ASR::symbol_t *)mfn, m->m_name, nullptr, 0,
          mfn->m_name, dflt_access);
      ...
      throw LCompilersException(
          "Only Subroutines, Functions, Variables and Derived supported in "
          "'use'");
    }
  }
  ...
  void visit_Use(const AST::Use_t &x) {
    ...
    if (!t) {
      ...
      t = (ASR::symbol_t *)(ASRUtils::load_module(
          al, current_scope->parent, msym, x.base.base.loc, false, rl_path,
          true, [&](const std::string &msg, const Location &loc) {
            throw SemanticError(msg, loc);
          }));
    }
    ...
    if (x.n_symbols == 0) {
      ...
      for (size_t i = 0; i < x.n_symbols; i++) {
        ...
        switch (x.m_symbols[i]->type) {
          ...
          default:
            throw SemanticError("Symbol with use not supported yet",
                                x.base.base.loc);
        }
        std::string local_sym;
      ...
      // importing the same function twice i.e., if the user has already
      // imported the required procedures manually then importing later avoids
...
Result<ASR::asr_t *> symbol_table_visitor(
