        ...
        for (size_t i=0; i<x.n_decl; i++) {
            ...
            if(x.m_decl[i]->type == AST::unit_decl2Type::Declaration) {
                ...
                if(decl.m_vartype) {
                    ...
                    if(type && type->m_type == AST::decl_typeType::TypeProcedure &&
                           type->m_name && (type->m_name != nullptr && std::string(type->m_name != nullptr ? type->m_name : "") == sym_name)) {
                        procedure_decl_indices.push_back(al, i);
                        continue;
                    }
                }
        ...
        for (size_t i=0; i<x.n_decl; i++) {
            ...
            if(x.m_decl[i]->type == AST::unit_decl2Type::Declaration) {
                ...
                if(decl.m_vartype) {
                    ...
                    LCOMPILERS_ASSERT(type);
                    if(type && type->m_type == AST::decl_typeType::TypeProcedure &&
                           (type->m_name != nullptr && std::string(type->m_name != nullptr ? type->m_name : "") == sym_name)) {
                        procedure_decl_indices.push_back(al, i);
                        continue;
                    }
                }
