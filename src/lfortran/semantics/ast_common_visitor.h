...
public:
    ...
    ASR::ttype_t* determine_type(const Location &loc, std::string& sym, AST::decl_attribute_t* decl_attribute, bool is_pointer, Vec<ASR::dimension_t>& dims){
        ...
        if (sym_type->m_type == AST::decl_typeType::TypeReal) {
            ...
            LFORTRAN_ASSERT(ASR::is_a<ASR::Function_t>(*v));
        ...
        for (size_t i=0; i<x.n_args; i++) {
            ...
            if (ASR::is_a<ASR::Var_t>(*var_expr) &&
                ...
                v = ASR::down_cast<ASR::symbol_t>(
                    ...
                    variable_dependencies_vec.size(), LFortran::ASRUtils::intent_unspecified,
                    ...
                    if( current_scope->resolve_symbol(imported_proc_name) == nullptr ) {
                        ...
                        if( current_scope->resolve_symbol(module_name) == nullptr ) {
                            ...
                            if( current_scope->resolve_symbol(imported_module_name) == nullptr ) {
                                LFORTRAN_ASSERT(ASR::is_a<ASR::Module_t>(*ASRUtils::get_asr_owner(module_sym)));
