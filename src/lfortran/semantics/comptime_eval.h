...
struct IntrinsicProceduresAsASRNodes {
 ...
 public:
  IntrinsicProceduresAsASRNodes() {
    intrinsics_present_in_ASR = {"size",    "lbound", "ubound",   "transpose",
                                 "matmul",  "pack",   "transfer", "cmplx",
                                 "reshape", "ichar"};
  }

  bool is_intrinsic_present_in_ASR(std::string &name) {
    return intrinsics_present_in_ASR.find(name) !=
           intrinsics_present_in_ASR.end();
  }
};
...
struct IntrinsicProcedures {
  ...
  */
  ...
  typedef ASR::expr_t *(*comptime_eval_callback)(Allocator &, const Location &,
                                                 Vec<ASR::expr_t *> &);
  std::map<std::string, std::tuple<std::string, comptime_eval_callback, bool>>
      comptime_eval_map;
  ...
  std::string get_module(std::string name, const Location &loc) const {
    ...
    if (search != comptime_eval_map.end()) {
      ...
      throw SemanticError(
          "Function '" + name + "' not found among intrinsic procedures", loc);
    }
  }

  ASR::expr_t *comptime_eval(std::string name, Allocator &al,
                             const Location &loc,
                             Vec<ASR::call_arg_t> &args) const {
    auto search = comptime_eval_map.find(name);
    if (search != comptime_eval_map.end()) {
  ...
  static ASR::expr_t *eval_kind(Allocator &al, const Location &loc,
    ...
    // TODO: Check that the expression reduces to a valid constant expression
    // (10.1.12)
    switch (kind_expr->type) {
      case ASR::exprType::IntegerConstant: {
        kind_num =
            ASR::down_cast<ASR::Integer_t>(
                ASR::down_cast<ASR::IntegerConstant_t>(kind_expr)->m_type)
                ->m_kind;
        break;
      }
      case ASR::exprType::RealConstant: {
        kind_num = ASR::down_cast<ASR::Real_t>(
                       ASR::down_cast<ASR::RealConstant_t>(kind_expr)->m_type)
                       ->m_kind;
        break;
      }
      case ASR::exprType::LogicalConstant: {
        kind_num =
            ASR::down_cast<ASR::Logical_t>(
                ASR::down_cast<ASR::LogicalConstant_t>(kind_expr)->m_type)
                ->m_kind;
        break;
      }
    ...
    ASR::ttype_t *type =
        LFortran::ASRUtils::TYPE(ASR::make_Integer_t(al, loc, 4, nullptr, 0));
    return ASR::down_cast<ASR::expr_t>(
        ASR::make_IntegerConstant_t(al, loc, kind_num, type));
  }

  static ASR::expr_t *eval_bit_size(Allocator &al, const Location &loc,
                                    Vec<ASR::expr_t *> &args) {
    ...
    return ASRUtils::EXPR(
        ASR::make_IntegerConstant_t(al, loc, bit_size_val, int32_type));
  }
  ...
  static ASR::expr_t *eval_not(Allocator &al, const Location &loc,
    ...
    ASR::IntegerConstant_t *arg_int =
        ASR::down_cast<ASR::IntegerConstant_t>(arg);
    int64_t not_arg = ~(arg_int->m_n);
    ASR::ttype_t *int_type = LFortran::ASRUtils::TYPE(
        ASR::make_Integer_t(al, loc, arg_int_type->m_kind, nullptr, 0));
    return ASRUtils::EXPR(
        ASR::make_IntegerConstant_t(al, loc, not_arg, int_type));
  }
  ...
  static ASR::expr_t *eval_tiny(Allocator &al, const Location &loc,
    ...
    if (ASR::is_a<LFortran::ASR::Real_t>(*tiny_type)) {
      // We don't actually need the value yet, it is enough to know it is a
      // double but it might provide further information later (precision)
      // double tiny_val =
      // ASR::down_cast<ASR::RealConstant_t>(LFortran::ASRUtils::expr_value(tiny_expr))->m_r;
      int tiny_kind = LFortran::ASRUtils::extract_kind_from_ttype_t(tiny_type);
      if (tiny_kind == 4) {
        float low_val = std::numeric_limits<float>::min();
        return ASR::down_cast<ASR::expr_t>(
            ASR::make_RealConstant_t(al, loc,
                                     low_val,  // value
                                     tiny_type));
      } else {
      ...
      throw SemanticError("Argument for tiny must be Real", loc);
    }
  }
  ...
  static ASR::expr_t *eval_floor(Allocator &al, const Location &loc,
    ...
    if (LFortran::ASR::is_a<LFortran::ASR::Real_t>(*func_type)) {
      ...
      ASR::ttype_t *type =
          LFortran::ASRUtils::TYPE(ASR::make_Integer_t(al, loc, 4, nullptr, 0));
      return ASR::down_cast<ASR::expr_t>(
          ASR::make_IntegerConstant_t(al, loc, ival, type));
    } else {
  ...
  typedef std::complex<double> (*trig_eval_callback_complex_double)(
      std::complex<double>);
  static ASR::expr_t *eval_trig(
    ...
    LFORTRAN_ASSERT(ASRUtils::all_args_evaluated(args));
    if (args.size() != 1) {
      throw SemanticError("Intrinsic trig function accepts exactly 1 argument",
                          loc);
    }
    ...
    if (LFortran::ASR::is_a<LFortran::ASR::Real_t>(*t)) {
      ...
      return ASR::down_cast<ASR::expr_t>(
          ASR::make_RealConstant_t(al, loc, val, t));
      ...
      if (trig_complex_double) {
        ...
        im = std::imag(result);
        return ASR::down_cast<ASR::expr_t>(
            ASR::make_ComplexConstant_t(al, loc, re, im, t));
      } else {
        return nullptr;
      }
    } else {
      throw SemanticError("Argument for trig function must be Real or Complex",
                          loc);
    }
  }
  ...
  static ASR::expr_t *eval_2args(Allocator &al, const Location &loc,
    ...
    LFORTRAN_ASSERT(ASRUtils::all_args_evaluated(args));
    if (args.size() != 2) {
      throw SemanticError("This intrinsic function accepts exactly 2 arguments",
                          loc);
    }
    ...
    if (ASR::is_a<LFortran::ASR::Real_t>(*t1) &&
        ASR::is_a<LFortran::ASR::Real_t>(*t2)) {
      ...
      return ASR::down_cast<ASR::expr_t>(
          ASR::make_RealConstant_t(al, loc, val, t1));
    } else {
      throw SemanticError("Arguments for this intrinsic function must be Real",
                          loc);
    }
  }
  ...
  static ASR::expr_t *eval_2args_ri(Allocator &al, const Location &loc,
    ...
    if (ASR::is_a<LFortran::ASR::Real_t>(*t1) &&
      ...
      return ASR::down_cast<ASR::expr_t>(
          ASR::make_RealConstant_t(al, loc, val, t1));
    } else if (ASR::is_a<LFortran::ASR::Integer_t>(*t1) &&
      ...
      throw SemanticError(
          "Arguments for this intrinsic function must be Real or Integer", loc);
    }
  }

#define TRIG_CB(X)                                                    \
  static std::complex<double> lfortran_z##X(std::complex<double> x) { \
    return std::X(x);                                                 \
  }
#define TRIG_CB2(X)                                                \
  static ASR::expr_t *eval_##X(Allocator &al, const Location &loc, \
                               Vec<ASR::expr_t *> &args) {         \
    return eval_trig(al, loc, args, &X, &lfortran_z##X);           \
  }
#define TRIG2_CB(X, Y)                                                \
  static std::complex<double> lfortran_z##Y(std::complex<double> x) { \
    return std::X(x);                                                 \
  }
#define TRIG2_CB2(X, Y)                                            \
  static ASR::expr_t *eval_##Y(Allocator &al, const Location &loc, \
                               Vec<ASR::expr_t *> &args) {         \
...
#define TRIG2(X, Y) \
  ...
  static ASR::expr_t *eval_erf(Allocator &al, const Location &loc,
                               Vec<ASR::expr_t *> &args) {
    return eval_trig(al, loc, args, &erf, nullptr);
  }
  static ASR::expr_t *eval_erfc(Allocator &al, const Location &loc,
                                Vec<ASR::expr_t *> &args) {
    return eval_trig(al, loc, args, &erfc, nullptr);
  }
  static ASR::expr_t *eval_gamma(Allocator &al, const Location &loc,
                                 Vec<ASR::expr_t *> &args) {
    return eval_trig(al, loc, args, &tgamma, nullptr);
  }
  static ASR::expr_t *eval_log_gamma(Allocator &al, const Location &loc,
                                     Vec<ASR::expr_t *> &args) {
    return eval_trig(al, loc, args, &lgamma, nullptr);
  }
  static ASR::expr_t *eval_log10(Allocator &al, const Location &loc,
                                 Vec<ASR::expr_t *> &args) {
    return eval_trig(al, loc, args, &log10, nullptr);
  }
  static ASR::expr_t *eval_atan2(Allocator &al, const Location &loc,
                                 Vec<ASR::expr_t *> &args) {
    return eval_2args(al, loc, args, &atan2);
  }

  static ASR::expr_t *eval_len(Allocator &al, const Location &loc,
                               Vec<ASR::expr_t *> &args) {
    ...
    ASR::StringConstant_t *value_str =
        ASR::down_cast<ASR::StringConstant_t>(arg_value);
    int64_t len_str = to_lower(value_str->m_s).length();
    ASR::ttype_t *type =
        LFortran::ASRUtils::TYPE(ASR::make_Integer_t(al, loc, 4, nullptr, 0));
    return ASR::down_cast<ASR::expr_t>(
        ASR::make_IntegerConstant_t(al, loc, len_str, type));
  }
  ...
  static double lfortran_mod(double x, double y) { return std::fmod(x, y); }
  ...
  static ASR::expr_t *eval_modulo(Allocator &al, const Location &loc,
                                  Vec<ASR::expr_t *> &args) {
    return eval_2args_ri(al, loc, args, &IntrinsicProcedures::lfortran_modulo,
                         &IntrinsicProcedures::lfortran_modulo_i);
  }

  static ASR::expr_t *eval_mod(Allocator &al, const Location &loc,
                               Vec<ASR::expr_t *> &args) {
    return eval_2args_ri(al, loc, args, &IntrinsicProcedures::lfortran_mod,
                         &IntrinsicProcedures::lfortran_mod_i);
  }

  static double lfortran_min(double x, double y) { return std::fmin(x, y); }
  ...
  static ASR::expr_t *eval_min(Allocator &al, const Location &loc,
                               Vec<ASR::expr_t *> &args) {
    return eval_2args_ri(al, loc, args, &IntrinsicProcedures::lfortran_min,
                         &IntrinsicProcedures::lfortran_min_i, true);
  }

  static ASR::expr_t *eval_dmin1(Allocator &al, const Location &loc,
                                 Vec<ASR::expr_t *> &args) {
    return eval_2args_ri(al, loc, args, &IntrinsicProcedures::lfortran_min,
                         &IntrinsicProcedures::lfortran_min_i, true);
  }

  static ASR::expr_t *eval_min0(Allocator &al, const Location &loc,
                                Vec<ASR::expr_t *> &args) {
    return eval_2args_ri(al, loc, args, &IntrinsicProcedures::lfortran_min,
                         &IntrinsicProcedures::lfortran_min_i, true);
  }

  static double lfortran_max(double x, double y) { return std::fmax(x, y); }
  ...
  static ASR::expr_t *eval_max(Allocator &al, const Location &loc,
                               Vec<ASR::expr_t *> &args) {
    return eval_2args_ri(al, loc, args, &IntrinsicProcedures::lfortran_max,
                         &IntrinsicProcedures::lfortran_max_i, true);
  }

  static ASR::expr_t *eval_dmax1(Allocator &al, const Location &loc,
                                 Vec<ASR::expr_t *> &args) {
    return eval_2args_ri(al, loc, args, &IntrinsicProcedures::lfortran_max,
                         &IntrinsicProcedures::lfortran_max_i, true);
  }

  static ASR::expr_t *eval_max0(Allocator &al, const Location &loc,
                                Vec<ASR::expr_t *> &args) {
    return eval_2args_ri(al, loc, args, &IntrinsicProcedures::lfortran_max,
                         &IntrinsicProcedures::lfortran_max_i, true);
  }
  ...
  static ASR::expr_t *eval_abs(Allocator &al, const Location &loc,
    ...
    LFORTRAN_ASSERT(ASRUtils::all_args_evaluated(args));
    if (args.size() != 1) {
      throw SemanticError("Intrinsic abs function accepts exactly 1 argument",
                          loc);
    }
    ASR::expr_t *trig_arg = args[0];
    ASR::ttype_t *t = LFortran::ASRUtils::expr_type(args[0]);
    if (LFortran::ASR::is_a<LFortran::ASR::Real_t>(*t)) {
      ...
      return ASR::down_cast<ASR::expr_t>(
          ASR::make_IntegerConstant_t(al, loc, val, t));
    } else if (LFortran::ASR::is_a<LFortran::ASR::Complex_t>(*t)) {
      double re = ASR::down_cast<ASR::ComplexConstant_t>(trig_arg)->m_re;
      double im = ASR::down_cast<ASR::ComplexConstant_t>(trig_arg)->m_im;
      std::complex<double> x(re, im);
      ...
      return ASR::down_cast<ASR::expr_t>(
          ASR::make_RealConstant_t(al, loc, result, t));
    } else {
      throw SemanticError(
          "Argument of the abs function must be Integer, Real or Complex", loc);
    }
  }
  ...
  static ASR::expr_t *eval_range(Allocator &al, const Location &loc,
                                 Vec<ASR::expr_t *> &args) {
    if (args.size() != 1) {
      throw SemanticError("Intrinsic range function accepts exactly 1 argument",
                          loc);
    }
    ASR::ttype_t *t = LFortran::ASRUtils::expr_type(args[0]);
    int64_t range_val = -1;
    if (LFortran::ASR::is_a<LFortran::ASR::Real_t>(*t)) {
      ...
      if (t_real->m_kind == 4) {
        ...
        throw SemanticError(
            "Only 32 and 64 bit kinds are supported in range intrinsic.", loc);
      }
    } else if (LFortran::ASR::is_a<LFortran::ASR::Integer_t>(*t)) {
      ...
      if (t_int->m_kind == 4) {
        ...
        throw SemanticError(
            "Only 32, 64, 8 and 16 bit kinds are supported in range intrinsic.",
            loc);
      }
    } else if (LFortran::ASR::is_a<LFortran::ASR::Complex_t>(*t)) {
      ...
      throw SemanticError(
          "Argument of the range function must be Integer, Real or Complex",
          loc);
    }
    ASR::ttype_t *tmp_int_type =
        LFortran::ASRUtils::TYPE(ASR::make_Integer_t(al, loc, 4, nullptr, 0));
    return ASR::down_cast<ASR::expr_t>(
        ASR::make_IntegerConstant_t(al, loc, range_val, tmp_int_type));
    ;
  }
  ...
  static ASR::expr_t *eval_aimag(Allocator &al, const Location &loc,
    ...
    if (LFortran::ASR::is_a<LFortran::ASR::Complex_t>(*t)) {
      ...
      throw SemanticError("Argument of the aimag() function must be Complex",
                          loc);
    }
  }
  ...
  static ASR::expr_t *eval_int(Allocator &al, const Location &loc,
    ...
    if (int_expr->type == ASR::exprType::IntegerBOZ) {
      ...
      ASR::ttype_t *tmp_int_type =
          LFortran::ASRUtils::TYPE(ASR::make_Integer_t(al, loc, 4, nullptr, 0));
      return ASR::down_cast<ASR::expr_t>(
          ASR::make_IntegerConstant_t(al, loc, boz_expr->m_v, tmp_int_type));
      ;
    }
    ...
    if (LFortran::ASR::is_a<LFortran::ASR::Integer_t>(*int_type)) {
      if (int_kind == 4) {
        int64_t ival = ASR::down_cast<ASR::IntegerConstant_t>(
                           LFortran::ASRUtils::expr_value(int_expr))
                           ->m_n;
        return ASR::down_cast<ASR::expr_t>(
            ASR::make_IntegerConstant_t(al, loc, ival, int_type));
      } else {
        int64_t ival = ASR::down_cast<ASR::IntegerConstant_t>(
                           LFortran::ASRUtils::expr_value(int_expr))
                           ->m_n;
      ...
      if (int_kind == 4) {
        ...
        double rv = ASR::down_cast<ASR::RealConstant_t>(
                        LFortran::ASRUtils::expr_value(int_expr))
                        ->m_r;
        int64_t ival = static_cast<int64_t>(rv);
        return ASR::down_cast<ASR::expr_t>(
  ...
  static ASR::expr_t *not_implemented(Allocator & /*al*/,
                                      const Location & /*loc*/,
                                      Vec<ASR::expr_t *> & /*args*/) {
  ...
  static ASR::expr_t *eval_char(Allocator &al, const Location &loc,
    ...
    if (LFortran::ASR::is_a<LFortran::ASR::Integer_t>(*real_type)) {
      ...
      ASR::ttype_t *str_type = LFortran::ASRUtils::TYPE(
          ASR::make_Character_t(al, loc, 1, 1, nullptr, nullptr, 0));
      if (!(c >= 0 && c <= 127)) {
        throw SemanticError(
            "The argument 'x' in char(x) must be in the range 0 <= x <= 127.",
            loc);
      }
      char cc = c;
      std::string svalue;
      ...
      return ASR::down_cast<ASR::expr_t>(
          ASR::make_StringConstant_t(al, loc, str_val, str_type));
    } else {
  ...
  static ASR::expr_t *eval_achar(Allocator &al, const Location &loc,
    ...
    ASR::ttype_t *int_type = LFortran::ASRUtils::expr_type(int_expr);
    if (LFortran::ASR::is_a<LFortran::ASR::Integer_t>(*int_type)) {
      int64_t c = ASR::down_cast<ASR::IntegerConstant_t>(
                      LFortran::ASRUtils::expr_value(int_expr))
                      ->m_n;
      ASR::ttype_t *str_type = LFortran::ASRUtils::TYPE(
          ASR::make_Character_t(al, loc, 1, 1, nullptr, nullptr, 0));
  ...
  static ASR::expr_t *eval_iachar(Allocator &al, const Location &loc,
    ...
    if (LFortran::ASR::is_a<LFortran::ASR::Character_t>(*char_type)) {
      char *c = ASR::down_cast<ASR::StringConstant_t>(
                    LFortran::ASRUtils::expr_value(char_expr))
                    ->m_s;
  ...
  static ASR::expr_t *eval_epsilon(Allocator &al, const Location &loc,
    ...
    if (!LFortran::ASR::is_a<LFortran::ASR::Real_t>(*t)) {
      throw SemanticError(
          "Only inputs of real type are accepted in epsilon intrinsic.", loc);
    }
    ASR::Real_t *t_real = ASR::down_cast<ASR::Real_t>(t);
    if (t_real->m_kind == 4) {
      ...
      throw SemanticError(
          "Only 32 and 64 bit kinds are supported in epsilon intrinsic.", loc);
    }
    return nullptr;
  }

  static ASR::expr_t *eval_new_line(Allocator &al, const Location &loc,
                                    Vec<ASR::expr_t *> &args) {
    LFORTRAN_ASSERT(args.size() == 1);
    char *new_line_str = (char *)"\n";
    return ASR::down_cast<ASR::expr_t>(ASR::make_StringConstant_t(
        al, loc, new_line_str, ASRUtils::expr_type(args[0])));
  }
  ...
  static ASR::expr_t *eval_selected_int_kind(Allocator &al, const Location &loc,
    ...
    if (LFortran::ASR::is_a<LFortran::ASR::Integer_t>(*real_type)) {
      int64_t R = ASR::down_cast<ASR::IntegerConstant_t>(
                      LFortran::ASRUtils::expr_value(real_expr))
                      ->m_n;
      int a_kind = 4;
      if (R < 10) {
        a_kind = 4;
      ...
      ASR::ttype_t *type =
          LFortran::ASRUtils::TYPE(ASR::make_Integer_t(al, loc, 4, nullptr, 0));
      return ASR::down_cast<ASR::expr_t>(
          ASR::make_IntegerConstant_t(al, loc, a_kind, type));
    } else {
      throw SemanticError("integer_int_kind() must have one integer argument",
                          loc);
    }
  }
  static ASR::expr_t *eval_selected_real_kind(Allocator &al,
                                              const Location &loc,
                                              Vec<ASR::expr_t *> &args) {
    LFORTRAN_ASSERT(ASRUtils::all_args_evaluated(args));
    ...
    if (LFortran::ASR::is_a<LFortran::ASR::Integer_t>(*real_type)) {
      ...
      throw SemanticError("integer_real_kind() must have one integer argument",
                          loc);
    }
  }
  static ASR::expr_t *eval_selected_char_kind(Allocator &al,
                                              const Location &loc,
                                              Vec<ASR::expr_t *> &args) {
    LFORTRAN_ASSERT(ASRUtils::all_args_evaluated(args));
    ASR::expr_t *real_expr = args[0];
    ...
    if (LFortran::ASR::is_a<LFortran::ASR::Character_t>(*real_type)) {
      ASR::ttype_t *type =
          LFortran::ASRUtils::TYPE(ASR::make_Integer_t(al, loc, 4, nullptr, 0));
      return ASR::down_cast<ASR::expr_t>(
          ASR::make_IntegerConstant_t(al, loc, 1, type));
    } else {
      throw SemanticError(
          "integer_char_kind() must have one character argument", loc);
    }
  }
  ...
  static ASR::expr_t *eval_ibclr(Allocator &al, const Location &loc,
    ...
    if (args.size() != 2) {
      throw SemanticError(
          "The ibclr intrinsic function accepts exactly 2 arguments", loc);
    }
    ...
    if (ASR::is_a<LFortran::ASR::Integer_t>(*t1) &&
      ...
      if (t1_kind == 4 && pos >= 0 && pos < 32) {
        ...
        throw SemanticError(
            "ibclr(i, pos) for pos < 0 or pos >= bit_size(i) is not allowed",
            loc);
      }
    } else {
      throw SemanticError(
          "Arguments for this intrinsic function must be Integer", loc);
    }
  }
  ...
  static ASR::expr_t *eval_ibset(Allocator &al, const Location &loc,
    ...
    if (args.size() != 2) {
      throw SemanticError(
          "The ibset intrinsic function accepts exactly 2 arguments", loc);
    }
    ASR::expr_t *arg1 = args[0];
    ASR::expr_t *arg2 = args[1];
    ASR::ttype_t *t1 = LFortran::ASRUtils::expr_type(arg1);
    ...
    if (ASR::is_a<LFortran::ASR::Integer_t>(*t1) &&
      ...
      if (t1_kind == 4 && pos >= 0 && pos < 32) {
        ...
        throw SemanticError(
            "ibset(i, pos) for pos < 0 or pos >= bit_size(i) is not allowed",
            loc);
      }
    } else {
      throw SemanticError(
  ...
  static ASR::expr_t *eval_ieor(Allocator &al, const Location &loc,
    ...
    if (args.size() != 2) {
      throw SemanticError(
          "The ieor intrinsic function accepts exactly 2 arguments", loc);
    }
    ASR::expr_t *arg1 = args[0];
    ASR::expr_t *arg2 = args[1];
    ASR::ttype_t *t1 = LFortran::ASRUtils::expr_type(arg1);
    ...
    if (ASR::is_a<LFortran::ASR::Integer_t>(*t1) &&
      ...
      if (t1_kind == 4 && t2_kind == 4) {
        ...
        throw SemanticError(
            "ieor(x, y): x and y should have the same kind type", loc);
      }
    } else {
      throw SemanticError(
          "Arguments for this intrinsic function must be Integer", loc);
  ...
  static int32_t lfortran_ibset32(int32_t i, int pos) { return i | (1 << pos); }

  static int64_t lfortran_ibset64(int64_t i, int pos) {
    return i | (1LL << pos);
  }

  static int32_t lfortran_ieor32(int32_t x, int32_t y) { return x ^ y; }

  static int64_t lfortran_ieor64(int64_t x, int64_t y) { return x ^ y; }
  ...
  static ASR::expr_t *eval_huge(Allocator &al, const Location &loc,
    ...
    if (ASR::is_a<LFortran::ASR::Integer_t>(*huge_type)) {
      ...
      if (kind == 4) {
        ...
        return ASR::down_cast<ASR::expr_t>(
            ASR::make_IntegerConstant_t(al, loc, max_val, huge_type));
      } else if (kind == 8) {
      ...
      if (t1_kind == 4 && t2_kind == 4) {
        ...
        if (shift->type == ASR::exprType::IntegerConstant) {
          ...
          ASR::IntegerUnaryMinus_t *u =
              ASR::down_cast<ASR::IntegerUnaryMinus_t>(shift);
        ...
        if (abs(y) <= 31) {
          ...
          throw SemanticError("shift must be less than 32", loc);
        }
      } else if (t1_kind == 8 && t2_kind == 8) {
        ...
        if (abs(y) <= 63) {
          ...
          throw SemanticError("shift must be less than 64", loc);
        }
      } else {
        throw SemanticError(
