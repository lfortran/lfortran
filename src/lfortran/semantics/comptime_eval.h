...
public:
    ...
    {
        intrinsics_present_in_ASR = { "size", "lbound",   "ubound", "transpose", "matmul",
                                      "pack", "transfer", "cmplx",  "reshape" };
    }
    ...
    */
    ...
    std::string get_module(std::string name, const Location& loc) const
    {
        ...
        if (search != comptime_eval_map.end()) {
            ...
            throw SemanticError("Function '" + name + "' not found among intrinsic procedures",
                                loc);
        }
    }

    ASR::expr_t* comptime_eval(std::string name,
                               Allocator& al,
                               const Location& loc,
    ...
    static ASR::expr_t* eval_kind(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        ...
        switch (kind_expr->type) {
            case ASR::exprType::IntegerConstant: {
                kind_num = ASR::down_cast<ASR::Integer_t>(
            ...
            case ASR::exprType::LogicalConstant: {
                kind_num = ASR::down_cast<ASR::Logical_t>(
        ...
        return ASR::down_cast<ASR::expr_t>(ASR::make_IntegerConstant_t(al, loc, kind_num, type));
    }
    ...
    static ASR::expr_t* eval_tiny(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        ...
        if (ASR::is_a<LFortran::ASR::Real_t>(*tiny_type)) {
            ...
            if (tiny_kind == 4) {
                ...
                return ASR::down_cast<ASR::expr_t>(ASR::make_RealConstant_t(al,
                                                                            loc,
                                                                            low_val,  // value
                                                                            tiny_type));
            } else {
    ...
    static ASR::expr_t* eval_trig(Allocator& al,
                                  const Location& loc,
                                  Vec<ASR::expr_t*>& args,
    ...
    static ASR::expr_t* eval_2args(Allocator& al,
                                   const Location& loc,
                                   Vec<ASR::expr_t*>& args,
    ...
    static ASR::expr_t* eval_2args_ri(Allocator& al,
                                      const Location& loc,
                                      Vec<ASR::expr_t*>& args,
                                      eval2_callback_double eval2_double,
                                      ...
                                      bool is_variadic = false)
    {
        ...
        if (ASR::is_a<LFortran::ASR::Real_t>(*t1) && ASR::is_a<LFortran::ASR::Real_t>(*t2)) {
            ...
            throw SemanticError("Arguments for this intrinsic function must be Real or Integer",
                                loc);
        }
    }
...
#define TRIG_CB2(X)                                                                                \
    static ASR::expr_t* eval_##X(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)      \
    {                                                                                              \
...
#define TRIG2_CB2(X, Y)                                                                            \
    static ASR::expr_t* eval_##Y(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)      \
    {                                                                                              \
...
#define TRIG2(X, Y)                                                                                \
    ...
    static ASR::expr_t* eval_abs(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        ...
        if (LFortran::ASR::is_a<LFortran::ASR::Real_t>(*t)) {
            ...
            throw SemanticError("Argument of the abs function must be Integer, Real or Complex",
                                loc);
        }
    }

    static ASR::expr_t* eval_range(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        if (args.size() != 1) {
        ...
        if (LFortran::ASR::is_a<LFortran::ASR::Real_t>(*t)) {
            ...
            if (t_real->m_kind == 4) {
                ...
                throw SemanticError("Only 32 and 64 bit kinds are supported in range intrinsic.",
                                    loc);
            }
        } else if (LFortran::ASR::is_a<LFortran::ASR::Integer_t>(*t)) {
            ...
            if (t_int->m_kind == 4) {
                ...
                throw SemanticError(
                    "Only 32, 64, 8 and 16 bit kinds are supported in range intrinsic.", loc);
            }
        } else if (LFortran::ASR::is_a<LFortran::ASR::Complex_t>(*t)) {
            ...
            throw SemanticError("Argument of the range function must be Integer, Real or Complex",
                                loc);
        }
    ...
    static ASR::expr_t* eval_aimag(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        LFORTRAN_ASSERT(ASRUtils::all_args_evaluated(args));
        if (args.size() != 1) {
            throw SemanticError("Intrinsic trig function accepts exactly 1 argument", loc);
    ...
    static ASR::expr_t* eval_int(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        ...
        if (LFortran::ASR::is_a<LFortran::ASR::Integer_t>(*int_type)) {
            ...
            if (int_kind == 4) {
                float rv
                    = ASR::down_cast<ASR::RealConstant_t>(LFortran::ASRUtils::expr_value(int_expr))
                          ->m_r;
    ...
    static ASR::expr_t* eval_char(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        ...
        if (LFortran::ASR::is_a<LFortran::ASR::Integer_t>(*real_type)) {
            ...
            if (!(c >= 0 && c <= 127)) {
                throw SemanticError(
                    "The argument 'x' in char(x) must be in the range 0 <= x <= 127.", loc);
            }
            char cc = c;
            std::string svalue;
            svalue += cc;
    ...
    static ASR::expr_t* eval_achar(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        ...
        if (LFortran::ASR::is_a<LFortran::ASR::Integer_t>(*int_type)) {
            int64_t c
                = ASR::down_cast<ASR::IntegerConstant_t>(LFortran::ASRUtils::expr_value(int_expr))
                      ->m_n;
            ASR::ttype_t* str_type = LFortran::ASRUtils::TYPE(
                ASR::make_Character_t(al, loc, 1, 1, nullptr, nullptr, 0));
            char cc = c;
    ...
    static ASR::expr_t* eval_iachar(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        ...
        if (LFortran::ASR::is_a<LFortran::ASR::Character_t>(*char_type)) {
            char* c
                = ASR::down_cast<ASR::StringConstant_t>(LFortran::ASRUtils::expr_value(char_expr))
                      ->m_s;
    ...
    static ASR::expr_t* eval_epsilon(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        ...
        if (t_real->m_kind == 4) {
            ...
            return ASR::down_cast<ASR::expr_t>(ASR::make_RealConstant_t(al, loc, epsilon_val, t));
        } else if (t_real->m_kind == 8) {
            double epsilon_val = std::numeric_limits<double>::min();
            return ASR::down_cast<ASR::expr_t>(ASR::make_RealConstant_t(al, loc, epsilon_val, t));
        } else {
            throw SemanticError("Only 32 and 64 bit kinds are supported in epsilon intrinsic.",
                                loc);
        }
        return nullptr;
    }
    ...
    static ASR::expr_t* eval_new_line(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        ...
        return ASR::down_cast<ASR::expr_t>(
            ASR::make_StringConstant_t(al, loc, new_line_str, ASRUtils::expr_type(args[0])));
    }

    static ASR::expr_t* eval_selected_int_kind(Allocator& al,
                                               const Location& loc,
                                               Vec<ASR::expr_t*>& args)
    {
        LFORTRAN_ASSERT(ASRUtils::all_args_evaluated(args));
        ...
        if (LFortran::ASR::is_a<LFortran::ASR::Integer_t>(*real_type)) {
            int64_t R
                = ASR::down_cast<ASR::IntegerConstant_t>(LFortran::ASRUtils::expr_value(real_expr))
                      ->m_n;
            int a_kind = 4;
            if (R < 10) {
                a_kind = 4;
            ...
            ASR::ttype_t* type
                = LFortran::ASRUtils::TYPE(ASR::make_Integer_t(al, loc, 4, nullptr, 0));
            return ASR::down_cast<ASR::expr_t>(ASR::make_IntegerConstant_t(al, loc, a_kind, type));
        } else {
                                                ...
                                                Vec<ASR::expr_t*>& args)
    {
        ...
        if (LFortran::ASR::is_a<LFortran::ASR::Character_t>(*real_type)) {
            ASR::ttype_t* type
                = LFortran::ASRUtils::TYPE(ASR::make_Integer_t(al, loc, 4, nullptr, 0));
            return ASR::down_cast<ASR::expr_t>(ASR::make_IntegerConstant_t(al, loc, 1, type));
        } else {
    ...
    static ASR::expr_t* eval_ibclr(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        ...
        if (ASR::is_a<LFortran::ASR::Integer_t>(*t1) && ASR::is_a<LFortran::ASR::Integer_t>(*t2)) {
            ...
            if (t1_kind == 4 && pos >= 0 && pos < 32) {
                ...
                ASR::ttype_t* type
                    = LFortran::ASRUtils::TYPE(ASR::make_Integer_t(al, loc, t1_kind, nullptr, 0));
                return ASR::down_cast<ASR::expr_t>(ASR::make_IntegerConstant_t(al, loc, val, type));
                ...
                throw SemanticError(
                    "ibclr(i, pos) for pos < 0 or pos >= bit_size(i) is not allowed", loc);
            }
        } else {
    ...
    static ASR::expr_t* eval_ibset(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        ...
        if (ASR::is_a<LFortran::ASR::Integer_t>(*t1) && ASR::is_a<LFortran::ASR::Integer_t>(*t2)) {
            ...
            if (t1_kind == 4 && pos >= 0 && pos < 32) {
                ...
                throw SemanticError(
                    "ibset(i, pos) for pos < 0 or pos >= bit_size(i) is not allowed", loc);
            }
        } else {
            throw SemanticError("Arguments for this intrinsic function must be Integer", loc);
        }
    ...
    static ASR::expr_t* eval_ishft(Allocator& al, const Location& loc, Vec<ASR::expr_t*>& args)
    {
        LFORTRAN_ASSERT(ASRUtils::all_args_evaluated(args));
        if (args.size() != 2) {
            throw SemanticError("The ishft intrinsic function accepts exactly 2 arguments", loc);
        }
        ASR::expr_t* i = args[0];
        ...
        if (ASR::is_a<LFortran::ASR::Integer_t>(*t1) && ASR::is_a<LFortran::ASR::Integer_t>(*t2)) {
            ...
            if (t1_kind == 4 && t2_kind == 4) {
                ...
                throw SemanticError("ishft(x, y): x and y should have the same kind type", loc);
            }
        } else {
            throw SemanticError("Arguments for this intrinsic function must be an Integer", loc);
        }
    }
