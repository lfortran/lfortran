...
public:
    ...
    */
    ...
    void visit_Assignment(const AST::Assignment_t& x)
    {
        ...
        ASR::ttype_t* target_type = ASRUtils::expr_type(target);
        ASR::ttype_t* value_type = ASRUtils::expr_type(value);
        // Check for ArrayBroadcast - this catches cases with variable indices
        // Check for ArrayBroadcast - this catches cases with variable indices
        if (ASR::is_a<ASR::ArrayBroadcast_t>(*value)) {
            ASR::ArrayBroadcast_t* broadcast = ASR::down_cast<ASR::ArrayBroadcast_t>(value);

            // m_shape contains what shape the array actually is
            // m_type contains what shape it's being broadcast to (should match target)
            ASR::ttype_t* actual_shape_type = ASRUtils::expr_type(broadcast->m_shape);
            ASR::ttype_t* broadcast_to_type = broadcast->m_type;

            ASR::dimension_t* actual_dims = nullptr;
            ASR::dimension_t* broadcast_to_dims = nullptr;
            size_t actual_n_dims
                = ASRUtils::extract_dimensions_from_ttype(actual_shape_type, actual_dims);
            size_t broadcast_to_n_dims
                = ASRUtils::extract_dimensions_from_ttype(broadcast_to_type, broadcast_to_dims);

            if (actual_n_dims > 0 && broadcast_to_n_dims > 0
                && actual_n_dims == broadcast_to_n_dims) {
                for (size_t i = 0; i < actual_n_dims; i++) {
                    int64_t actual_size = -1, broadcast_to_size = -1;

                    if (actual_dims[i].m_length
                        && ASR::is_a<ASR::IntegerConstant_t>(*actual_dims[i].m_length)) {
                        actual_size
                            = ASR::down_cast<ASR::IntegerConstant_t>(actual_dims[i].m_length)->m_n;
                    }

                    if (broadcast_to_dims[i].m_length
                        && ASR::is_a<ASR::IntegerConstant_t>(*broadcast_to_dims[i].m_length)) {
                        broadcast_to_size
                            = ASR::down_cast<ASR::IntegerConstant_t>(broadcast_to_dims[i].m_length)
                                  ->m_n;
                    }

                    if (actual_size > 0 && broadcast_to_size > 0
                        && actual_size != broadcast_to_size) {
                        std::string msg
                            = "Different shape for array assignment at (1) on dimension "
                              + std::to_string(i + 1) + " (" + std::to_string(broadcast_to_size)
                              + " and " + std::to_string(actual_size) + ")";
        ...
        {
            // Check if we have an ArrayBroadcast - this indicates a dimension mismatch
            // that LFortran is trying to broadcast
            if (ASR::is_a<ASR::ArrayBroadcast_t>(*value)) {
                ASR::ArrayBroadcast_t* broadcast = ASR::down_cast<ASR::ArrayBroadcast_t>(value);

                // Get the actual array being broadcast (before broadcasting)
                ASR::ttype_t* broadcast_array_type = ASRUtils::expr_type(broadcast->m_array);

                // Get dimensions
                ASR::dimension_t* target_dims = nullptr;
                ASR::dimension_t* broadcast_dims = nullptr;
                size_t target_n_dims
                    = ASRUtils::extract_dimensions_from_ttype(target_type, target_dims);
                size_t broadcast_n_dims
                    = ASRUtils::extract_dimensions_from_ttype(broadcast_array_type, broadcast_dims);

                // If both have dimensions and we can evaluate them
                if (target_n_dims > 0 && broadcast_n_dims > 0
                    && target_n_dims == broadcast_n_dims) {
                    for (size_t i = 0; i < target_n_dims; i++) {
                        int64_t target_size = -1;
                        int64_t broadcast_size = -1;

                        if (target_dims[i].m_length
                            && ASR::is_a<ASR::IntegerConstant_t>(*target_dims[i].m_length)) {
                            target_size
                                = ASR::down_cast<ASR::IntegerConstant_t>(target_dims[i].m_length)
                                      ->m_n;
                        }

                        if (broadcast_dims[i].m_length
                            && ASR::is_a<ASR::IntegerConstant_t>(*broadcast_dims[i].m_length)) {
                            broadcast_size
                                = ASR::down_cast<ASR::IntegerConstant_t>(broadcast_dims[i].m_length)
                                      ->m_n;
                        }

                        // If both sizes are known and different, report error
                        if (target_size > 0 && broadcast_size > 0
                            && target_size != broadcast_size) {
                            std::string msg
                                = "Different shape for array assignment at (1) on dimension "
                                  + std::to_string(i + 1) + " (" + std::to_string(target_size)
                                  + " and " + std::to_string(broadcast_size) + ")";
                            diag.add(Diagnostic(msg,
                                                Level::Error,
                                                Stage::Semantic,
                                                { Label("", { x.base.base.loc }) }));
            ...
            // Only check if both target and value are arrays (original check for ArraySection)
            if (ASRUtils::is_array(target_type) && ASRUtils::is_array(value_type)) {
                ...
                // Get dimensions for value
                // Special handling for ArraySection
                if (ASR::is_a<ASR::ArraySection_t>(*value)) {
                    ASR::ArraySection_t* array_section = ASR::down_cast<ASR::ArraySection_t>(value);
                    size_t value_n_dims = array_section->n_args;

                    // Check if rank matches
                    if (target_n_dims != value_n_dims) {
                        diag.add(Diagnostic("Different number of dimensions in array assignment",
                                            Level::Error,
                                            Stage::Semantic,
                                            { Label("", { x.base.base.loc }) }));
                        throw SemanticAbort();
                    ...
                    for (size_t i = 0; i < target_n_dims; i++) {
                        ...
                        // Calculate value dimension size from array section
                        ASR::array_index_t* arg = &array_section->m_args[i];
                        int64_t value_size = -1;

                        if (arg->m_right != nullptr) {
                            // This is a range like i0:i0 or 1:5
                            // Try to compute the size
                            int64_t left_val = -1, right_val = -1, step_val = 1;

                            if (arg->m_left && ASR::is_a<ASR::IntegerConstant_t>(*arg->m_left)) {
                                left_val = ASR::down_cast<ASR::IntegerConstant_t>(arg->m_left)->m_n;
                            } else if (arg->m_left) {
                                ASR::expr_t* left_value = ASRUtils::expr_value(arg->m_left);
                                if (left_value && ASR::is_a<ASR::IntegerConstant_t>(*left_value)) {
                                    left_val
                                        = ASR::down_cast<ASR::IntegerConstant_t>(left_value)->m_n;
                                }
                            }

                            if (arg->m_right && ASR::is_a<ASR::IntegerConstant_t>(*arg->m_right)) {
                                right_val
                                    = ASR::down_cast<ASR::IntegerConstant_t>(arg->m_right)->m_n;
                            } else if (arg->m_right) {
                                ASR::expr_t* right_value = ASRUtils::expr_value(arg->m_right);
                                if (right_value
                                    && ASR::is_a<ASR::IntegerConstant_t>(*right_value)) {
                                    right_val
                                        = ASR::down_cast<ASR::IntegerConstant_t>(right_value)->m_n;
                                }
                            }

                            if (arg->m_step && ASR::is_a<ASR::IntegerConstant_t>(*arg->m_step)) {
                                step_val = ASR::down_cast<ASR::IntegerConstant_t>(arg->m_step)->m_n;
                            }

                            if (left_val != -1 && right_val != -1 && step_val != 0) {
                                value_size = (right_val - left_val) / step_val + 1;
                            }
                        } else if (arg->m_left) {
                            // Single index, size is 1
                            value_size = 1;
                        }

                        // Compare sizes if both are known
                        if (target_size > 0 && value_size > 0 && target_size != value_size) {
                            std::string msg
                                = "Different shape for array assignment at (1) on dimension "
                                  + std::to_string(i + 1) + " (" + std::to_string(target_size)
                                  + " and " + std::to_string(value_size) + ")";
                            diag.add(Diagnostic(msg,
                                                Level::Error,
                                                Stage::Semantic,
                                                { Label("", { x.base.base.loc }) }));
                    ...
                    // Regular array (not ArraySection)
                    ASR::dimension_t* value_dims = nullptr;
                    size_t value_n_dims
                        = ASRUtils::extract_dimensions_from_ttype(value_type, value_dims);
                    ...
                    for (size_t i = 0; i < target_n_dims; i++) {
                        ...
                        if (value_dims[i].m_length
                            && ASR::is_a<ASR::IntegerConstant_t>(*value_dims[i].m_length)) {
                            value_size
                                = ASR::down_cast<ASR::IntegerConstant_t>(value_dims[i].m_length)
                                      ->m_n;
                        }

                        // If both are known constants and they differ, error
                        if (target_size > 0 && value_size > 0 && target_size != value_size) {
                            std::string msg
                                = "Different shape for array assignment at (1) on dimension "
                                  + std::to_string(i + 1) + " (" + std::to_string(target_size)
        ...
        target_type = ASRUtils::type_get_past_allocatable(ASRUtils::expr_type(target));
        value_type = ASRUtils::type_get_past_allocatable(ASRUtils::expr_type(value));
        ...
        // Before creating ArrayBroadcast, check if dimensions would mismatch
        // This catches cases like: a(2) = temp(i0:i0) where i0 makes it a scalar
        ASR::dimension_t* target_dims = nullptr;
        size_t target_n_dims
            = ASRUtils::extract_dimensions_from_ttype(ASRUtils::expr_type(target), target_dims);

        // If target is an array but value is not (or has different dimensions),
        // and we're about to broadcast, check if it's a valid broadcast
        if (target_n_dims > 0 && target_dims != nullptr) {
            ASR::dimension_t* value_dims = nullptr;
            size_t value_n_dims
                = ASRUtils::extract_dimensions_from_ttype(ASRUtils::expr_type(value), value_dims);

            // Broadcasting from scalar (value_n_dims=0) to array
            // Only block if the value comes from an ArrayItem (array section result)
            // Allow true scalar constants like: a = 5
            if (value_n_dims == 0 && ASR::is_a<ASR::ArrayItem_t>(*value)) {
                // This is an array section that resulted in a scalar (single element)
                // Check if target has more than 1 element total
                int64_t target_total_size = 1;
                for (size_t i = 0; i < target_n_dims; i++) {
                    if (target_dims[i].m_length
                        && ASR::is_a<ASR::IntegerConstant_t>(*target_dims[i].m_length)) {
                        int64_t dim_size
                            = ASR::down_cast<ASR::IntegerConstant_t>(target_dims[i].m_length)->m_n;
                        target_total_size *= dim_size;
                    } else {
                        target_total_size = -1;  // Unknown size
                        break;
                    }
                }

                // If we can determine the target size and it's > 1, this is an error
                if (target_total_size > 1) {
                    std::string msg = "Different shape for array assignment at (1) on dimension 1 ("
                                      + std::to_string(target_total_size) + " and 1)";
                    diag.add(Diagnostic(
                        msg, Level::Error, Stage::Semantic, { Label("", { x.base.base.loc }) }));
                    throw SemanticAbort();
                }
            }
        }
        // Check for dimension mismatch before creating ArrayBroadcast
        if (ASRUtils::is_array(ASRUtils::expr_type(target))
            ...
            if (target_n_dims > 0 && value_n_dims > 0 && target_n_dims == value_n_dims) {
                for (size_t i = 0; i < target_n_dims; i++) {
                    ...
                    if (value_dims[i].m_length
                        && ASR::is_a<ASR::IntegerConstant_t>(*value_dims[i].m_length)) {
                        value_size
                            = ASR::down_cast<ASR::IntegerConstant_t>(value_dims[i].m_length)->m_n;
                    }
                    ...
                    if (target_size > 0 && value_size > 0 && target_size != value_size) {
                        std::string msg
                            = "Different shape for array assignment at (1) on dimension "
                              + std::to_string(i + 1) + " (" + std::to_string(target_size) + " and "
