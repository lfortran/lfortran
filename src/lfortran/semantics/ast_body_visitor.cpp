...
public:
    ...
    */
    ...
    // Called from visit_Assignment when either:
    //   (a) x.m_infer_type is true (:= syntax), or
    //   (b) compiler_options.infer_mode is true and LHS is an undeclared bare name
    bool try_infer_and_declare(const AST::Assignment_t &x) {
        ...
        if (!x.m_infer_type && !compiler_options.infer_mode) {
            return false;
        }
