...
class BodyVisitor : public CommonVisitor<BodyVisitor> {
 ...
 public:
  ...
  void visit_Block(const AST::Block_t &x) {
    ...
    ASR::asr_t *block = ASR::make_Block_t(al, x.base.base.loc, current_scope,
                                          s2c(al, name), body.p, body.size());
  ...
  void transform_stmts(Vec<ASR::stmt_t *> &body, size_t n_body,
                       AST::stmt_t **m_body) {
    tmp = nullptr;
    for (size_t i = 0; i < n_body; i++) {
      ...
      if (tmp != nullptr) {
        ...
        if (tmp_stmt->type == ASR::stmtType::SubroutineCall) {
          ASR::stmt_t *impl_decl =
              create_implicit_deallocate_subrout_call(tmp_stmt);
  ...
  void visit_Open(const AST::Open_t &x) {
    ASR::expr_t *a_newunit = nullptr, *a_filename = nullptr,
                *a_status = nullptr;
    if (x.n_args > 1) {
      throw SemanticError(
    ...
    for (std::uint32_t i = 0; i < x.n_kwargs; i++) {
      ...
      if (m_arg_str == std::string("newunit") ||
        ...
        if ((m_arg_str == std::string("newunit") &&
          ...
          throw SemanticError(
              "`newunit`/`unit` must be a variable of type, Integer or "
              "IntegerPointer",
              x.base.base.loc);
        }
        ...
        ASR::ttype_t *a_filename_type =
            LFortran::ASRUtils::expr_type(a_filename);
        if (!ASR::is_a<ASR::Character_t>(
                *ASRUtils::type_get_past_pointer(a_filename_type))) {
          throw SemanticError(
        ...
        if (!ASR::is_a<ASR::Character_t>(
                *ASRUtils::type_get_past_pointer(a_status_type))) {
          throw SemanticError(
    ...
    if (a_newunit == nullptr) {
      throw SemanticError(
          "`newunit` or `unit` must be specified either in argument or keyword "
          "arguments.",
          x.base.base.loc);
    }
    tmp = ASR::make_FileOpen_t(al, x.base.base.loc, x.m_label, a_newunit,
                               a_filename, a_status);
  }
  ...
  void visit_Close(const AST::Close_t &x) {
      ...
      if (m_arg_str == std::string("unit")) {
        ...
        if (a_iomsg->type != ASR::exprType::Var ||
            (!ASR::is_a<ASR::Character_t>(
                *ASRUtils::type_get_past_pointer(a_iomsg_type)))) {
          throw SemanticError(
        ...
        if (kwarg.m_value->type != AST::exprType::Num) {
          throw SemanticError("`err` must be a literal integer",
                              x.base.base.loc);
        }
        this->visit_expr(*kwarg.m_value);
    ...
    tmp = ASR::make_FileClose_t(al, x.base.base.loc, x.m_label, a_unit,
                                a_iostat, a_iomsg, a_err, a_status);
  }
  ...
  void create_read_write_ASR_node(const AST::stmt_t &read_write_stmt,
    ...
    ASR::expr_t *a_unit, *a_fmt, *a_iomsg, *a_iostat, *a_id, *a_separator,
        *a_end;
    ...
    for (std::uint32_t i = 0; i < n_kwargs; i++) {
      ...
      if (m_arg_str == std::string("unit")) {
        ...
        if (kwarg.m_value != nullptr) {
          ...
          if (!ASR::is_a<ASR::Integer_t>(
                  *ASRUtils::type_get_past_pointer(a_unit_type))) {
        ...
        if (kwarg.m_value != nullptr) {
          ...
          if (!ASR::is_a<ASR::Character_t>(
                  *ASRUtils::type_get_past_pointer(a_fmt_type))) {
    ...
    if (_type == AST::stmtType::Write) {
      tmp = ASR::make_FileWrite_t(al, loc, m_label, a_unit, a_fmt, a_iomsg,
                                  a_iostat, a_id, a_values_vec.p, n_values,
                                  a_separator, a_end);
    } else if (_type == AST::stmtType::Read) {
  ...
  template <typename T>
  void fill_args_for_rewind_inquire_flush(
      const T &x, const size_t max_args, std::vector<ASR::expr_t *> &args,
      const size_t args_size, std::map<std::string, size_t> &argname2idx,
    ...
    for (size_t i = 0; i < x.n_kwargs; i++) {
      if (x.m_kwargs[i].m_value) {
        ...
        if (args[argname2idx[m_arg_string]]) {
          throw SemanticError(m_arg_string + " has already been specified.",
                              x.base.base.loc);
        }
  ...
  void visit_Rewind(const AST::Rewind_t &x) {
    ...
    tmp = ASR::make_FileRewind_t(al, x.base.base.loc, x.m_label, unit, iostat,
                                 err);
  }
  ...
  void visit_Instantiate(const AST::Instantiate_t &x) {
    ...
    if (template_type_parameters.find(x.m_name) ==
        template_type_parameters.end()) {
    ...
    if (template_type_parameters[template_name].size() != x.n_types) {
      throw SemanticError("Number of template arguments don't match",
                          x.base.base.loc);
    }
    ...
    for (size_t i = 0; i < x.n_types; i++) {
      ASR::ttype_t *type =
          determine_type(x.base.base.loc, x.m_types[i], false, dims);
      subs[ASR::down_cast2<ASR::TypeParameter_t>(
               template_type_parameters[template_name][i])
               ->m_param] = type;
    }
    ...
    for (size_t i = 0; i < x.n_symbols; i++) {
      ...
      ASR::symbol_t *s =
          resolve_symbol(x.base.base.loc, use_symbol->m_remote_sym);
      pass_instantiate_generic_function(al, subs, current_scope,
                                        use_symbol->m_local_rename,
  ...
  void visit_Inquire(const AST::Inquire_t &x) {
    ...
    ASR::expr_t *unit = args[0], *file = args[1], *iostat = args[2],
                *err = args[3];
    ASR::expr_t *exist = args[4], *opened = args[5], *number = args[6],
                *named = args[7];
    ASR::expr_t *name = args[8], *access = args[9], *sequential = args[10],
                *direct = args[11];
    ASR::expr_t *form = args[12], *formatted = args[13],
                *unformatted = args[14], *recl = args[15];
    ASR::expr_t *nextrec = args[16], *blank = args[17], *position = args[18],
                *action = args[19];
    ASR::expr_t *read = args[20], *write = args[21], *readwrite = args[22],
                *delim = args[23];
    ASR::expr_t *pad = args[24], *flen = args[25], *blocksize = args[26],
                *convert = args[27];
    ...
    tmp = ASR::make_FileInquire_t(
        al, x.base.base.loc, x.m_label, unit, file, iostat, err, exist, opened,
        number, named, name, access, sequential, direct, form, formatted,
        unformatted, recl, nextrec, blank, position, action, read, write,
        readwrite, delim, pad, flen, blocksize, convert, carriagecontrol,
        iolength);
  }
  ...
  void visit_Flush(const AST::Flush_t &x) {
    ...
    if (!args[0]) {
      throw SemanticError("unit must be present in flush statement arguments",
                          x.base.base.loc);
    }
    ASR::expr_t *unit = args[0], *err = args[1], *iomsg = args[2],
                *iostat = args[3];
    tmp = ASR::make_Flush_t(al, x.base.base.loc, x.m_label, unit, err, iomsg,
                            iostat);
  }
  ...
  void visit_Associate(const AST::Associate_t &x) {
    ...
    if (!(is_target_pointer && !is_value_pointer)) {
      throw SemanticError(
          "Only a pointer variable can be associated with a non-pointer "
          "variable.",
          x.base.base.loc);
    }
  ...
  void visit_AssociateBlock(const AST::AssociateBlock_t &x) {
    ...
    for (size_t i = 0; i < x.n_syms; i++) {
      ...
      if (ASR::is_a<ASR::Var_t>(*tmp_expr)) {
        ...
        ASR::Variable_t *variable =
            ASR::down_cast<ASR::Variable_t>(tmp_var->m_v);
        tmp_storage = variable->m_storage;
        tmp_type = ASRUtils::TYPE(
            ASR::make_Pointer_t(al, tmp_type->base.loc, variable->m_type));
      ...
      ASR::asr_t *v = ASR::make_Variable_t(
          al, x.base.base.loc, new_scope, name_c, ASR::intentType::Local,
          nullptr, nullptr, tmp_storage, tmp_type, ASR::abiType::Source,
          ASR::accessType::Private, ASR::presenceType::Required, false);
      new_scope->add_symbol(name, ASR::down_cast<ASR::symbol_t>(v));
      ASR::expr_t *target_var = ASRUtils::EXPR(
          ASR::make_Var_t(al, v->loc, ASR::down_cast<ASR::symbol_t>(v)));
      if (create_associate_stmt) {
        ASR::stmt_t *associate_stmt = ASRUtils::STMT(ASR::make_Associate_t(
            al, tmp_expr->base.loc, target_var, tmp_expr));
        body.push_back(al, associate_stmt);
      } else {
        ASR::stmt_t *assign_stmt = ASRUtils::STMT(ASR::make_Assignment_t(
            al, tmp_expr->base.loc, target_var, tmp_expr, nullptr));
    ...
    ASR::asr_t *associate_block = ASR::make_AssociateBlock_t(
        al, x.base.base.loc, new_scope, s2c(al, name), body.p, body.size());
    current_scope->add_symbol(name,
  ...
  void visit_Allocate(const AST::Allocate_t &x) {
    ...
    ASR::ttype_t *int32_type = LFortran::ASRUtils::TYPE(
        ASR::make_Integer_t(al, x.base.base.loc, 4, nullptr, 0));
    ASR::expr_t *const_1 = LFortran::ASRUtils::EXPR(
    ...
    for (size_t i = 0; i < x.n_args; i++) {
      ...
      } else if (x.m_args[i].m_start && !x.m_args[i].m_end &&
                 x.m_args[i].m_step) {
      ...
      if (ASR::is_a<ASR::ArraySection_t>(*tmp_stmt)) {
        ASR::ArraySection_t *array_ref =
            ASR::down_cast<ASR::ArraySection_t>(tmp_stmt);
        new_arg.m_a = ASR::down_cast<ASR::symbol_t>(
            (ASR::asr_t *)ASRUtils::EXPR2VAR(array_ref->m_v));
        ...
        ASR::ArrayItem_t *array_ref =
            ASR::down_cast<ASR::ArrayItem_t>(tmp_stmt);
        new_arg.m_a = ASR::down_cast<ASR::symbol_t>(
            (ASR::asr_t *)ASRUtils::EXPR2VAR(array_ref->m_v));
        Vec<ASR::dimension_t> dims_vec;
        dims_vec.reserve(al, array_ref->n_args);
    ...
    if (x.n_keywords >= 1) {
      ...
      errmsg_cond =
          !errmsg_cond && (to_lower(x.m_keywords[0].m_arg) == "errmsg");
      source_cond =
          !source_cond && (to_lower(x.m_keywords[0].m_arg) == "source");
    ...
    if (x.n_keywords >= 2) {
      ...
      errmsg_cond =
          !errmsg_cond && (to_lower(x.m_keywords[1].m_arg) == "errmsg");
      source_cond =
          !source_cond && (to_lower(x.m_keywords[1].m_arg) == "source");
      cond = cond && (stat_cond || errmsg_cond || source_cond);
      if (stat_cond) {
    ...
    if (x.n_keywords >= 3) {
      ...
      errmsg_cond =
          !errmsg_cond && (to_lower(x.m_keywords[2].m_arg) == "errmsg");
      source_cond =
          !source_cond && (to_lower(x.m_keywords[2].m_arg) == "source");
      cond = cond && (stat_cond || errmsg_cond || source_cond);
      if (stat_cond) {
    ...
    tmp = ASR::make_Allocate_t(al, x.base.base.loc, alloc_args_vec.p,
                               alloc_args_vec.size(), stat, errmsg, source);
  }

  // If there are allocatable variables in the local scope it inserts an
  // ImplicitDeallocate node with their list. The ImplicitDeallocate node will
  // deallocate them if they are allocated, otherwise does nothing.
  ASR::stmt_t *create_implicit_deallocate(const Location &loc) {
    ...
    for (auto &item : current_scope->get_scope()) {
      if (item.second->type == ASR::symbolType::Variable) {
        const ASR::symbol_t *sym =
            LFortran::ASRUtils::symbol_get_past_external(item.second);
    ...
    return LFortran::ASRUtils::STMT(
        ASR::make_ImplicitDeallocate_t(al, loc, del_syms.p, del_syms.size()));
  }
  ...
  void visit_Deallocate(const AST::Deallocate_t &x) {
    ...
    tmp = ASR::make_ExplicitDeallocate_t(al, x.base.base.loc, arg_vec.p,
                                         arg_vec.size());
  }
    ...
    switch (x.type) {
      case AST::case_stmtType::CaseStmt: {
        ...
        if (AST::is_a<AST::CaseCondExpr_t>(*(Case_Stmt->m_test[0]))) {
          ...
          tmp = ASR::make_CaseStmt_t(al, x.base.loc, a_test_vec.p,
                                     a_test_vec.size(), case_body_vec.p,
                                     case_body_vec.size());
          break;
        } else {
          ...
          if (Case_Stmt->n_test != 1) {
            throw SemanticError(
                "Not implemented: more than one range condition", x.base.loc);
          }
          ...
          if (condrange->m_start != nullptr) {
            ...
            if (LFortran::ASRUtils::expr_type(m_start)->type !=
                ASR::ttypeType::Integer) {
              throw SemanticError(
                  R"""(Expression in Case selector can only be an Integer)""",
                  x.base.loc);
            }
          }
          if (condrange->m_end != nullptr) {
            ...
            if (LFortran::ASRUtils::expr_type(m_end)->type !=
                ASR::ttypeType::Integer) {
              throw SemanticError(
                  R"""(Expression in Case selector can only be an Integer)""",
                  x.base.loc);
  ...
  void visit_Module(const AST::Module_t &x) {
    ...
    // We have to visit unit_decl_2 because in the example, the Template is
    // directly inside the module and Template is a unit_decl_2
  ...
  ASR::stmt_t *create_implicit_deallocate_subrout_call(ASR::stmt_t *x) {
    ASR::SubroutineCall_t *subrout_call =
        ASR::down_cast<ASR::SubroutineCall_t>(x);
    const ASR::symbol_t *subrout_sym =
        LFortran::ASRUtils::symbol_get_past_external(subrout_call->m_name);
    ...
    for (size_t i = 0; i < subrout_call->n_args; i++) {
      if (subrout_call->m_args[i].m_value &&
        ...
        const ASR::symbol_t *sym =
            LFortran::ASRUtils::symbol_get_past_external(arg_var->m_v);
        if (sym->type == ASR::symbolType::Variable) {
          ASR::Variable_t *var = ASR::down_cast<ASR::Variable_t>(sym);
          const ASR::Var_t *orig_arg_var =
              ASR::down_cast<ASR::Var_t>(subrout->m_args[i]);
          const ASR::symbol_t *orig_sym =
              LFortran::ASRUtils::symbol_get_past_external(orig_arg_var->m_v);
    ...
    return LFortran::ASRUtils::STMT(ASR::make_ImplicitDeallocate_t(
        al, x->base.loc, del_syms.p, del_syms.size()));
  }
  ...
  void visit_Assignment(const AST::Assignment_t &x) {
    ...
    if (LFortran::ASRUtils::use_overloaded_assignment(
            target, value, current_scope, asr, al, x.base.base.loc,
  ...
  ASR::asr_t *create_CFPointer(const AST::SubroutineCall_t &x) {
    ...
    if (shape) {
      int shape_rank = ASRUtils::extract_dimensions_from_ttype(
          ASRUtils::expr_type(shape), shape_dims);
      if (shape_rank != 1) {
        throw SemanticError(
  ...
  void visit_SubroutineCall(const AST::SubroutineCall_t &x) {
    ...
    if (x.n_member == 1) {
      ...
      original_sym =
          resolve_deriv_type_proc(x.base.base.loc, to_lower(x.m_name),
    ...
    if (x.n_keywords > 0) {
      ASR::symbol_t *f2 =
          LFortran::ASRUtils::symbol_get_past_external(original_sym);
    ...
    switch (original_sym->type) {
      ...
      case (ASR::symbolType::GenericProcedure): {
        ASR::GenericProcedure_t *p =
            ASR::down_cast<ASR::GenericProcedure_t>(original_sym);
        int idx;
        if (x.n_member == 1) {
          idx = ASRUtils::select_generic_procedure(
              args_with_mdt, *p, x.base.base.loc,
              [&](const std::string &msg, const Location &loc) {
                throw SemanticError(msg, loc);
              });
        } else {
          idx = ASRUtils::select_generic_procedure(
              args, *p, x.base.base.loc,
              [&](const std::string &msg, const Location &loc) {
                throw SemanticError(msg, loc);
              });
        }
      ...
      case (ASR::symbolType::ExternalSymbol): {
        ASR::ExternalSymbol_t *p =
            ASR::down_cast<ASR::ExternalSymbol_t>(original_sym);
        ...
        if (ASR::is_a<ASR::GenericProcedure_t>(*final_sym)) {
          ASR::GenericProcedure_t *g =
              ASR::down_cast<ASR::GenericProcedure_t>(final_sym);
          int idx = ASRUtils::select_generic_procedure(
              args, *g, x.base.base.loc,
              [&](const std::string &msg, const Location &loc) {
                throw SemanticError(msg, loc);
              });
          // FIXME
          ...
          if (!ASR::is_a<ASR::Function_t>(*final_sym)) {
            throw SemanticError("ExternalSymbol must point to a Subroutine",
                                x.base.base.loc);
          }
    ...
    tmp = ASR::make_SubroutineCall_t(al, x.base.base.loc, final_sym,
                                     original_sym, args.p, args.size(), v_expr);
  }
  ...
  void visit_Print(const AST::Print_t &x) {
    ...
    tmp = ASR::make_Print_t(al, x.base.base.loc, fmt, body.p, body.size(),
                            nullptr, nullptr);
  }
  ...
  void visit_If(const AST::If_t &x) {
    ...
    tmp = ASR::make_If_t(al, x.base.base.loc, test, body.p, body.size(),
                         orelse.p, orelse.size());
  }
  ...
  void visit_IfArithmetic(const AST::IfArithmetic_t &x) {
    ...
    if (is_int) {
      ...
      ASR::expr_t *right = ASRUtils::EXPR(
          ASR::make_IntegerConstant_t(al, x.base.base.loc, 0, type0));
      ASR::ttype_t *type = ASRUtils::TYPE(
          ASR::make_Logical_t(al, x.base.base.loc, 4, nullptr, 0));
      ASR::expr_t *value = nullptr;
      test_lt = ASRUtils::EXPR(
          ASR::make_IntegerCompare_t(al, test_int->base.loc, test_int,
                                     ASR::cmpopType::Lt, right, type, value));
      test_gt = ASRUtils::EXPR(
          ASR::make_IntegerCompare_t(al, test_int->base.loc, test_int,
                                     ASR::cmpopType::Gt, right, type, value));
    } else {
      ASR::ttype_t *type0 = ASRUtils::TYPE(
          ASR::make_Real_t(al, x.base.base.loc, kind, nullptr, 0));
      ASR::expr_t *right = ASRUtils::EXPR(
          ASR::make_RealConstant_t(al, x.base.base.loc, 0, type0));
      ASR::ttype_t *type = ASRUtils::TYPE(
          ASR::make_Logical_t(al, x.base.base.loc, 4, nullptr, 0));
      ASR::expr_t *value = nullptr;
      test_lt = ASRUtils::EXPR(
          ASR::make_RealCompare_t(al, test_int->base.loc, test_int,
                                  ASR::cmpopType::Lt, right, type, value));
      test_gt = ASRUtils::EXPR(
          ASR::make_RealCompare_t(al, test_int->base.loc, test_int,
                                  ASR::cmpopType::Gt, right, type, value));
    ...
    Vec<ASR::stmt_t *> body;
    body.reserve(al, 1);
    body.push_back(al, ASRUtils::STMT(ASR::make_GoTo_t(al, x.base.base.loc,
                                                       x.m_lt_label)));
    Vec<ASR::stmt_t *> orelse;
    orelse.reserve(al, 1);
    ...
    body_gt.push_back(al, ASRUtils::STMT(ASR::make_GoTo_t(al, x.base.base.loc,
                                                          x.m_gt_label)));
    Vec<ASR::stmt_t *> orelse_gt;
    orelse_gt.reserve(al, 1);
    orelse_gt.push_back(al, ASRUtils::STMT(ASR::make_GoTo_t(al, x.base.base.loc,
                                                            x.m_eq_label)));

    orelse.push_back(al, ASRUtils::STMT(ASR::make_If_t(
                             al, x.base.base.loc, test_gt, body_gt.p,
                             body_gt.size(), orelse_gt.p, orelse_gt.size())));
    tmp = ASR::make_If_t(al, x.base.base.loc, test_lt, body.p, body.size(),
                         orelse.p, orelse.size());
  }
  ...
  void visit_WhileLoop(const AST::WhileLoop_t &x) {
    ...
    body.reserve(al, x.n_body);
    transform_stmts(body, x.n_body, x.m_body);
    tmp = ASR::make_WhileLoop_t(al, x.base.base.loc, test, body.p, body.size());
  }
  ...
  void visit_ImpliedDoLoop(const AST::ImpliedDoLoop_t &x) {
    ...
    // std::string a_var_name = std::to_string(iloop_counter) +
    // std::string(x.m_var); iloop_counter += 1; Str a_var_name_f;
    // a_var_name_f.from_str(al, a_var_name);
    // ASR::asr_t* a_variable = ASR::make_Variable_t(al, x.base.base.loc,
    // current_scope, a_var_name_f.c_str(al),
    ...
    if (current_scope->get_symbol(var_name) == nullptr) {
      throw SemanticError(
          "The implied do loop variable '" + var_name + "' is not declared",
          x.base.base.loc);
    };
    ...
    ASR::expr_t *a_var =
        LFortran::ASRUtils::EXPR(ASR::make_Var_t(al, x.base.base.loc, a_sym));
    tmp = ASR::make_ImpliedDoLoop_t(
        al, x.base.base.loc, a_values, n_values, a_var, a_start, a_end,
        a_increment, LFortran::ASRUtils::expr_type(a_start), nullptr);
  }
  ...
  void visit_DoLoop(const AST::DoLoop_t &x) {
    ...
    if (x.m_var) {
      var = LFortran::ASRUtils::EXPR(
          resolve_variable(x.base.base.loc, to_lower(x.m_var)));
    }
    if (x.m_start) {
    ...
    if (x.m_goto_label) {
      if (AST::is_a<AST::Num_t>(*x.m_goto_label)) {
        ...
        if (!ASR::is_a<ASR::Variable_t>(*sym)) {
          throw SemanticError("Symbol needs to be a variable", x.base.base.loc);
        }
        // n_labels GOTO
        Vec<ASR::case_stmt_t *> a_body_vec;
        ...
        for (size_t i = 0; i < x.n_labels; ++i) {
          if (!AST::is_a<AST::Num_t>(*x.m_labels[i])) {
            ...
            body.push_back(al, ASRUtils::STMT(ASR::make_GoTo_t(
                                   al, x.base.base.loc, l->m_n)));
            Vec<ASR::expr_t *> comparator_one;
            comparator_one.reserve(al, 1);
            ASR::ttype_t *int32_type = LFortran::ASRUtils::TYPE(
                ASR::make_Integer_t(al, x.base.base.loc, 4, nullptr, 0));
            comparator_one.push_back(
                al, LFortran::ASRUtils::EXPR(ASR::make_IntegerConstant_t(
                        al, x.base.base.loc, i + 1, int32_type)));
            a_body_vec.push_back(
                al, ASR::down_cast<ASR::case_stmt_t>(ASR::make_CaseStmt_t(
                        al, x.base.base.loc, comparator_one.p, 1, body.p, 1)));
          }
        }
        ASR::expr_t *target_var =
            ASRUtils::EXPR(ASR::make_Var_t(al, x.base.base.loc, sym));
        tmp =
            ASR::make_Select_t(al, x.base.base.loc, target_var, a_body_vec.p,
                               a_body_vec.size(), def_body.p, def_body.size());
      } else {
        throw SemanticError("A goto label must be an integer", x.base.base.loc);
      }
    } else {
...
Result<ASR::TranslationUnit_t *> body_visitor(
    Allocator &al, AST::TranslationUnit_t &ast, diag::Diagnostics &diagnostics,
    ASR::asr_t *unit, CompilerOptions &compiler_options,
    std::map<std::string, std::vector<ASR::asr_t *>>
