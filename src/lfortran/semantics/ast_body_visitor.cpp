...
{
private:
    void check_ArraySectionShapeConformance(ASR::expr_t* target,
                                            ASR::expr_t* value,
                                            const Location& loc);

    bool get_array_dims_at_compile_time(ASR::expr_t* expr,
                                        ASR::ttype_t* type,
                                        std::vector<int64_t>& dims);

    bool compute_array_section_dims(ASR::expr_t* arr_section,
                                    std::vector<int64_t>& dims,
                                    const Location& loc);
...
public:
    ...
    */
    ...
    void visit_Assignment(const AST::Assignment_t& x)
    {
        ...
        // Handle assumed-rank arrays (from main branch)
        ...
        check_ArraySectionShapeConformance(target, value, x.base.base.loc);
        ...
        if (overloaded_stmt == nullptr) {
            ...
            if (!ASRUtils::is_array(ASRUtils::expr_type(target))
                && ASRUtils::is_struct(*ASRUtils::expr_type(target))
                ...
                ASR::stmt_t* alloc_stmt = ASRUtils::STMT(ASR::make_Allocate_t(
                    al, target->base.loc, alloc_args.p, alloc_args.n, nullptr, nullptr, nullptr));
                Vec<ASR::stmt_t*> if_body;
                if_body.reserve(al, 1);
                ...
                ASR::expr_t* allocated_expr = ASRUtils::EXPR(ASR::make_IntrinsicImpureFunction_t(
                    al,
                    target->base.loc,
                    ...
                    allocated_args.n,
                    0,
                    ASRUtils::TYPE(ASR::make_Logical_t(al, target->base.loc, 4)),
                    nullptr));
                ASR::expr_t* if_test_expr = ASRUtils::EXPR(ASR::make_LogicalNot_t(
                    al,
                    target->base.loc,
                    allocated_expr,
                    ASRUtils::TYPE(ASR::make_Logical_t(al, target->base.loc, 4)),
                ...
                ASR::stmt_t* if_stmt = ASRUtils::STMT(ASR::make_If_t(
                    al, target->base.loc, nullptr, if_test_expr, if_body.p, if_body.n, nullptr, 0));
...
bool
BodyVisitor::compute_array_section_dims(ASR::expr_t* arr_section,
                                        std::vector<int64_t>& dims,
                                        const Location& /* loc */)
{
    ASR::ArraySection_t* section = ASR::down_cast<ASR::ArraySection_t>(arr_section);

    for (size_t i = 0; i < section->n_args; i++) {
        ASR::array_index_t& arg = section->m_args[i];

        // If no right bound, this is a scalar index, not a section
        if (!arg.m_right)
            continue;

        // This is a section (left:right:step)
        ASR::expr_t* left = arg.m_left ? ASRUtils::expr_value(arg.m_left) : nullptr;
        ASR::expr_t* right = ASRUtils::expr_value(arg.m_right);
        ASR::expr_t* step = arg.m_step ? ASRUtils::expr_value(arg.m_step) : nullptr;
        ...
        if (step) {
            if (!ASR::is_a<ASR::IntegerConstant_t>(*step)) {
                return false;
            }
            step_val = ASR::down_cast<ASR::IntegerConstant_t>(step)->m_n;
        }
