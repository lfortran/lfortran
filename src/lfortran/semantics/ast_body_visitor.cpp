...
public:
    ...
    void create_read_write_ASR_node(const AST::stmt_t& read_write_stmt, AST::stmtType _type) {
        ...
        ASR::expr_t *a_unit, *a_fmt, *a_iomsg, *a_iostat, *a_size, *a_id, *a_separator, *a_end, *a_fmt_constant, *a_advance, *a_rec;
        a_unit = a_fmt = a_iomsg = a_iostat = a_size = a_id = a_separator = a_end = a_fmt_constant = a_advance = a_rec = nullptr;
        ...
        for( std::uint32_t i = 0; i < n_kwargs; i++ ) {
            ...
            if( m_arg_str == std::string("unit") ) {
                ...
                if (ASRUtils::is_array(a_rec_type)) {
                    diag.add(Diagnostic(
                        "`rec` must be scalar",
                        Level::Error, Stage::Semantic, {
                            Label("",{loc})
                ...
                if (!ASR::is_a<ASR::Integer_t>(*ASRUtils::type_get_past_pointer(a_rec_type))) {
                    diag.add(Diagnostic(
                ...
                if (ASR::is_a<ASR::StringConstant_t>(*adv_val_expr)) {
                    ...
                    body.push_back(al, ASRUtils::STMT(
                        ...
                        nullptr, 0, nullptr, newline, nullptr, formatted, nullptr, nullptr)));
            ...
            if (format_statements.find(label) == format_statements.end()) {
                if (_type == AST::stmtType::Write) {
                    tmp = ASR::make_FileWrite_t(al, loc, m_label, a_unit, a_iomsg, a_iostat,
                        a_id, a_values_vec.p, a_values_vec.size(), a_separator, a_end, nullptr, true, nullptr, a_rec);
                    ...
                    tmp = ASR::make_FileRead_t(al, loc, m_label, a_unit, a_fmt, a_iomsg,
                        ...
                        nullptr, formatted, nullptr, a_rec);
        ...
        if (!a_fmt
            ...
            a_values_vec.size(), a_separator, a_end, overloaded_stmt, formatted, nullptr, a_rec);
            ...
            tmp = ASR::make_FileRead_t(al, loc, m_label, a_unit, a_fmt, a_iomsg,
               a_iostat, a_advance, a_size, a_id, a_values_vec.p, a_values_vec.size(), overloaded_stmt, formatted, nullptr, a_rec);
        }
            ...
            if (inserted_iostat) {
                ...
                if (end_label == -1) {
                    ...
                    body.push_back(al, ASRUtils::STMT(ASR::make_FileRead_t(al, loc, m_label,
                        ...
                        a_values_vec.p, a_values_vec.size(), overloaded_stmt, formatted, nullptr, nullptr)));
                    tmp_vec.push_back(ASR::make_If_t(al, loc, nullptr, eof_test,
                        body.p, body.size(), nullptr, 0));
                }
                if (err_label == -1) {
