...
public:
    ...
    void visit_Block(const AST::Block_t& x)
    {
        ...
        ASR::asr_t* block = ASR::make_Block_t(
            al, x.base.base.loc, current_scope, s2c(al, name), body.p, body.size());
    ...
    void visit_Open(const AST::Open_t& x)
    {
        ...
        for (std::uint32_t i = 0; i < x.n_kwargs; i++) {
            ...
            if (m_arg_str == std::string("newunit") || m_arg_str == std::string("unit")) {
                ...
                if ((m_arg_str == std::string("newunit") && a_newunit->type != ASR::exprType::Var)
                ...
                if (!ASR::is_a<ASR::Character_t>(
                    ...
                    throw SemanticError("`file` must be of type, Character or CharacterPointer",
                                        x.base.base.loc);
                }
                ...
                if (!ASR::is_a<ASR::Character_t>(*ASRUtils::type_get_past_pointer(a_status_type))) {
                    throw SemanticError("`status` must be of type, Character or CharacterPointer",
                                        x.base.base.loc);
                }
            }
        }
        ...
        tmp = ASR::make_FileOpen_t(al, x.base.base.loc, x.m_label, a_newunit, a_filename, a_status);
    }
        ...
        for (std::uint32_t i = 0; i < x.n_kwargs; i++) {
            ...
            if (m_arg_str == std::string("unit")) {
                ...
                if (a_iomsg->type != ASR::exprType::Var
                    ...
                    throw SemanticError("`iomsg` must be of type, Character or CharacterPointer",
                                        x.base.base.loc);
                }
            } else if (m_arg_str == std::string("status")) {
                if (a_status != nullptr) {
    ...
    void create_read_write_ASR_node(const AST::stmt_t& read_write_stmt, AST::stmtType _type)
    {
        ...
        if (n_args > 2) {
            throw SemanticError(
                "Number of arguments cannot be more than 2 in Read/Write statement.", loc);
        }
        ...
        if (_type == AST::stmtType::Write) {
            ...
            tmp = ASR::make_FileRead_t(
                al, loc, m_label, a_unit, a_fmt, a_iomsg, a_iostat, a_id, a_values_vec.p, n_values);
        }
    }
    ...
    void visit_Instantiate(const AST::Instantiate_t& x)
    {
        ...
        for (size_t i = 0; i < x.n_types; i++) {
            ...
            subs[ASR::down_cast2<ASR::TypeParameter_t>(template_type_parameters[template_name][i])
                     ->m_param]
                = type;
        }
        ...
        for (size_t i = 0; i < x.n_symbols; i++) {
            ...
            pass_instantiate_generic_function(al,
                                              subs,
                                              current_scope,
    ...
    void visit_AssociateBlock(const AST::AssociateBlock_t& x)
    {
        ...
        for (size_t i = 0; i < x.n_syms; i++) {
            ...
            if (ASR::is_a<ASR::Var_t>(*tmp_expr)) {
                ...
                tmp_type
                    = ASRUtils::TYPE(ASR::make_Pointer_t(al, tmp_type->base.loc, variable->m_type));
            ...
            if (create_associate_stmt) {
                ASR::stmt_t* associate_stmt = ASRUtils::STMT(
                    ASR::make_Associate_t(al, tmp_expr->base.loc, target_var, tmp_expr));
                body.push_back(al, associate_stmt);
            } else {
                ASR::stmt_t* assign_stmt = ASRUtils::STMT(
                    ASR::make_Assignment_t(al, tmp_expr->base.loc, target_var, tmp_expr, nullptr));
    ...
    void visit_Allocate(const AST::Allocate_t& x)
    {
        ...
        ASR::ttype_t* int32_type
            = LFortran::ASRUtils::TYPE(ASR::make_Integer_t(al, x.base.base.loc, 4, nullptr, 0));
    ...
    void visit_Select(const AST::Select_t& x)
    {
        ...
        for (std::uint32_t i = 0; i < x.n_body; i++) {
            ...
            if (AST::is_a<AST::CaseStmt_Default_t>(*body)) {
                if (def_body.size() != 0) {
                    throw SemanticError("Default case present more than once", x.base.base.loc);
                }
    ...
    void visit_Assignment(const AST::Assignment_t& x)
    {
        ...
        if (target->type == ASR::exprType::Var && !ASRUtils::is_array(target_type)
            ...
            throw SemanticError("ArrayInitalizer expressions can only be assigned array references",
                                x.base.base.loc);
        }
        if (overloaded_stmt == nullptr) {
            if (target->type == ASR::exprType::Var || target->type == ASR::exprType::ArrayItem
                ...
                ImplicitCastRules::set_converted_value(
                    al, x.base.base.loc, &value, value_type, target_type);
            }
        ...
        tmp = ASR::make_Assignment_t(al, x.base.base.loc, target, value, overloaded_stmt);
    }
    ...
    void visit_SubroutineCall(const AST::SubroutineCall_t& x)
    {
        ...
        if (x.n_member == 1) {
            ...
            original_sym = resolve_deriv_type_proc(
                x.base.base.loc, to_lower(x.m_name), to_lower(x.m_member[0].m_name), scope);
        } else {
        ...
        if (x.n_keywords > 0) {
            ...
            if (ASR::is_a<ASR::Function_t>(*f2)) {
                ...
                if (!ASR::is_a<ASR::Function_t>(*f4)) {
                    throw SemanticError(std::string(f3->m_proc_name) + " is not a subroutine.",
                                        x.base.base.loc);
                }
        ...
        tmp = ASR::make_SubroutineCall_t(
            al, x.base.base.loc, final_sym, original_sym, args.p, args.size(), v_expr);
    }
    ...
    void visit_Print(const AST::Print_t& x)
    {
        ...
        tmp = ASR::make_Print_t(al, x.base.base.loc, fmt, body.p, body.size(), nullptr, nullptr);
    }
    ...
    void visit_IfArithmetic(const AST::IfArithmetic_t& x)
    {
        ...
        if (!is_int && !is_real) {
            throw SemanticError("Arithmetic if (x) requires an integer or real for `x`",
                                test_int->base.loc);
        }
        ...
        if (is_int) {
            ASR::ttype_t* type0
                = ASRUtils::TYPE(ASR::make_Integer_t(al, x.base.base.loc, kind, nullptr, 0));
            ASR::expr_t* right
            ...
            ASR::ttype_t* type0
                = ASRUtils::TYPE(ASR::make_Real_t(al, x.base.base.loc, kind, nullptr, 0));
            ASR::expr_t* right
        ...
        Vec<ASR::stmt_t*> body;
        body.reserve(al, 1);
        body.push_back(al, ASRUtils::STMT(ASR::make_GoTo_t(al, x.base.base.loc, x.m_lt_label)));
        Vec<ASR::stmt_t*> orelse;
        orelse.reserve(al, 1);

        Vec<ASR::stmt_t*> body_gt;
        body_gt.reserve(al, 1);
        body_gt.push_back(al, ASRUtils::STMT(ASR::make_GoTo_t(al, x.base.base.loc, x.m_gt_label)));
        Vec<ASR::stmt_t*> orelse_gt;
        orelse_gt.reserve(al, 1);
        orelse_gt.push_back(al,
                            ASRUtils::STMT(ASR::make_GoTo_t(al, x.base.base.loc, x.m_eq_label)));
        ...
        orelse.push_back(al,
                         ASRUtils::STMT(ASR::make_If_t(al,
                                                       ...
                                                       orelse_gt.size())));
        tmp = ASR::make_If_t(
            al, x.base.base.loc, test_lt, body.p, body.size(), orelse.p, orelse.size());
    }
    ...
    void visit_ImpliedDoLoop(const AST::ImpliedDoLoop_t& x)
    {
        ...
        // ASR::asr_t* a_variable = ASR::make_Variable_t(al, x.base.base.loc, current_scope,
        ...
        if (current_scope->get_symbol(var_name) == nullptr) {
            throw SemanticError("The implied do loop variable '" + var_name + "' is not declared",
                                x.base.base.loc);
        };
    ...
    void visit_DoConcurrentLoop(const AST::DoConcurrentLoop_t& x)
    {
        ...
        if (!h.m_var) {
            throw SemanticError("Do loop: loop variable is required for now", x.base.base.loc);
        }
        if (!h.m_start) {
            throw SemanticError("Do loop: start condition required for now", x.base.base.loc);
        }
        if (!h.m_end) {
            throw SemanticError("Do loop: end condition required for now", x.base.base.loc);
        }
        ASR::expr_t* var
            = LFortran::ASRUtils::EXPR(resolve_variable(x.base.base.loc, to_lower(h.m_var)));
        ...
        head.m_increment = increment;
        head.loc = head.m_v->base.loc;
        tmp = ASR::make_DoConcurrentLoop_t(al, x.base.base.loc, head, body.p, body.size());
    }
    ...
    void visit_ForAllSingle(const AST::ForAllSingle_t& x)
    {
        ...
        AST::ConcurrentControl_t& h = *(AST::ConcurrentControl_t*) x.m_control[0];
        if (!h.m_var) {
            throw SemanticError("Forall statement: loop variable is required", x.base.base.loc);
        }
        if (!h.m_start) {
            throw SemanticError("Forall statement: start condition is required", x.base.base.loc);
        }
        if (!h.m_end) {
            throw SemanticError("Forall statement: end condition is required", x.base.base.loc);
        }
        ASR::expr_t* var
            = LFortran::ASRUtils::EXPR(resolve_variable(x.base.base.loc, to_lower(h.m_var)));
        visit_expr(*h.m_start);
    ...
    void visit_GoTo(const AST::GoTo_t& x)
    {
        if (x.m_goto_label) {
            ...
            throw SemanticError("Only 'goto INTEGER' is supported currently", x.base.base.loc);
        }
    }
