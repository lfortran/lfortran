...
std::map<std::string, int> identifiers_map = { { "end of file", END_OF_FILE },
                                               { "newline", TK_NEWLINE },
                                               { "\n", TK_NEWLINE },
                                               { "name", TK_NAME },
                                               { "def_op", TK_DEF_OP },
...
std::vector<std::string> declarators{ "integer",  "real",      "complex",   "doubleprecision",
                                      "external", "dimension", "character", "logical",
                                      "bytes",    "data" };
...
void
FixedFormTokenizer::set_string(const std::string& str)
{
...
struct FixedFormRecursiveDescent {
    ...
    {
        return !s.empty()
               && std::all_of(s.begin(), s.end(), [](char c) { return ::isdigit(c) || c == ' '; });
    }
    ...
    int64_t eat_label_inline(unsigned char*& cur)
    {
        ...
        if (is_integer(label) && count > 0) {
            YYSTYPE yy;
            lex_int_large(m_a, cur, cur2, yy.int_suffix.int_n, yy.int_suffix.int_kind);
    ...
    void tokenize_line(const std::string& chop, unsigned char*& cur)
    {
        ...
        for (;;) {
            ...
            if (next_is(t.tok, "goto") && token != yytokentype::KW_GOTO) {
                ...
                lex_int_large(m_a, t.tok + 4, t.cur, y3.int_suffix.int_n, y3.int_suffix.int_kind);
                tokens.push_back(yytokentype::TK_INTEGER);
            ...
            if (token == yytokentype::TK_INTEGER) {
                lex_int_large(m_a, t.tok, t.cur, y2.int_suffix.int_n, y2.int_suffix.int_kind);
    ...
    bool lex_do_terminal(unsigned char*& cur, int64_t do_label)
    {
        ...
        if (next_is(cur, "enddo")) {
            ...
            // only append iff (tokens[tokens.size()-2] == yytokentype::TK_LABEL &&
            // tokens[tokens.size()-1 == yytokentype::KW_CONTINUE])
    ...
    void lex_if(unsigned char*& cur)
    {
        ...
        if (tokens[tokens.size() - 2] != yytokentype::KW_THEN)
            return;
                        ...
                        const std::vector<std::string>& keywords)
    {
        ...
        while (decls.size() != 0) {
            ...
            for (auto kw = kw_found.begin(); kw != kw_found.end(); ++kw) {
                if (next_is(cpy, *kw)) {
                    error(cpy,
                          "Syntax error: keyword " + *kw + "cannot occur multiple times in "
    ...
    void lex_global_scope_item(unsigned char*& cur)
    {
        ...
        std::vector<std::string> subroutine_keywords{ "recursive", "pure", "elemental" };
        std::vector<std::string> function_keywords{ "recursive", "pure",      "elemental",
                                                    "real",      "character", "complex",
                                                    "integer",   "logical",   "doubleprecision" };

        if (next_is(cur, "include"))
            tokenize_line("include", cur);
        if (is_program(cur)) {
...
bool
FixedFormTokenizer::tokenize_input(diag::Diagnostics& diagnostics, Allocator& al)
{
...
int
FixedFormTokenizer::lex(Allocator& /*al*/,
                        YYSTYPE& yylval,
                        Location& loc,
