...
void
section(const std::string& s)
{
    std::cout << color(LFortran::style::bold) << color(LFortran::fg::blue) << s
              << color(LFortran::style::reset) << color(LFortran::fg::reset) << std::endl;
}
...
std::string
p(Allocator& al, const std::string& s)
{
    LFortran::AST::TranslationUnit_t* result;
    LFortran::diag::Diagnostics diagnostics;
    LFortran::Result<LFortran::AST::TranslationUnit_t*> res = LFortran::parse(al, s, diagnostics);
    if (res.ok) {
        result = res.result;
    } else {
        throw ParserError0();
...
TEST_CASE("Multiple units")
{
    ...
    results = LFortran::TRY(LFortran::parse(al, s, diagnostics));
    CHECK(results->n_items == 2);
    CHECK(LFortran::pickle(*results->m_items[0])
          == "(= 0 x (+ x 1) (TriviaNode [] [(Semicolon) (Semicolon)]))");
    CHECK(LFortran::pickle(*results->m_items[1]) == "(= 0 y (+ z 1) ())");
    ...
    CHECK(results->n_items == 2);
    CHECK(LFortran::pickle(*results->m_items[0])
          == "(= 0 x (+ x 1) (TriviaNode [] [(Semicolon) (EndOfLine) (EndOfLine) (Semicolon)]))");
    CHECK(LFortran::pickle(*results->m_items[1]) == "(= 0 y (+ z 1) ())");
...
TEST_CASE("declaration")
{
    ...
    CHECK_THROWS_AS(P("integer, parameter, pointer x"), ParserError0);
