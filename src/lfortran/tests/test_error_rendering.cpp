...
TEST_CASE("Error Render: no labels") {
  ...
  out = render_diagnostic_human(
      Diagnostic("Error no label", Level::Error, Stage::Parser), false);
  ref = S(R"""(
syntax error: Error no label
  ...
  out = render_diagnostic_human(
      Diagnostic("Error no label", Level::Error, Stage::Semantic), false);
  ref = S(R"""(
semantic error: Error no label
  ...
  out = render_diagnostic_human(
      Diagnostic("Error no label", Level::Warning, Stage::Semantic), false);
  ref = S(R"""(
...
TEST_CASE("Error Render: primary/secondary labels, single line") {
  ...
  // 1 Label 1 Span
  auto d = Diagnostic("Error with label no message", Level::Error,
                      Stage::Semantic, {Label("", {loc1})});
...
1 | One line text
  ...
  // 2 Label 1 Span
  d = Diagnostic(
      "Error with two labels and message", Level::Error, Stage::Semantic,
...
1 | One line text
  ...
  d = Diagnostic("Error with two labels and message", Level::Error,
                 Stage::Semantic,
...
1 | One line text
  ...
  d = Diagnostic("Error with three labels and message, two spans", Level::Error,
                 Stage::Semantic,
...
TEST_CASE("Error Render: primary/secondary labels, multi line") {
  ...
  // 1 Label 1 Span
  auto d = Diagnostic("Error with label no message", Level::Error,
                      Stage::Semantic, {Label("Multilines", {loc1})});
  out = render_diagnostic_human(d, input, lm, false, false);
  ref = S(R"""(
semantic error: Error with label no message
 --> input:1:5 - 2:11
...
2 |    Second line text
  ...
  // 1 Label 2 Span
  d = Diagnostic("Error with label, two spans", Level::Error, Stage::Semantic,
