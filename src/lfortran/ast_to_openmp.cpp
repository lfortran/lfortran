...
#define ATTRTYPE2(x, y)          \
  ...
  void visit_AttrType(const AttrType_t &x) {
    ...
    if (x.n_kind > 0) {
      ...
      if (x.n_kind == 1 &&
        ...
        k[0].m_id = nullptr;
      } else if (x.n_kind == 1 && x.m_type == decl_typeType::TypeCharacter &&
                 (k[0].m_id == nullptr || std::string(k[0].m_id) == "len")) {
        k[0].m_id = (char *)"len";
      } else if (x.n_kind == 2 && x.m_type == decl_typeType::TypeCharacter &&
                 (k[0].m_id != nullptr && k[1].m_id != nullptr) &&
                 std::string(k[1].m_id) == "len") {
        std::swap(k[0], k[1]);
      }
      if (x.m_type == decl_typeType::TypeCharacter && k[0].m_id == nullptr) {
        k[0].m_id = (char *)"len";
      }
  ...
  void visit_DoConcurrentLoop(const DoConcurrentLoop_t &x) {
    if (x.n_control != 1) {
      throw LCompilersException(
          "Do concurrent: exactly one control statement is implemented for "
          "now");
    }
    ...
    for (size_t i = 0; i < x.n_locality; i++) {
      if (x.m_locality[i]->type ==
          AST::concurrent_localityType::ConcurrentReduce) {
  ...
  void visit_Num(const Num_t &x) { s = std::to_string(x.m_n); }
  ...
  void visit_Name(const Name_t &x) { s = std::string(x.m_id); }
  ...
  void visit_arg(const arg_t &x) { s = std::string(x.m_arg); }
