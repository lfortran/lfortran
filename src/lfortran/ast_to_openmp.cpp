...
#define ATTRTYPE2(x, y)                                                                            \
    case (decl_typeType::Type##x):                                                                 \
        ...
        std::string kind_value(const AST::kind_item_typeType& type, const AST::expr_t* value)
        {
            switch (type) {
        ...
        void visit_DoConcurrentLoop(const DoConcurrentLoop_t& x)
        {
            if (x.n_control != 1) {
                throw LCompilersException(
                    "Do concurrent: exactly one control statement is implemented for now");
            }
...
std::string
ast_to_openmp(LFortran::AST::ast_t& ast)
{
