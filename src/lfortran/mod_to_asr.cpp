...
#else
    uint8_t * /*out*/, uint64_t * /*out_size*/, uint8_t * /*in*/,
    uint64_t /*in_size*/
#endif
) {
#ifdef HAVE_ZLIB
  ...
  switch (res) {
    ...
    case Z_DATA_ERROR:
      throw LCompilersException(
          "ZLIB: the input data was corrupted or incomplete");
    default:
...
// 'abc' -> abc
std::string str_(const std::string &s) { return s.substr(1, s.size() - 2); }
...
struct Item {
  enum { list, str, integer, token } kind;
  std::string s;
  uint64_t i;
  std::vector<Item> l;
};
...
struct GSymbol {
  ...
  Item info;
  enum { variable, procedure, module } kind;
  struct {
    int intent;
    bool dummy;
    ASR::ttype_t *type;
...
ASR::TranslationUnit_t *parse_gfortran_mod_file(Allocator &al,
                                                const std::string &s) {
  std::vector<std::string> s2 = split(s);
  int version = std::atoi(&str_(s2[3])[0]);
  if (version != 14) {
    throw LCompilersException(
        "Only GFortran module version 14 is implemented so far");
  }
  ...
  if (mod.l.size() == 8) {
    ...
    for (size_t i = 0; i < symtab.size(); i += 6) {
      ...
      if (kind == "VARIABLE") {
        ...
        Location loc;
        ASR::asr_t *asr = ASR::make_Variable_t(
            al, loc, nullptr, name, ASR::intentType::In, nullptr, nullptr,
        ...
        a.from_str_view(s.name);
        char *name = a.c_str(al);
        ASR::asr_t *asr = ASR::make_Function_t(
            al, loc, proc_symtab, name, nullptr, 0, nullptr, 0, nullptr, 0,
            nullptr,  // return var
            ASR::abiType::GFortranModule, ASR::Public, ASR::Interface, nullptr,
    ...
    for (size_t i = 0; i < public_symbols.size(); i += 3) {
      ...
      if (gsymtab.find(symbol_id) == gsymtab.end()) {
        throw LCompilersException(
            "Public symbol not defined in the symbol table");
      }
  ...
  for (auto &item : gsymtab) {
      std::cout << item.second.id << " " << item.second.name << " " <<
  item.second.module_name << " " << item.second.is_public << " "; std::cout <<
  format_item(item.second.info) << std::endl;
  ...
  */
  ...
  ASR::asr_t *asr;
  Location loc;
  asr = ASR::make_TranslationUnit_t(al, loc, parent_scope, nullptr, 0);
