...
Item
read_from_tokens(const std::vector<std::string>& tokens, size_t& pos)
{
    if (pos == tokens.size()) {
...
ASR::ttype_t*
parse_type(Allocator& al, const std::vector<Item>& l)
{
...
parse_gfortran_mod_file(Allocator& al, const std::string& s)
{
    ...
    if (mod.l.size() == 8) {
        ...
        for (size_t i = 0; i < symtab.size(); i += 6) {
            ...
            if (kind == "VARIABLE") {
                ...
                Location loc;
                ASR::asr_t* asr = ASR::make_Variable_t(al,
                                                       loc,
                                                       nullptr,
                                                       name,
                                                       ASR::intentType::In,
                ...
                a.from_str_view(s.name);
                char* name = a.c_str(al);
                ASR::asr_t* asr = ASR::make_Function_t(al,
                                                       loc,
                                                       proc_symtab,
                                                       name,
                                                       nullptr,
    ...
    for (auto &item : gsymtab) {
        std::cout << item.second.id << " " << item.second.name << " " << item.second.module_name <<
    " " << item.second.is_public << " "; std::cout << format_item(item.second.info) << std::endl;
...
ASR::TranslationUnit_t*
mod_to_asr(Allocator& al, std::string filename)
{
