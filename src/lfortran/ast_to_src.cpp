            ...
            for (size_t i = 0; i < x.n_items; i++) {
                ...
                if (i < x.n_items - 1) {
                    if (is_a<expr_t>(*x.m_items[i])
                        || ((is_a<mod_t>(*x.m_items[i]) || is_a<program_unit_t>(*x.m_items[i]))
...
std::string
ast_to_src(AST::TranslationUnit_t& ast, bool color, int indent, bool indent_unit)
{
