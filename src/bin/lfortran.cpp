...
std::string
remove_path(const std::string& filename)
{
...
get_kokkos_dir()
{
    ...
    if (env_p)
        return env_p;
    std::cerr << "The code C++ generated by the C++ LFortran backend uses the Kokkos library"
              << std::endl;
    std::cerr << "(https://github.com/kokkos/kokkos). Please define the LFORTRAN_KOKKOS_DIR"
              << std::endl;
...
int
emit_tokens2(const std::string& input,
             std::vector<std::string>& tok_strings,
             std::vector<int>& toks,
...
prompt(bool verbose)
{
    ...
    std::cout << "Interactive Fortran. Experimental prototype, not ready for end users."
              << std::endl;
    ...
    while (true) {
        ...
        switch (r.type) {
            case (LFortran::FortranEvaluator::EvalResult::integer4): {
                if (verbose)
                    std::cout << "Return type: integer" << std::endl;
                if (verbose)
                    section("Result:");
            ...
            case (LFortran::FortranEvaluator::EvalResult::integer8): {
                if (verbose)
                    std::cout << "Return type: integer(8)" << std::endl;
                if (verbose)
                    section("Result:");
            ...
            case (LFortran::FortranEvaluator::EvalResult::real4): {
                if (verbose)
                    std::cout << "Return type: real" << std::endl;
                if (verbose)
                    section("Result:");
            ...
            case (LFortran::FortranEvaluator::EvalResult::real8): {
                if (verbose)
                    std::cout << "Return type: real(8)" << std::endl;
                if (verbose)
                    section("Result:");
            ...
            case (LFortran::FortranEvaluator::EvalResult::complex4): {
                if (verbose)
                    std::cout << "Return type: complex" << std::endl;
                if (verbose)
                    section("Result:");
            ...
            case (LFortran::FortranEvaluator::EvalResult::complex8): {
                if (verbose)
                    std::cout << "Return type: complex(8)" << std::endl;
                if (verbose)
                    section("Result:");
                std::cout << std::setprecision(17) << "(" << r.c64.re << ", " << r.c64.im << ")"
                          << std::endl;
                break;
            }
...
int
emit_prescan(const std::string& infile, CompilerOptions& compiler_options)
{
    std::string input = read_file(infile);
    LFortran::LocationManager lm;
    lm.in_filename = infile;
    std::string prescan = LFortran::fix_continuation(input, lm, compiler_options.fixed_form);
    std::cout << prescan << std::endl;
    return 0;
}

int
emit_tokens(const std::string& infile, bool line_numbers, const CompilerOptions& compiler_options)
{
    std::string input = read_file(infile);
    // Src -> Tokens
    Allocator al(64 * 1024 * 1024);
    ...
    auto res = LFortran::tokens(
        al, input, diagnostics, &stypes, &locations, compiler_options.fixed_form);
    lm.in_filename = infile;
    lm.init_simple(input);
...
int
emit_ast(const std::string& infile, CompilerOptions& compiler_options)
{
    std::string input = read_file(infile);
...
int
emit_ast_f90(const std::string& infile, CompilerOptions& compiler_options)
{
    std::string input = read_file(infile);
    LFortran::FortranEvaluator fe(compiler_options);
    LFortran::LocationManager lm;
    LFortran::diag::Diagnostics diagnostics;
    lm.in_filename = infile;
    LFortran::Result<LFortran::AST::TranslationUnit_t*> r = fe.get_ast2(input, lm, diagnostics);
    std::cerr << diagnostics.render(input, lm, compiler_options);
    if (r.ok) {
...
int
format(const std::string& infile,
       bool inplace,
       bool color,
       int indent,
       bool indent_unit,
...
int
python_wrapper(const std::string& infile,
               ...
               CompilerOptions& compiler_options)
{
    ...
    auto pxd_fname
        = prefix + "_pxd.pxd";  // the "_pxd" is an ugly hack, see comment in asr_to_py.cpp
    ...
    auto chdr_fname_forcodegen = chdr_fname;
    {
        ...
        if (lastpos > 0UL)
            chdr_fname_forcodegen.erase(0, lastpos + 1);
    }
...
int
emit_asr(const std::string& infile,
         ...
         CompilerOptions& compiler_options)
{
    ...
    lm.in_filename = infile;
    LFortran::diag::Diagnostics diagnostics;
    LFortran::Result<LFortran::ASR::TranslationUnit_t*> r = fe.get_asr2(input, lm, diagnostics);
    std::cerr << diagnostics.render(input, lm, compiler_options);
    if (!r.ok) {
        LFORTRAN_ASSERT(diagnostics.has_error())
        return 2;
    ...
    std::cout << LFortran::pickle(
        *asr, compiler_options.use_colors, compiler_options.indent, with_intrinsic_modules)
              << std::endl;
    return 0;
}

int
emit_cpp(const std::string& infile, CompilerOptions& compiler_options)
{
    std::string input = read_file(infile);
...
int
emit_c(const std::string& infile, CompilerOptions& compiler_options)
{
    std::string input = read_file(infile);
...
int
save_mod_files(const LFortran::ASR::TranslationUnit_t& u)
{
    for (auto& item : u.m_global_scope->get_scope()) {
        if (LFortran::ASR::is_a<LFortran::ASR::Module_t>(*item.second)) {
            ...
            Allocator al(4 * 1024);
            LFortran::SymbolTable* symtab = al.make_new<LFortran::SymbolTable>(nullptr);
...
int
emit_llvm(const std::string& infile,
          LCompilers::PassManager& pass_manager,
          CompilerOptions& compiler_options)
{
    std::string input = read_file(infile);
    ...
    lm.in_filename = infile;
    LFortran::diag::Diagnostics diagnostics;
    LFortran::Result<std::string> llvm = fe.get_llvm(input, lm, pass_manager, diagnostics);
    std::cerr << diagnostics.render(input, lm, compiler_options);
    if (llvm.ok) {
...
int
emit_asm(const std::string& infile, CompilerOptions& compiler_options)
{
    std::string input = read_file(infile);
...
int
compile_to_object_file(const std::string& infile,
                       ...
                       CompilerOptions& compiler_options)
{
    ...
    if (!compiler_options.generate_object_code && !LFortran::ASRUtils::main_program_present(*asr)) {
...
int
compile_to_assembly_file(const std::string& infile,
                         const std::string& outfile,
                         CompilerOptions& compiler_options)
{
...
int
emit_wat(const std::string& infile, CompilerOptions& compiler_options)
{
    std::string input = read_file(infile);
...
int
compile_to_binary_x86(const std::string& infile,
                      const std::string& outfile,
                      bool time_report,
                      CompilerOptions& compiler_options)
{
    ...
    // AST -> ASR
    {
        diagnostics.diagnostics.clear();
        auto t1 = std::chrono::high_resolution_clock::now();
        LFortran::Result<LFortran::ASR::TranslationUnit_t*> result = fe.get_asr3(*ast, diagnostics);
        auto t2 = std::chrono::high_resolution_clock::now();
    ...
    // ASR -> x86 machine code
    {
        diagnostics.diagnostics.clear();
        auto t1 = std::chrono::high_resolution_clock::now();
        LFortran::Result<int> result = LFortran::asr_to_x86(*asr, al, outfile, time_report);
        auto t2 = std::chrono::high_resolution_clock::now();
    ...
    if (time_report) {
        std::cout << "Allocator usage of last chunk (MB): " << al.size_current() / (1024. * 1024)
                  << std::endl;
        ...
        int total = time_file_read + time_src_to_ast + time_ast_to_asr + time_asr_to_x86;
...
int
compile_to_binary_wasm(const std::string& infile,
                       const std::string& outfile,
                       bool time_report,
                       CompilerOptions& compiler_options)
{
    ...
    if (time_report) {
        ...
        int total = time_file_read + time_src_to_ast + time_ast_to_asr + time_asr_to_wasm;
        std::cout << "Total:       " << std::setw(5) << total << std::endl;
    }
...
int
compile_to_object_file_cpp(const std::string& infile,
                           const std::string& outfile,
                           bool assembly,
                           bool kokkos,
                           const std::string& rtlib_header_dir,
                           CompilerOptions& compiler_options)
{
    std::string input = read_file(infile);
...
int
link_executable(const std::vector<std::string>& infiles,
                const std::string& outfile,
                const std::string& runtime_library_dir,
                Backend backend,
                bool static_executable,
                bool link_with_gcc,
                bool kokkos,
                CompilerOptions& compiler_options)
{
    ...
    */
    ...
    if (backend == Backend::llvm) {
        if (t == "x86_64-pc-windows-msvc") {
            ...
            cmd += +" -L" + base_path + " -Wl,-rpath," + base_path + " -l" + runtime_lib + " -lm";
            int err = system(cmd.c_str());
            if (err) {
                std::cout << "The command '" + cmd + "' failed." << std::endl;
                return 10;
        ...
        std::string cmd = "cp " + infiles[0] + " " + outfile + " && " + "cp " + infiles[0] + ".js"
                          + " " + outfile + ".js";
        int err = system(cmd.c_str());
        if (err) {
            std::cout << "The command '" + cmd + "' failed." << std::endl;
...
int
emit_c_preprocessor(const std::string& infile, CompilerOptions& compiler_options)
{
    std::string input = read_file(infile);
...
emit_ast_from_source(char* input)
{
    INITIALIZE_VARS;
    LFortran::Result<std::string> r = fe.get_ast(input, lm, diagnostics);
...
EMSCRIPTEN_KEEPALIVE char*
emit_asr_from_source(char* input)
{
    INITIALIZE_VARS;
...
EMSCRIPTEN_KEEPALIVE char*
emit_wat_from_source(char* input)
{
    INITIALIZE_VARS;
    LFortran::Result<std::string> r = fe.get_wat(input, lm, diagnostics);
    out = diagnostics.render(input, lm, compiler_options);
...
EMSCRIPTEN_KEEPALIVE char*
emit_cpp_from_source(char* input)
{
    INITIALIZE_VARS;
...
EMSCRIPTEN_KEEPALIVE char*
emit_wasm_from_source(char* input)
{
    INITIALIZE_VARS;
...
main(int argc, char* argv[])
{
    ...
    try {
        ...
        app.add_option("-D",
                       compiler_options.c_preprocessor_defines,
        ...
        app.add_flag(
            "--fixed-form", compiler_options.fixed_form, "Use fixed form Fortran source parsing");
        ...
        app.add_flag(
            "--with-intrinsic-mods", with_intrinsic_modules, "Show intrinsic modules in ASR");
        app.add_flag(
            "--show-ast-f90", show_ast_f90, "Show Fortran from AST for the given file and exit");
        ...
        app.add_flag(
            "--show-cpp", show_cpp, "Show C++ translation source for the given file and exit");
        ...
        app.add_flag(
            "--no-error-banner", compiler_options.no_error_banner, "Turn off error banner");
        app.add_option("--error-format",
                       compiler_options.error_format,
                       "Control how errors are produced (human, short)")
            ->capture_default_str();
        app.add_option("--backend", arg_backend, "Select a backend (llvm, cpp, x86, wasm)")
            ->capture_default_str();
        ...
        app.add_option("--target", compiler_options.target, "Generate code for the given target")
            ->capture_default_str();
        app.add_flag("--print-targets", print_targets, "Print the registered targets");
        app.add_flag(
            "--implicit-typing", compiler_options.implicit_typing, "Allow implicit typing");
        ...
        fmt.add_flag(
            "-i", arg_fmt_inplace, "Modify <file> in-place (instead of writing to stdout)");
        fmt.add_option("--spaces", arg_fmt_indent, "Number of spaces to use for indentation")
            ->capture_default_str();
        fmt.add_flag(
            "--indent-unit", arg_fmt_indent_unit, "Indent contents of sub / fn / prog / mod");
        ...
        pywrap.add_option("--array-order", arg_pywrap_array_order, "Select array order (c, f)")
            ->capture_default_str();
        ...
        if (arg_version) {
            ...
            std::cout << std::endl;
#ifdef HAVE_LFORTRAN_LLVM
            std::cout << "Default target: " << LFortran::LLVMEvaluator::get_default_target_triple()
                      << std::endl;
#endif
            return 0;
        }
...
#else
            std::cerr
                << "The --print-targets option requires the LLVM backend to be enabled. Recompile with `WITH_LLVM=yes`."
                << std::endl;
            return 1;
#endif
        }
...
#else
            std::cerr
                << "The kernel subcommand requires LFortran to be compiled with XEUS support. Recompile with `WITH_XEUS=yes`."
                << std::endl;
            return 1;
#endif
        }
        ...
        if (pywrap) {
            return python_wrapper(arg_pywrap_file, arg_pywrap_array_order, compiler_options);
        }
...
#else
            std::cerr
                << "Interactive prompt requires the LLVM backend to be enabled. Recompile with `WITH_LLVM=yes`."
                << std::endl;
            return 1;
#endif
        }
        ...
        if (show_asr) {
            return emit_asr(
                arg_file, lfortran_pass_manager, with_intrinsic_modules, compiler_options);
        }
...
#else
            std::cerr
                << "The --show-llvm option requires the LLVM backend to be enabled. Recompile with `WITH_LLVM=yes`."
                << std::endl;
            return 1;
#endif
        }
...
#else
            std::cerr
                << "The --show-asm option requires the LLVM backend to be enabled. Recompile with `WITH_LLVM=yes`."
                << std::endl;
            return 1;
#endif
        }
...
#else
                std::cerr
                    << "The -S option requires the LLVM backend to be enabled. Recompile with `WITH_LLVM=yes`."
                    << std::endl;
                return 1;
#endif
            } else if (backend == Backend::cpp) {
        ...
        if (arg_c) {
            if (backend == Backend::llvm) {
#ifdef HAVE_LFORTRAN_LLVM
                return compile_to_object_file(arg_file, outfile, false, compiler_options);
#else
                std::cerr
                ...
                return compile_to_object_file_cpp(
                    arg_file, outfile, false, true, rtlib_header_dir, compiler_options);
            } else if (backend == Backend::x86) {
        ...
        if (endswith(arg_file, ".f90")) {
            ...
            if (backend == Backend::llvm) {
                ...
                err = compile_to_object_file_cpp(
                    arg_file, tmp_o, false, true, rtlib_header_dir, compiler_options);
            } else {
                throw LFortran::LCompilersException("Backend not supported");
            }
            if (err)
                return err;
            return link_executable({ tmp_o },
                                   outfile,
                                   runtime_library_dir,
                                   backend,
                                   static_link,
            ...
            return link_executable(arg_files,
                                   outfile,
                                   runtime_library_dir,
                                   backend,
                                   static_link,
