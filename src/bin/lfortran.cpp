    ...
    &stypes)
{
    ...
    for (size_t i=0; i < toks.size(); i++) {
        ...
        //std::cout << LCompilers::LFortran::pickle(toks[i], stypes[i]) << std::endl;
    }
    return 0;
}
...
int prompt(bool verbose)
{
    ...
    Allocator al(64*1024*1024);
    CompilerOptions cu;
    cu.runtime_library_dir = LCompilers::get_runtime_library_dir();
...
#define INITIALIZE_VARS CompilerOptions compiler_options; \
                        ...
                        compiler_options.runtime_library_dir = LCompilers::get_runtime_library_dir(); \
                        LCompilers::FortranEvaluator fe(compiler_options); \
                        LCompilers::LocationManager lm; \
                        LCompilers::diag::Diagnostics diagnostics; \
                        { \
...
int main(int argc, char *argv[])
{
    ...
    try {
        int dirname_length;
        LCompilers::get_executable_path(LCompilers::binary_executable_path, dirname_length);

        // TODO: This is now in compiler options and can be removed
        std::string runtime_library_dir = LCompilers::get_runtime_library_dir();

        std::string rtlib_header_dir = LCompilers::get_runtime_library_header_dir();
        Backend backend;
