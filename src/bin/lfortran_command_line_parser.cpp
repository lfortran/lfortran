...
namespace LCompilers::CommandLineInterface {
    ...
    auto LFortranCommandLineParser::parse() -> void {
        ...
        app.add_flag("--gpu-decline-stats", compiler_options.po.gpu_decline_stats, "Print one line per parallel loop that was not offloaded, naming the unsupported construct or classifying the offloading failure as not-implemented or backend-cannot")->group(group_backend_codegen_options);
